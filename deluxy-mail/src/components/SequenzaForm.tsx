...
const PASSO_VUOTO: PassoInput = { giorniAttesa: 3, oggetto: '', corpo: '' }
    ...
    <div className="card" style={{ marginBottom: 16 }}>
      <div className="form-grid">
        ...
        <div className="full" style={{ fontSize: 12.5, color: 'var(--text-secondary)', lineHeight: 1.5 }}>
          ...
          mail iniziale). Ogni passo parte solo se il destinatario NON ha ancora risposto.
        </div>

        {passi.map((p, i) => (
          <div key={i} className="full" style={{ border: '1px solid var(--hairline)', borderRadius: 12, padding: 14 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 10, flexWrap: 'wrap' }}>
              ...
              <span style={{ fontSize: 12.5, color: 'var(--text-secondary)' }}>
                giorni {i === 0 ? 'dall’invio della mail' : 'dal passo precedente'}
              </span>
              {passi.length > 1 && (
                <button
                  type="button"
            ...
            <textarea
              ...
              placeholder={'Testo — es. Ciao {{nome}}, ti riscrivo in merito a "{{oggetto}}": hai avuto modo di dare un’occhiata?'}
              rows={4}
              style={{ width: '100%' }}
            />
          </div>
        ...
        <div className="full">
          <button type="button" className="btn secondary small" onClick={() => setPassi((p) => [...p, { ...PASSO_VUOTO }])}>
            + Aggiungi un passo
          </button>
        </div>
      </div>
  ...
  return (
    <div className="card" style={{ marginBottom: 12 }}>
      ...
      <div style={{ fontSize: 12.5, color: 'var(--text-tertiary)', marginTop: 8 }}>
        {sequenza.passi.map((p, i) => `+${p.giorniAttesa}g: ${p.oggetto || '(senza oggetto)'}`).join('  ·  ')}
      </div>
    </div>
  )
}
