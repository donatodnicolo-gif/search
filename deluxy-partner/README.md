...
2. **`npx vercel promote <url-del-deploy> --yes`** — obbligatorio dal 10/09/2026: il 02/09 un rollback ha spento l'assegnazione automatica dei domini di produzione, e da allora ogni deploy restava `STAGED` (pagine spostate a mano con `alias set`, ma **i cron continuavano a girare sul deploy del 31/08**). `promote` promuove, riaccende l'automatismo e riallinea i cron.
