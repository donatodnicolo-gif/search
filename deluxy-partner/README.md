...
Vercel, progetto `deluxy/deluxy-partner`, dalla cartella:

1. `npx vercel deploy --prod --yes` (build su Vercel; il `vercel build` locale non funziona su Windows per i symlink).
2. **`npx vercel promote <url-del-deploy> --yes`** — serve **dopo un rollback**: il 02/09/2026 un rollback ha spento l'assegnazione automatica dei domini di produzione, e fino al 10/09 ogni deploy restava `STAGED` (pagine spostate a mano con `alias set`, ma **i cron continuavano a girare sul deploy del 31/08**). Il `promote` del 10/09 ha riacceso l'automatismo: il deploy successivo è risultato promosso da solo (`promote` risponde 409 «already the current production deployment», ed è la risposta giusta). Se un giorno `readySubstate` torna `STAGED`, qualcuno ha fatto un rollback: rifare il `promote`.
3. Verifica: `npx vercel inspect https://deluxy-partner.vercel.app` deve mostrare il deploy appena fatto; dall'API `GET https://api.vercel.com/v9/projects/prj_GHPnElvYTaHUUmqhMaFKaD5yWn24?teamId=team_vt9JRBhnxbY4spm5LzhNyxoY` (token in `%APPDATA%com.vercel.cliDataauth.json`) `targets.production.id` e `crons.deploymentId` devono coincidere con quel deploy, e il suo `readySubstate` deve essere `PROMOTED`.
 Env di produzione: `DATABASE_URL` (pooler 6543 con `?pgbouncer=true&connection_limit=1`), `DIRECT_URL` (pooler 5432), `PARTNER_APP_PASSWORD`.
