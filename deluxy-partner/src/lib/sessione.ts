...
// conoscere: se la chiave deriva da PARTNER_APP_PASSWORD, cambiare la password
...
function segreto(): string {
  return (
    ...
    process.env.PARTNER_APP_PASSWORD ||
    "deluxy-partner-sviluppo-locale"
  );
}
...
async function chiave(): Promise<CryptoKey> {
  return crypto.subtle.importKey(
    "raw",
    new TextEncoder().encode(`deluxy-partner:sessione:v2:${segreto()}`),
...
export async function leggiSessione(cookie: string | undefined): Promise<DatiSessione | null> {
  ...
  try {
    ...
    if (d.ruolo !== "admin" && d.ruolo !== "sola_lettura") return null;
    return d;
  } catch {
    return null;
  }
