...
- **Fatture in Cloud** ✅ collegato (app "FINANCE", azienda "Deluxy srl", id 712328). OAuth con refresh automatico (`fic.*` nel DB). Emissione fatture commissioni non-SDI con numero di ritorno; lettura clienti e fatture. `src/lib/fic.ts`.
  **Stato incasso allineato app → FIC (23/07/2026)**: quando una fattura servizi viene segnata saldata/riaperta nell'app (pulsante «Saldata», spunta nella scheda fattura, o riconciliazione di un movimento bancario in `/transazioni`), lo stesso stato viene scritto sui `payments_list` della fattura su Fatture in Cloud — prima restava «Da incassare» in `/registrazioni/fatture`. Helper `ficAllineaStatoFattura()` + `ficIdDaNumero()`: il match è per numero+anno (`"474/2026"`), e se FIC non è collegato/il numero non si trova l'azione locale **non fallisce** (lo stato resta da allineare a mano). Il verso opposto (saldata su FIC → saldata in app) resta da fare.
