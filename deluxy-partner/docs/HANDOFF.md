...
> 🏢 **11/09/2026 (sessione piattaforma) — L'INTESTAZIONE VA AL CAPOGRUPPO quando la sede non paga da sé** (decisione dell'utente dopo il confronto fra piattaforma, Anagrafiche e Finance). Prima Finance intestava pro-forma, fattura e cliente FIC alla SINGOLA scheda (nome della sede) pur ricevendo dal registro P.IVA/SDI/IBAN del capogruppo: nome di Diptyque Brera con la P.IVA di Olfattorio. Ora `src/lib/anagrafiche.ts` porta `pagaDaSe` e `capogruppo {id, nome}` nel tipo `Anagrafica` (il registro li manda già) e la funzione `intestatarioFattura(a, locale)`: con `pagaDaSe === false` e capogruppo, il nome è la ragione sociale del capogruppo e la sede sta sotto come «per conto di …»; altrimenti come prima (ragione sociale locale → del registro → nome). Usata in `proforma-documento.ts` (documento e PDF, riga `insegna`) e in `fic/fattura/page.tsx` (nome del cliente FIC e «crea da registro»). ⚠️ L'INDIRIZZO resta quello della sede: il capogruppo nel registro non ha un indirizzo suo. Commit su `scout-ui`, deploy in cloud dalla cartella (`npx vercel deploy --prod --yes`), id nel messaggio di sessione della piattaforma (handoff `deluxy-platform-next/docs/HANDOFF.md`, voce 11/09 «mattina, 20»).
>
...
> - **DECISO E FATTO dall'utente, stessa sera**: «i €15 sono da segnare come se fosse un errore che gli abbiamo mandato in più», «quindi andrebbero scalati dai prossimi pagamenti», «se annullo il bonifico anche questo plus o minus deve scomparire», «fai anche l'ipotesi in cui abbiamo inviato di meno».
>   - **Dove si applica**: SOLO dove l'extra è importato (nessuna riga in `ExtraSaldo`) **e un bonifico è davvero uscito**. Senza bonifico non c'è prova di un errore e l'extra resta un dovuto: così la riclassificazione tocca **31 mesi, non 211** — gli 82.681,70 € restano fermi.
>   - **Si CALCOLA, non si memorizza**: `pagatoInPiu` nasce da `bonificoImporto`, quindi annullando il bonifico torna zero da sé. Nessuna riga da cancellare, nessun numero che sopravvive alla sua causa (regola d'oro n.1, e richiesta esplicita dell'utente).
>   - **Nel mese**: pillola rossa «inviato in più X € — da recuperare» (con la spiegazione di dove finiva l'extra importato) e pillola ambra «inviato in meno X € — ancora da versare» nell'altro verso.
>   - **Nell'anno** (`rolling`): il surplus si **scala** da quello che resta da bonificare (`daBonificare` è ora AL NETTO), e ciò che avanza compare come **«Surplus da recuperare»** — denaro nostro che sta dal partner.
>   - **MISURA su tutti i partner, 2026**: «da bonificare» da **48.502,14 € a 42.485,97 €** (−6.016,17 € che erano già usciti), **2.815,93 €** di surplus che avanza, **13 partner** toccati. I più grossi: CLIVATI (2.832,96 € in più, tutti riassorbiti), FIORAVANTI (2.771,75 €, ne avanzano 1.586,27), FLOR (1.220,00 €), CAKELAB FIRENZE (698,84 €, ne avanzano 549,41).
>   - **Verificato in locale** su CAKELAB FIRENZE: febbraio mostra «inviato in più 15,01 € — da recuperare» (dovuto vendite 99,38 contro 114,39 bonificati), e il totale annuo «già scalato 149,43 €» più «Surplus da recuperare 549,41 €». `tsc` verde.
> - ⚠️ **Resta fermo, e va detto**: i **180 mesi** con extra importato **senza** bonifico continuano a contare quell'extra come dovuto (per loro non esiste una prova). Se anche quelli non sono dovuti, è un secondo passo da decidere a parte.
>
...
> - **La causa**: la scheda e la riconciliazione facevano **due domande diverse**. Il motore (`matchPartner`) confronta per **parole intere** e vuole un token ≥5 lettere **o** due token; la scheda cercava per **sottostringa** (`controparte contains 'ARTE'`), e una sottostringa qualsiasi bastava. Su ARTE E FIORI il nome dà un solo token — `ARTE`, 4 lettere (`FIORI` è in `PAROLE_GENERICHE`, «E» è troppo corto) — quindi **103 candidati per sottostringa, di cui 0 che il motore avrebbe riconosciuto**.
> - **Fatto**: la sottostringa resta solo come **prefiltro del database** (tetto 200 righe: la regola del motore non si può chiedere a Postgres senza una regex per riga), e il giudizio lo dà `matchPartner`. **Due query invece di una, di proposito**: i movimenti **già attribuiti** non passano dal filtro — sono una certezza scritta da una persona — e non possono essere spinti fuori dal rumore.
> - ⚠️⚠️ **LA TRAPPOLA IN CUI SONO CADUTO, e come si riconosce.** La prima versione chiedeva che il partner **VINCESSE** il confronto globale (`matchPartner(testo, tuttiIPartner)?.id === id`). Misurando prima di committare: svuotava a torto **quattro schede** — CHANEL ROMA e CHANEL FIRENZE (perdono contro CHANEL MILANO), Giada Cake Lab, e **VINCENZO D'ASCANIO (88 candidati → 0)**, dove «VINCENZO DASCANIO S.R.L.» **è proprio lui**: il token `VINCENZO` vale uguale per `MARYFLOR DI GERARDI VINCENZO`, e a parità di punteggio `matchPartner` tiene il primo dell'elenco. La regola giusta è **«il motore lo RICONOSCE»** (`matchPartner(testo, [partner])`), non «lo assegna a lui»: questo elenco è fatto di **sospetti da confermare** e ha il bottone per dire di no — perdere i movimenti veri è peggio che mostrarne qualcuno in più.
> - **In pagina**: i movimenti che la riconciliazione darebbe a un ALTRO partner non spariscono, si **dichiarano** — pillola ambra «per nome — il motore lo darebbe a *X*». Ed è nata una differenza importante nello stato vuoto: «non c'è niente» e «il nome non è riconoscibile» sono due cose diverse, e la seconda **non si risolve aspettando** — la riga lo dice, con il numero di movimenti scartati e la via d'uscita (attribuirne uno in riconciliazione, che impara la controparte).
> - **MISURA su tutti i partner**: candidati «per nome» da **3.209 → 1.671**, di cui **206 contesi** (dichiarati). 78 schede tengono candidati, 18 si svuotano — e quelle 18 sono partner il cui nome è fatto **solo di parole del mestiere**, che il motore non può riconoscere: per loro serve un'`AssociazioneControparte`, non un filtro più largo.
> - **Verificato in locale**: ARTE E FIORI → vuota con la spiegazione; VINCENZO D'ASCANIO → tiene i candidati, 20 marcati come contesi; CHANTILLITTY ed ECI → i 5 movimenti attribuiti ci sono tutti. `tsc` verde.
>
...
> - **«Nella scheda indica se un partner ha compensazione o meno»**: c'era, ma in fondo alla griglia delle condizioni. Ora è una **pillola sotto il nome** — «In compensazione» (blu) · «Senza compensazione» (grigia) · «Compensazione mai decisa» (ambra, **cliccabile**, porta a Modifica). Conta perché cambia come si leggono tutti i numeri della pagina: con la compensazione il mese è un saldo netto unico, senza sono due partite che non si toccano mai.
> - 🔴 **«Sotto è indicato in compensazione, quindi non dovrebbe apparire qui»** (riquadro «Contatto amministrativo», con «Invia sollecito» sulla 459/2026). Verificato: **la 459/2026 era già `compensata: true`** — e infatti «Salute del credito», dieci centimetri più su, diceva «Nessuna esposizione». La query di `fattureAperte` filtrava `eFatturaVera` **ma non `compensata`**, mentre `stato-credito.ts` quel filtro ce l'aveva dal 04/09: due metà della stessa pagina si contraddicevano. Aggiunto `compensata: false` **qui e in `/scadenzario`**, che aveva lo stesso buco. Una fattura in compensazione non si sollecita: non passa dalla banca. **Misura**: fra i 12 partner in compensazione ci sono **13 fatture aperte vere**, 6 già segnate `compensata` (che sparivano da «esposizione» ma restavano sollecitabili) e **7 non segnate, 2.893,50 € residui** — quelle restano visibili, vedi i punti aperti.
> - ⭐ **«Riconosci automaticamente… e se sono per commissioni e il partner ha compensazione, per definizione sono sempre saldate»** (card «Fatture su Fatture in Cloud»). Fatto: `FicFattura` porta ora l'**`oggetto`** del documento (`subject`/`visible_subject`, aggiunti ai `fields` della chiamata), e la tendina usa **lo stesso `eFatturaCommissioni` dell'import notturno** — due criteri diversi per la stessa domanda darebbero due risposte. Le righe riconosciute nascono con **«Fee vendor» già scelto** e la `descrizione` è l'oggetto vero («Commissioni Deluxy Agosto 2026») invece di «FIC 624/2026», così scattano anche le regole sulla tipologia. Se il partner è in compensazione lo stato diventa **«Saldata per compensazione»** invece di «Da incassare»: la commissione è già tolta dal dovuto sulle vendite, cioè pagata nel momento in cui è nata. ⚠️ **Su FIC il documento resta aperto** e la pillola lo dichiara nel suo titolo: qui si dice come stanno i conti, non si scrive su Fatture in Cloud.
> - ⚠️ **TRAPPOLA DELLA CACHE, da ricordare**: `ficFattureCached` è `unstable_cache` con `revalidate: 300`. Aggiungendo `oggetto` alla forma del dato, la voce già in cache non ce l'aveva e **per cinque minuti il riconoscimento sembrava semplicemente non funzionare** — nessun errore, solo un campo `undefined`. La chiave porta ora una versione (`fic-fatture-v2-oggetto`). **Quando cambia la FORMA di ciò che si mette in cache, cambia anche la chiave.**
> - **Verificato in locale sui dati veri** (142 RESTAURANT): le 6 fatture della card sono tutte riconosciute, tendina su «Fee vendor», stato «Saldata per compensazione», descrizione con l'oggetto FIC; il riquadro del contatto amministrativo dice «Nessuna fattura aperta»; in testata «In compensazione». BASARA → «Compensazione mai decisa», CLIVATI → «Senza compensazione». `tsc` verde.
>
> ### 08/09/2026 (3) — La compensazione ha tre stati: sì, no, e «mai deciso»
>
> Domanda dell'utente: «hai l'informazione se il partner vuole andare in compensazione per vendite oppure no?». La risposta misurata: **c'è il campo, non c'è l'informazione.**
> - **Censimento della fonte** (`prisma/seed-data.json`, estratto da PARTNER.xlsx il 16/07/2026): la colonna `compensazione` era valorizzata per **8 partner su 92** (6 sì, 2 no) — **84 celle bianche**. Idem `commissioniADetrazione` (6/2/84). `addebitoDiretto` e `cartaCreditoApp`: **92 celle bianche su 92**. Nel DB i sì sono 12 (i 6 del foglio più 6 accesi a mano dopo). **Non c'era niente da importare**: l'import aveva già preso tutto.
> - **Il difetto**: `compensazione` è un booleano con default `false`, quindi «no, deciso» e «nessuno l'ha mai chiesto» erano la stessa cosa a schermo. Su 119 schede, **106 stavano sul valore di partenza** — e **60 di quelle hanno vendite vendor**, cioè per loro la compensazione è una domanda vera rimasta senza risposta. È lo stesso difetto dell'IBAN del 05/09: «non ce l'ha» e «non lo so» portano a due azioni diverse.
> - **Fatto** (scelta dell'utente fra tre opzioni: «campo a tre stati»): colonna nuova **`Partner.compensazioneDecisa`** (`Boolean @default(false)`). `compensazione` resta il booleano che usa il motore di calcolo — «mai deciso» si comporta come «no», i conti non cambiano di un centesimo — e la colonna nuova distingue le due cose. Nel modulo partner la spunta è diventata **tre radio** (Mai deciso · Sì · No), un solo campo `compensazioneScelta`; la scheda mostra la pillola «mai deciso» al posto di «No»; l'elenco ha il filtro **«Compensazione mai decisa · con vendite»**; `GET /api/clienti/stato` espone `condizioni.compensazioneDecisa` per le altre app.
> - **Riempimento** (`scripts/segna-compensazione-decisa.mjs`, con prova a secco): segnati come decisi **13** partner, e solo dove la scelta è DIMOSTRABILE — il flag acceso (il default è spento: un `true` ci è arrivato dal foglio o da una mano) oppure il «no» esplicito del foglio (CLIVATI). Gli altri **106 restano «mai deciso»**, che è la verità. Non si deduce dai numeri: è un patto commerciale.
> - ⚠️⚠️ **TRAPPOLA GRAVE EVITATA: NON usare `prisma db push` su questo database.** `prisma migrate diff` prima di scrivere ha mostrato che, oltre alla colonna, avrebbe eseguito **`DROP TABLE "EsclusioneMovimentoPartner"` e `DROP TABLE "PartnerPivaEntita"`**: due tabelle che esistono nel Postgres ma **non stanno in `schema.prisma`** (la prima la legge la scheda partner via `$queryRaw` — è quella dei movimenti esclusi a mano). La colonna è stata aggiunta con un `ALTER TABLE ... ADD COLUMN IF NOT EXISTS` a mano, poi `prisma generate`; verificato dopo che le due tabelle ci sono ancora. **Prima di qualunque modifica allo schema di quest'app: `npx prisma migrate diff --from-schema-datasource prisma/schema.prisma --to-schema-datamodel prisma/schema.prisma --script` e leggere cosa c'è oltre a quello che ci si aspetta.** (E il dev server va fermato prima, o Prisma non riesce a riscrivere la DLL su Windows.)
> - **Da fare**: le 60 schede che vendono e non hanno una scelta si trovano in `/partner?stato=comp-mai-decisa`. La decisione è dell'utente, una per una.
>
...
> ### 🔖 PUNTI APERTI — aggiornati al 05/09/2026 (il 5 è chiuso: deroga confermata) (tutto il resto è già in produzione)
>
> Sono usciti lavorando su GIADA CAKE. Il codice della sera è **live** (`cp4unalaj`): questi sono i pezzi **non fatti apposta**, in attesa di una decisione.
>
> 1. **I due bottoni nel riquadro ambra della scheda partner.** Il riquadro elenca le fatture senza documento e dice che vanno emesse o tolte, ma non dà modo di farlo: si deve aprire la fattura. Proposta: per ogni riga **«Emetti su Fatture in Cloud»** (apre l'emissione già compilata; la strada esiste: `/fic/emetti`, `/fic/fattura`, `ficCreaFattura`; a emissione fatta il numero torna sulla riga e la riga rientra nei conti) e **«Elimina»** (la conferma a due passaggi ora c'è già sulla scheda della fattura). Prevenzione: chi vuole SEGNARSI una fee da fatturare dovrebbe fare una **pro-forma** — ha già «Emetti su FIC» e «Annulla» — non una fattura senza numero; nel modulo basterebbe dirlo quando il numero è vuoto.
> 2. **Il numero 105/2025 da scrivere sulla riga di ADOLFO STEFANELLI (TASTE 20), febbraio 2025, 18,52 €** (verificato il 04/09: su FIC è di TASTE 2.0 S.R.L., 04/03/2025, incassata; nessuna riga di Finance porta quel numero, quindi non è un doppione; il 05/09 la riga è ancora senza numero). ⚠️ Di fianco: la **104/2025 da 230,76 €**, stesso cliente e stesso giorno, **in Finance non esiste affatto** — quel partner ha 14 fatture su FIC nel 2025 e 6 righe qui. Il backfill di FIC guardava indietro 245 giorni: l'inizio 2025 non è mai entrato.
> 3. **Le 115 righe dello storico «Import PARTNER.xlsx»**, che oggi restano dentro i conti per scelta dichiarata in `fattura-vera.ts`. Numeri per decidere: 7.429,35 € di imponibile, ma **90 righe su 115 valgono 0,00 €** e solo **1.611,10 €** non è pagato; 15 partner hanno importi diversi da zero (Taveggia 1.348,73 · Martesana 747,43 · Fusto 619,23). Se devono uscire si cancella UNA riga di `eFatturaVera`. Fuori dai conti oggi restano solo **2 righe**: Fiori Rimini 950 € e Bottega dei Fiori Firenze 300 € (quella di GIADA CAKE l'utente l'ha eliminata il 04/09 alle 22:06).
> 4. **I candidati «per nome» che pescano gli omonimi.** `PAROLE_GENERICHE` in `riconciliazione.ts` non contiene CAKE/CAKES/TORTE/BAKERY, e la scheda partner confronta per SOTTOSTRINGA accontentandosi di UNA parola, mentre `matchPartner` vuole parole intere + una parola lunga o due parole. Misurato: i candidati passerebbero da **3.290 a 1.684**; ARMANI FIORI da 284 a 1 (la sua parola «ALIA» sta dentro «ITALIA»), FLOR da 176 a 0, Arte e Fiori da 103 a 0. Da applicare **misurando prima** quanti candidati veri si perdono.
> 5. ✅ **CHIUSO il 05/09/2026 — deroga UX CONFERMATA dall'utente** («confermo deroga»): nella scheda partner i movimenti si aprono in una finestra invece che in una pagina, in deroga al §8 del Libro. Scritta in `README.md` dell'app (sezione «Deroghe al Libro UX&UI (approvate)») con dove vale, perché, e le condizioni rispettate del §9. Vale SOLO per le due tabelle di movimenti della scheda partner: negli altri elenchi la riga continua a portare a una pagina. Se un domani lo schema vale per tutti gli elenchi che si scorrono per confronto, va scritto nel Libro e smette di essere una deroga.
> 6. **«Città / area servita» nella scheda partner** (chiesto il 05/09): un campo di testo libero scritto a mano, vuoto per 4 partner su 119, e che **mescola due dati con due padroni**: la CITTÀ (identità → registro Anagrafiche, che Finance già legge) e l'AREA SERVITA (le province → piattaforma consegne, tabella `PartnerProvince`). I 35 valori attuali lo mostrano: «Milano» 53 volte ma anche «MILANO» 5, e aree scritte a mano come «Milano / Roma». La piattaforma espone già tutto: **`GET /api/v1/app/partner`** con chiave a scope, sola lettura, torna id, insegna, città e le **sigle delle province servite**. Servono: la chiave + l'URL della piattaforma nel `.env` di Finance (oggi ci sono Anagrafiche, Orders, Budgets, Tasks, Transactions, non la piattaforma); il join **Finance → `anagraficaId` → registro → `platformId` → piattaforma** (per nome sarebbe fragile); e mostrare senza copiare (Standard §7). ⚠️ Prima di tutto: verificare se GIADA CAKE **esiste** sulla piattaforma — è nato dal registro il 03/09 via Scout; se non c'è, l'area servita resta vuota comunque e il posto dove riempirla è la piattaforma.
> 7. **L'Artifact del manuale non è stato ripubblicato.** `docs/COME-FUNZIONA-FINANCE.html` ha quattro voci nuove del 04–05/09 (finestra dei movimenti, tipologia dalla descrizione, fattura vera solo se sta su FIC, elenco «fattura o vendita», IBAN dal registro, fine del 404): il file nel repo è aggiornato, l'artifact allo stesso URL no.
...
> - **Da dove venivano i 450 €**: riga `FatturaServizio` scritta A MANO in Finance da Renato Cassoli il 03/08/2026 alle 16:07 (registro: «Creata fattura servizi s.n. (450,00 €)»). Competenza luglio 2026, descrizione «Fee affiliazione Deluxy», 450 € + IVA 22 % = 549 €, non pagata, **senza numero, senza emissione e senza scadenza**. Non arriva da Fatture in Cloud e non è calcolata da nessuna formula.
> - ❌ **Su FIC quella fattura NON è mai stata emessa** (chiesto dall'utente, verificato sul token vero, non dedotto): fra le **fatture emesse 2026** nessun cliente contiene «giada» — l'unico simile è «CAKE LAB DI STEFANIA MANNELLI», che è un altro partner; le fatture da 549 € lordi / 450 € netti del 2026 sono di MARTESANA, EZI, IL CHIOSCO, RECARLO, ENRICO RIZZI. Quindi in Finance c'è un ricavo che **nessun documento sostiene**; e senza scadenza non entra nemmeno nello scadenzario, quindi nessuno la sollecita. ⚠️ Da decidere con l'utente: emetterla su FIC o cancellarla. **Non toccata.**
> - ✅ **Tipologia riparata**: era «Consegne» con descrizione «Fee affiliazione Deluxy» — le altre tre fee di affiliazione di luglio (LIJOI 1.000 con n. 536/2026, Fiori Rimini 950, Bottega dei Fiori 300) stavano già su «Affiliazioni». Spostata con `scripts/ripara-tipologia-affiliazioni.mjs --esegui` (prova a secco prima; riga nel registro modifiche con la tipologia di partenza, per tornare indietro). Ricontato dopo: **0** righe con «affiliazione» fuori posto, e **0** righe in tutta l'app la cui descrizione nomina una tipologia diversa dalla sua.
> - ⭐ **La regola è entrata nel codice**, così non si ripresenta: nuovo `src/lib/tipologie.ts` (`tipologiaNominata`, `tipologiaDaDescrizione`, `tipologiaDaUsare`) — un punto solo, perché le fatture nascono da due strade. Usato da `createFattura`/`updateFattura` (la descrizione vince sulla tendina) e da `fic-mancanti.ts`, dove la tipologia si IMPARA dall'ultima fattura del partner: **ed è proprio lì che una fee di affiliazione ereditava «Consegne» da una consegna**. La correzione non è muta: finisce nel `dettaglio` della riga di registro («Tipologia messa su X al posto di Y: la descrizione nomina l'affiliazione»).
> - ⚠️ **Una parola sola nell'elenco, di proposito**: `affiliazion`. «Consegne» NON ci va — compare nelle descrizioni di mezzo mondo e trascinerebbe righe giuste nella voce sbagliata. Quando la parola non c'è, decide la persona.
> - **Verifica**: `tsc` verde; la funzione provata sui casi veri — «Fee affiliazione Deluxy» e «Fee di affiliazione annuale» → Affiliazioni, «Servizi Deluxy Luglio» e descrizione vuota → nessuna correzione; con la tendina già giusta non tocca niente; con «Consegne di luglio» su Consegne lascia com'era.
> - ℹ️ **7 fatture servizio del 2026 su 289 sono senza numero**: nessun documento vero dietro. Da guardare a parte.

> ### 04/09/2026 (21:10) — Il dettaglio del movimento si apre in una FINESTRA, non cambiando pagina (chiesto dall'utente)
>
> «Il dettaglio di una transazione lo apri con un pop-up non cambiando pagina». Sostituisce la navigazione messa in piedi mezz'ora prima (blocco sotto): nella scheda partner i movimenti si guardano uno dopo l'altro per capire quali sono del partner, e cambiare pagina fa perdere il posto nell'elenco.
> - **Nuovo componente `src/components/MovimentoModale.tsx`** (client): `RigaMovimento` (la `<tr>` cliccabile + la finestra) e `ApriDettaglio` (il testo-link dentro la riga, che serve alla TASTIERA: una riga cliccabile col Tab non si raggiunge). La finestra esce da un **portale su `document.body`**: dentro un `<td>` erediterebbe gli overflow della tabella e verrebbe tagliata.
> - **Contenuto**: lo stesso record di `/movimenti/[id]` — data, importo+divisa, controparte, IBAN, causale per intero, fonte, data d'archivio, stato in riconciliazione con l'esito, partner collegato, categoria di costo e chi l'ha decisa. Nel piede: «Apri la scheda intera →» (dove ci sono gli altri movimenti della stessa controparte, che nella finestra non ci sono) e «Cambia/Assegna categoria in Spese →».
> - ⭐ **La finestra dice a chiaro quando il movimento NON è del partner**: «nessuno — questo movimento non è attribuito a nessun partner: compare in questa scheda solo perché la controparte somiglia al nome». È il caso GIADA CAKE del blocco sotto: in elenco il badge «per nome — da confermare» si legge di sfuggita, qui la frase è intera.
> - **Regole del Libro v1.7 §9 rispettate** (le classi `modal-*` esistevano già, collaudate a 375×812 e 1366×768): ✕ obbligatoria in testata sticky, ESC, click sullo scrim, tetto d'altezza, corpo che scorre. In più — e `RiconciliaModale` non le ha — **fuoco che entra nella finestra, non ne esce col Tab e torna al punto di partenza** alla chiusura (ARIA APG, il Libro lo chiede: «0 modali su 10 app lo fanno»).
> - **Dati**: la query della scheda ora prende i campi che servono alla finestra (`MOVIMENTO_SELECT`, condiviso dalle due tabelle) — dieci righe già caricate, nessuna seconda query al click. Il colore/nome del tipo di costo li calcola il SERVER e li passa già pronti: `TIPI_PL` sta in un file che parla con Budgets e non deve finire in un componente client. Nuova classe `.btn-testo` in `globals.css`.
> - **Verifica reale, e cosa NON è stato verificato**: `tsc` verde; `next build` verde. Il componente è stato collaudato **da solo**, su una pagina di prova temporanea poi cancellata (`/prova-finestra`): click sull'importo → finestra aperta **sotto `<body>`** con tutte e 11 le righe di dettaglio e il piede giusto, **URL invariato**; ESC chiude; click sullo scrim chiude; click DENTRO non chiude; click sul bottone «Non è di questo partner» **non** apre la finestra; il testo-link apre. La scheda del partner rende bene lato server (10 righe `riga-link`, 20 `btn-testo`). ⚠️ **Il click sulla scheda vera non è stato provato**: nel pannello browser di questa sessione (nascosto, `visibilityState: "hidden"`) le pagine grandi **non completano l'idratazione** — il flusso RSC si ferma a 12 pezzi e resta idratata solo la shell. Non è colpa di questa modifica: succede uguale su **`/fatture`**, che non è stata toccata, e lì restano morti anche i bottoni preesistenti. Una persona lo conferma con un click.
> - ⚠️ **Da far ratificare al custode del layout** (`architetto-ux`, registro `deluxy-design-system/SEGNALAZIONI-UX.md` — che vive sul branch della piattaforma, non su questo): il Libro §8 dice «la riga si apre col click» e in tutta l'app la riga PORTA A UNA PAGINA. Qui, su richiesta dell'utente, la riga apre una finestra: o diventa una regola nuova («elenco che si scorre per confronto → finestra»), o resta una deroga scritta nel README dell'app.

> ### 04/09/2026 (20:30) — Scheda partner: la riga dei movimenti bancari si apre col click (⚠️ SUPERATO alle 21:10: ora apre una finestra, non la pagina)
>
...
> ⚠️⚠️ **L'ALIAS `deluxy-partner.vercel.app` NON SEGUE I DEPLOY.** Dopo il deploy il dominio serviva ancora `dpl_BorQpaSLyu9bB9AMePhFdt8t15c9` = deployment `hi23pu4ge` del **03/09 18:20** — cioè nemmeno `i37xrfvpi` (19:57, il fix dei font del PDF pro-forma) era mai stato live sul dominio: «prod = 9f3274b3» scritto ieri era vero per `vercel ls`, falso per chi apre l'app. L'alias automatico del progetto è `deluxy-partner-deluxy.vercel.app`; `deluxy-partner.vercel.app` è un alias messo a mano che resta dov'è. Sistemato con `npx vercel@latest alias set https://deluxy-partner-5od7ydnyq-deluxy.vercel.app deluxy-partner.vercel.app`; verificato: `/login` serve `dpl=dpl_59JEJd8U…` (id di `5od7ydnyq`). **Da oggi ogni deploy di Finance finisce con `alias set` + `vercel inspect https://deluxy-partner.vercel.app` per leggere l'id servito** — `vercel ls` che dice «Production Ready» non basta. Con questo il webhook di produzione conosce le richieste multi-mese: si può pagare la 48,30 senza la finestra descritta sotto. Restano all'utente: chiudere le 7 «pagate fuori dall'app» su Transactions, poi «Paga il netto 48,30 €» da ANTOFLOWERS.
> ✅ **17:05 — `TRX-2026-000049` ANNULLATA** («Annulla tu» dell'utente) con il nuovo script `scripts/ritira-richiesta-transactions.mjs <TRX> "<motivo>" --esegui` (prova a secco senza `--esegui`; usa chiave e firma di Finance, funziona solo su `in_attesa`/`sospesa`). Risposta 200, stato `annullata`.
>
...
> - Alle 13:15 Renato ha premuto «Paga» su **agosto 2026** di ANTOFLOWERS (una vendita da 245 € → commissione 49 + IVA = 59,78 → dovuto **185,22 €**). Finance ha mandato a Transactions `TRX-2026-000049` per 185,22 €: Transactions l'ha registrata fedelmente (`riferimentoEsterno saldo-…-2026-08`, sigillo v2).
> - Il **48,30** è il totale dell'anno della scheda («Da bonificare 48,30 € (netto)»): il partner è in **compensazione**; giugno +196,56, luglio +71,82, agosto +185,22 (Deluxy deve 453,60) contro aprile −71,52 e maggio −333,78 (il partner deve 405,30). Il bottone del mese ignorava tutto questo e mandava il lordo del mese: due cifre per lo stesso dovuto, e la più alta era in coda per il pagamento.
> - **Correzione (codice, tsc verde, NON ancora deployata)**: `richiediPagamento` (`src/lib/pagamenti-partner-actions.ts`) per un partner in compensazione calcola le partite aperte dell'anno (`src/lib/saldo-netto.ts`, sui 12 mesi come il rolling della scheda) e chiede il **netto**; l'`importo` passato dal bottone vale solo senza compensazione. Causale «Saldo netto 2026 - <partner>», nota con la spiegazione mese per mese; `riferimentoEsterno` resta `saldo-<id>-<anno>-<mm>[-rN]` sul mese premuto (compatibile col webhook vecchio). **Tutti i mesi coinvolti** ricevono lo stesso `richiestaRif`/stato (mostrano «in corso», niente seconda richiesta da giugno o luglio); se un altro mese ha già una richiesta in corso si rifiuta con messaggio; se il netto è ≤ 0 si rifiuta («il partner deve ancora X»). Webhook `/api/pagamenti/notifica`: i mesi della richiesta sono quelli col suo `richiestaRif` (fallback: il mese del riferimento esterno); alla `pagata` di una richiesta multi-mese ogni mese riceve `bonificoImporto += delta` (delta = daBonificare − daIncassare calcolato al momento) così tutti chiudono; se la somma non torna col pagato lo scrive nel registro con ⚠️. UI: `PagamentoMese` e la dashboard mostrano «Paga il netto 48,30 €» con la riga «non i 185,22 € del mese», e non mostrano il bottone se il netto ≤ 0.
> - 🔴 **La richiesta sbagliata `TRX-2026-000049` (185,22 €) è ANCORA `in_attesa` su Transactions**: l'annullo via API (chiave di Finance, `POST /api/v1/richieste/TRX-2026-000049/annulla`) è stato bloccato dal permesso della sessione. Va annullata da https://deluxy-transactions.vercel.app/richieste (dettaglio → «annulla», motivo: importo lordo del mese al posto del netto). Il webhook rimette agosto «rifacibile»; poi da Finance (scheda partner, agosto) «Paga il netto 48,30 €». Finché non è annullata, NON pagarla.
> - ⚠️ Finestra tra invio e deploy: se la nuova richiesta parte dal server locale (3040 sul DB di produzione) prima del deploy, il webhook arriva al Finance di produzione col codice vecchio, che alla `pagata` scriverebbe 48,30 solo su agosto. Deployare prima di pagare.
> - **Verifica delle richieste precedenti (chiesta dall'utente, 16:00)** — confrontate tutte le 17 richieste aperte di Finance su Transactions con i dati di oggi:
>   - **Solo ANTOFLOWERS aveva mesi a debito** che abbassano il netto: le altre 6 richieste di partner in compensazione (142 RESTAURANT, TASTE, AMIR, ANGOLO FIORITO, ARTE E FIORI) portano il mese giusto; i loro mesi di agosto non richiesti restano da chiedere a parte. Per questo la regola è diventata: **i mesi con una richiesta già in corso si ESCLUDONO dal netto** (prima la mia versione delle 15:30 rifiutava con «c'è già una richiesta in corso»: avrebbe bloccato agosto su 5 partner).
>   - 🔴🔴 **7 richieste in attesa su Transactions sono di mesi GIÀ SEGNATI PAGATI A MANO in Finance** («Registrato bonifico al partner … scheda partner», quindi bonifico fatto dal portale della banca): `TRX-2026-000004` CHANTILLITTY giu 249,28 (24/08) · `-000005` TASTE giu 484,00 (05/08) · `-000006` AMIR giu 9,83 (27/08) · `-000007` ANGOLO FIORITO giu 2.336,59 (21/08, «Paga rapido») · `-000008` FIORISTA TONINO mag 942,42 (05/08) · `-000009` FIORISTA TONINO apr 686,31 (05/08) · `-000011` CAKELAB giu 85,48 (24/08, registrato 9 volte = 769,32: sono i bonifici gemelli dello script 2). **Se qualcuno le paga da Transactions, paga due volte.** Vanno chiuse in Transactions come «già pagata fuori dall'app» con la data del bonifico (dal dettaglio; l'annullo via API da qui è bloccato dal classificatore). Il webhook non riscrive il bonifico (c'è già).
>   - `TRX-2026-000013` TASTE luglio: mandati 347,48, oggi il mese vale 351,08 (aggiunta di 3,60 € dopo la richiesta): pagherebbe 3,60 in meno, da sapere.
>   - `TRX-2026-000012` FAG TORINO aprile: 494,28 contro 494,29 di oggi, arrotondamento.
> - Non toccato: la dashboard somma ancora i lordi nel KPI «Bonifici da fare ai partner» (per i partner in compensazione sovrastima); il mese di **maggio** ha la fattura 386/2026 `compensata` e non pagata: il netto la conta per intero (488 €), coerente con la nota «debito 405,3 € riportato ai prossimi mesi».
...
> - 🔴→✅ **Il primo deploy crashava su Vercel** («butta giù l'app», segnalato dall'utente sul PDF di PF 33/2026): in locale il PDF usciva, in produzione la funzione moriva. Causa: i font standard sono file `.afm` in `node_modules/pdfkit/js/data` letti a runtime con `readFileSync` — il tracciamento di Next non li includeva (l'nft.json della rotta aveva l'ICC ma **zero .afm**). Fix: `outputFileTracingIncludes` per `/proforma/[id]/pdf` in `next.config.ts`. Regola da ricordare: **con `serverExternalPackages` i file letti a runtime dal pacchetto vanno inclusi a mano**, e si verifica sull'`.nft.json` della rotta prima del deploy, non dopo.
> - **«Emetti su FIC…» anche in alto a destra** (chiesto il 02/09): nella barra azioni della scheda, accanto a Stampa/Scarica PDF, finché il documento non ha una fattura vera collegata (non annullato).
...
> 🔴 **Difetto 1 — il cron FIC importa la fattura COMMISSIONI come servizio.** `460/2026 «Commissioni Deluxy Giugno 2026»` (3,75 € = 15 % di 25 €) è già `commFattNumero` del `SaldoMensile`; il 31/08 `fic-mancanti` l'ha registrata come `FatturaServizio` «Consegne» (tipologia imparata). Risultato: commissione **sottratta nel dovuto E sommata ai servizi ivati** (+4,58 €) → saldo sbagliato e fatturato per tipologia gonfiato (Budgets lo legge). Da fare: escludere dall'import le fatture il cui numero è un `commFattNumero` o la cui descrizione inizia per «Commissioni Deluxy» (e ripulire quelle già entrate — censimento da fare).
> 🔴 **Difetto 2 — bonifico registrato due volte.** Registro: «Registrato bonifico al partner 5,79 € (Giugno 2026)» alle 13:07:29 **e** 13:07:34 del 24/08 (doppio clic) → `bonificoImporto` 11,58. Il «da incassare 5,79 €» di oggi è tutto lì. La richiesta vera è TRX-2026-000003 (5,79 €, «in_lotto» dal 31/07). Da fare: bottone «Registra bonifico» con pending/disable (Libro UX) e/o idempotenza sul saldo. **Nulla corretto: solo diagnosi**, in attesa della decisione dell'utente.
> ℹ️ Le voci 31/08 (competenza = mese del servizio, import FIC automatico + `/fatture/da-fic`, audit doppioni 4/194) sono nei commit `49245439`, `8387aba7`, `8790bf78` e nella memoria di progetto: qui mancava il diario.
...
> - **Allineamento UI al Design System** (audit agente `architetto-ux` vs DS v1.4 + peer Hub/Orders/Anagrafiche): FINANCE era già molto allineato (token identici, sidebar/tabelle/badge a norma). Unica divergenza grossa: la **navigazione mobile** era una fascia orizzontale che andava a capo → ora è il pattern canonico (Libro §2): **topbar vetro 56px con hamburger + drawer off-canvas da sinistra + velo `--scrim`** (chiude al tocco e a ogni navigazione), stato `apertaMobile` in `Sidebar.tsx` distinto dal collasso-a-icone desktop (`chiusa`). Micro-fix: **logout** non più `solo-estesa` (resta a sidebar collassata, Libro §1; footer collassato in colonna); **`aria-current="page"`** sulla voce attiva. Igiene token: badge `--*-soft`, gradiente logo `--logo-dark-a/b`, banner sola-lettura `--orange-soft` (valori identici agli rgba di prima). Deroghe LASCIATE (a norma peer): shell senza topbar desktop (modello AI Mail), classi in inglese, input a bordo+anello oro. ⚠️ Drawer da verificare sul telefono (è dietro il login).
