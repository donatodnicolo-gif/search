...
## Chi possiede i soldi degli ordini (30/07/2026)

**I movimenti bancari sono di Finance (`deluxy-partner/`), il controllo degli
ordini è di Orders (`deluxy-orders/`).** Finance espone l'estratto conto in sola
lettura (`GET /api/v1/movimenti`) e il controllo che ha già fatto
(`GET /api/v1/ordini-controllo`); Orders se li legge, tiene uno specchio e decide
**a quale ordine appartiene** ciascun incasso e ciascun pagamento al fornitore —
da cui le pagine `/controllo` e `/margini`. Orders **non scrive** in Finance.
Non duplicare questa logica in una terza app.
...
> ⚠️ Da non confondere con l'app **[deluxy-scripts](deluxy-scripts/README.md)** (porta 3170): lì «script» vuol dire **copione commerciale** — i testi di vendita, inviti e presentazioni che si mandano ai clienti. Gli script di codice stanno qui, in questo catalogo.
