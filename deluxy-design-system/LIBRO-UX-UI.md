...
- **Campi a valore corto** (ora, numero, importo) — v1.11: **larghezza fissa pari al valore** (`time` 112 px, `number` 88 px), mai `1fr`; **etichetta ≤ 22 caratteri su una riga**; le righe di campi sono una griglia a celle fisse (150 px, gap 14×16) con **`align-items: end`**, così gli input della stessa riga stanno allineati al pixel anche se un'etichetta è più lunga; le spiegazioni vanno in **un solo `testo-guida` per blocco**, non sotto ogni campo. *Perché*: un input di 300 px per «08:00» sposta l'occhio e un'etichetta a tre righe disallinea gli input. *Misura*: nessuna label che va a capo a 1440 px, input della stessa riga allineati in basso. Riferimento: Customer Service, scheda «Orari negozi» (`.riga-campi`). **Anteprima di un form** (il risultato calcolato dai valori scritti): è il feedback, non contenuto — sopra i 1100 px sta in una colonna laterale `sticky` (360 px), sotto si impila dopo i campi.
...
8-bis. **Ogni pagina di elenco ha ricerca, filtri principali e le scorciatoie di periodo** *(v1.9, 28/08/2026 — regola dell'utente, da Finance/fatture che non aveva né ricerca né periodo rapido)*: **(a)** una **ricerca testuale** sui campi con cui l'operatore riconosce il record (nome, numero, controparte) — un elenco senza ricerca si scorre a occhio, e a 200 righe è un lavoro; **(b)** i **filtri principali** utili a quella pagina (stato, tipologia, controparte…), composti secondo la zona filtri qui sopra; **(c)** se i record hanno una data, le **scorciatoie di periodo**: `Mese in corso · Mese scorso · Trimestre · Anno` (+ «Sempre/Tutto» come azzeramento) — chip a selezione singola, MAI un doppio campo data come unica via (le date libere restano come opzione avanzata dove già ci sono). Sul web lo stato va nell'URL (§8); il periodo è UN parametro (`periodo=mese|scorso|trimestre|anno`), non quattro date ricopiate. Riferimenti: Scout `ordini` (gruppo Periodo), Marketing `SceltaPeriodo`, e per le pagine server-rendered Finance `/fatture` (chips come link GET).
