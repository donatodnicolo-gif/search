...
|---|---|---|---|
| 11/09 | **Hub** (per tutte le app che leggono la cassaforte) | ⚠️ **`GET /api/chiavi` può costare 12–20 s A FREDDO, poi 0,13 s.** Misure (`curl` da Windows su deluxy-hub.vercel.app): tre letture consecutive **20,11 · 20,10 · 12,00 s**, e mezz'ora dopo cinque letture di fila **0,15 · 0,13 · 0,12 · 0,14 · 0,13 s**. ⚠️ **Correzione di una mia diagnosi sbagliata**: avevo scritto «non è cold start, perché `/api/health` risponde in 0,12 s» — non regge, sono due lambda diverse e quella della salute era calda. Quindi la lettura onesta è: la rotta è veloce quando è calda, e la prima chiamata dopo il riposo costa **fino a 20 s** (avvio della lambda + client Prisma + connessione al pooler; la decifratura per riga resta da misurare, non da accusare). **Perché riguarda tutti**: ogni app con `HUB_KEYS_TOKEN` chiama questa rotta con timeout 4 s, quindi a ogni risveglio del Hub paga 4 s buttati — misurato sul CRM l'11/09 appena gli è stato dato il token: `/` 7,07 s · `/clienti` 9,43 s · `/reclami` 8,53 s. **Da decidere in casa Hub**: se i 20 s a freddo siano normali per quella lambda (e allora le app devono aspettarseli), o se ci sia dentro qualcosa che si può evitare | CRM, 11/09 |
| 11/09 | **CRM** | ✅ **APPLICATO — la lentezza del Hub resa innocua, senza smettere di leggere la cassaforte** (§7 resta). `src/lib/chiavi-app.ts`: (1) **una sola chiamata in volo** — le pagine che chiedono due chiavi insieme ne aprivano due, e pagavano il timeout **due volte**; (2) **si ricorda anche il buco** per 60 s (prima, dopo un fallimento, la pagina dopo ripagava i 4 s interi: la cache si scriveva solo in caso di successo). Misura **prima → dopo**, stesso dev server, pagine calde: `/` 7,07 → **0,37 s** · `/clienti` 9,43 → **1,05 s** · `/reclami` 8,53 → **0,81 s** · `/ricorrenze` 13,11 → **1,74 s**. ⚠️ La causa vera resta la riga qui sopra (il Hub a freddo): qui si è tolto il danno, non il difetto — e con la cache del buco un risveglio del Hub costa 4 s **una volta**, non 4 s a pagina | CRM, 11/09 |
