...
|---|---|---|---|
| NUOVA DIFESA | Hub ↔ CRM | **API utenti del Hub scrivibile con token di servizio** (`/api/utenti?app=crm`, 11/09/2026, richiesta utente «creare utenti da Impostazioni del CRM»). Chi ha il token del CRM (scope `crm`) può: leggere l'elenco utenti del Hub (nome, email, ruolo, attivo, ultimo accesso — niente hash), creare un utente NON admin già abilitato al CRM (password ≥ 8, salata dal Hub), aggiungere/togliere SOLO `crm` da `appAbilitate` di un non-admin. Confini scritti nel codice: 403 su admin, mai ruolo/password/attivo di un esistente, scope obbligatorio. Percorso da valutare: un token `crm` rubato = un insider crea un commerciale che entra nel CRM (non in altre app). Da decidere: basta lo scope, o serve un flag `scrittura` sul `TokenApi`? Nel CRM l'azione è per admin del Hub (SSO) o con la password di team | cap. 3, 4 |
| NUOVA DIFESA | Piattaforma consegne | **Cambio IBAN dal profilo partner, con codice per email** (regola utente 08/09/2026). ⚠️ **L'ostile ha demolito la prima versione: tre falle confermate, tutte chiuse in `c3ae0eda`.** (1) Il destinatario del codice lo sceglieva l'attaccante: `Partner.email` e' nella whitelist del profilo del partner, quindi bastavano due salvataggi sulla stessa schermata — riscrivo il recapito, chiedo il codice, me lo trovo in casella — e l'IBAN fraudolento arrivava fino ad Anagrafiche da cui FINANCE paga. (2) `PUT /partners/:id` scriveva l'IBAN per PROJECT_MANAGER/OPERATION/ADMIN scavalcando codice, checksum e avvisi. (3) Il tetto dei 5 tentativi era aggirabile due volte (l'annullamento azzerava la pausa di 60 s; il contatore letto-e-riscritto non reggeva il parallelo, e l'app non ha throttler). Toppe: blocco di 7 giorni dopo un cambio di recapito, codice a TUTTI gli indirizzi noti (incluso il precedente), avviso all'ufficio gia' sulla richiesta, IBAN fuori dalla rotta generica per ogni ruolo, `bankCodeSentAt` che sopravvive all'annullamento, contatore atomico con `increment`. 🟡 Secondo giro dell'ostile sulle toppe in corso. | cap. 1, 3, 11 |
| ✅ CONCESSO 09/09 | Customer Service ↔ Anagrafiche | **Chiedo l'ambito «Dati finanziari» per la chiave `deluxy-messaging`, e non me lo prendo da solo.** Il CS **scrive** IBAN e intestatario nel registro a ogni pagamento verificato (`registro-fornitori.ts:371`, `scritturaPartner: true`) ma non può **rileggerli**: `leggeDatiFinanziari` è `false`. Conseguenza misurata il 09/09/2026: il registro ha **80 IBAN distinti**, i pagamenti del CS ne conoscono 70, **24 stanno solo nel registro** (Enrico Rizzi Milano, Il Fiore Di Fabula, FAG Torino Fiori, MARYFLOR, Angolo Fiorito, Mastrofioraio…) e per tutti il modulo dice «l'IBAN va scritto a mano» pur avendolo in casa — cioè si ribattono a mano coordinate bancarie, che è la fonte di errore da cui quella funzione era nata. ⚠️⚠️ **Il rischio da pesare è vero e va detto**: l'ambito fu ristretto il 27/08/2026 con la motivazione «col registro usato anche per i FORNITORI — cioè per chi paghiamo — è una superficie da frode, e camminando sulle pagine si raccoglieva l'intero registro». Concederlo al CS rimette 80 IBAN dietro una chiave in più. **Mitigazioni già scritte nel codice del CS (09/09), che rendono l'ambito il solo interruttore mancante**: (a) `partnerAttivi` ha `conDatiFinanziari`, **predefinito a falso**, e `normalizza` BUTTA i due campi se non è stato chiesto — seconda serratura dopo quella del registro; (b) lo chiede **una sola rotta**, `/api/fornitori/cerca`, che gira sul server ed è il modulo da cui parte un bonifico; (c) verificato sull'app in funzione che `/api/partner` (tendina dei reclami, risposta che finisce nel browser) restituisce **96 partner e 0 IBAN**; (d) l'IBAN del registro non scavalca mai quello provato da un bonifico, non riempie il rifiuto deliberato sugli IBAN discordi, e a schermo si legge **da dove viene** (prova: `scripts/prova-iban-da-registro.mts`, 6 casi). Senza l'ambito il codice è **inerte**: verificato oggi che «FAG Torino Fiori» torna ancora con IBAN vuoto. ✅ **CONCESSO il 09/09/2026 su ok esplicito dell'utente** («si fai pure»): `leggeDatiFinanziari = true` sulla chiave `deluxy-messaging`. Ora l'ambito ce l'hanno tre chiavi su undici — `deluxy-delivery`, `deluxy-messaging`, `deluxy-partner` — cioè le tre app che hanno a che fare col denaro dei fornitori. ⚠️ Concesso con un `UPDATE` diretto sulla tabella `ApiKey` del registro (non esiste una rotta per farlo): se un domani nasce una pagina di amministrazione delle chiavi, quel cambio non ha lasciato una riga nel registro modifiche di Anagrafiche — sta qui. **Verificato sull'app pubblicata** (`deluxy-messaging-31eepkqu3`, 09/09 11:20): `/api/fornitori/cerca` restituisce «FAG Torino Fiori» con IBAN `IT25B0…4971` e conto «EMMEGIFLOWERS SRL», «Il Fiore Di Fabula» con conto «Martina Cozzella» (due nomi sul conto DIVERSI dall'insegna: è il caso per cui il campo esiste), e `/api/partner` resta a **96 partner e 0 IBAN**. | Chiavi a scope · deny-by-default |
| 🟡 IGIENE | Piattaforma consegne | **`body.partner.email` non e' validata come formato** (trovato dall'ostile 08/09/2026). Il controllo del formato vale solo per l'email dell'ACCOUNT, e `@IsEmail()` del DTO non si applica perche' `aggiornaProfilo` chiama `partners.update()` con un oggetto nudo, fuori dal ValidationPipe. Percorso: un PARTNER punta il proprio recapito su un indirizzo qualsiasi e cicla richiedi/annulla → posta indesiderata verso terzi firmata dall'infrastruttura Deluxy (casella `mailUtente`). Contenuto fisso, quindi non phishing su misura: e' reputazione del mittente. | cap. 6, 10 |
