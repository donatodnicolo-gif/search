...
|---|---|---|---|
| 09/09 | **piattaforma** | Consegne, **ordine CORPORATE** (`/deliveries/:id` e `/deliveries`) · segnalazione dell'utente in tre punti sul flusso «Crea l'acquisto» (la seconda consegna, quella con cui si compra la merce dal fornitore): **(1)** il modulo chiedeva di riscegliere la tipologia di servizio invece di proporre la vendita — il prefill ereditava il servizio CORPORATE del cliente, che il fornitore non ha in listino, e appena si sceglieva il partner veniva azzerato; **(2)** si perdeva l'orario di consegna — senza servizio non esistono fasce, e la fascia ereditata veniva scartata; **(3)** l'indirizzo di ritiro restava quello del cliente corporate e non passava a quello del fornitore scelto — un indirizzo ereditato è indistinguibile da uno scritto a mano, e la regola «un ritiro scritto a mano non si tocca» lo proteggeva. **Correzioni applicate nello stesso giro**: nel ramo `?acquistoDa` servizio e ritiro si azzerano subito, l'orario si tiene da parte e si rimette appena esistono di nuovo le fasce (con l'orario di ritiro che ne discende); la vendita si sceglie dal listino DEL FORNITORE, non dal catalogo. Inoltre, su richiesta dell'utente, il comando è ora **anche nella riga dell'elenco consegne** e nel dettaglio è diventato **un avviso in cima** («Manca l'acquisto della merce») finché l'acquisto non esiste, invece di un link in fila con gli altri. **Da valutare al custode, due regole candidate**: *(a)* quando un modulo nasce COPIANDO un altro record, ogni campo ereditato va marcato come «messo dal sistema», altrimenti le automazioni successive lo scambiano per una scelta dell'utente e non lo toccano più; *(b)* il comando che completa un lavoro in due pezzi non vive solo nel dettaglio del primo pezzo: finché il secondo manca, lo si dice dove il lavoro si guarda (elenco compreso). Misura del perché: in tutta la piattaforma **0 consegne su 205 corporate** hanno mai avuto l'acquisto collegato. | utente |
| 08/09 | **piattaforma** | **Pattern nuovo da portare (o respingere) nel Libro: la colonna che dice se un messaggio è ARRIVATO.** In Vendite è nata la colonna «Trasmessa»: lo stato della riga («proposta») diceva solo che il sistema aveva deciso, non che il destinatario lo avesse saputo — e i due canali (campanello in app, mail) falliscono per conto proprio e in silenzio. Forma adottata: badge a quattro stati (in attesa / inviata / **parziale** / non arrivata), i due canali nominati sotto in piccolo con quello fallito barrato, il motivo per esteso nel `title`, ordinamento per gravità. Domanda al custode: **serve un pattern di Libro per «esito di una comunicazione in uscita»**, oggi assente? E «parziale» è uno stato legittimo o va spezzato in due colonne? | sessione piattaforma 08/09 |
| 08/09 | **piattaforma** | **Pattern nuovo: due riquadri affiancati per i due VERSI del denaro.** Scheda partner → «Condizioni di pagamento»: a sinistra i servizi vendor con «Paghiamo noi» (ambra), a destra i servizi a fatturazione con «Incassiamo noi» (verde). Affiancarli è il punto: sono rapporti opposti con tempi diversi, e finché erano un campo solo (in FINANCE) nessuno sapeva a quale verso si riferisse. Sotto 720px si impilano. Domanda al custode: il **colore semantico sul verso di un flusso di denaro** (non su uno stato di successo/errore) è ammesso dal Libro? | sessione piattaforma 08/09 |
| 08/09 | **piattaforma** | **Pattern nuovo: le due strade in cima a un modulo, come pillole.** Modulo «Nuova riconciliazione»: «Da un ordine» / «Senza ordine», che cambiano i passi successivi. Alternativa scartata: due voci di menu separate (avrebbero duplicato un modulo identico all'80%). Nella stessa schermata una casella «Cerca anche fra gli ordini già chiusi» allarga un elenco filtrato di suo — come «Attivi / Tutti» nell'elenco partner (29/08). Se approvato, vale la pena scriverlo una volta per tutte: **un elenco filtrato deve sempre dire come si allarga**. | sessione piattaforma 08/09 |
| 08/09 | **piattaforma** | **Trappola tecnica per il Libro (Angular): `@else if` non accetta l'alias `as`.** Spostando un blocco in `product-reconciliations.component.ts` la build è fallita con `NG9: Property 'v' does not exist` su `@else if (venditaScelta(); as v)`. L'alias vale solo su `@if`: si annida un `@if` dentro `@else`. Vale per tutte le app Angular dell'ecosistema. | sessione piattaforma 08/09 |
