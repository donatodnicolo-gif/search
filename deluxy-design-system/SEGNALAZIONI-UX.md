...
|---|---|---|---|
| 10/09 | **AI Mail · Cestino** | 🔴 **«Svuota cestino» sparisce col filtro — e quando si vede, la conferma dichiara il numero SBAGLIATO su un'azione irreversibile.** Segnalato dall'utente («se non c'è una mail in cestino da posta in arrivo il pulsante sembra sparire»). In `src/app/cestino/page.tsx:63` i due pulsanti stanno dietro a `{messaggi.length > 0 && …}`, dove `messaggi` è la lista **filtrata** (chip di periodo, ricerca) e **tagliata a `take: 800`**. Due difetti distinti: **(1)** con un filtro che non pesca nulla la pagina dice giustamente «Niente nel cestino *per questa ricerca*» — cioè sa che il cestino non è vuoto — ma nasconde lo stesso il pulsante che agisce su **tutto**; misurato oggi: Eleonora ha 2 mail in cestino e 0 cestinate oggi, Martina 4 e 0 → col chip «oggi» il pulsante sparisce per entrambe. **(2) Più grave**: `<SvuotaCestino quanti={messaggi.length} />` passa il conteggio **filtrato/troncato** alla conferma, che scrive «Rimuovo {quanti} messaggi», mentre `svuotaCestinoDi()` svuota **tutto** il cestino e cancella dal server IMAP. Misurato: **Renato ha 1.076 mail in cestino, la pagina ne mostra 800 → la conferma direbbe «800» e ne cancellerebbe 1.076**, definitivamente. Su un'azione irreversibile la conferma non può contare una popolazione diversa da quella su cui agisce. **Rimedio proposto (non applicato)**: una `count()` secca sul cestino intero (`utenteId + cestinato: true`, senza filtri né `take`) usata sia per mostrare il pulsante sia per il numero della conferma — il ripristino invece resta giustamente sulle righe mostrate, ma va detto nell'etichetta. **Da valutare come regola del Libro**: *un comando che agisce su TUTTA una popolazione non si mostra, né si conta, sulla sua sottopopolazione filtrata.* ✅ **CORRETTO E PUBBLICATO il 10/09 alle 12:07** (`deluxy-mail-akd6nj5bn`), su richiesta esplicita dell'utente: `count()` sul cestino intero dentro lo stesso `Promise.all` della lista (nessuna ondata in più), usata sia per mostrare il pulsante sia per il numero della conferma. 🔴 **Terzo difetto trovato correggendo, ed era il più grave: la conferma diceva il FALSO** — «le mail restano sulla casella» — mentre lo svuotamento le cerca sul server per Message-ID e le cancella (`eliminaDalServer`). Il riepilogo *finale* lo diceva già: a mentire era **la conferma**, cioè l'unico momento in cui si può ancora tornare indietro. Riscritti conferma e sottotitolo della pagina. **Regola aggiuntiva proposta**: *quando cambia ciò che un'azione irreversibile fa, il testo che la conferma va riletto — il riepilogo che arriva dopo non serve a chi deve decidere prima.* | utente |
| 08/09/2026 ✅ DECISA | Piattaforma · Listino fiorista | Tabella allargata con 5 colonne di colore valorizzate su 1 riga su 16 (le altre 15 mostravano trattini); conflitto di larghezze che comprimeva l'ultima colonna mandandola a capo su ogni riga; overflow-x finito su tutte le card invece che sulla tabella. Corretto su richiesta esplicita dell'utente: contenitore di scorrimento dedicato, prima colonna ancorata, celle non applicabili vuote, ultima colonna che non va a capo. **Verdetto del custode**: caso non coperto → **regola nuova del Libro §8 v2.2** («una colonna è un attributo della popolazione»). Correzione locale: tabella a 3 colonne + riga di espansione coi colori, `.table-wrap` canonico al posto di `.card {overflow-x}`, larghezze per classe, tabular-nums, spinner nascosti. **Respinte**: celle vuote al posto dei trattini (cura il sintomo, lascia 75 celle morte) e blocco separato sotto la tabella (stacca il prezzo del colore dal prezzo base che gli fa da riferimento). ⚠️ Il custode ha trovato anche che **i colori non venivano salvati** (il client non li mandava): esito verde su scrittura mai avvenuta, Libro §7. |
...
## Decisa il 05/09/2026 — tabelle più larghe della finestra (piattaforma `/vendite`, poi Libro)

**Segnalazione** (utente, 05/09, con screenshot): nella piattaforma, elenco **Vendite**, scorrendo a destra per raggiungere i bottoni Accetta/Rifiuta/Inserisci/Modifica sparivano le prime colonne — numero d'ordine e data — e non si capiva più quale vendita fosse la riga; il nome del prodotto restava tagliato a metà.

**Verdetto del custode**: il caso non era coperto. Il Libro §8 dava la tabella canonica e le schede sotto gli 800px, ma non diceva niente su cosa succede fra i due mondi, dove `overflow-x: auto` da solo porta via l'identità del record. **Regola nuova, vale per tutte le app** — entrata nel Libro §8 (v2.0): una colonna identità congelata a sinistra (dev'essere la prima), una colonna azioni congelata a destra, tetto del 40% con le azioni che si scongelano per prime, niente congelamento sotto la soglia mobile, e le quattro insidie obbligatorie (fondo opaco + hover ridipinto, z-index 1/2/3, `border-collapse: separate` col separatore a `box-shadow`, niente `display:flex` su un `td` sticky).

**Respinta** l'idea di congelare una colonna centrale lasciandole scorrere sotto le altre (sembra un difetto di render e non è il pattern di nessun riferimento), e respinto il ripiego di nascondere colonne su desktop o anticipare le schede sopra gli 800px (due deroghe al §8 per un problema che si risolve con venti righe di CSS).

**Applicato su `/vendite`** lo stesso giorno: `.table-wrap.col-fisse` nel foglio condiviso della piattaforma (`web/src/styles.css`), la prima colonna (data di consegna, prima per regola dell'utente del 04/09) porta il numero d'ordine come sotto-testo così l'identità viaggia nella cella congelata senza congelarne due, il flex delle azioni spostato su un `div` interno. **Due difetti trovati durante l'esame e corretti**: `td:nth-child(3)` dava 220px di respiro alla colonna sbagliata (era pensato per Prodotto, colpiva Ordine) e prendeva in pieno anche la riga di modifica col `colspan` → sostituito con la classe `.prodotto`; `overflow-x` ridondante nel componente, che mascherava la regola canonica → tolto.

**Da uniformare**, per impatto: `deliveries-list` → `partners-list` → `payments-list` → `invoices-list`; le altre 26 `.table-wrap` solo se eccedono davvero (la classe è opt-in).
...
| 11/09 (5) | **CRM** (Reclami: sezione nuova, scheda cliente) | Utente: «aggiungi sezioni reclami all'app e fatti passare i reclami dall'app customer service» | **Fatto**: pagina `/reclami` col pattern delle altre liste (4 KPI, pillole di stato con «Da lavorare» come vista di partenza, filtri in `<details>`, riga che apre il dettaglio in finestra §9); card «Reclami» nella scheda cliente sotto gli Ordini, con badge «N da lavorare»; voce di menu in «Clientela» col pallino (rosso se c'è un grave o una domanda senza risposta). Sola lettura, con «Apri nel Customer Service» in fondo alla finestra. **Difetto trovato e corretto alla radice**: `.cella-principale`/`.cella-sotto` non avevano `display:block`, e dentro un `<button>`/`<a>` (dove sono `<span>`) titolo e sottotitolo finivano sulla stessa riga — si vedeva anche nella ricerca viva del cliente, da prima. ⚠️ Per il custode: due app mostrano lo stesso reclamo con due badge di stato disegnati da codici diversi (CS e CRM); i colori arrivano dall'API, ma se il Libro vuole un componente unico di «stato», è il caso di dirlo |
| 11/09 (4) | **CRM** (Nuovo ordine) · **CS** (API) | Utente dal modulo live: «le fasce orarie devono essere dei negozi o a inserimento manuale; copia tutte le opzioni da app customer service, chiedi espressamente all'app quali ha» | **Fatto**: il modulo chiede al CS le opzioni del negozio e le mostra con le stesse parole della schermata del CS (giorno con motivo in rosso se chiuso, spunta «Eccezione concordata» + motivo, fascia dalle fasce del giorno o «Flessibile: la scrivo io…» con «Torna alle fasce del negozio», «Riceve un'altra persona», «Consegna anonima», consenso acceso, spedizione a gruppi «Voci che il negozio usa» / «Tariffe del sito per questo indirizzo» + stima con «Usa la stima», mezzo con `datalist` dei metodi visti, IVA con la spiegazione). Se il CS non risponde: banner e modulo usabile a mano. ⚠️ Per il custode: due moduli (CS e CRM) con le stesse scelte ma due implementazioni; da valutare nel Libro §4 se un form condiviso via API debba anche dettare l'ordine dei campi (oggi il CRM segue il CS a parole, non a codice) |
| 11/09 (3) | **CRM** (pop-up dettaglio ricorrenza, Calendario e Ricorrenze) | Utente (screenshot): form «Correggi» del pop-up schiacciato in una riga, etichette e campi sovrapposti, testo che esce dalla finestra | **Corretto**: una regola CSS generica sul contenitore delle azioni della modale (`.modale-azioni form {display:flex}`, pensata per il bottone unico della conferma di eliminazione) si applicava anche a un form a più campi. Regola scoperta solo sulla conferma (`.modale-conferma`). ⚠️ Per il custode: da valutare nel Libro §9 una regola «la modale non impone il layout ai form che contiene»: i selettori sul contenitore vanno per classe di scopo, non per tag |
| 11/09 (2) | **CRM** (Impostazioni → Utenti) | Utente: «mi manca la possibilità di creare utenti da impostazioni» → «Procedi» all'API nel Hub | **Fatto**: card a tutta larghezza con tabella «chi entra» (nome/email, ruolo, stato attivo, ultimo accesso, «Togli dal CRM»), sotto due colonne: form «Nuovo utente» (nome, email, password iniziale, ruolo) e «Già nel Hub, fuori dal CRM» con «Abilita al CRM» a un click; esito con le parole («Maria può entrare nel CRM con…»). Se il token manca, la card dice cosa serve e dove si fa. ⚠️ Per il custode: è la prima card di Impostazioni che è anche un elenco + due form; se il Libro preferisce una pagina propria (`/impostazioni/utenti`), si sposta |
| 11/09 | **CRM** (liste, eventi, impostazioni) | Utente: «in liste metti tutte le condizioni che desume Orders», «un evento collega liste di clienti (mostra quale nell'elenco)», «manca creare utenti da impostazioni» | **Fatto**: form a mano con le 41 liste di Orders **raggruppate per famiglia** (titoletto maiuscoletto + caselle: 41 caselle in fila non si leggevano), sezione «Occasioni in arrivo» (tipo + entro giorni / mese, con la regola scritta sotto), canale di arrivo, ordine medio, cliente da N anni; eventi: caselle «Liste collegate» nel form, card con «Aggiungi gli invitati» ed esito con le parole (quanti entrati, quanti c'erano già), chip «☰ lista» sotto il titolo nell'elenco; Impostazioni: card «Utenti del CRM» che porta al Hub (casa unica) invece di un form doppione. ⚠️ Per il custode: il form a mano delle liste è lungo (7 sezioni); da valutare se piegare le sezioni meno usate in `<details>` (§8.9) |
| 10/09 notte | **CRM** (scheda, libro clienti, nuovo ordine) | Utente dopo il deploy: «non vedo possibilità di modifica del cliente e manca il pop-up degli ordini» (era Orders in timeout: regressione mia sui brand, corretta), «modifica chi è con una matitina», «consenso marketing con disattivazione», «consenso CRM come flag, anche in tabella, di default sì», «bottone unisci in tabella», «ordine per cliente nuovo come dal CS» | **Fatto**: matitina a pillola in testata (era un link piccolo che sfuggiva); «Chi è» con `TestoModificabile` (matitina → textarea inline, Salva/Annulla: pattern «modifica in visualizzazione», da valutare per il Libro §4); card «Consensi» con badge (sì/no/mai detto) e bottoni Attiva/Disattiva per canale — scrivono in Orders, casa del dato; consenso CRM come pallino oro/rosso nella colonna «CRM» del libro; spunte di riga + barra «Unisci i selezionati» sotto la tabella (checkbox con attributo `form=`, niente JS; il principale è chi ha più ordini, scritto in chiaro); cliente nuovo: pagina del modulo tollerante quando Orders non conosce l'email. ⚠️ Per il custode: il pallino della colonna CRM è un segnale a colore solo, con `title`/`aria-label` — se il Libro vuole anche la forma (§5), si cambia |
| 10/09 sera | **CRM** (revisione completa: custode desktop + ux-mobile + ux-ostile) | Utente: «rivedi tutta l'app con il custode ux&ui e ottimizzala». Referti: desktop 3 P1 / 15 P2 / 15 P3, mobile 4 P1 / 6 P2 / 3 P3 — tutti misurati; l'ostile ne ha confermati 31, refutati 2, ridimensionati 15 | **Applicato oggi (10 correzioni + seconda fascia), tsc/build 0, commit nel repo `app/`**: (1) **conferma narrativa sulle 4 eliminazioni irreversibili** (nota, lista, template mail, template WA) con `ConfermaElimina.tsx` su `Modale.tsx` (variante `stretta`), verbo nel bottone rosso, conseguenza scritta; le rimozioni reversibili (membro, invito, programmazione → nel Calendario resta annullata) restano a un click, e i due «✕» sono diventati parole; (2) **griglie a due colonne**: 11 stili inline → classi `.griglia.scheda/.lavoro` con ramo mobile a 1 colonna (a 375 px la colonna destra era 115-140 px, `scrollWidth` 478-489, «Salva» fuori schermo); (3) `.filtri .riga-chips-scorri` nella MQ 900 (specificità 0,2,0 vinceva: chip di /clienti su 3 righe); (4) MQ 900: `.form-riga` a capo, input 16 px (zoom iOS), label cluster 12,5; MQ 700: `.griglia.due/tre/quattro` `minmax(0,1fr)`, `.tabella-fette th` a capo, `code.chip` a capo (overflow 385/379 → 375); (5) `aria-current="page"` in Sidebar; (6) `input[type=time/month/week]` nel selettore dei campi, `file` con `font: inherit`; (7) Calendario: una mappa sola, tutte le ricorrenze viola come la legenda (l'anniversario in oro si confondeva con un evento); (8) `BottoneInvio` (stato «in corso», doppio click impossibile) sui tre form d'invio mail/WhatsApp/lista — verificato in react-dom 19.2.8 che il submit non deduplica; il «nonce» monouso è demolito (copre solo il doppio-tab); (9) gap 6 → 10 fra Modifica/Archivia/Elimina; (10) neri di troppo: «Cerca» e i 30 «Crea ordine» di riga di /nuovo-ordine, Salva/Registra/Aggiungi/Unisci della scheda → ghost (in scheda restano neri solo il CTA del form aperto e «Scrivi una mail»). Seconda fascia: 6 `aria-label` sulle ricerche e sul brief; hex → token (`--on-ink`, `--scrim`, `--red-soft`, via il fallback `var(--ink,#1d1d1f)`); **«Archivia» anche sui template WhatsApp** (l'action c'era, la UI no); `creaListaManuale` torna su `/liste?modo=manuale`; «Azzera ricerca e filtri» nel vuoto di /clienti; quattro titoli di sezione nel form a condizioni; `WaAssistito` guarda il ritorno di `window.open` (popup bloccato → avviso; diario non registrato → lo dice) — chiude la riga del 28/08. **Da fare in giri dedicati (registrate, non toccate)**: B1 drawer mobile + topbar 56 (la sidebar a 375 occupa 453 px = 56% dello schermo; poi B6 bersagli 44 con padding/margin negativo, B8 tabelle a schede, B4 calendario mobile a conteggi, B13 foglio dal basso); A2.2 inversione dell'API `.btn` insieme al Customer Service (Appendice B); A2.3 `htmlFor/id` su ~100 campi (la conversione in blocco di `.campo` produrrebbe label annidate); A2.1 `useActionState` sui 2 compositori e sulla lista a condizioni; A2.4/A2.5 tabella Clienti (togliere Medio, Brand come sotto-testo; `ThSort`); A2.6 + A1.3 classe avviso oro e riordino dei rami del troncamento (la proposta «n/d sotto il 50%» è **demolita**: il Libro §6 dice «dichiara la copertura O si rifiuta», e la pagina dichiara; il 50% è della Tessera KPI con confronto); A2.11 esito «non in agenda» quando il Calendario configurato non risponde; A2.12/B11 piede sticky; `error.tsx`; i 3 delete ancora muti (`rimuoviInvito`, `eliminaLista`, `rimuoviMembro`; `eliminaAttivita` è irraggiungibile). **Al DS**: contrasto `text-tertiary` sui contenuti (3,3-3,6:1) e `chip.oro` (3,77:1). **Verdetti del custode**: (a) il gruppo ripetibile (`RicorrenzeMultiple`) entra nel Libro §4 v2.3 con cinque condizioni (fieldset numerati, «Togli la ricorrenza N» ≥ 28 px, tetto dichiarato, focus gestito, esito per riga; ≤ 6 campi, un gruppo per pagina, oltre 10 voci → lista + form) — le righe cluster di Impostazioni NON sono questo pattern (griglia fissa); (b) Ricorrenze fino a 3000 righe ordinate in casa: **deroga temporanea** annotata nel README del CRM con soglia di rientro (`troncato` in produzione, o > 1500 ricorrenze a 30 giorni → ordinamento e filtri in Orders); oggi 786 a 30 giorni |
| 10/09 | **CRM** (scheda, Ricorrenze, Calendario, Performance, Liste, Template, Impostazioni) | Utente, 17 richieste in sessione: profilo cliente modificabile (professione, foto, note), form «Programmazione», sezione Calendario, «Oggi» nelle Ricorrenze, tabella ordinabile e con filtri, sito di provenienza, ricorrenze multiple, liste a condizioni, «questo non fa nulla» (Costruisci la lista), template in tabella con archivio, prodotti/collezioni da Merchandising, nuovo ordine dal menù, note multiple, Performance, soglie e cluster con punteggio, «Unisci» clienti, ordini in pop-up | **Fatto in locale (commit `8ee4c0c5`), non deployato.** Pattern usati senza regole nuove: form con label e asterischi (cap. 4), stato «in corso» sui bottoni lunghi (`BottoneInvio`, `useFormStatus`: il «non fa nulla» era un'azione di 30-60 s senza segnale — le liste nascevano, due per due click), modale §9 per gli ordini (`<dialog>` centrata via flex: il reset dei margini annulla il centramento nativo, annotato in `Modale.tsx`), zona filtri §8 (riga fissa ricerca+Filtra, «Filtri (N)» in `<details>`, ordinamento nei `th` con `ThSort` copiato da Finance), chip scorrevoli §8.9, griglia mese copiata dal Calendario. ⚠️ Due cose da far vedere al custode: (1) **il form a righe ripetibili** (`RicorrenzeMultiple`: fieldset per riga, «+ Un'altra», «togli», tetto 10) non è nel Libro — proposta di pattern §4 se piace; (2) la **tabella Ricorrenze legge fino a 3000 righe e ordina in casa** perché l'API di Orders non ordina né filtra per sito/parola: il troncamento è dichiarato in pagina (Libro PERFORMANCE: niente take silenziosi), ma se il volume cresce va portato l'ordinamento in Orders. Rubrica Gmail NON fatta (nessuna app espone i contatti Google) |
