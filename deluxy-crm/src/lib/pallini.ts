...
// di riferimento: Customer Service (deluxy-messaging, src/lib/pallini.ts).
//
// ⚠️⚠️ Sta in un file SUO e senza `prisma`: la barra laterale è un componente del
...
export function decidiPallini(
  ...
  for (const [href, s] of Object.entries(sezioni)) {
    ...
    // chiuse (completate, annullate, archiviate), quindi chiudendo la cosa più
    // recente la data precedente diventa quella corrente — e il menu
    // segnalerebbe come «novità» qualcosa che è **sparito**.
...
/**
 * ⚠️ La stessa regola con cui il menu decide quale voce è accesa: `/` conta come
 * «ci sono sopra» solo sulla pagina iniziale, le altre voci anche sulle proprie
 * sottopagine. Due regole diverse per la stessa domanda finirebbero per dire
 * due cose diverse.
 */
