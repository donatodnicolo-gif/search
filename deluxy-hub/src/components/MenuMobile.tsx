 ...
 * e le azioni restano in linea nella topbar (Libro UX cap.1: la topbar è
 * ammessa per un launcher a ≤3 destinazioni; su mobile diventa menu).
 */
export function MenuMobile({ children }: { children: React.ReactNode }) {
  const [aperto, setAperto] = useState(false);
  const pathname = usePathname();

  // Alla navigazione il drawer si richiude da solo.
  useEffect(() => {
    setAperto(false);
  }, [pathname]);

  // Esc chiude; con il drawer aperto blocca lo scroll del fondo.
  useEffect(() => {
    if (!aperto) return;
