...
// Sessione firmata (HMAC-SHA256) dentro un cookie: il middleware può validarla
// sull'Edge senza interrogare il database. Usa solo Web Crypto, quindi funziona
// sia nel middleware sia nelle server action.
...
function segreto(): string {
  ...
  if (!s) throw new Error("HUB_SESSION_SECRET non impostata: la sessione non può essere firmata.");
  return s;
}
...
export async function leggiSessione(token: string | undefined): Promise<Sessione | null> {
  ...
  try {
    ...
    if (typeof dati.uid !== "string" || !isRuolo(dati.ruolo)) return null;
    return dati;
  } catch {
    return null;
  }
