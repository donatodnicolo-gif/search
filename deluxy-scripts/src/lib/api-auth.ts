...
// Autenticazione delle API: chiave nell'header "x-api-key" (o "Authorization:
// Bearer …"). Nel database c'è solo lo SHA-256 della chiave. Le chiavi si
// creano con `npm run chiave -- <nome-app> [--scrittura]`.
...
export function erroreApi(status: number, messaggio: string) {
  return NextResponse.json({ errore: messaggio }, { status, headers: { "Cache-Control": "no-store" } });
}
...
// Verifica la chiave. Con { scrittura: true } richiede una chiave di scrittura;
// di default basta una chiave di sola lettura.
