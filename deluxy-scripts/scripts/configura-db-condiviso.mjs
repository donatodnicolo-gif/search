...
// dedicato "scripts". Non stampa mai le stringhe di connessione.
//
// Uso: node scripts/configura-db-condiviso.mjs <percorso-env-sorgente>
// Es.: node scripts/configura-db-condiviso.mjs ../deluxy-orders/.env
...
function conSchemaScripts(url) {
  if (!url) return null;
  if (/[?&]schema=/.test(url)) return url.replace(/([?&]schema=)[^&]*/, "$1scripts");
  return url + (url.includes("?") ? "&" : "?") + "schema=scripts";
}

const db = conSchemaScripts(prendi("DATABASE_URL"));
const direct = conSchemaScripts(prendi("DIRECT_URL"));
if (!db || !direct) {
...
// si riscrivono solo le due stringhe di connessione.
...
const contenuto = [
  '# Postgres condiviso delle app Deluxy (stesso cluster di hub/partner), schema "scripts"',
  `DATABASE_URL="${db}"`,
  `DIRECT_URL="${direct}"`,
  ...esistenti,
  "",
...
console.log('Scritto .env con schema=scripts (stringhe di connessione non mostrate).');
