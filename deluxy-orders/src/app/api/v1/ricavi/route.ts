...
export async function GET(req: NextRequest) {
  ...
  const perBrand = new Map<string, { brand: string; ordini: number; lordo: number; mesi: number[]; ordiniMese: number[] }>();
  ...
  for (const r of righe) {
    ...
    if (!b) {
      b = { brand: r.brand, ordini: 0, lordo: 0, mesi: Array(12).fill(0), ordiniMese: Array(12).fill(0) };
  ...
  return NextResponse.json({
    ...
    totali: { ordini: ordiniTotali, lordo: lordoTotale, mesi: mesiTotali },
    esclusi: {
