...
async function conteggi(): Promise<{ ordini: number; daClassificare: number; clienti: number; liste: number; automazioni: number; script: number; eventi: number }> {
  try {
    const [ordini, daClassificare, clienti, automazioni, script, eventi] = await Promise.all([
      prisma.ordine.count(),
    ...
    return { ordini, daClassificare, clienti, liste: LISTE.length, automazioni, script, eventi };
  } catch {
    return { ordini: 0, daClassificare: 0, clienti: 0, liste: LISTE.length, automazioni: 0, script: 0, eventi: 0 };
  }
}
