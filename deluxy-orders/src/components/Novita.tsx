...
// deluxy-messaging/Novita.tsx. Qui l'unico arrivo esterno vero sono gli
// ORDINI, che li scrive la sync Shopify mentre nessuno guarda: finché non si
// apriva il registro non lo sapeva nessuno.
//
...
//
// ⚠️ La navigazione di quest'app è a ricaricamento pieno: il segnaposto sta in
// `sessionStorage` (per scheda, come il cookie di sessione del registro) così
// un cambio pagina non fa ripartire il giro da zero perdendo le novità.
...
const RESPIRO = 25000;
const CHIAVE_DA = "orders-novita-da";
...
export function Novita() {
  ...
  const chiedi = useCallback(async () => {
    ...
    try {
      ...
      // ⚠️ Il ramo /api/novita del middleware risponde 401; se un giorno
      // finisse dietro un redirect, `fetch` lo seguirebbe tornando la pagina
      // di login con stato 200: si guardano tutte e tre le spie.
          ...
          return [
            {
              ...
              tipo: "riassunto",
              gruppo: "novità",
              titolo: `${nuove.length}${d.troncato ? "+" : ""} nuovi ordini`,
              dettaglio: "Aperti nel registro mentre non guardavi",
      ...
      {coda.map((a) => (
        <div key={a.id} className="novita">
          {/* ⚠️ Tutto il riquadro porta alla cosa di cui parla: un avviso che
              dice «nuovo ordine» e poi lascia cercare fra 14.000 righe fa
