...
// dedicato "orders". Non stampa mai le stringhe di connessione.
//
...
function conSchemaOrders(url) {
  if (!url) return null;
  if (/[?&]schema=/.test(url)) return url.replace(/([?&]schema=)[^&]*/, "$1orders");
  return url + (url.includes("?") ? "&" : "?") + "schema=orders";
}

const db = conSchemaOrders(prendi("DATABASE_URL"));
const direct = conSchemaOrders(prendi("DIRECT_URL"));
if (!db || !direct) {
...
const contenuto = [
  '# Postgres condiviso delle app Deluxy (stesso cluster di hub/partner), schema "orders"',
  `DATABASE_URL="${db}"`,
  `DIRECT_URL="${direct}"`,
  ...esistenti,
  "",
...
console.log('Scritto .env con schema=orders (stringhe di connessione non mostrate).');
