...
> **Dettaglio della vendita (04/09/2026, regola utente).** Nel pop-up: bottone **«Vedi su Shopify ↗»** (link all'ordine nell'admin, costruito dal **server** — `SalesService.linkShopify`: base in `Impostazioni → Admin Shopify`, chiave `shopifyAdminUrl`, di default il negozio storico `deluxygifts`; solo per l'ufficio, mai nella risposta del partner) e riga **«Prezzo partner»** = importo × (1 − sconto%), mostrata **anche all'ufficio** accanto all'importo del cliente.
>
