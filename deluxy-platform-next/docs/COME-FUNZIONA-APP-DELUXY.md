...
**Plus/minus e regola carnet (regola dell'utente, 04/09/2026).** Il plus/minus del partner (`Delivery.additionalPrice`) non è un valore in più rispetto alla regola carnet agganciata (`DeliveryRule.partnerBillingAdjustment`): la **recepisce**. In `prezzoConsegna` l'extra da sommare al prezzo è: con regola → il plus scritto se diverso da zero, altrimenti l'aggiustamento della regola; senza regola → il plus scritto. Mai la somma dei due. Una regola con `toBill = false` continua a vincere su tutto (non si fattura).
