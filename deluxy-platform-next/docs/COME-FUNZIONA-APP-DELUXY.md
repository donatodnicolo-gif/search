...
| **Aziendale (corporate)** | ⚠️ **La descrizione precedente («replica la consegna a un altro partner, trasformando il servizio da prezzo fisso a vendita») NON è sostenuta dai dati** — misurato il 24/08/2026. Il campo `delivery.parentDeliveryId` esiste e ha **72 legami** (tutti importati), ma: **47 su 72 hanno lo STESSO partner** della madre (è una seconda tratta, non una replica); delle 25 che cambiano partner, **24 vanno a “Magazzino Deluxy” a prezzo ZERO** e una sola a un partner vero; e il modello di prezzo **non si trasforma mai** — 25 su 25 restano `sales→sales`, le altre 28 `fixedprice→fixedprice`. Il vero Corporate Service passa dai **prodotti condivisi** (vedi §3.9, *Servizi Partner*), non dalle consegne. |
