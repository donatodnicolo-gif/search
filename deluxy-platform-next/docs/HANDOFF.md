...
>
> 🔴 **11/09/2026 (mattina, 27) — CONSEGNE IN ERRORE PER IL POOL DI CONNESSIONI** (manuale 129). Sintomo: «Internal server error» sulla lista Consegne. Log: `Timed out fetching a new connection from the connection pool (timeout: 10, connection limit: 3)`. Causa: il blocco del margine (10/09) in `deliveries.service` faceva `Promise.all` di QUATTRO query per ogni pagina — con tre connessioni nel pool le prendeva tutte. Corretto: letture in sequenza (commit `3e768f20`), 🚀 **LIVE `delivery-229pvicym`**, log puliti dopo il deploy. ⚠️ Visto anche `FATAL: (EMAXCONN) max client connections reached, limit: 200` sul pooler Supabase (porta 6543, condiviso con le altre app): rientrato da solo; il Postgres stava bene (29 connessioni su 60, nessuna query lenta). 📌 Da valutare col custode PERFORMANCE: alzare `connection_limit` nella variabile di produzione e non aggiungere altre query per riga nelle liste.
