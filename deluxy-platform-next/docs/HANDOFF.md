...
> 🔗 **10/09/2026 (sera, dal Customer Service) — NUOVA ROTTA A CHIAVE `GET /api/v1/app/prodotti-unici/calendario?codici=SKU1,SKU2&giorni=14`** (`app-api.module.ts`, `AppApiController.calendarioProdottiUnici`; `AppApiModule` importa `CalendarioUniciModule`). Restituisce lo STESSO calendario che ogni mezz'ora va a Merchandising (`CalendarioUniciService.calendario()`), filtrato per SKU: per ogni prodotto unico `{codice, nome, partner, senzaOrari, calendario:[{data, aperto, dalle, alle, origine}]}`. Lo legge il Customer Service (`deluxy-messaging/src/lib/piattaforma.ts: calendarioUniciPiattaforma`, cache 5 min) per la sua rotta pubblica `/api/pubblico/consegna?codici=` che i temi Shopify chiamano dal carrello: un giorno in cui il partner è chiuso si spegne sul sito col suo nome, la sua ora di apertura alza l'orario minimo, passata la sua ora di chiusura oggi non si ordina più. Additiva, `tsc` API verde. ⚠️ Scritta da un'altra sessione (quella del CS) in questo worktree perché il dato ha casa qui: **da pubblicare col prossimo deploy della piattaforma** — finché non è live il CS risponde `partner.errore: piattaforma: non-trovato` e il calendario esce senza il vincolo del partner.
> 📌 **STATO AL 12/09/2026 (sera) — per chi riprende.** Tutto pushato e LIVE: piattaforma `delivery-5s6ncm464`, Merchandising `icufl4arz`, Finance `deluxy-partner-i8z44lxve`, Anagrafiche col capogruppo diretto; manuale **171**. Regole del deploy: dalla RADICE del worktree, dopo `nest build` + avvio locale di Nest; Merchandising e Finance dalle loro cartelle, build in cloud.
>
> **PUNTI APERTI (aspettano una decisione dell'utente, nessuno è bloccante):**
> 0. ⚠️ **La consegna #101331 non è recuperabile** (11/09): creata alle 11:47 per Chanel Test ed eliminata col codice vecchio, che cancellava la riga per davvero. Da oggi «Elimina» annulla e conserva, ma quella è persa: se serve, va rifatta a mano (DDT «test», 11/09/2026, 10:00–11:00, ritiro 09:00–10:00, Piazza Duca d'Aosta 9 Milano, luogo Excelsior Hotel Gallia) e prenderà un numero nuovo.
> 1. 🔎 **Paga del valet uguale ai chilometri su 1.351 consegne** (segnalazione dell'11/09 sulla #62926: paga 6,43 € = `distanceKm` 6,43). Su 19.631 consegne con entrambi i valori, 1.351 hanno `valetSalary` identico a `distanceKm` — 35 in agosto, 51 a luglio, 85 a giugno, 91 a maggio, 60 ad aprile, 63 a marzo, 54 a febbraio, 16 a gennaio. Sospetto scambio di colonne nell'import dal sistema precedente; **non dimostrato**, perché il database legacy non è collegato a questo ambiente (`api/.env.legacy` non esiste, c'è solo l'esempio). Prossimo passo concordato: **preparare uno script di sola lettura** che confronti quelle consegne col listino del valet e mostri quanto cambierebbe, PRIMA di toccare qualsiasi paga. Niente scritture senza «vai».
> 2. «vai» per `applica-indice-sale-deliveryid.mjs` (poi aggancio della vendita 12895 alla #101104).
> 3. Decisione sulle 115 consegne chiuse di giugno-luglio.
> 4. Ripubblicare gli artifact dei manuali (URL vivi: «Come funziona Deluxy» cd3c9488…, «Manuale Deluxy» f0e0e84e…).
> 5. I capogruppi degli esempi (Chanel, Diptyque/Olfattorio) li crea l'ufficio dal modulo partner o dalla pagina Capogruppi.
...
>
> 💶 **12/09/2026 (sera, 44) — FINANZA: TRE CORREZIONI E UNA CORSA NUOVA** (manuale 166-171, deploy `delivery-5s6ncm464`).
> - **Valore vendita dell ordine**: le consegne di uno stesso ordine possono risolvere a due fonti (ordine Shopify vero o ripiego `vendita:<id>`), e `recap()` prendeva la PRIMA che capitava. Ora l ordine vero comanda, più ordini veri si sommano, solo-ripieghi si sommano fra loro. Misurato su 60 giorni: 4 ordini su 3.236 cambiano, tutti in aumento (+425 €).
> - **Consegne pagate**: la paga valet o è scritta o è stimata dal listino, e si contavano insieme. Nuovo campo `costoStimato` su CorrispettivoRow; `consegnePagate` e `piuPagheStessoGiorno` contano solo le paghe vere. Su 60 giorni: 536 consegne su 1.692 hanno il costo solo stimato; gli ordini «con più di una pagata» scendono da 55 a 24.
> - **ddt + marchio → ordine del cliente** (chiesto dal Customer Service, caso #101402 / Flowers #2923): le consegne del canale app portano il costo del FORNITORE. ⚠️ Il marchio è obbligatorio: in copia il numero 2923 esiste ma è di deluxy.it e vale 51 €. Si accetta solo l abbinamento UNICO — 160 su 167 ne trovano uno, 7 nessuno, **zero** ne trovano due. 55 consegne cambiano davvero, +12.777,93 € di valore cliente nei conti.
> - **Copia degli ordini ogni ora** (regola utente): `GET /api/v1/cron/copia-ordini` al minuto 35, finestra 7 giorni, opzione `soloCopia` su `spingiMargini`. ⚠️ LEGGE e basta: non scrive su Orders, perché ogni PATCH lascia una riga nella storia dell ordine. I margini restano notturni. Provata: copia Flowers da #2918 a #2926, la #101402 da «ricavi 0» a ricavi 60 € e margine 46,08 € (30,72%).
> - ⚠️ **Aperto**: la #101259/#101260 (DDT 12919) resta rossa perché tre consegne dello stesso giro pagano tre volte il valet (42,18 € stimati su 78 € di ordine). Da decidere se la regola carnet deve valere anche fra partner diversi allo stesso indirizzo.
> 📦 **11/09/2026 (notte tarda, 43) — MERCE IN SEDE: dati puliti, campi a Merchandising, prodotti di servizio** (manuale 157-159, deploy `delivery-hck8ii2ic`).
> - **Date impossibili: 92 consegne corrette.** `api/scripts/sistema-merce-in-sede.mjs` (le 52 aperte) e `api/scripts/date-impossibili-chiuse.mjs` (le 40 chiuse), tutti e due con anteprima e `--applica`. Il dump legacy del 06/09 (`C:Users
icolDownloadslocalhost.sql`, 233 MB) ha le STESSE date: l'errore è alla nascita, non nella migrazione. La data vera viene dagli id vicini (progressivi, stesso caricamento); in tre casi su quattro la sbagliata è l'anagramma della giusta. Backup: `scripts/merce-prima-di-*.json` e `scripts/date-chiuse-prima-di-*.json`.
> - **Stati riverificati su tutte le 311 aperte con data passata**: 232 identiche, 45 con enum VUOTA nel legacy (la migrazione le ha lette «created», lettura prudente), 1 più avanti qui, 33 nate qui. **Zero chiuse di là e aperte qui.**
> - **1.587 mancati recapiti senza destinazione chiusi** con `productManagement = closedNoInfo` (dal 2021 all'08/09). Le 3 di oggi restano aperte: la misura riparte da lì. ⚠️ Non si è scritta una delle tre destinazioni vere: sarebbe inventare dove sta la merce.
> - **Verso Merchandising (contratto §2-quater)**: `/app/partner` manda `indirizzo` (126/126) e `minimoOrario` (89), `/app/prodotti` manda `giorniMinimi` (anche variante) e `minimoOrario`; gli stessi campi viaggiano col prodotto in `merchandising-sync`. La `citta` passa da 13 a 126 perché si ricava dall'indirizzo, DICHIARANDOLO con `cittaDaIndirizzo`. Le due funzioni stanno in `api/src/app-api/dati-partner.ts` (file a sé per non creare un ciclo di import).
> - **Prodotti di SERVIZIO**: `Product.servizio` + tabella `PartnerProductStock`, migrazione `20260911000000_merce_di_servizio` **già applicata** in produzione con `api/scripts/applica-migrazione.cjs` (esegue i comandi uno alla volta e tiene interi i blocchi `DO 53721`). Merce in sede: colonna «In dotazione» e pannello di assegnazione (scrive solo l'ufficio, il partner legge la sua). ⚠️ È la prima giacenza VERA: non segue il periodo.
> - ⚠️ **Resta aperto**: quando un prodotto approvato viene modificato torna «da approvare» qui, ma **Merchandising non viene avvisato** — `merchandising.spingi` è chiamata solo alla creazione. Se il prodotto è già su Shopify resta in vendita coi dati vecchi. Serve una decisione sul contratto.
...
> - ⚠️ Il partner si **rilegge da Prisma dentro `inviaOra`**, non si prende da chi chiama: `spingi` arriva da più punti e non tutti includono le province.
> - ⚠️ `legacyId` si manda solo se `typeof === 'number'`: di là è il metafield `custom.partner_id`, dichiarato `number_integer` su Shopify, e un cuid farebbe rifiutare tutto il prodotto. Misura dell'11/09: 32 partner su 297 non hanno l'id numerico.
> - `app-api.module.ts`: `POST /api/v1/app/prodotti/:id/approvato` (guard `ScritturaRichiestaGuard`; l'id è il nostro `Product.id`, che loro conoscono come `idEsterno`). **Scrive solo `Product.approved`**: prezzo, sku e nome arrivano nel corpo e tornano nella risposta come `ignorati` — l'offerta del partner ha casa qui (Standard Deluxy §7), la decisione ha casa là. Provata in locale: 401 senza chiave, cioè esiste ed è protetta.
> - 🔴 **Perché il giro non si chiude ancora**: di là manca la configurazione «Piattaforma consegne» (`PIATTAFORMA_URL`, `PIATTAFORMA_API_KEY`), e la chiave che useranno deve avere il permesso di SCRITTURA. Finché non c'è, l'approvazione vale solo dentro Merchandising.

> 🔔 **11/09/2026 (notte, 39) — CHAT CHE SI APRE DA SOLA E MODIFICA A DUE ORE** (manuale 148-149).
> - `chat-panel.component.ts`: un `effect` guarda `nonLetti()` (dal `NovitaService`, giro di 30 s) e quando SALE, per ADMIN/OPERATION, apre il pannello e chiama `suona()` — due oscillatori Web Audio, niente file (`media-src` non esiste nel CSP). ⚠️ Il primo giro NON suona: `nonLettiPrima` parte a `null` e serve solo a prendere la misura.
> - Regola delle DUE ORE: `DeliveriesService.mancanoDueOre(d, adesso?)` — statica, usa `istanteRoma` di `common/puntualita` (ora legale compresa). Il partner può modificare con stato `created` (come prima) **o** `assigned` se mancano più di due ore. Stessa regola ricopiata in `delivery-detail.component.ts` e `deliveries-list.component.ts` per mostrare il bottone: se i conti divergono, chi clicca prende un rifiuto senza capire perché.
> - Casi limite provati (adesso = 11/09 12:00 Roma): oggi 15:00 → sì; oggi 14:00 (due ore esatte) → no; oggi 13:00 → no; domani 09:00 → sì; ieri → no; oggi senza fascia → no; domani senza fascia → sì.
...
> - Misure prima/dopo (produzione, contesto Nest locale): ufficio senza date 38,6 s → 4,5 s a freddo, 2,4 s a caldo; storico 29,2 s → 2,4 s; partner Chanel 262 ms → 213 ms.
> - ⚠️ **Perché era sfuggito**: le prime misure erano state fatte sul caso del partner (73 righe, sempre veloce). Il difetto cresceva con le righe, e l'ufficio senza filtri di data è il caso peggiore. Misurare il caso segnalato non basta: va misurato il caso GRANDE.
> - Lato browser restano le correzioni del 145 (collegamento aggiunto alla pagina, revoca ritardata, messaggio col codice HTTP, timeout di 60 s).

> 🧾 **11/09/2026 (notte, 36) — CONFRONTO IN OGNI TABELLA, E L'EXPORT CHE SALVAVA A METÀ** (manuale 144-145).
> - `statistiche-partner.module.ts`: le sette classifiche passano da una funzione sola, `classifica(chiave, {da, peso, filtro, limite})`, che conta il periodo scelto e quello di confronto con due somme condizionate sulle stesse righe. Ogni riga torna `{nome, quantita, prima}`. Filtri nuovi: `confronto` (precedente | anno | personalizzato | nessuno), `confrontoDa`, `confrontoA`. Misurato su Clivati (7.962 consegne): ~800 ms in tutti e quattro i modi.
> - ⚠️ `ORDER BY 2 DESC, 3 DESC` con `HAVING chiave IS NOT NULL AND chiave <> ''`: le voci presenti SOLO nel confronto restano, con quantità 0. È voluto.
> - Web: «Confronta con» nei filtri (le due date compaiono solo su «personalizzato»), colonna «prima» in ogni classifica con la variazione, intestazione con un paragone solo. ⚠️ Se `prima` è 0 la percentuale è `null`: non si divide per zero e «+∞%» non è un dato.
> - **Export Excel**: i due difetti erano nel salvataggio lato browser — `<a>` non inserito nel DOM (Firefox/Safari ignorano il clic) e `revokeObjectURL` chiamato nella stessa riga del clic. Ora si aggiunge alla pagina, si clicca, si toglie dopo 2 s. Aggiunti: messaggio per foglio vuoto, codice HTTP nell'errore (con `responseType: 'blob'` `e.error.message` è vuoto), e un timeout di 60 s che spegne «Preparo il file…». ⚠️ Il SERVER non era il problema: 250-400 ms misurati, e i log di produzione non mostravano nessuna richiesta a `/deliveries/esporta` fallita.

> 📈 **11/09/2026 (sera, 35) — CONFRONTI NELLE STATISTICHE, ACCESSO D'UFFICIO, «OGGI/DOMANI»** (manuale 142-143).
> - `statistiche-partner.module.ts`: `intervalliDiConfronto(da, a)` dà periodo precedente (stessa lunghezza, finisce il giorno prima) e anno prima (stesse date −1 anno); null senza entrambi gli estremi. I tre periodi si contano in **UNA query sola** con somme condizionate (`SUM(CASE WHEN … )`), che ha sostituito il `groupBy` dei servizi: stesse righe lette una volta invece di tre. Risposta `confronto: { venduto, precedente, annoPrima }` e `servizi[].precedente/.annoPrima`. Misurato su Clivati: ultimo mese 66 contro 99 (prec) e 96 (anno prima), 1,4 s.
> - Web: intestazione col totale e i due paragoni in percentuale; `?partnerId=…` letto dalla rotta (l'API lo accetta solo da ufficio, il partner prende l'id dal token); pulsante «Statistiche» nella scheda partner (`partner-detail.component.ts`, dentro `canEdit()`).
> - `delivery-form.component.ts`: `SCORCIATOIE_DATA`, `dataFra()`, `dataAmmessa()`, `scegliData()`. ⚠️ La data si costruisce con l'ora LOCALE, non con `toISOString()`: in UTC dopo mezzanotte «Oggi» scriverebbe ieri.

> 🗂️ **11/09/2026 (sera, 34) — ANNULLAMENTO, LUOGHI RICOSTRUITI, STATISTICHE PARTNER** (manuale 139-141).
> - **`remove()` non cancella più** (`deliveries.service.ts`): niente `prisma.delivery.delete`, la consegna passa a `DeliveryStatus.INVALIDATED` («annullata d'ufficio», già in `DELIVERY_CLOSED_STATUSES` → Storico) e il `DeliveryLog` registra chi e quando. Vale anche per `PATCH massa/elimina`, che chiama lo stesso metodo. Verificato: `findAll(view:'storico', status:'invalidated')` → 228 righe. ⚠️ La #101331 di oggi NON è recuperabile: è stata cancellata dal codice vecchio.
> - **Luoghi delle consegne passate**: `api/scripts/riconcilia-luoghi.mjs` + dizionario `api/scripts/luoghi-hotel.json` (482 indirizzi, 452 nomi confermati da Google Places). **Applicato: 7.136 consegne**. ⚠️⚠️ Due trappole già cadute e risolte, non ripeterle: (1) la chiave dell'indirizzo vuole VIA + CIVICO + CAP — col confronto sull'intera stringa, 7.605 consegne con indirizzo «Milano MI, Italia» finivano tutte al Park Hyatt; (2) i nomi cercati dentro citofono e note vogliono i confini di parola, se no «Panettone Mandarino» diventa l'hotel Mandarin. E dove Google risponde con un civico diverso si richiede il solo indirizzo: così Piazza della Repubblica 17 è Principe di Savoia e non ME Milan Il Duca.
> - **Statistiche partner**: `api/src/statistiche/statistiche-partner.module.ts` (rotta `GET /statistiche-partner`, ruoli PARTNER/ADMIN/OPERATION; il partner legge l'id dal token) e `web/src/app/pages/statistiche-partner.component.ts` (rotta `/statistiche-partner`, voce di menù per i PARTNER). Le sette classifiche partono a **onde di tre** query: in fila erano 6,9 s su Clivati (7.962 consegne), ora ~1 s a cache calda, ~5 s alla prima lettura a freddo. Tre e non di più: il pool del pooler è piccolo (`connection_limit=5` in locale, 3 in produzione) ed è già stato esaurito il 10/09.

> 📤 **11/09/2026 (sera, 33) — ESTRAZIONE DI BASE + LUOGO IN MODIFICA** (manuale 138).
> - `DELIVERY_EXPORT_SELECT` in `deliveries.service.ts`: selezione propria dell'estrazione, più corta di quella dell'elenco. `findAll(…, { leggero: true, soloBase: true })`. ⚠️ `serviceType.hideCustomerInfo` e `deliveredByPartner` restano nella selezione: la mascheratura dei dati del cliente li legge.
> - Tolte le colonne «Prodotti» e «Margine €/%»: uscivano SEMPRE vuote (l'elenco non seleziona `products`, e il margine arriva dalla Finanza, che la modalità leggera salta).
> - `delivery-form.component.ts`: caricando una consegna, `ultimoLuogoAuto` prende il luogo salvato — se no il modulo lo scambia per scritto a mano e non lo sostituisce col posto del nuovo indirizzo.
> - 🔎 **Misura, per chi riprende il tema «export lento»**: NON è il server. Contesto Nest locale sul DB di produzione — ufficio 32 righe 232 ms, 265 righe 251 ms; partner Chanel 72 righe 396 ms. L'API in produzione risponde in 74 ms (`/api/v1/deliveries/esporta`, 401 senza token). ⚠️ Attenzione: `app.deluxy.it/api/...` senza `v1` risponde 200 con la PAGINA ANGULAR — un controllo fatto lì non prova nulla. Se l'utente vede ancora «Preparo il file» all'infinito, il prossimo passo è guardare rete e console DEL BROWSER, non il servizio.

> 🈯 **11/09/2026 (sera, 32) — LE CHIAVI DEL CAMPO «LUOGO»** (segnalazione dell'utente: a schermo si leggeva `deliveryForm.fields.luogo`). Il campo era nato con le chiavi `deliveryForm.fields.*`, ma il dizionario qui usa `field` / `hint` / `placeholder` al SINGOLARE: senza traduzione il pipe stampa la chiave. Corretto il componente e aggiunte `deliveryForm.field.luogo`, `.hint.luogo`, `.placeholder.luogo` in `web/public/i18n/it.json` e `en.json`. ⚠️ Il dizionario si modifica a mano: riscriverlo con `json.dumps` espande gli oggetti scritti su una riga sola e sporca il diff di centinaia di righe.
> **Controllo che conviene rifare**: uno script estrae tutte le `'x.y' | translate` da `web/src/**/*.ts` e le cerca nei due dizionari — oggi 2.147 chiavi, 0 mancanti in italiano e in inglese. Le chiavi composte a runtime (`'status.delivery.' + d.status`) non le vede.

> 📍 **11/09/2026 (sera, 31) — LUOGO IN ELENCO + TRE QUANTITÀ CORRETTE** (manuale 136).
> - **Elenco consegne**: nella cella dell'indirizzo, `@if (d.recipientPlace)` aggiunge una seconda riga «📍 …» (stile `.luogo-riga`); `recipientPlace` dichiarato in `web/src/app/core/models.ts` (l'API lo mandava già: è in `DELIVERY_LIST_SELECT`). `tsc` web verde.
> - **Quantità a 1** su #101230 (35 → 1 × 80 €), #101207 (15 → 1 × 65 €), #101229 (10 → 1 × 60 €) — script `api/scripts/quantita-extra-a-uno.mjs`, **applicato in produzione** l'11/09 dopo anteprima; si ferma da solo se una consegna risulta fatturata, e scrive una nota nel registro della consegna. Non è più un punto aperto.
...
> - **Estrazione più leggera**: `findAll(user, query, { leggero: true })` salta margini, puntualità e aggancio vendite; la usa `esporta`. Era il motivo del «Preparo il file» infinito su 72 righe.

> 🔢 **11/09/2026 (pomeriggio, 29) — MARGINI DALLA FINANZA + ESTRAZIONE EXCEL DELLE CONSEGNE** (manuale 131-132). 🚀 **LIVE `delivery-7ticd1jiu`**.
> - **Margini**: `FinanceService.margineDiConsegna(id)` e `marginiDiConsegne(ids)` (nuovi, pubblici) riusano `computeRow` + `recap`, cioè il conto della pagina Finanza; `DeliveriesService` importa `FinanceModule` e li chiama per il dettaglio e per la lista. Tolto il vecchio calcolo (`margineVendita` fatto in casa) e le sue quattro query per pagina. Web: riquadro riscritto sui campi nuovi, i18n `deliveryDetail.margini.*` rifatte.
> - **Estrazione**: `GET /deliveries/esporta` (CSV con BOM e punto e virgola, tetto 20.000 righe, intestazione `X-Troncato`) passa da `findAll`, quindi stessi filtri e stessi permessi della lista: il partner scarica le sue senza i numeri del valet, l'ufficio scarica tutto con valet, paga, «da pagare» e margine. Web: bottone «Esporta per Excel» accanto a «Ricerca avanzata» e `parametriRicerca()` estratto da `load()` (una sola lista di filtri per lista ed estrazione).

> 💶 **11/09/2026 (mattina, 28) — ANTICIPO/EXTRA DA STIPENDI** (manuale 130). 🚀 **LIVE `delivery-fz9mj3dbc`**. Web `salaries-list`: bottone «Richiesta pagamento» nella riga di «Da pagare» + finestra in fondo al template (`richiestaPer`, `ibanValet`, `apriRichiesta(valetId, nome)`, `richiestaValida`, `inviaRichiesta`), stili della modale copiati dalla scheda valet; l'IBAN si legge con `GET /valets/:id` all'apertura. Nessuna rotta nuova: `POST /valets/:id/richiesta-pagamento` è quella del 09/09. i18n riusate (`valetDetail.pagamento.*`). `ng build` ok.

> 🔴 **11/09/2026 (mattina, 27) — CONSEGNE IN ERRORE PER IL POOL DI CONNESSIONI** (manuale 129). Sintomo: «Internal server error» sulla lista Consegne. Log: `Timed out fetching a new connection from the connection pool (timeout: 10, connection limit: 3)`. Causa: il blocco del margine (10/09) in `deliveries.service` faceva `Promise.all` di QUATTRO query per ogni pagina — con tre connessioni nel pool le prendeva tutte. Corretto: letture in sequenza (commit `3e768f20`), 🚀 **LIVE `delivery-229pvicym`**, poi **`delivery-kkdhqissx`** (rebase sul commit «fee sulle vendite al registro» arrivato da un'altra sessione: il deploy precedente non lo conteneva, ripubblicato includendolo). Log puliti dopo il deploy. ⚠️ Visto anche `FATAL: (EMAXCONN) max client connections reached, limit: 200` sul pooler Supabase (porta 6543, condiviso con le altre app): rientrato da solo; il Postgres stava bene (29 connessioni su 60, nessuna query lenta). 📌 Da valutare col custode PERFORMANCE: alzare `connection_limit` nella variabile di produzione e non aggiungere altre query per riga nelle liste.
> - 🔎 **Paga = chilometri: lo script c'è** (`api/scripts/paga-uguale-ai-km.mjs`, SOLA LETTURA, anche `--csv`, `--mese`, `--valet`). ⚠️ Trappola trovata scrivendolo: `pagaConsegna` dà la precedenza alla paga SCRITTA (`origine: consegna`), quindi il confronto va fatto passando `valetSalary: null`, se no «combacia» sempre. Esito su 1.351 consegne: **8 combaciano col listino, 109 listino muto, 1.234 diverse** (pagato 23.464,68 € contro 11.801,72 € che direbbe il listino di oggi). Dicembre 2025 da solo pesa 581 casi su 1.351. Su #62926 il listino direbbe 13,50 € invece di 6,43 €. ⚠️ Il listino di OGGI non è un metro giusto per consegne del 2025: resta un punto aperto, nessuna scrittura fatta.

> 🧾 **11/09/2026 (mattina, 26) — STIPENDI, FATTURAZIONE E CORRELATE** (manuale 128). 🚀 **LIVE `delivery-memlcydky`** (verificato `it.json` con le chiavi nuove). API `salaries`: `pending/:valetId` porta `partner`, `payable`, `plusScritto`, `plusRegola` (query con `partner.insegna`); `dettaglio/:id` include partner, orari, `payable`, `valetAdditionalPrice`, regola. API `invoices.findAll` (ora `async`): campo `venduto` per fattura, calcolato dalle righe con `calcolaValoreProdotti` sulle sole consegne VENDITA, una lettura per pagina. Web: colonne nuove nelle due tabelle di Stipendi (+ `orarioRiga`/`regolaRiga`/`plusRiga`), chip per `pricingModel` in Fatturazione (`modelliPresenti`, `scegliModello`) e colonna Venduto, colspan 8/9; `delivery-detail`: cella «cosa» sempre resa nelle consegne con lo stesso DDT + ellissi sulle altre celle. `tsc` verdi, `ng build` ok, avvio Nest ok.
> - 🔎 **Perché la paga della consegna #62926 è 6,43 €**: è il valore SCRITTO su `Delivery.valetSalary` (importato dal sistema precedente), non un calcolo: il valet non ha listino per «Vendita Deluxy» (ha Consegna Standard 12,50 € e Servizio a Ora 12,50 €). ⚠️ Coincide esattamente con `distanceKm` (6,43) e lo stesso accade su **1.351 consegne** su 19.631 con entrambi i valori (35 in agosto, 51 a luglio, 85 a giugno…): sospetto scambio di colonne nell'import. Il database legacy non è collegato (`api/.env.legacy` non esiste), quindi la prova non è stata fatta: **da decidere con l'utente** se ricalcolare quelle paghe dal listino.

> 📁 **11/09/2026 (mattina, 25) — CARTELLA DRIVE DA LINK + «NOME ALTERNATIVO» NASCOSTO AL PARTNER** (manuale 127). API `settings.module`: `SettingsService.idCartellaDrive(v)` (statica) usata da `cartellaFileApp` e `driveConfigurato` — accetta id, `/folders/<id>`, `?id=<id>`; senza, ogni upload dava «File not found» perché in `driveFolderId` c'era il link intero. Web `product-form`: toggle e campo `alternateName` dietro `@if (!isPartner())`. `tsc` verdi, `ng build` ok, avvio Nest ok.

> 🗺️ **11/09/2026 (mattina, 24) — ELENCO PARTNER CON LE AREE** (manuale 126). 🚀 **LIVE `delivery-pv3mcl4dw`**. Web `partners-list`: `primeAree/altreAree/restoAree`, colonna `partners.col.aree`, ripiego province; chiave di ricerca `aree.area.nome`.

> 🧩 **11/09/2026 (mattina, 23) — MODULO PRODOTTO DEL PARTNER + CARICAMENTO FILE** (manuale 125). 🚀 **LIVE `delivery-qo8f7la6z`** (verificato: `/products/upload` 401, `it.json` con «Carica dai tuoi file»). Web `product-form`: blocchi `@if (!isPartner())` su tipologia+legenda, Partner, prezzo pubblico (prodotto e varianti), flag unico, Shopify, partner aggiuntivi, superprodotto, campi personalizzati; categorie filtrate da `GET /partners/:id` (le sue); default `isUnique=true, tipologiaVendita='unico'`; scheda «Immagini» a parte con `caricaFile()`/`caricaFileVariante()` → `POST /products/upload` (multipart, memoryStorage 25 MB, `SettingsService.caricaSuDrive`; `ProductsModule` importa `SettingsModule`); `product-detail` galleria con `<video>`. API `products.service`: create per PARTNER impone UNICO/unico e toglie publicPrice, superprodotto, visibilità; update scarta i campi dell'ufficio. i18n `productForm.images.carica*`. `tsc` verdi, `ng build` ok, avvio Nest ok.

> 🏷️ **11/09/2026 (mattina, 22) — NOME PARTNER NELLA CONSEGNA + NOMI SPECCHIATI** (manuale 124). 🚀 **LIVE `delivery-bm7c0x8pl`**. API `deliveries.service`: `DELIVERY_INCLUDE.products.product` porta `alternateName/useAlternateName`; nel blocco PARTNER di `soloIMieiSoldi` il nome del prodotto diventa il nome partner (e i due campi non escono). Dati: `api/scripts/nomi-partner-da-merchandising.mjs --applica` → 3.005 prodotti (198 con nome diverso dal commerciale). 🔖 «Allinea note» in Impostazioni resta la strada per i prossimi allineamenti (note e nomi); valutare un cron.

> 🔘 **11/09/2026 (mattina, 21) — «DISATTIVA / RIATTIVA» IN TABELLA PARTNER** (manuale 123). 🚀 **LIVE `delivery-gzdrynx5l`** (verificato `it.json` con `partners.reactivate`). Web `partners-list`: `cambiaAttivazione(p)` + `attivazioneInCorso`, i18n `partners.reactivate`, `partners.deactivateConferma`.

> 🧾 **11/09/2026 (mattina, 20) — FINANCE INTESTA AL CAPOGRUPPO** (decisione utente dopo la tabella delle tre app). `deluxy-partner`: `intestatarioFattura()` in `src/lib/anagrafiche.ts` (tipo `Anagrafica` + `pagaDaSe`, `capogruppo`), usata in `proforma-documento.ts` (nome = capogruppo, riga «per conto di <sede>») e `fic/fattura/page.tsx` (cliente FIC). Commit `cf541303` su `scout-ui`, 🚀 **LIVE `deluxy-partner-i8z44lxve`** (Production, Ready). ⚠️ L'indirizzo resta della sede (il capogruppo del registro non ha indirizzo). Handoff aggiornati anche in Finance (`deluxy-partner/docs/HANDOFF.md`), Anagrafiche (`HANDOFF.md`) e Merchandising (`docs/HANDOFF.md`), commit `986e0bb6`.

> 🏢 **11/09/2026 (mattina, 19) — DATI DI FATTURAZIONE DAL CAPOGRUPPO + SEZIONE CAPOGRUPPI** (manuale 121-122). 🚀 **LIVE `delivery-o5s39de91`**, poi **`delivery-6e54cs1le`** (CSS della pagina Capogruppi: tabella, pillole e azioni erano senza stile perché quelle classi le altre liste le dichiarano in locale, non in `styles.css`). ⚠️ **Nota per Finance**: oggi Finance intesta pro-forma e fatture alla SINGOLA scheda del registro (P.IVA/SDI/IBAN della scheda), non al capogruppo — l'API del registro manda `capogruppo` e `pagaDaSe` ma la sostituzione (`leggiFatturazione`) la fa solo il registro nelle sue pagine; Finance usa il capogruppo solo per «fatturato per gruppo». Deciso e fatto l'11/09 (voce «mattina, 20»): Finance intesta al capogruppo. API `partners.service`: `datiDalCapogruppo(capogruppoId, pagaDaSe)` → {businessName, vatNumber, fiscalCode, sdiCode, certifiedEmail, invoiceEmail} (solo i compilati) applicato in create, update (stato DOPO), `rispecchiaCapogruppoDalRegistro`, `mettiSottoEntita`; `CapogruppiService.aggiorna` propaga alle sedi con `pagaDaSe=false`. Web: `campoBloccato()`/`valoreMostrato()` nel modulo partner (6 campi readonly + avviso + link), nuova pagina `pages/capogruppi-list.component.ts` (rotta `/capogruppi`, voce `nav.capogruppi`, bottone nell'elenco partner), i18n `capogruppi.*`. `tsc` verdi, `ng build` ok, avvio Nest ok.

> 🛍️ **11/09/2026 (mattina, 18) — PRODOTTI DEL PARTNER → MERCHANDISING DA APPROVARE → SHOPIFY** (manuale 120). 🚀 **LIVE piattaforma `delivery-63sul1gr8` + Merchandising `deluxy-merchandising-icufl4arz`** (verificato: API 401, `it.json` con `namePartnerHint`). Piattaforma: `products.service.create` (partner → `alternateName`/`useAlternateName`; `spingi(creato, { partner })`), `merchandising-sync.inviaOra(product, extra)` (origine partner, nomePartner, fase prototipo, noteSviluppo), hint `productForm.field.namePartnerHint`. Merchandising (scout-ui): `POST /api/v1/prodotti` accetta nomePartner/nomePartnerAttivo/noteSviluppo; `cambiaFase` → se manca `shopifyId` e la fase è approvato/in_vendita, redirect a `/prodotti/:id/modifica?fase=…` (banner `.avviso-info`, fase preimpostata). `tsc` verdi (piattaforma e Merchandising), `ng build` ok, avvio Nest ok.

> 💶 **11/09/2026 (mattina, 17) — MARGINI SUL TOTALE PAGATO DAL CLIENTE** (manuale 119). 🚀 **LIVE `delivery-jaqtz39no`** (verificato `it.json` con «Totale pagato dal cliente»). API: nuovo `orders/orders-client.module.ts` (`OrdersClientService.mappa(da)` cache 2′ con chiavi id Orders / numero gid / `n:<numero>`, `perVendita(v)` con ripiego sulla rotta del singolo `/api/v1/ordini/:id`); `SalesService.statiDaOrders` delega al client (tolta la cache locale); `DeliveriesService` lo inietta (DeliveriesModule importa `OrdersClientModule`): dettaglio → `margineVendita.{prodottoVendita,totaleOrdine,righeProdotti,extraOrdine,venditeNellOrdine,prezzoCliente}`, elenco → `prezzoCliente = amount + (totale − Σ amount dell'ordine)/n`. Web: righe nuove nel riquadro, i18n `deliveryDetail.margini.*`. `tsc` verdi, `ng build` ok, avvio Nest ok.

> 🔢 **10/09/2026 (sera, 16) — NUMERO DI RECORD SOPRA OGNI TABELLA** (manuale 118). 🚀 **LIVE `delivery-99jdlc58a`** (il primo tentativo di deploy non ha pubblicato — nessun URL in uscita; ripetuto e verificato: `it.json` servito con `comune.record`, API 401). Web soltanto: `<p class="conto-record">` prima del `table-wrap` in 16 liste (Fatturazione e Stipendi ne hanno due), stile in `styles.css`, i18n `comune.record`/`recordFiltrati`; tolte le vecchie `conto-righe` condizionali (Vendite, Pagamenti, Ricevute, Attività). `ng build` ok.

> ✅ **10/09/2026 (sera, 15) — APPENA ACCETTATE** (manuale 117). 🚀 **LIVE `delivery-fpbv0uk5j`** (radice del worktree; verificato `/deliveries` 401 e `it.json` con «Appena accettate»). **Fine giornata 10/09: tutto pushato e pubblicato; aperti solo i punti che aspettano l'utente (indice Sale.deliveryId, 115 chiuse giugno-luglio, «allinea note», artifact dei manuali).** Web: `appenaAccettate` signal + `mostraAppenaAccettata(id)` (GET /deliveries/:id dopo `POST /sales/:id/accetta`), sezione `.appena-accettate` sopra la tabella, i18n `deliveries.proposte.appena*`. `ng build` ok.

> 🌆 **10/09/2026 (sera, 14) — CINQUE RICHIESTE** (manuale 116). 🚀 **LIVE `delivery-15chra7fd`** (dalla radice del worktree; verificato: `/sales/stati-esterni`, `/deliveries`, `/sales` rispondono 401; `it.json` servito con «Ordina per», 8 chiavi `sales.filtri`, «Vendita accettata ✓»).
> - Dettaglio consegna: `puoRispondereVendita()`, `accettaVendita()`, `rifiutaVendita(motivo)` (rotte già esistenti `accetta-vendita` / `rifiuta-vendita`), pillola «Vendita accettata»; `DeliveryDetail.acceptSale`.
> - Elenco consegne: `ORDINAMENTI` + `invertiVerso()` nel pannello filtri (`.ordina-mobile`, visibile ≤ 800 px); `margineBasso(d)` → `tr.margine-basso` rosso; API lista: `r.margine {euro, percent, conValet}` per ADMIN/OPERATION sulle righe VENDITA (4 letture per pagina: DeliveryProduct, PartnerService, ValetService, Valet), `productValue` nel select, `amount` nel join delle vendite.
> - Vendite: `findAll` non aspetta più Orders/CS; nuova `GET /sales/stati-esterni` (`statiEsterni(user)`); web `caricaStatiEsterni()` fonde `ordine`/`customerService`; filtri `fProvincia/fPartner/fBrand/fTipologia/fDal/fAl` + `filtriVersione` signal per il `computed`.
> - Clivati: le due accettazioni di ieri sera esistono (#101246, #101248); erano sepolte sotto 616 consegne vecchie chiuse stamattina.

> 💶 **10/09/2026 (notte, 13) — MARGINI DELLA VENDITA NEL DETTAGLIO CONSEGNA** (manuale 115). 🚀 **LIVE `delivery-4n0yq9l3q`** (dalla radice del worktree, progetto «delivery»; verificato: API 401 su `/deliveries`, `it.json` servito con «Chi paga (capogruppo)» e le 14 chiavi `deliveryDetail.margini`).
> - API `deliveries.service.findOne`: `margineVendita` (solo ADMIN/OPERATION; per PARTNER e VALET il campo è tolto) da `Sale` (`amount` = totale pagato dal cliente, `discountPercent`, `externalOrderNumber`) + `economiaVendita` + paga valet (`valetSalary ?? valetSalaryDalListino` + `valetAdditionalPrice`).
> - Web `delivery-detail`: sezione `margini-vendita` sotto il conto della vendita; i18n `deliveryDetail.margini.*`.
> - 🐛 Corretto: le 8 chiavi `partnerForm.payments.capogruppo*`/`pagaDaSe*` erano nel blocco sbagliato degli i18n (uscivano grezze nel modulo partner, «illegibile»): spostate in `partnerForm.payments`. Va pubblicato per sistemare il modulo in produzione.

> 🔴 **10/09/2026 16:46–16:52 — API GIÙ IN PRODUZIONE per ~6 minuti** dopo `delivery-h5cqxlksm`: ogni rotta 500 (`FUNCTION_INVOCATION_FAILED`), nei log «Nest can't resolve dependencies of the PartnersService (…, CalendarioUniciService, ?)» — `PartnersService` iniettava `CapogruppiService` ma `PartnersModule` non importava `CapogruppiModule`. `tsc` non lo vede: lo vede SOLO l'avvio. Corretto (commit `1b125f5b`) e ripubblicato: **LIVE `delivery-d3sseze1f`**, verificate 401 su `/capogruppi`, `/salaries/servizi-valet`, `/cron/tipologie`, `/salaries/pending`. 📏 **Regola da oggi: prima di ogni deploy dell'API, `nest build` + avvio locale (`PORT=3999 node dist/main.js`) fino a «Nest application successfully started», poi `taskkill` del processo (altrimenti la dll di Prisma resta bloccata e `vercel build` fallisce con EPERM).**
>
...
> 🧾 **10/09/2026 (notte, 9) — FATTURA DELLE CONSEGNE IN FINANCE: nasce dal mese; la pro-forma non porta più le vendite; all'invio riuscito la fattura interna va in Storico.** (segnalazioni utente su Rizzi agosto)
> - **Finance** `/api/consegne-mese` accetta `consegne: { imponibile, descrizione }` → crea/aggiorna una **FatturaServizio** tipologia «Consegne» (non pagata, marcatore `[consegne <rif>]`) — prima la faceva l'ufficio a mano (648/2026 di Diptyque). `venduto` può essere 0 se ci sono consegne; `VenditaVendor` si scrive solo con venduto > 0. Risposta: `fatturaConsegne`.
> - **Piattaforma** `invoices.module.ts`: `inviaBozzaAFinance` manda solo le righe NON VENDITA (le vendite viaggiano col mese: in PF 36/2026 c'erano 2 righe di vendita, contate due volte) e con solo vendite non crea pro-forma; `inviaMeseAFinance` manda `consegne.imponibile` (somma delle righe non vendita) e non si ferma più senza vendite; **all'invio riuscito (pro-forma o mese) la fattura interna diventa `archived=true`** («ho fatto invio ma non è andata in storico»). Fatturazione: periodo rapido con stato attivo.
> - **Riparato a mano Rizzi agosto** (`scratchpad/ripara-rizzi.mjs`): PF 36/2026 da 20 a 18 righe (tolte Commenda 12 · 5,20 e Carlone 4 · 7,00), FatturaServizio «Consegne» agosto 2026 **193,65 €** creata col marcatore `[consegne FAT-2026-565]`, FAT-2026-565 archiviata. Il saldo di agosto in Finance ora ha Fatture IVATE 236,25 € contro dovuto vendite 46,12 €.
> - 🔖 Capogruppo in piattaforma: tabella `Capogruppo` + `Partner.capogruppoId/pagaDaSe` CREATE in prod (`applica-migrazione-capogruppo.mjs`), modulo `capogruppi/capogruppi.module.ts` scritto ma NON ancora cablato (app.module, DTO partner, sync col registro, web): in corso.

> 🔴 **10/09/2026 (notte, 8) — INCIDENTE E RIMEDIO: «Sotto un'altra entità» ha riscritto una scheda del registro. Ripristinata. Guardie messe in tutt'e due le app.**
> - **Cosa è successo** (registro delle modifiche di Anagrafiche, ora UTC): la scheda `cmrovn4m8006gi6pc3p8rf0sn` era «OLFATTORIO», Firenze, prospect, da Excel. Alle **13:38** la sincronizzazione automatica della piattaforma (salvataggio del partner «Olfattorio Roma») l'ha trovata **per P.IVA** (08699330018, comune a 6 sedi) e l'ha riscritta con nome, indirizzo, email, telefono e condizioni vendor di Olfattorio Roma. Alle **13:40** il bottone «Fattura sotto questo» su Diptyque Brera → Olfattorio: `mettiSottoCapogruppo` ha fatto `sincronizzaOra(p, trovato.id)` per tutt'e due i partner sulla STESSA scheda trovata per P.IVA → riscritta prima con Diptyque Brera (platformId compreso) poi con Olfattorio (ragione sociale = un indirizzo email). Poi la rotta capogruppo non ha trovato la sede per platformId → 404, nessun capogruppo creato; l'esito era mostrato in fondo alla sezione, non sotto i bottoni: «sembrano non fare nulla».
> - **Ripristino** (`scratchpad/registro-ripristino.mjs`, scrittura diretta sul registro): 13 campi riportati al primo «da» del registro delle modifiche (nome OLFATTORIO, stato prospect, Via de' Tornabuoni 6, email/telefono/condizioni vuoti, platformId null, fonte excel, ragione sociale OLFATTORIO SRL); la coda delle note originali oltre «(31/07/20…» non era conservata nel log e resta persa — scritto nelle note. 13 righe di Modifica con origine «deluxy-delivery (ripristino 10/09)».
> - **Guardie in Anagrafiche** (`0d1179c8`, pubblicato): POST agganci per P.IVA/CF **solo se la sede è una** e non porta il platformId di un altro; la ricerca per nome esclude le schede già di un altro platformId; PATCH rifiuta (409) di spostare il platformId di una scheda già collegata a un altro punto vendita.
> - **Guardie in piattaforma**: `mettiSottoCapogruppo` non fa più PATCH su una scheda «trovata»: vale solo quella con `platformId === partner.id`, altrimenti POST di una scheda nuova (che il registro ora non fonde più per P.IVA). L'esito compare sotto i tre bottoni.
> - ⚠️ **Resta da guardare**: la sincronizzazione automatica al salvataggio (`sincronizza()` → POST) prima di oggi poteva fondere per P.IVA anche altri partner multi-sede (Basara, Chanel, Martesana, Clivati…): con la guardia del registro non succede più, ma le fusioni già avvenute prima del 10/09 non sono state cercate.

> 🚀 **10/09/2026 (notte, 7) — LIVE: piattaforma (entità di fatturazione, filtri Dal/Al, lista sul mese generato, fino a `e94844a2`), Anagrafiche (rotta capogruppo, `a98b0e29`), Finance (pro-forma col motore, `678fc1ed`).** Comando utente «fai deploy». Piattaforma precompilata dalla radice del worktree; Anagrafiche e Finance in cloud (`npx vercel --prod --yes` dalla cartella).

> 🧾 **10/09/2026 (notte, 6) — RIZZI AGOSTO: la pro-forma delle consegne non era partita perché Finance non riconosceva il nome.** FAT-2026-565 (01–31/08, 20 righe: 18 consegne standard 193,65 € + 2 vendite 12,20 €, creata oggi 14:53) è DRAFT con `financeRef = null`: in Finance esistono il mese (VenditaVendor 61 € fee 20 %, SaldoMensile agosto) e la fattura commissioni **650/2026**, ma NESSUNA pro-forma. Causa: `/api/proforma` di Finance cercava il partner per id, nome esatto o «contiene» — «Enrico Rizzi Milano» non è contenuto in «ENRICO RIZZI» —, mentre `/api/consegne-mese` usa `matchPartner` (regola del motore). Corretto in Finance (`scoutwt/deluxy-partner`, la pro-forma ora ripiega su `matchPartner`), pubblicato in cloud. 🔖 Da fare dall'ufficio: Fatturazione → Fatture → «Mese scorso» → FAT-2026-565 → «Invia a Finance» (financeRef è vuoto, quindi riparte senza forza). «È scomparso tutto»: le 20 consegne sono uscite dal Da fatturare (fatturate) e la fattura di agosto non compariva perché la lista era sul mese in corso → ora dopo la generazione il periodo si allinea al mese generato. Anche: filtri «Dal/Al» in un blocco solo, tipologie su una riga (segnalazione utente).

> 🏢 **10/09/2026 (notte, 5) — ENTITÀ DI FATTURAZIONE: le tre strade nella sezione Registro della scheda partner (capogruppo, scelto fra i partner).** In locale, `tsc` verdi su piattaforma API/web e Anagrafiche; NON deployato (piattaforma da precompilato, Anagrafiche in cloud come Merchandising).
> - **Registro** (`scoutwt/deluxy-anagrafiche`): nuova `POST /api/v1/partners/:id/capogruppo` (scrittura): risolve sede e `capofila` per id o platformId; usa il capogruppo della capofila o lo CREA (nome = ragione sociale della capofila, dati fiscali copiati, la capofila ci entra restando «paga da sé»); la sede entra con `pagaDaSe=false`; `registraModifica` su tutt'e due.
> - **Piattaforma**: `AnagraficheSyncService.mettiSottoCapogruppo(sede, capofila)` (prima assicura le due schede nel registro con cerca→sincronizzaOra), `cercaPerNome(q)`; `PartnersService.mettiSottoEntita`, `cercaAnagrafiche`; `confrontaAnagrafica` risponde anche `capogruppo` e `pagaDaSe`; rotte `POST :id/anagrafica/capogruppo` e `GET anagrafiche/cerca`. Web: tre bottoni dopo il confronto (crea · aggancia con ricerca libera · sotto un'altra entità con ricerca fra i partner della piattaforma), riga «Fattura sotto: …», i18n `partnerAnagrafica.entita.*`.
> - 🔖 Da provare a schermo dopo il deploy: Diptyque Brera → «Sotto un'altra entità» → Olfattorio Rinascente (o Olfattorio) → il registro crea il capogruppo «OLFATTORIO SRL» e la sezione mostra «Fattura sotto: OLFATTORIO SRL». Poi in Finance controllare che la scheda di Diptyque Brera legga i dati dal capogruppo (`lib/fatturazione.ts` del registro).

> 🚀 **10/09/2026 (notte, 4) — LIVE piattaforma `main-43TAJKNR` (attività con partner/assegnazione, conto per l'ufficio) e MERCHANDISING pubblicato in cloud.** Il server locale 3120 (PID 8276) è stato fermato su comando dell'utente; `vercel deploy --prebuilt` per Merchandising FALLISCE sempre su Windows (120 collegamenti simbolici in `.vercel/output/functions`, come dice il suo handoff): si pubblica con `npx vercel deploy --prod --scope deluxy` (compilazione in cloud). 12927: la vendita è giusta a 85 € (riga d'ordine: Bouquet Pink Grace, variante Medio, sku SHUXWA2, 85 €); i 45 € sono il prezzo base di Merchandising (variante Piccolo). Il prodotto in Merchandising HA la categoria (FIORI), in piattaforma i 5 «Pink Grace» (uno per taglia, dal legacy) no: la sincronizzazione mette la categoria solo ai prodotti che crea. Misura: 899 attivi senza categoria, 668 hanno una categoria in Merchandising (417 DA_CLASSIFICARE, 133 FIORI, 63 TORTE_DOLCI, 32 BOUQUET, 8 REGALI, 7 VINI_SPIRITS, 4 ORIGINALI_DELUXY). Serve la mappa Merchandising→piattaforma (proposta in chat) prima di scrivere.

> 🧑‍🔧 **10/09/2026 (notte, 3) — LIVE su app.deluxy.it (bundle `main-G3KYCEA3`) tutto il locale fino a `6b2f41c1`; poi ATTIVITÀ con partner e assegnazione, ed etichette del conto vendita per l'ufficio (in locale, da pubblicare).**
> - **Deploy**: `vercel pull/build/deploy --prebuilt --prod` dalla radice del worktree; verificato dal vivo: `/api/v1/cron/calendario-unici` e `/merchandising-sync/calendario/prova` rispondono 401 (esistono). Push su origin `platform-0409` fino a `6b2f41c1`. ⚠️ **Merchandising NON pubblicato**: `vercel build` fallisce in `npm install` → `prisma generate` con EPERM su `query_engine-windows.dll.node`, tenuto aperto da un server locale di Merchandising sulla porta **3120 (PID 8276)**, di un'altra sessione: va fermato prima di ricompilare. Finché Merchandising non è live, il calendario dei prodotti unici e i nomi partner NON arrivano (la piattaforma chiama rotte che di là non esistono ancora: la POST risponderà 404 e il log lo dirà).
> - **Consegne ferme**: chiuse **1.103** come `approved` + `invoiced=true` (decisione utente «metti già fatturate»): fuori dalle Attive e dal Da fatturare, reversibili dal registro. Di queste **115 sono di giugno-luglio 2026** (Artista Locale 101, Deluxy Flowers 14, tutte «Vendita Deluxy» mai chiuse): segnalato all'utente, in attesa se vanno riportate indietro.
> - **Attività** (manuale 108): `GET /activities` porta partner/provincia/servizio della consegna; `PATCH /activities/:id/assegna {valetId}` (ADMIN, OPERATION, VALET team leader) → `assignValet` della consegna (cambia consegna + tutte le attività + registro). Web: colonna Partner, tendina «Assegna a…» filtrata per provincia e servizio, `puoAssegnare` = ufficio o `isTeamLeader`. `ActivitiesModule` importa `DeliveriesModule`.
> - **Conto vendita per l'ufficio** (manuale 107): `contoKey()` nel dettaglio sceglie `saleAccount.*` (partner) o `saleAccountOffice.*` (ufficio: «Totale dovuto al partner: il suo guadagno»).
> - 🔖 Restano in attesa dell'utente: «vai» per `applica-indice-sale-deliveryid.mjs` (l'indice unico rompe «due vendite → una consegna» in prod); fermare il server 3120 per pubblicare Merchandising; le 115 di giugno-luglio.

> 💶 **10/09/2026 (notte, 2) — QUOTA SULLA CONSEGNA: decisa e corretta; agganci 4/5; TROVATO l'indice UNICO su Sale.deliveryId ancora in produzione.**
> - **Decisione utente**: «60 € è il valore da cui incassare la nostra commissione + IVA» → la riga della consegna nata da vendita vale `amount × (1 − discountPercent) / pezzi` quando la quota è > 0 (`prezzoRigaVendita`, precedenza invertita: prima vinceva il listino della variante, che sui non unici è il pubblico). `Delivery.price` resta vuoto: la scheda applica fee% del listino + IVA sul valore di riga. Corrette in prod (`correggi-riga-vendita-quota.mjs`): #101251 85→60, #101138 130→108. Misura: 69 vendite dal 06/09, 11 già giuste, 31 senza riga abbinabile, 27 diverse di cui **solo 2** al pubblico; le altre 25 hanno righe più basse del pubblico (listino/patto) e non si toccano.
> - **Agganci vendita→consegna** (`aggancia-vendite-a-consegne.mjs`): fatti 12804→#63031, 12818→#63124, 2729→#62868, 12901→#101130. **12895→#101104 NO: P2002**, perché in produzione esiste ancora `Sale_deliveryId_key` UNIQUE mentre lo schema dal 07/09 dice `@@index`. ⚠️ Vuol dire che la regola «due prodotti stesso partner → una consegna» **fallisce in prod all'accettazione della seconda riga**. Script pronto, NON applicato (schema condiviso): `applica-indice-sale-deliveryid.mjs --applica` (DROP unico + CREATE non unico).
> - **Chiusura consegne ferme**: `approved` È fatturabile (`NON_BILLABLE_STATUSES` = cancelled, invalidated, not_accepted): chiudere 1.103 righe del 2012-2024 come approvate le metterebbe in «Da fatturare». Proposta all'utente: `invalidated` («Annullata d'ufficio»). In attesa.

> 📅 **10/09/2026 (notte) — CALENDARIO DEI PARTNER → SITO per i prodotti unici, e flag «PRODOTTO APP».** In locale, `tsc` API/web/Merchandising verdi, NON deployato. Migrazioni APPLICATE in prod: 4 colonne su `merchandising."Prodotto"` (ggDispEffettivo, minimoOrarioEffettivo, disponibilitaCalcolataIl, disponibilitaMotivo) e `platform."Product".prodottoApp`.
> - **Piattaforma** `api/src/merchandising-sync/calendario-unici.module.ts`: `calendario(giorni)` (unici attivi con SKU e proprietario, non prodottoApp; per partner i prossimi N giorni con `statoDelGiorno`; `senzaOrari` se nessun orario) e `manda()` → `POST <merchandisingUrl>/api/v1/prodotti/disponibilita` con la chiave di Merchandising già in Impostazioni. Rotte ADMIN/OPERATION `merchandising-sync/calendario/prova|anteprima|calendario`; cron `GET /cron/calendario-unici` (CRON_SECRET) in vercel.json **ogni mezz'ora** (`10,40 * * * *`). ⭐ Poi (regola utente, stessa sera): «se Clivati oggi chiude alle 15 e sono le 16, i suoi prodotti sono acquistabili da domani» → nel calendario OGGI è `aperto=false` con `origine=chiuso-per-oggi` quando l'ora di Roma ha passato l'ultima chiusura del giorno; il payload porta anche `alle`. È il motivo per cui l'orologio resta a mezz'ora e non a un giro al giorno: lo scatto avviene all'ora di chiusura di ciascun partner. ⭐ Su osservazione dell'utente («non è meglio ogni volta che viene aggiornato un orario?»): lo scatto vero è **al salvataggio** — `PartnersService.aggiornaCalendarioSito(partnerId)` dopo orari (create/update), eccezioni del giorno (upsert/delete) e cambio di attivazione, solo per quel partner, best-effort; il giro notturno resta per lo scorrere dei giorni (chiuso sabato: giovedì «da domani», venerdì «fra 2 giorni» senza che nessuno salvi niente).
> - **Merchandising** (`scoutwt/deluxy-merchandising`): `src/lib/disponibilita-unici.ts` (`calcolaDisponibilita` pura + `applicaCalendario` che scrive i metafield `prodotto.consegna` e `custom.minimo_orario` con `metafieldsSet` su ogni negozio dove il prodotto è pubblicato, tipo dalle definizioni in cache, solo se cambia); rotta `POST /api/v1/prodotti/disponibilita` (`?anteprima=1`); API `/api/v1/prodotti` espone `nomePartner/nomePartnerAttivo`. Il tema legge davvero quei due metafield (`delivery_date_hour_c.liquid` righe 28-49 e 54/256; `product-delivery-date.liquid`): verificato.
> - **Decisioni utente**: unico = un solo partner; senza orari → ore 9; chiusura lunga → in futuro avviso + blocco acquisto (oggi: giorni = fine finestra e motivo scritto). Base mancante: resta vuota (40 unici su 224).
> - **Flag «prodotto app»**: schema + DTO + form (toggle, i18n it/en) + esclusione dal calendario e dagli allineamenti per SKU in `merchandising-sync` (note/tipologie/nomi). Non c'è nessun push di stato verso Merchandising nella piattaforma (grep: zero), quindi non c'era altro da escludere.
> - 🔖 **Da fare dopo il deploy**: (1) `POST /merchandising-sync/calendario/anteprima` e leggere la risposta (riepilogo aggiornato/invariato/saltato/errore); (2) primo giro vero; (3) controllare sul negozio la Colazione Clivati (`prodotto.consegna` = 1 se Clivati è aperto domani, `minimo_orario` = 7 in settimana / 8 nel weekend); (4) mettere una chiusura di prova a un partner e vedere il numero salire entro mezz'ora. ⚠️ Non provato end-to-end in locale: la POST scriverebbe sui negozi veri.

> 🏷️ **10/09/2026 (sera, 2) — NOME PER IL PARTNER OVUNQUE: importato in Merchandising, mostrato nel catalogo del partner.** (regola utente: «il catalogo prodotti dei partner mostra il nome partner; se vuoto, importa i nomi attuali»)
> - **Merchandising, APPLICATO in produzione** (`scoutwt/deluxy-merchandising/scripts/nome-partner-da-nome.mjs --applica`): 5.080 prodotti con `nomePartner` vuoto → riempiti col nome attuale; `nomePartnerAttivo` acceso su tutti i 5.081 (l'unico scritto a mano, la Colazione Clivati, non toccato). Reversibile: i riempiti hanno nome partner IDENTICO al nome.
> - **Piattaforma** (`products.service.ts`, `colNomeDelPartner`): al ruolo PARTNER `findAll` e `findOne` restituiscono `name = alternateName` quando `useAlternateName` e il nome è diverso, SENZA il nome commerciale accanto (regola utente: «il partner non lo vede») — vale per catalogo, modulo consegna e ogni lista che passa da lì; ufficio invariato. `tsc` verde. 🔖 Dopo il deploy: `POST /merchandising-sync/note` porta i 5.081 nomi in `Product.alternateName` (finché non gira, in piattaforma i campi restano vuoti e il partner vede il nome commerciale).
> - **Sami, ricontrollo chiesto dall'utente**: le 6 rimesse «da pagare» sono TUTTE a Roma (CAP 001xx, valet #304 provincia RM); zero consegne Stefanelli assegnate a Sami in archivio. Il flag resta.
> - 📐 **Da progettare (richiesta utente, confermata)**: (a) i GIORNI DI DISPONIBILITÀ dei prodotti UNICI seguono le chiusure del partner (base 1 → 2 se domani è chiuso → torna 1 quando riapre; ricalcolo quotidiano); (b) il MINIMO ORARIO di consegna dei prodotti unici segue l'orario di apertura del partner per giorno. Misure: 619 unici attivi con proprietario, 62 proprietari, 51 con orari settimanali, 0 chiusure future oggi; in Merchandising 1.535 prodotti in vendita su 3.052 hanno `gg_disp_min` (Colazione Clivati = 1). Il metafield `custom.minimo_orario` il tema lo legge come UN numero per prodotto. Disegno proposto in chat del 10/09 sera; non ancora codice.

> 🏷️ **10/09/2026 (sera) — IL NOME PER IL PARTNER viaggia da Merchandising alla proposta di vendita; chiusure decise dall'utente APPLICATE.** In locale (`tsc` API e Merchandising verdi), NON deployato.
> - **Nome per il partner** (regola utente: «il nome dovresti prendere quello indicato come nome partner da piattaforma merchandising se valorizzato»). Merchandising ha `Prodotto.nomePartner` + `nomePartnerAttivo` (importati dal legacy `alternateProductName`/`isAlternateProductName`); la piattaforma ha da sempre `Product.alternateName` + `useAlternateName` ma **nessuno li riempiva** e la proposta non li leggeva. Ora: (1) `scoutwt/deluxy-merchandising` `GET /api/v1/prodotti` espone i due campi (commit nel worktree scoutwt); (2) `allineaNote` della sincronizzazione Merchandising copia anche `alternateName`/`useAlternateName` per codice (risposta con `nomiPartnerInMerchandising`/`nomiPartnerDaCambiare`; si applica con `POST /merchandising-sync/note` come le note); (3) `SalesService.nomePerIlPartner()` nel campanello e nella mail «Nuova proposta di vendita»: se attivo, il fornitore legge il SUO nome. Misura: **1 prodotto** in Merchandising ha il nome attivo — proprio la «Colazione 5 Stelle - Clivati Milano (2 persone)» → **«2 Colazioni in Famiglia»**. 🔖 Dopo il deploy: lanciare l'allineamento note (che porta anche i nomi) e controllare che `Product.alternateName` della Colazione sia valorizzato.
> - ⚠️ **Orari di apertura dei partner ↔ metafield Shopify «Minimo Orario»: NON esiste nessuna sincronizzazione.** Verificato nel codice: `custom.minimo_orario` è un metafield DI PRODOTTO («l'ora del giorno da cui si può consegnare», 7 = dalle 7, letto dal tema nel carrello); Merchandising lo **importa** da Shopify in `Prodotto.minimoOrario` e lo usa nelle regole delle collezioni, **non lo scrive**; la piattaforma non espone gli orari di apertura (`OpeningHour`) a nessuna app e Merchandising non li legge. Se lo si vuole, il disegno è: piattaforma espone gli orari via `/api/v1/app/...` → Merchandising li legge e scrive il metafield sui prodotti UNICI di quel partner. Da decidere.
> - ✅ **Chiusure applicate** (`api/scripts/chiusure-chanel-10-09.mjs --applica`): le 5 «cancellazione richiesta» → annullate (#30407, #59338, #59341 Montenapoleone; #61708 Rizzi; #62518 P.di Spagna); Rinascente #27364/#27365 (legacy 04/12/2024) → annullate; #63113 e #100876 (Sant'Andrea) → consegnate, `deliveredAt` a fine fascia. Attività chiuse come fa l'app, riga di registro su ognuna. Verifica: 0 in cancellazione richiesta.
> - ✅ **Le 6 vendite di agosto di Sami → `payable=true`** (vedi sotto).
> - **Province non risolte dall'indirizzo (1.238)**: per anno 2021=556, 2024=510, 2023=87, 2022=49, 2026=20, 2020=13, 2025=2, 2001=1; **1.230 in storico, 8 attive**; per partner Basara Vimercate 347, Chanel Montenapoleone 147, Clivati-Consegne 105, Basara Corso Italia 104. Sono presìdi in negozio e hinterland senza sigla: non pesano sul lavoro di oggi.
> - **Vendite senza consegna, i 5 agganci proposti** (non fatti): 12804→#63031, 12818→#63124, 2729→#62868, 12895→#101104, 12901→#101130.
> - Nuovo punto aperto: sulle consegne nate da una vendita la quota del CS non viaggia sulla consegna (#101251: mail 60 €, scheda 62,19 €). In attesa della scelta «netto o lordo».

> 🏭 **10/09/2026 (pomeriggio) — L'ACQUISTO CORPORATE NASCE DA SOLO; migrazione IBAN applicata; sei verifiche chieste dall'utente; due script pronti (NON applicati).** In locale, `tsc` API verde, NON deployato.
>
> **Codice** — `DeliveriesService.acquistiCorporateAutomatici()` (chiamato in fondo a `create()`): su un servizio CORPORATE, le righe con prodotto **UNICO di un partner diverso** dal cliente generano **un acquisto per fornitore** via `this.create()` con utente d'ufficio sintetico (`acquisto-corporate:<sub>`, ruolo OPERATION): servizio di vendita del fornitore (`servizioVenditaDelPartner`: «Vendita Deluxy» dal suo listino, altrimenti dal catalogo), ritiro alla sua sede (default di `create`), stessi giorno/fascia/destinatario, righe dal catalogo, `ddtNumber = CPR<code>`, registro `acquisto-automatico` su tutt'e due. Idempotente per (DDT, fornitore); best-effort (fallimento → riga di registro «Resta il bottone»). Nessuna ricorsione: l'acquisto è VENDITA. ⚠️ Solo alla nascita; `update()` non riallinea. Misura: 7 «ORDINE BRIOCHE» di Casati 14 dal 08/09 (#100922, 101146–101150, 101266), 9 righe uniche di Mali'A ciascuno, **0 acquisti** — la #101150 citata dall'utente è fra queste. Docs aggiornati: manuale (101), guida-visiva (regole del posto), MANUALE-DELUXY (registro). 🔖 Da guardare a schermo dopo il deploy: un nuovo ORDINE BRIOCHE deve mostrare subito «Acquisto della merce: #…» invece di «Manca l'acquisto».
>
> **Migrazione IBAN applicata in produzione** (`20260908_cambio_iban_in_due_passi.sql`, comando utente «Procedi»): 5 colonne su 7 c'erano già; aggiunte `emailCambiataIl` e `emailPrecedente`. Verifica: 7 righe in `information_schema`, 297 partner leggibili col client Prisma. La scheda partner non si romperà al deploy del codice dell'08/09.
>
> **Verifiche chieste dall'utente, con l'esito vero**:
> - ✅ **Plugin WooCommerce di Clivati: FUNZIONA.** La rotta `POST /api/deliveries/sync/woo-order` è viva su app.deluxy.it e deluxy-delivery.vercel.app (401 «Chiave partner mancante»), e sono arrivati **2 ordini veri**: #15197 il 06/09 (→ #101080, consegnata il 09/09) e #15328 il 09/09 (→ #101241, per il 13/09). Punto chiuso.
> - ✅ **«Codice richiesto valet»: la logica ESISTE** (`verificaRitiro`, `POST /deliveries/:id/ritiro/verifica`, pop-up nel dettaglio per il partner, `pickupVerifiedAt`, blocco «in consegna» al valet finché non è verificato, forzatura d'ufficio registrata). La voce del 05/09 «flag senza logica» era vecchia.
> - ⚠️ **`scontoPartnerPercent`: NESSUNA app lo manda nell'ordine, e non serve.** Verificato nel codice: Orders non lo espone, il Customer Service (`scoutwt/deluxy-messaging`) non lo manda; `orders-sync` lo legge da un campo che non esiste. La quota arriva perché è la **piattaforma a chiederla** (`quotaDaOrders` → `GET /api/v1/quota-fornitore` del CS, o di Orders che delega al CS) alla nascita della vendita, col prezzo pubblico, e ricava la % dal prezzo arrotondato. La #12922 lo prova: «sconto dal Customer Service (territorio: fornitore 70%, 60 € arrotondati)». Quindi il punto «Orders deve mandarlo» è superato dall'architettura del 06/09: si può togliere la lettura morta in `orders-sync`, non aggiungere un campo.
> - 🔴 **#101251 (vendita #12922, Hokusai, Lijoi Roma): la scheda e la vendita dicono due numeri.** La vendita ha promesso al partner **60,00 €** (85 − 29,41 %), e la mail lo diceva. La scheda consegna calcola `economiaVendita` da **fee% del listino di Lijoi (22 %)** perché `Delivery.price` è vuoto (canone 01/09): commissione 18,70 + IVA 4,11 = 22,81 → **dovuto netto 62,19 €** (lordo 66,30). Al partner la scheda mostra incasso 85, commissione 18,70, dovuto 62,19; l'ufficio vede la riga a 85 e nessun prezzo sulla consegna. La `discountPercent` della vendita **non viaggia** sulla consegna. 🔖 Da decidere: o la consegna nata da una vendita scrive la quota (`price = amount − prezzo partner`, cioè 25 €), o la scheda legge la vendita collegata. Finché no, fattura e mail dicono cifre diverse.
> - ✅ **7 vendite di agosto `payable=false` senza regola → le 6 CONSEGNATE sono tornate «da pagare»** (decisione utente «quelle consegnate vanno pagate», applicato con `api/scripts/rimetti-da-pagare-sette-agosto.mjs --applica`, verificato payable=true su #62414, #62415, #62822, #62974, #62975, #63028; riga di registro su ognuna). La #62748 è annullata e resta fuori. Entreranno nel prossimo stipendio di Sami (4 con paga già scritta, 61,54 €; 2 dal listino).
> - **Vendite accettate senza consegna**: sono **14**, non 13 — la #12901 (Rizzi, 5 praline) ha la riga aggiunta alla #101130 dallo script del 07/09 ma `deliveryId` è rimasto NULL. Le altre: #12804/#12818 Stefanelli (consegne #63031/#63124 già consegnate, DDT uguale), #2729 Lami Fiori (fatta da Artista Locale #62868), 8 «Recupero registro» del 03/09 (già evase fuori piattaforma: 2650, 1758, 2684, 1780, 2717, 1785, 2732, 2789), #12895 La Dolceroma (#101104 consegnata), #12879 Stefanelli 5 € (#101007 Clivati non consegnata + #101109 magazzino). 🔖 Da decidere: agganciare `deliveryId` dove la consegna c'è (12804, 12818, 2729, 12895, 12901) e chiudere le 8 storiche.
> - **Chanel, elenco Attive (405)**: 334 **ricorrenti future** create il 31/08 (Chanel Firenze/Milano/Roma a ora, fino al 31/12: sono il calendario, giusto che ci siano); 46 legacy `created` del 2024-2026 mai chiuse; 3 in `cancellation_requested` di Montenapoleone (dic 2024, giu 2026) che aspettano l'ufficio; le altre sono di settembre.
>
> **Due script pronti, anteprima fatta, NON applicati** (decisione dell'utente):
> - `api/scripts/chiudi-consegne-ferme-vecchie.mjs --partner … --stato approved|delivered --applica`: chiude le `created/assigned` con data > 30 giorni, con riga di registro che dice lo stato di prima. ⚠️ **Nel csv legacy NON sono «approvate»**: Basara Tortona 582/582 `created` anche lì; Martesana Ecommerce 387 con stato **vuoto**. Lo stato di chiusura lo sceglie l'utente. Anteprima sui 5 partner chiesti: **1.103** (Basara Tortona 582, Martesana Ecommerce 390, Artista Locale 106, Deluxy Flowers 14, BAR Nicol 11); su tutti: 1.377.
> - `api/scripts/province-dall-indirizzo.mjs --applica`: **30.776 su 32.014** province ricavate dall'indirizzo (24.814 dalla sigla, 5.962 dal nome), 1.238 restano (hinterland senza sigla). Il csv legacy non aiuta: `province` valorizzata su **9** righe. Zero chiamate a Google.
>
> **Elenco Attive per partner (2.159)**: Basara Tortona 582, Martesana Ecommerce 390, Chanel Deluxy 385, Basara Washington 152, Artista Locale 127, Basara Corso Italia 104, Clivati-Consegne 83, Mali'A 83, Nikky Sushi 49, DR Vranjes 49… 57 partner. **1.125 sono legacy `created` del 2012-2024**: il 52 % dell'elenco.
...
> ✅ **10/09/2026 — «INVIA A FINANCE» FUNZIONA END-TO-END** (collaudo dell'utente: «fatto funziona»). Riscontro dalla nostra parte: **`FAT-2026-564`** · 01→31/08 · imponibile 48,68 € · totale 59,39 € · **`financeRef = PF 35/2026`**, mandata il 09/09 alle 14:50. Il giro è chiuso: fattura interna → pro-forma in Finance → mese delle consegne.

> 🗳️ **10/09/2026 — TRE RISPOSTE DELL'UTENTE, da tenere ferme.**
> 1. **TRANSACTIONS PER I PARTNER: sarà FINANCE a chiedere.** Decisione presa («si corretto sarà finance a chiedere»): il bonifico ai partner non resta sulla distinta SEPA — il ponte verso Transactions si costruisce **in Finance**, che è la casa dei conti col partner. La piattaforma non c'entra: manda il mese, e basta. 📌 Da passare alla sessione di Finance.
...
> - **Web**: bottone e finestra nella scheda valet (`valet-detail`), con l'IBAN mostrato prima di chiedere e i campi che si svuotano dopo l'invio (un secondo click distratto non chiede due volte). La pagina non aveva modali: gli stili `.overlay/.dialog/...` sono stati scritti nel componente — vedi [[trappola-classi-modale-senza-css]].
> - **Elenco Pagamenti**: colonna **«Richiesta a Transactions»** (solo ufficio) con lo stato o l'errore nel `title`. `findAll` non ha `select`, quindi i campi `richiesta*` arrivavano già al client.
> - **Le tre decisioni dell'utente, prese il 09/09 e già applicate per metà**: *(1)* «se è anticipo si scala» → due tipi distinti alla nascita, **`ADVANCE`** (anticipo) e **`PAYOUT`** (extra), scelti nella finestra con la conseguenza scritta accanto; il valore di default è l'anticipo. *(2)* «se lo stipendio è già approvato o pagato si scala al prossimo». *(3)* «il valet lo vede nel suo riepilogo paghe» → riquadro **«Anticipi e pagamenti»** in cima a `/salaries` per il solo VALET (la voce Pagamenti è fuori dal menu dal 03/09, quindi da lì non li avrebbe visti).
> - 🚀 **LO SCALO AUTOMATICO È LIVE — `delivery-o5wrasbok`** (assenso dell'utente, 09/09: «fai anche quello»). **Migrazione applicata dall'utente e verificata**: `recuperatoImporto` (double precision, default 0, NOT NULL) e `recuperatoSuSalaryId` (text, null) esistono su `platform."Payment"`, e una lettura vera contro il database di produzione le restituisce senza P2022 (679 pagamenti, 0 di tipo ANTICIPO: il conteggio parte da zero). ⚠️ L'SQL nudo non si incolla in PowerShell — c'è `api/scripts/applica-migrazione-anticipi.mjs` (anteprima, `--applica`, `IF NOT EXISTS`, rilegge dal DB alla fine). Il file di migrazione resta — `api/prisma/migrations-manuali/20260909_recupero_anticipi_valet.sql`, due colonne su `Payment`: `recuperatoImporto` (Float, default 0) e `recuperatoSuSalaryId` (Text). Aggiungere colonne è retrocompatibile, quindi l'ordine è **migrazione → deploy**; al contrario ogni query su `Payment` darebbe P2022 e la pagina Pagamenti resterebbe vuota.
> - **Come recupera** (`salaries.module.ts`, dentro `generate()`): prende gli anticipi `ADVANCE` non chiusi **dal più vecchio**, trattiene **fino a capienza del netto** (parziale se non ci sta tutto, il resto aspetta il mese dopo), scrive una `SalaryLine` con `origin = 'anticipo'` e importo negativo, e aggiorna il pagamento. **Stipendio e anticipi in una transazione**: se il conto si salvasse senza marcare gli anticipi, il mese dopo verrebbero recuperati di nuovo. L'`updateMany` ha il filtro sul residuo, così due conteggi in parallelo non possono portare il recuperato oltre l'importo.
> - **Due scelte da tenere a mente**: il **lordo non si tocca** (l'anticipo non è meno lavoro: è denaro già dato, quindi è una trattenuta, e ritenuta e documento restano corretti) e il **netto non va sotto zero** (uno stipendio negativo diventerebbe una richiesta di bonifico negativa).
> - **Il valet vede il residuo**: nel riquadro «Anticipi e pagamenti» ogni anticipo dice «da recuperare» / «resta X €» / «recuperato».
...
> - ⚠⚠ **LA CASCATA «APERTO O CHIUSO» ORA STA IN UN POSTO SOLO**: `api/src/common/disponibilita-partner.ts`. Era in DUE punti (smistamento, tabellone) e con questo badge sarebbe diventata TRE. Tre copie divergono al primo cambiamento e nessuno se ne accorge, perché ognuna sembra giusta da sola.
>   - ⚠️ La **presentazione** resta diversa di proposito: senza nessuna fonte lo smistamento tratta il partner come sempre aperto, ma il tabellone scrive **«non indicata»** — all'ufficio serve la differenza fra «non so» e «so che non lavora».
>   - 🐛 **Difetto trovato riunendo le copie**: nel tabellone gli orari settimanali erano raccolti con `new Map(...)`, che tiene solo L'ULTIMO di ogni partner — chi ha due fasce nello stesso giorno ne perdeva una.
> - **LISTINO: i colori della rosa, in orizzontale** (regola utente). Cinque colonne (Rossa, Bianca, Rosa, Gialla, Blu) che compaiono solo se un fiore le chiede. Vuoto = «non lo faccio»; il colore senza prezzo si **disattiva**, non si cancella. La tabella scorre dentro la card.
>   - **Perché**: su 1.461 steli dallo storico, **12 partner su 14** con almeno due colori li pagano diverso, schema costante — rossa di più, rosa 1-2 € meno, bianca in mezzo. Il prezzo unico era quasi sempre quello della rossa. Le **36 varianti** create dallo storico si correggono da qui.

> 🪗 **08/09/2026 (39) — ORARI E CHIUSURE PER GIORNO NEL PROFILO DEL PARTNER** (regola utente, richiesta rimasta in sospeso dal 08/09 mattina). In locale, `tsc` API e web verdi, `ng build` verde.
> - Nel Profilo un riquadro nuovo: si sceglie un giorno, si dichiara **chiuso tutto il giorno** oppure un **orario diverso**, con una nota facoltativa. L'elenco mostra solo i giorni da oggi in avanti, e ogni riga si toglie con la ×. L'API esisteva già (`GET/PUT/DELETE /partners/:id/day-exceptions`, aperta al PARTNER): mancava solo l'interfaccia.
> - ⚠⚠ **LA TRAPPOLA CHE RENDEVA LA FUNZIONE INUTILE, misurata.** La cascata della disponibilità guardava **prima** `PartnerDaySlot` e solo dopo `PartnerDayException`. Ma **34 partner hanno 102.874 fasce future** — nessuno le ha scritte una per una, sono generate in massa. Per tutti loro l'eccezione impostata dal profilo sarebbe stata **semplicemente ignorata**: il partner dichiarava «giovedì chiudo», l'app continuava a proporgli consegne, e nessuno avrebbe capito perché.
>   - **Invertita la precedenza in TUTTI E DUE i punti** che la implementano — `sales.module.ts` (smistamento) e `availability.module.ts` (tabellone): due cascate che dicono cose diverse mostrerebbero «aperto» un partner a cui lo smistamento non propone nulla.
>   - **La regola**: fra un dato scritto APPOSTA per quel giorno e uno generato in blocco, vince quello scritto apposta.
>   - ✅ **Cambio sicuro sul campo**: le eccezioni future in archivio oggi sono **ZERO**, quindi nessun partner cambia comportamento da adesso. Misurato prima di invertire.
> - ✅ **`body.partner.email` ora è validata** (regola utente, dopo la segnalazione dell'ostile): il controllo del formato c'era solo per l'email dell'ACCOUNT, e `@IsEmail()` del DTO non si applica perché quella strada chiama `partners.update()` con un oggetto costruito a mano, fuori dal ValidationPipe. ⚠️ Non è un campo qualsiasi: è la casella su cui poggia la verifica del cambio IBAN.
> - 🟢 **Chiuso su decisione dell'utente** («non serve per ufficio»): il riquadro delle coordinate bancarie resta solo nel profilo del partner. L'ufficio scrive dalla scheda, col checksum, il log e gli avvisi al partner e a sé stesso.

> 🪗 **08/09/2026 (38) — SECONDO GIRO DELL'OSTILE: LA TOPPA AVEVA CREATO IL CANALE CHE DOVEVA CHIUDERE.** Corretto; `tsc` API e web verdi, `ng build` verde.
> - 🔴 **CONFERMATA — il blocco dei 7 giorni si aggirava da `POST /auth/profilo`.** Per non dipendere da un solo indirizzo avevo messo fra i destinatari anche l'email dell'ACCOUNT (`User.email`). Ma `User.email` si riscrive dal profilo **senza codice, senza conferma, senza attesa**, e senza lasciare la traccia che fa scattare il blocco (che guarda solo `Partner.email`): tre chiamate con una sessione rubata, nessuna attesa, IBAN sostituito fino al registro Anagrafiche. **Prima della mia toppa, riscrivere `User.email` non serviva a niente.**
>   - **La regola che ne esce, e che vale oltre questo caso**: *un secondo fattore non può essere un dato che il primo fattore riscrive*. Ogni indirizzo raggiungibile dalla sessione va o congelato o tolto dai destinatari.
>   - **Correzione**: `User.email` fuori dai destinatari (restano `Partner.email`, protetta dal blocco, e il precedente); e al cambio del recapito parte un **avviso al VECCHIO indirizzo** — senza il quale il blocco dei 7 giorni, per chi ha la password, sarebbe solo un'attesa.
> - 🟠 **Il consumo della richiesta non era atomico** (solo il contatore lo era): la guardia decideva su uno stato letto a inizio richiesta, quindi N `conferma` in parallelo passavano tutte. Ora la condizione sta nella `where` di un `updateMany` — impronta, scadenza e tentativi — e l'update che azzera l'impronta è lo stesso che scrive l'IBAN: solo una richiesta vince.
> - 🟠 **`POST /partners/:id/anagrafica/importa` era l'ultima porta**: `CAMPI_IMPORTABILI` conteneva `IBAN`, `Intestatario conto` ed `Email`, e scriveva senza codice, senza checksum, senza avvisi. Tolti dall'import: restano nel CONFRONTO (la differenza col registro si vede), ma applicarla passa dalla verifica.
> - ⚠️ **E la toppa aveva TOLTO UN'AZIONE all'ufficio** (feedback utente: non si tolgono azioni). Vietando la scrittura a tutti, ADMIN/OPERATION non avevano più nessuna via per correggere un IBAN sbagliato — il riquadro con la verifica sta nel profilo del PARTNER e manda il codice a lui — e il **solo intestatario** diventava immodificabile *in silenzio*. Ora: ADMIN e OPERATION scrivono, ma l'IBAN passa dal checksum, il gesto lascia un log e **l'avviso parte sia al partner sia all'ufficio**; PARTNER e PROJECT_MANAGER restano fuori. Rischio accettato e dichiarato: una difesa che costringe a lavorare da un'altra parte non protegge, sposta.
> - ✅ Anche il **solo intestatario** ora si cambia dalla rotta con la verifica (prima era rifiutato a monte con «questo è già l'IBAN registrato»).
> - **Refutata**: `bankCodeSentAt` che sopravvive non blocca nessuno oltre i 60 secondi previsti.
> - 🔖 **Resta aperto, da valutare col custode**: il riquadro delle coordinate bancarie esiste solo nel profilo del partner, non nella scheda dell'ufficio; e `body.partner.email` non è validata come formato.

> 🪗 **08/09/2026 (37) — L'AGENTE OSTILE HA DEMOLITO IL CAMBIO IBAN: TRE FALLE, TUTTE CHIUSE** (commit `c3ae0eda`). 🟡 **Secondo giro dell'ostile sulle TOPPE in corso** (il Libro: non si corregge una difesa senza far smontare anche la correzione). ✅ **Migrazione applicata dall'utente: 7 colonne su 7 presenti in produzione.**
> 1. 🔴 **IL DESTINATARIO DEL CODICE LO SCEGLIEVA L'ATTACCANTE.** Il codice andava «alla mail in anagrafica» per non farla scegliere a chi chiede il cambio — ma `Partner.email` **è nella whitelist del profilo del partner** (`partners.service.ts:663`, `auth.service.ts:336`). Percorso: `POST /auth/profilo {partner:{email:'mia@…'}}` → chiedo il codice → mi arriva → confermo. **Due salvataggi sulla stessa schermata**, nessuna password richiesta, e l'IBAN fraudolento si propagava fino al registro Anagrafiche da cui FINANCE legge per pagare. La difesa non fermava il suo stesso attaccante dichiarato.
>    - **Toppe (valgono solo insieme)**: (a) un recapito cambiato da meno di **7 giorni** non apre un cambio IBAN — chiude proprio l'ORDINE delle mosse; (b) il codice parte verso **tutti** gli indirizzi noti (scheda, account `User`, e il precedente se il cambio è di meno di 30 giorni); (c) l'ufficio riceve l'avviso **già sulla richiesta**, mentre l'IBAN è ancora quello di prima. Il cambio del recapito ora lascia una traccia (`emailCambiataIl`, `emailPrecedente`).
> 2. 🔴 **LA PORTA ACCANTO.** `PUT /partners/:id` accettava `bankAccount` dal DTO e il ramo restrittivo scattava **solo per il ruolo PARTNER**: PROJECT_MANAGER, OPERATION e ADMIN scrivevano l'IBAN dalla rotta generica, senza codice, senza checksum, senza avvisi — e per il project manager in contraddizione col commento che dichiara «non tocca il denaro di nessuno». **Toppa**: da `update()` l'IBAN non passa più per nessun ruolo (si solleva un errore che rimanda alla rotta con la verifica).
> 3. 🟠 **IL TETTO DEI TENTATIVI ERA AGGIRABILE DUE VOLTE.** `azzeraRichiestaBanca` metteva `bankCodeSentAt: null`, quindi la pausa di 60 s si annullava con un `DELETE` (o esaurendo i 5 tentativi, che chiama proprio quel metodo): «cinque prove» diventava «cinque prove per codice, e un codice nuovo ogni giro di rete». E il contatore era letto-e-riscritto col valore assoluto: N `conferma` in parallelo leggevano 0 e scrivevano 1 — il tetto valeva solo provando in fila, e **l'app non ha nessun throttler** (`grep Throttle` → zero). **Toppe**: l'ora dell'ultimo invio resta scritta anche quando la richiesta cade; l'incremento lo fa il database (`increment`) e si decide sul valore che TORNA.
> - **Refutate dall'ostile (4)**: partner su partner (l'id viene riletto dal database a ogni richiesta, non dal token); `bankCodeHash`/`bankPending` fuori da ogni risposta (`PARTNER_OMIT` su tutti i punti, verificato anche fuori dal modulo); corse su richiesta/conferma; stati intermedi di annulla/scadenza.
> - 🔖 **Aperto, segnalato dall'ostile e non ancora chiuso**: `body.partner.email` non è validata come formato (il controllo `/^[^\s@]+@[^\s@]+\.[^\s@]+$/` vale solo per l'email dell'ACCOUNT, e `@IsEmail()` del DTO non si applica perché `aggiornaProfilo` chiama `partners.update()` con un oggetto nudo, fuori dal ValidationPipe). Effetto: un partner può puntare il proprio recapito su un indirizzo qualsiasi — **posta indesiderata verso terzi firmata dall'infrastruttura Deluxy**. Da valutare col custode.

> 🪗 **08/09/2026 (36) — CAMBIO DELLE COORDINATE BANCARIE DAL PROFILO, COL CODICE PER EMAIL** (regola utente). In locale, `tsc` API e web verdi, `ng build` verde. 🔴 **Migrazione DA LANCIARE prima del deploy**: `api/prisma/migrations-manuali/20260908_cambio_iban_in_due_passi.sql` (5 colonne in aggiunta su `platform."Partner"`, tutte nullable/default, IF NOT EXISTS). Senza, il client Prisma non legge piu' il Partner e la scheda non si apre.
> - **Il disegno**: la richiesta **non scrive**. `POST /partners/:id/banca/richiedi` valida l'IBAN (formato + checksum mod-97), parcheggia i valori in `bankPending`, manda un codice a 6 cifre alla mail **dell'anagrafica** (mai una passata dal client). `POST /partners/:id/banca/conferma` è l'unico punto in cui `bankAccount`/`bankAccountName` cambiano davvero.
> - **Le difese**: impronta SHA-256 (il codice non si conserva), scadenza 15 minuti, 5 tentativi poi la richiesta decade, confronto a **tempo costante**, un invio ogni 60 s, IBAN col **checksum** (intercetta la cifra sbagliata e le due invertite), avviso all'ufficio a cambio avvenuto, `bankCodeHash` e `bankPending` in `PARTNER_OMIT` (non escono da nessuna risposta).
> - ⚠️ **Se la mail non parte, la richiesta si annulla**: un codice valido che nessuno ha ricevuto sarebbe una porta senza campanello.
> - **Registrato** in `deluxy-design-system/SEGNALAZIONI-SICUREZZA.md` e passato dall'agente `sicurezza-ostile` prima del deploy, come vuole il CLAUDE.md.
> - **Condizioni di pagamento**: ora **visibili** al partner nel profilo, in sola lettura (regola utente: «solo admin e operation le possono modificare»). Viaggiano in un oggetto `condizioni` a parte, non sciolte accanto agli altri campi, così non possono finire per sbaglio nel payload che il profilo rimanda al salvataggio.

> 🪗 **08/09/2026 (35) — PERCHE' LA 12908 NON VA IN AUTOMATICO** (domanda utente). Non è un difetto: «Spumante - Set Sommelier» sta nella categoria **Vini**, mestiere **Enoteca**, che ha `smistamentoAutomatico = false`. Su 9 mestieri l'interruttore è acceso solo su **Fiorista** e **Cake designer**. La lista di priorità Enoteca su Milano però **esiste già, con 7 partner** (142 Restaurant, Clivati-Consegne, Cantina Franco, Basara Corso Italia, Mazzetti D'Altavilla, Capjari, Deluxy): per farla smistare da sola basta accendere l'interruttore del mestiere. 🔖 Decisione dell'utente.
...
> - ⚠️ **Numero ambiguo = colonna vuota**: il CS risponde con un ELENCO (lo stesso numero esiste su più negozi) e si tiene solo quando la corrispondenza è **una sola**. Meglio vuoto che lo stato di un altro ordine.
> - ⚠️ **Copia consapevole nel web**: `GESTIONI_CS` in `sales-list.component.ts` ha nomi e colori presi da `deluxy-messaging/src/lib/gestione.ts`, perché il loro endpoint manda la **chiave** e basta. Se di là nasce uno stato nuovo, qui si mostra **la chiave così com'è** invece di inventare un'etichetta. Se un giorno l'endpoint manderà anche il nome, quella mappa si butta.
> - 🔴 **NON provata end-to-end**: `customerServiceUrl`/`customerServiceApiKey` **non sono in AppSetting** e in locale non ci sono — il codice ripiega su `CUSTOMER_SERVICE_URL`/`CUSTOMER_SERVICE_API_KEY`, che **esistono nelle env di Vercel** (viste nell'elenco, valori mascherati). Verificato solo che l'endpoint c'è ed è protetto (401 senza chiave). **In locale la colonna resta vuota**: si collauda in produzione.

> 🪗 **08/09/2026 (28) — il ritiro del partner si applica anche dal pop-up delle vendite** (segnalazione utente; `ng build` verde). `caricaVendita()` impostava `partnerId` ma non chiamava `applicaRitiroPartner()`: quella girava solo nel callback di `/partners`, sotto la condizione `if (this.model.partnerId && !this.editId())`. Le due chiamate corrono in parallelo e `/partners` arriva quasi sempre prima → partner ancora vuoto → condizione falsa → nessuno ci riprova. Aggiunta la chiamata in `caricaVendita`, come già fatto per `forzaServizioVendita` («chi arriva ultimo vince»). `applicaRitiroPartner` era già prudente: non fa nulla se la lista partner non c'è, e non sovrascrive un indirizzo scritto a mano.
> - Misura: **129 partner attivi, 0 senza indirizzo** → il campo si riempie sempre. ⚠️ **22 su 129 hanno un indirizzo senza numero civico** («Milano», «Firenze», «Viale Premuda, 20129 Milano MI»): si compilano lo stesso, ma valgono poco per un valet — si correggono in anagrafica, non nel modulo.
> - ⚠️ Nota sullo screenshot della segnalazione: il campo mostrava «55100 Lucca LU», che **non è** l'indirizzo del partner — «Omnistore Flowers Pisa» ha «Via Venezia Giulia, 14». Quel valore veniva da un'altra strada (indirizzo digitato o residuo del prefill), non da `applicaRitiroPartner`.

> 🪗 **08/09/2026 (27) — DDT `CPR` per i corporate service** (regola utente; in locale, `tsc` e `ng build` verdi).
> - **Il legame sta nel DDT**, non in una colonna nuova: l'acquisto porta `CPR<code dell'ordine corporate>` e da lì si risale nei due versi. Scelta deliberata: `legacyCorrespondDeliveryId` è solo letto da FINANCE e legato a `legacyId` (le consegne nuove non ce l'hanno), e `parentDeliveryId` è già delle riconsegne — usarlo anche qui sarebbe stato mettere due tassonomie sullo stesso campo. Così **niente migrazione sul Postgres condiviso**.
> - **API**: `DeliveriesService.legameCorporate(d, user)` chiamata in `findOne` → `legameCorporate` nel payload. Verso «acquisto»: `/^CPR(\d+)$/i` sul `ddtNumber` → la consegna con quel `code` (se non esiste si restituisce comunque `{verso, numero, consegna: null}` e la scheda lo dichiara). Verso «ordine»: se il servizio è `CORPORATE`, cerca le consegne con `ddtNumber = CPR<code>`. **Solo ADMIN e OPERATION**, null per tutti gli altri.
> - **Web**: sezione `.legame-corporate` col link (stessa forma della riconsegna), bottone «Crea l'acquisto» su `pricingModel === 'CORPORATE'`, e nel form `?acquistoDa=<id>` che precompila DDT `CPR<code>` + servizio di vendita (`venditaPreferita` del catalogo, perché senza partner non c'è un listino) e **svuota il partner**.
> - Provato in lettura sui dati veri: ordine **101150 Casati 14 · ORDINE BRIOCHE · CORPORATE**; regexp verificata su `CPR101150`, `cpr101150` (ok), `CPR 101150`, `CPRABC`, `DDT101150`, vuoto e null (nessun legame); risalita da un numero inesistente gestita. **Nessun DDT CPR esisteva prima**: si parte puliti.
> - ⚠️ **Trappola dei backtick, la quarta volta oggi**: due commenti dentro il `template` di `delivery-detail.component.ts` contenevano `` `CPR` `` e hanno chiuso il template literal («Incorrect number of arguments to @Component decorator», «Cannot find name 'CPR'»). Nei commenti DENTRO un template non si scrivono backtick.

> 🪗 **08/09/2026 (26) — il ripiego sulla riga di consegna non è più una prova** (in locale, `tsc` verde). `datoAlPartner` ripiegava sull'unica riga della consegna anche quando quella riga era **un'altra cosa**: su prodotti composti la consegna di un fornitore copre solo la sua parte, e così «Cofanetto Pregiate Praline» (30 €) diventava il patto per «Rose Rosse e Praline d'Autore» (175 €).
> - Ora `reale` è vero se la riga ha lo stesso `productId` **oppure** se il suo nome parla dello stesso prodotto (`parlaDelloStessoProdotto`, funzione di modulo). Aggiunta una mappa `nomeProdotti` con **una lettura sola** per tutto il giro, non una per riga.
> - ⚠️ **Il criterio è stato misurato prima di scriverlo**, ed è la parte che conta: col solo `productId` finivano fra i suggerimenti **74 su 109** (il flag sarebbe diventato rumore — trappola nota); col nome ne restano **15**, e sono i casi veri («Wine» su un bouquet, «Colazione Malià» su un bouquet, «Bouquet Beethoven» su una Red Velvet **al 106% del pubblico**).
> - 🔖 **Da decidere all'utente, NON toccate d'ufficio**: «Dolci Abbracci — Orsacchiotto e Cappelliera», proposta a **35,12 € su 255 € (14%)**. Il nome della riga corrisponde, quindi il criterio non la marca, e non ho elementi per dire che il prezzo sia sbagliato: potrebbe essere il costo della sola parte di Fao Schwarz. Serve una persona che sappia com'è composto quel prodotto. L'altra («Rose Rosse e Praline», 30 € su 175) è già **rifiutata**: nessun danno.

> 🔴🔴 **08/09/2026 — QUESTIONE APERTA IN CARICO AL CUSTODE DELLE PERFORMANCE: non chiuderla, non «correggerla» da soli** (decisione dell'utente: «tieni aperta la questione per il custode»).
> - **Cosa succede**: `app.deluxy.it` cade a intermittenza con **HTTP 500 su TUTTE le rotte**, comprese le pubbliche. Log: `EMAXCONN max client connections reached, limit: 200` in `PrismaService.onModuleInit` — fallisce all'avvio del modulo, prima di ogni rotta.
> - **⚠️ È RICORRENTE**: la prima volta è comparso dopo il deploy delle 11:28, riassorbito (12/12 buone), **tornato ~15 minuti dopo SENZA deploy** (3/3 in errore), poi rientrato. Quindi «colpa della pubblicazione» NON basta a spiegarlo: considerare l'app instabile.
> - **Cosa NON è** [misurato, non dedotto]: non è il database (**16-28 connessioni** in tutto durante i blocchi) e non è il pooler condiviso in generale (nelle stesse finestre **Orders rispondeva 401 in 247 ms**, CRM/FINANCE/Anagrafiche 307). Sono i client Supavisor **della sola piattaforma** a sfondare i 200.
> - ➕ **08/09, dal custode: la causa è probabilmente un BUG NOTO DI SUPAVISOR** (discussione Supabase #40671, fix `supavisor#783` in rollout per regione): `ClientHandler` che sopravvivono a errori TLS fatali e **non rilasciano lo slot** del pooler — client oltre 200 mentre i backend restano ~11-13, e ripristino solo col riavvio del pooler. Combacia con le misure prese qui (16-28 backend durante i blocchi) e col ritorno del guasto **senza deploy**, che la spiegazione «valanga da pubblicazione» non copriva. **Non è codice della piattaforma.**
> - ❌ **La mia proposta `connection_limit=1` era SBAGLIATA: va ignorata.** Vale sul modello lambda classico, non su **Fluid Compute**, dove le invocazioni concorrenti condividono istanza e pool — Vercel documenta che abbassare il pool «non riduce le connessioni totali e danneggia la concorrenza». Corretta anche nel registro del custode, per non lasciarla lì come cosa da fare.
> - **⚠️ COSA MANCA per decidere, e che NON si può dedurre**: il valore vero del `DATABASE_URL` di produzione (Vercel, progetto `delivery`). `vercel pull` lo dà `[SENSITIVE]`. Due domande: porta **6543** (transaction) o **5432** (session)? Ci sono `pgbouncer=true` e `connection_limit`? Riferimento buono: **Tasks** usa `6543 + pgbouncer=true&connection_limit=5`.
> - **⚠️ Errore da non ripetere**: in questa sessione ho prima affermato «non ha connection_limit» basandomi su un `grep` di un file che contiene `[SENSITIVE]` — cioè su niente. Era una deduzione travestita da misura. Finché quel valore non lo legge una persona dalla dashboard, ogni diagnosi resta un sospetto.
> - ✅ **08/09 — SQL GREZZO: rassegna fatta su richiesta del custode, tutto qualificato.** Sul cluster il ruolo `postgres` ha **`mail` davanti nel `search_path`**: un raw NON qualificato colpirebbe le tabelle di AI Mail — e `Partner` esiste in **tre** schemi (`anagrafiche`, `platform`, `public`). I raw della piattaforma sono **10 in 3 file** e nominano tutti lo schema: `finance.module.ts` (1, lettura), `statistiche.module.ts` (8, letture), `orders-sync.module.ts` (1, **l'unica scrittura raw**: `INSERT INTO platform."OrdineCliente"`). Nessun `DELETE` raw. ⚠️ **Regola per chi scrive raw qui: sempre `platform."Tabella"`, mai il nome nudo** — un `UPDATE` non qualificato non dà errore, va a segno sulla tabella di un'altra app. Nota: con `?schema=platform` nell'URL Prisma imposta il `search_path` (verificato: `current_schema()` = `platform`), ma l'URL di produzione non è leggibile e non va dato per scontato.
> - **Registrata** in `deluxy-design-system/SEGNALAZIONI-PERFORMANCE.md` («In attesa») e **report completo consegnato** alla sessione custode «Raccolta feedback prestazioni app».
> - ⚠️ **Prima di pubblicare di nuovo**: sapere che ogni deploy può riaprire la finestra di 500.

> ✅ **08/09/2026 (25) — LIVE `delivery-1hdlsem6c`** (prebuilt dalla RADICE del worktree, bundle `main-CYEFTZ53.js` su app.deluxy.it, pushato `c3066b75`). Dentro: moltiplicazione 8×15 sulle riconciliazioni, patto senza ordine + ricerca fra le vendite chiuse, colonna «Trasmessa», statistiche dello smistamento, condizioni di pagamento sulla scheda partner. Verificato dal vivo: bundle giusto, le 6 rotte nuove rispondono 401 (una rotta inventata dà 404, quindi il controllo discrimina), home 200.
> - ✅ **Le 5 proposte col prezzo di un pezzo sono corrette** (`correggi-patti-per-pezzo.mjs --scrivi`): Rose Rosse 12 TO → 96 €, Rose Rosse Deluxy 3 RM → 24 €, Rose Rosse 15 MI → **120 €**, White Set 9 MI → 63 €, Regina di Cuori Principessa BG → 270 €. Rilanciato dopo: **0 rimaste**. Nessuna accettata: restano proposte da decidere.
> - 🔴 **INCIDENTE AL DEPLOY, da leggere prima del prossimo**: subito dopo la pubblicazione `app.deluxy.it` ha dato **500 su tutte le rotte per ~8 minuti**, anche sulle pubbliche. Log: `EMAXCONN max client connections reached, limit: 200` in `PrismaService.onModuleInit` — fallisce all'AVVIO, prima di ogni rotta. ⚠️ **La prima diagnosi era sbagliata** (avevo detto «pooler condiviso saturo»): misurando dalla connessione di sessione, sul Postgres c'erano **28 connessioni in tutto** e **le altre app rispondevano** (Orders 401 in 247 ms, CRM/FINANCE/Anagrafiche 307). Erano i client attaccati a Supavisor **dalla sola piattaforma**. Causa vera: il `DATABASE_URL` di produzione **non ha `connection_limit`**, quindi ogni lambda apre il pool di default (3-5 connessioni) e un deploy — che invalida tutte le funzioni calde — fa aprire pool nuovi mentre i vecchi sono ancora attaccati. Si è riassorbito da solo (da 1 richiesta buona su 5 a **12 su 12** dopo ~9 minuti). **Rimedio proposto e registrato in SEGNALAZIONI-PERFORMANCE: `connection_limit=1` nel DATABASE_URL di produzione**, da concordare con l'utente perché cambia una variabile d'ambiente. Finché non si fa, **ogni deploy può rifare questo**. Verificato anche il sospetto peggiore: il `api/.env` creato per il dev locale **NON è finito nel bundle** (nessun `.env` in `.vercel/output`).
> - ⚠️ Per far girare l'API nel worktree serve `api/.env` col `DATABASE_URL` **Postgres** (quello di `deluxy-tasks/.env`): il `.env` della cartella principale punta a uno **SQLite** (`file:./dev.db`) e l'API non parte. È in `.gitignore`, non si committa.
> - 📋 **Registrate ai custodi**: 1 segnalazione PERFORMANCE (il `connection_limit`, con tutte le misure) e 4 UX (colonna «esito di una comunicazione in uscita»; due riquadri per i versi del denaro; pillole «due strade» + elenco filtrato che dice come si allarga; la trappola Angular dell'`@else if` senza alias `as`).

> 🪗 **08/09/2026 (24) — condizioni di pagamento sulla scheda partner** (in locale; `tsc` e `ng build` verdi). ⚠️ **MIGRAZIONE DA LANCIARE PRIMA DI PROVARE**: `scratchpad/migrazione-condizioni-pagamento.mjs --scrivi` — senza, l'API non legge il Partner (Prisma chiede colonne che non ci sono).
> - **Quattro campi** su `platform."Partner"`: `pagamentoVendorGiorni Int?`, `compensazioneIncassi Boolean`, `incassoServiziGiorni Int?`, `incassoServiziFineMese Boolean`. Colonne solo in AGGIUNTA, nullable o con default, `IF NOT EXISTS`: le altre 13 app che leggono Partner non se ne accorgono.
> - **La divisione dei servizi non è stata inventata**: esisteva già `ServiceType.pricingModel` — `VENDITA` (5 servizi attivi) contro tutto il resto (PREZZO_FISSO 17, MAGAZZINO 11, A_ORA 10, CORPORATE 4 = 42). Nessun criterio sul nome, che sarebbe stato fragile.
> - **Decorrenze diverse, come da regola utente**: vendor = **fine mese** (fisso, scritto nell'interfaccia); fatturazione = **flag** `incassoServiziFineMese`. Campo giorni vuoto = `null` = «non concordato», e il payload manda `null`, non `0` (con `0` la scheda direbbe «0 giorni» come se fosse un patto).
> - **Difesa**: sono economia, e `partners.service.update()` riassegna già il dto ai soli contatti quando chi scrive è un PARTNER — quindi questi campi non passano nemmeno mandandoli a mano.
> - **Import da FINANCE** (decisione utente: «importa da finance ma il dato sarà caricato su di te»): `scratchpad/importa-condizioni-da-finance.mjs` (a secco, poi `--scrivi`). FINANCE ha **29 partner** con condizioni (16 con giorni > 0, 12 con `compensazione`, 14 con `commissioniADetrazione`) e **un solo `ggPagamento`** per due versi opposti: lo script deduce il verso dalle etichette `servizi` (Vendor/Consegne/Magazzino) e **dichiara riga per riga che è una deduzione**. Abbinamento per insegna/ragione sociale normalizzata + la parte fra parentesi (in FINANCE i nomi sono tipo «ARMANI FIORI (ALIA SRL)») + email; le chiavi che portano a due partner diversi si buttano (un abbinamento ambiguo non si vede, uno mancante sì). ⚠️ **Chanel saltata sempre** (regola utente). Chi resta senza corrispondenza viene elencato per il caricamento a mano.
> - ✅ **FATTO IN PRODUZIONE 08/09** (l'utente ha detto «alcuni dati sono già disponibili su finance importali da lì»): migrazione applicata (4 colonne su 4, 130 partner leggibili dopo) e import eseguito — **18 partner scritti**, 3 Chanel saltati, 8 senza corrispondenza. ⚠️ **Quello che l'import NON ha potuto dare**: in FINANCE quasi tutti i partner «Vendor» avevano `ggPagamento = 0` (cioè «a vista»), quindi i **giorni sul vendor sono valorizzati solo su 2** (Mazzetti D'Altavilla e Nikky Sushi, 30 gg); 4 hanno solo i giorni di incasso (Armani Fiori, Bonpoint, Wicuisine, Zegna — 30 gg) e **12 hanno solo il flag compensazione**. Restano **111 partner attivi su 129** senza condizioni: vanno compilati a mano, non c'era altro da importare. Prima di scrivere ho aggiunto allo script il controllo di ambiguità anche sulle **email** (ce l'aveva solo sui nomi): un solo abbinamento è passato di lì (Arte E Fiori Firenze) e nessuna email risulta condivisa fra due partner — senza quel controllo un indirizzo condiviso avrebbe scritto le condizioni sul partner sbagliato, in silenzio.
> - 🔖 **Da fare**: guardare `http://localhost:4200/partner` → un partner → Modifica; i **8 senza corrispondenza** da mettere a mano (DIPTYQUE, FENDI, PASTICCERIA TAVEGGIA, PASQUALE BRUNI, BASARA, MONCLER, BOTTEGA DEI FIORI FIRENZE, **CLIVATI** 30 gg Vendor+Consegne); le tre **Chanel** (60 gg) se e quando si vorranno.

> 🪗 **08/09/2026 (23) — il prezzo per pezzo si moltiplica; il patto anche senza ordine; la colonna «Trasmessa»; statistiche dello smistamento** (in locale, `tsc` API e `ng build --configuration production` verdi).
> - **⭐ 8×15** (regola utente): `riconciliazioni.module.ts · datoAlPartner` legge ora anche `quantity` della `DeliveryProduct` e restituisce `price × quantity`. Il fioraio scrive il SUO listino sulla riga («rosa rosa maryflor €8» × 15, price 8): erano 8 € per UNA rosa, letti come prezzo dell'intera vendita da 180 €. Le `stats` portano ora `perPezzi: { unitario, pezzi }` col conto in chiaro. **Misura prima di applicare**: 19 righe di consegna con q>1 e price>0, **18 unitarie**, la 19ª un artefatto (consegna con due righe contro una sola vendita). ⚠️ **5 proposte già scritte** hanno il prezzo di un pezzo (Maryflor 8×15 MI, FAG Torino 8×12 TO, Lijoi Roma 8×3 RM, Maryflor 7×9 MI, Fiorista Tonino 6×45 BG): tutte in stato **proposta**, nessuna vendita smistata a quel prezzo. Script pronto **da lanciare**: `scratchpad/correggi-patti-per-pezzo.mjs` (a secco senza argomenti, `--scrivi` per applicare; cerca **per la forma del problema**, non per id — così ne sono uscite 5 invece delle 3 note; non accetta niente, restano proposte). Il giro notturno NON le ricalcola: sono su prodotti `tipologiaVendita: 'quantita'`, fuori dal filtro delle proposte automatiche.
> - **Patto senza ordine + ordini già chiusi** (regole utente). API: `venditeDaRiconciliare(q, tutte)` con `?tutte=1` che toglie il filtro sugli stati e porta `partner` in ogni riga; nuovo `contestoDelPatto(b)` che risolve prodotto/variante/provincia/prezzo-al-cliente **da una vendita oppure dai campi diretti** (senza vendita il prezzo pubblico è il **listino**, e l'anteprima lo dichiara con `fonte: 'listino'` + avviso «il margine è una previsione»); `anteprimaManuale(b)` e `creaManuale(body)` passano da lì; nuove `GET /riconciliazioni/cerca-prodotti?q=` e `GET /riconciliazioni/province`. Web: due pillole «Da un ordine / Senza ordine», casella «Cerca anche fra gli ordini già chiusi», il passo del prezzo **estratto** dal ramo «con la vendita» (`prontoPerIlPrezzo()`), `suCosa()` come unico punto che decide su cosa vale il patto. ⚠️ **Trappola Angular**: `@else if` NON accetta l'alias `as` («Property 'v' does not exist») — si annida un `@if` dentro `@else`.
> - **Colonna «Trasmessa»** (regola utente). La trasmissione non lasciava traccia: `avvisaProposta` in un `catch` muto e `mailProposta` chiamata con `void`. Ora ogni esito scrive un `SaleLog` `trasmissione-app` / `trasmissione-mail` col motivo per esteso (partner senza utenti attivi, senza indirizzo, indirizzo non valido, notifiche spente, errore dell'invio); il marcatore del fallimento è «NON » in testa al messaggio, scritto da chi invia. `findAll` li rilegge con **UNA query per pagina** (solo le vendite in stato proposta) e mette `trasmissione: { esito, app, mail }` su ogni riga; esiti `attesa | inviata | parziale | fallita`. Colonna solo per l'ufficio, ordinabile per gravità. i18n `sales.col.sent` e `sales.sent.*`.
> - **Statistiche: smistamento automatico o a mano** (regola utente). `statistiche.module.ts · smistamento(corrente, confronto, filtri)`: raggruppa le `Sale` del periodo per `assignmentReason` in **tre** famiglie — `auto` (una regola ha scelto), `mano` (l'automatismo si è fermato), `fuori` (recuperi del registro e ordini già evasi altrove, che non ci sono mai passati: **76 righe su 577** in 30 giorni, e contarle fra i manuali avrebbe gonfiato il lavoro dell'ufficio). `percentualeAuto` si calcola su `auto + mano`, non sul totale. Nel payload di `GET /statistiche` sotto `smistamento`; riquadro con i tre numeri, il confronto col periodo prima e la **tabella dei motivi** (le righe «a mano» sono la lista di quello che si può ancora automatizzare).
> - 🔖 **Da fare**: lanciare `correggi-patti-per-pezzo.mjs --scrivi`; guardare i link localhost; poi push e deploy a comando.

> 🪗 **08/09/2026 (22) — il patto vince su tutte le altre regole della provincia** (in locale; segnalazione utente sulla 12904). Nuovo `SalesService.daPatto(c)` (prezzoPartner definito + motivo che inizia con «riconciliazione prodotto/provincia»): nel ramo `tipologiaVendita === 'quantita'` i candidati da patto NON vengono riscritti dal calcolo pezzi × unitario. Il ramo `preventivo` già lo faceva (07/09). Diagnosi della 12904: la regola che la bloccava «nella percezione» era una riconciliazione in stato **proposta** (8 €, Negozio Fiori Cannavo) — il codice non bloccava affatto, ma il modulo la mostrava in ROSSO come se fosse un divieto. Ora tre messaggi distinti (`gia_proposta` / `gia_accettata` / `gia_rifiutata`), rosso solo sulla regola attiva. Provata la decisione sui 4 casi (patto + più pezzi → resta il patto; senza patto → pezzi × unitario; senza unitario → percentuale; un pezzo → il ramo non scatta). `tsc` e `ng build` verdi.
> 🪗 **08/09/2026 (21) — pop-up riconciliazione, ricerca partner, prodotti in tabella** (in locale). `consegneAllIndirizzo`: `NON_ANNULLATE` ({ status: { notIn: [CANCELLED, NOT_ACCEPTED] } }) su entrambe le strade (DDT e indirizzo) — misurato: DDT 12883 da 3 a 2 candidate, 12833 da 5 a 4. Nel template la pastiglia `.stato-consegna` con `deliveries.status.*`. `product-reconciliations`: il partner si CERCA (`qPartner`, `partnerTrovati`, filtro client sui 129 già in memoria, `partnerScelto2` computed) e la ricerca prodotto passa `partnerId` — con un partner scelto parte anche a campo vuoto (pageSize 30). `sales-list`: cella prodotto con `×{{quantity}}` (aggiunto `quantity` al tipo `Sale`; l'API lo mandava già, `findAll` usa include) e `altriProdotti(s)` che legge le vendite sorelle già in pagina (stesso `externalOrderNumber`, non annullate, senza doppioni) — nessuna chiamata in più. i18n it/en. `ng build` e `tsc` verdi.
> 🪗 **08/09/2026 (20) — mail sulla proposta di vendita + prodotto di riferimento** (in locale). `avvisaProposta` chiama ora `mailProposta(saleId, partnerId, alPartner, pezzi)`: legge partner e vendita, esce se manca l'indirizzo o `mailNotifications` è spento, e manda HTML via `SettingsService.inviaHtmlViaAiMail` (prodotto, importo al partner, giorno, destinatario, indirizzo, provincia, note, pulsante). Best-effort e in coda con `void`. Il prezzo passa da `prezzoAlPartner` (arrotondato a 5): prima la mail diceva 94,50 dove la regola dice 95. Nuovo `riferimentoDelPatto(productId, variantId, provinceId, partnerId)`: rilegge `ProductReconciliation.stats` (lo scrive `creaManuale`) e porta al partner il SUO prodotto — provato sul patto vero: «Per lei corrisponde a: Chantilly Classica · 6 (CHT410-1)», sia nel campanello sia nella mail. 🔖 Vale solo per le proposte NUOVE (decisione utente): le 18 in attesa non ricevono nulla. `tsc --noEmit` verde.
> 🪗 **07/09/2026 (19) — prezzo dal CS arrotondato, avviso col prezzo del partner, riconciliazione a mano, nomi somiglianti** (in locale, tutto compilato).
> - **Prezzo dal custode**: `quotaDaOrders(provinceId, categoryId, prezzoPubblico)` passa ora `prezzoPubblico` al Customer Service e legge `prezzoFornitore` (già arrotondato a 5/0 da lui); in `create` la percentuale si ricava con `quotaPerDare(importo, prezzoFornitore)`. Cache con il prezzo nella chiave. Ripiego sulla percentuale se il CS non risponde. ⚠️ `customerServiceUrl/ApiKey` NON sono in AppSetting: valgono le env di Vercel, che il `pull` restituisce mascherate — verifica fatta sulla funzione vera del CS (`deluxy-messaging/src/lib/vendite.ts`), non sull'endpoint.
> - **Avviso**: `avvisaProposta` diceva `amount` (prezzo del cliente); ora dice il prezzo del partner con «a te» e i pezzi.
> - **Riconciliazione a mano**: servizio `venditeDaRiconciliare(q)`, `riferimento(productId, variantId)` (tre fonti dichiarate: vendite accettate, listino dell'unico, `PP-*` dai DDT), `anteprimaManuale(saleId, partnerId, prezzo)` col margine, `creaManuale()` che nasce **accettata**; rotte `GET /riconciliazioni/vendite-da-riconciliare`, `GET /riconciliazioni/riferimento/:productId`, `POST /riconciliazioni/anteprima`, `POST /riconciliazioni/manuale`; UI a passi in `product-reconciliations.component` + i18n `reconciliations.nuova.*` (29 voci it/en).
> - **Storico partner**: via il livello «altre province», dentro «nome somigliante nella stessa provincia» (parole ≥ 4 lettere, senza le parole di servizio); i18n `base_nome-simile`, tolto `base_altre-province`.
> - ✅ **DECISO dall'utente il 07/09 sera** («per ora ok così arrotondamento»): l'arrotondamento a 5 vale **da dove nasce la vendita** (prezzo chiesto al Customer Service) e **non** si estende retroattivamente al `partnerPrice` delle riconciliazioni già scritte né al prezzo di riga delle consegne. Le 21 riconciliazioni su 107 con un prezzo non tondo **restano come sono**: sono accordi già presi coi partner, e riscriverli d'ufficio cambierebbe una fattura senza che nessuno l'abbia pattuito. Se un giorno si vorrà allinearle, il conto è già fatto: `scratchpad/arrotondamento.mjs`.
> - 🔖 `homePartnerEmails` in AppSetting risulta **vuoto**: da riscrivere (era la segnalazione dell'altra sessione).
> 🪗 **07/09/2026 (18) — UNA consegna per ordine+partner, non due** (in locale; regola utente «se è due prodotti stesso partner unisci»). In `creaConsegna`: si cercano le vendite SORELLE (stesso `externalOrderId`, stesso `partnerId`, non annullate, con `deliveryId`) e la loro consegna se è ancora viva (CREATED/ASSIGNED/ACCEPTED/IN_PREPARATION); se c'è, si crea la `DeliveryProduct` lì (saltando il doppione), si scrive una riga di registro e si RESTITUISCE quella consegna. ⚠️ `Sale.deliveryId` è `@unique`: la seconda vendita non può puntarci finché il vincolo c'è, e l'update di `accetta` sarebbe fallito con P2002 — facendo fallire l'accettazione. Perciò: (a) nuovo helper `aggiornaVenditaConConsegna()` che su P2002 riscrive tutto il resto senza `deliveryId` e lo annota nel log — la riga è già sulla consegna, il lavoro è completo; (b) `schema.prisma`: `@unique` → `@@index([deliveryId])` col perché; (c) migrazione pronta in `scratchpad/migrazione-sale-deliveryid.mjs` (prova a secco fatta: 461 vendite collegate, indice attuale `Sale_deliveryId_key`) — **da lanciare con `--scrivi`**, toglie il vincolo e lascia `Sale_deliveryId_idx`. Estratta la funzione `prezzoRigaVendita()` (il prezzo di riga serviva in due punti e un calcolo copiato diverge). Misura: 3 ordini con più vendite allo stesso partner, 0 con consegne separate — difetto latente, preso prima che mordesse. `tsc --noEmit` verde.
> ✅ **07/09/2026 (17) — il 12902 È ENTRATO, e con lui tre altre vendite perse** (LIVE `delivery-6j2i8g0p5`, pushato `9d792bc8`). Alle 19:15 il cron ha creato: #12902 Bouquet Portofino 135 € su **PV** e #2880 Bouquet Girasoli 135 € su **CA** (prodotti unici di Deluxy Flowers, escluso dalle proposte), e — effetto non previsto ma giusto — le RIGHE MANCANTI di due ordini composti: #12889 e #12883, «Palloncini Numero» da 50 € e 25 € su MI (prodotto senza partner in provincia). Quegli ordini erano già in Vendite con la prima riga e perdevano la seconda: 75 € che non vedeva nessuno. Provincia, prodotto e importo corretti su tutte e quattro. ⚠️ Il motivo però diceva «Ordine estero» (testo cablato in `ingest`, ramo `senzaProposta`, nato il 03/09): corretto nel codice (`ingest` accetta `motivo`, l'estero resta il ripiego) e **riscritto sulle 4 vendite già create** con `scratchpad/correggi-motivo.mjs --scrivi`, lanciato dall'utente. Misura: vendite col motivo «estero» da 11 a 7, e le 7 sono tutte su provincia EE, cioè davvero estere. 🔖 Resta: il 12902 non ha la data di consegna in Orders (accettandolo la consegna non nasce finché non si mette a mano).
> 🪗 **07/09/2026 (16) — ordini senza partner che sparivano + servizio dal pagamento anche sull'automatico** (in locale). **(a)** `orders-sync`: `senzaCandidato = daCreare.filter(r => !conCandidato.includes(r))`; l'esito `senza-partner` scatta solo se non c'è NIENTE, e le righe senza candidato si creano con `ingest({ ...riga, senzaProposta: true })` → vendita DA GESTIRE col prodotto agganciato. Causa: prodotto UNICO di un partner con `esclusoDalleProposte` (Deluxy Flowers, Cakedesignme, Artista Locale) → `esisteCandidato` false su tutte le righe → l'ordine non entrava. Misura: 2 su 224 in 14 giorni (#12902, #2880). **(b)** `sales.servizioDaPagamento(externalOrderId)`: legge l'ordine da Orders, contrassegno da `classificazione.categoriaPagamento` o dal gateway (`contrassegno|cash on delivery|cod`), pezzi = righe CON SKU → «Vendita con Pagamento alla Consegna» / «Vendita Deluxy Multipla» / «Vendita Deluxy»; `creaConsegna` la usa quando la vendita non ha un servizio suo, e col contrassegno scrive `paymentOnDelivery` + `paymentAmount`. Best-effort: se Orders non risponde, ripiego su «Vendita Deluxy». Misura sui 320 ordini di 21 giorni: 33 → Multipla, 0 contrassegni. **(c)** regexp di `?da=` corretta (mancavano i backslash: `?da=` non ha mai funzionato). `tsc --noEmit` verde.
> 🪗 **07/09/2026 (15) — il ritiro a mano dell'ufficio vince sulla regola di Artista Locale** (segnalazione utente: «modifico l'indirizzo di ritiro e non me lo salva»). Causa: `PARTNER_RITIRO_IN_CITTA = 'artista locale'` in `deliveries.service.ts` — `ritiroInCittaDiConsegna()` riscriveva `pickupAddress` con l'indirizzo del destinatario a ogni create/update, e la `forzatura` sta DOPO `...scalar` nel `data` del Prisma update, quindi vinceva sempre. Correzione: quarto parametro `ritiroSceltoDallUfficio` (esce subito con `null`); in `create` = ufficio con un ritiro non vuoto; in `update` = `ritiroAMano`, cioè ufficio + `dto.pickupAddress` non vuoto **e diverso** da `delivery.pickupAddress` (il confronto è necessario: il form rimanda sempre tutti i campi). Log «Ritiro cambiato a mano» nel registro. `tsc --noEmit` verde; regola provata sui 5 casi (admin cambia → resta; operation cambia → resta; admin salva senza toccare → forzatura come prima; partner → forzatura; altro partner → resta). Dati: 63 consegne su 262 (da 3 giorni fa a domani) hanno già un ritiro diverso dalla sede del partner, quindi il salvataggio funzionava in generale — era solo Artista Locale. ⚠️ `Delivery.updatedAt` non ha indice: una query per data di modifica va in statement timeout. Da valutare col custode delle performance.
> ✅ **07/09/2026 sera — LIVE `delivery-f7tp5orqs`** (prebuilt dalla RADICE del worktree `deploy-delivery`, bundle `main-GKIYTGWX` su app.deluxy.it, 22 s di deploy). Dentro: recap giornaliero ai partner coi 2 cron, mail «Nuovo servizio» formale, vendita accettata → consegna sempre («Vendita Deluxy») + `POST /sales/:id/crea-consegna`, pezzi dalle righe d'ordine, fix `homePartnerEmails` in Impostazioni. Verificato: le nuove rotte rispondono 401 senza token. 🔖 Da fare a mano: Impostazioni → «Manda una prova a me»; email dei marchi interni in `recapPartnerEsclusi`; ricontrollare che `homePartnerEmails` (Chanel) sia ancora valorizzato.
> 🧷 **07/09/2026 (14) — VENDITA ACCETTATA → CONSEGNA SEMPRE («Vendita Deluxy»); #12901 sanata; pezzi dalle righe d'ordine** (regola utente: «il tipo di servizio è sempre vendita deluxy, crea anche le consegne»; «per le 15 ferme controlla non fossero già state create»). IN LOCALE, committato, NON pubblicato — ma la consegna #101130 è GIÀ scritta in produzione dallo script.
> - **Diagnosi** (lettura diretta di `orders` e `platform`, script nello scratchpad): #12901 importato da Orders alle 15:00, due vendite alle 15:15 (una per riga, entrambe a Enrico Rizzi come proprietario dei prodotti unici), accettate dall'utente partner di Rizzi alle 15:15:26, «consegna NON creata (dati mancanti)»: `creaConsegna` usciva perché `serviceTypeId` era null — le vendite da Orders non ne hanno mai uno. Stesso motivo su tutte le **15** accettate senza consegna dal 24/08. Seconda cosa: le righe erano ×2 a 130 € e ×5 a 65 €, le vendite erano nate 130 € × 1 e 65 € × 1 (`orders-sync` passava il prezzo unitario e nessuna quantità per i prodotti a catalogo).
> - **Codice**: `SalesService.servizioVenditaDeluxy()` (id per nome, in memoria) usato da `creaConsegna` quando la vendita non ha servizio; `accetta` scrive il servizio anche sulla vendita; nuova `creaConsegnaMancante(id)` + rotta `POST /sales/:id/crea-consegna` (ADMIN/OPERATION, idempotente, dice cosa manca). `orders-sync`: per i prodotti a catalogo `amount = prezzo × pezzi` e `quantity = pezzi` (come i generici). Typecheck verde.
> - **Script** `api/scripts/crea-consegne-vendite-ferme.ts` (ts-node; prova senza argomenti, `--applica`, `--solo`, `--salta`): cerca PRIMA una consegna esistente (DDT = numero d'ordine, o partner+data+cognome) e la aggancia; stesso ordine + stesso partner = UNA consegna con più righe; riallinea pezzi e importo dalla riga d'ordine (lettura una tantum di `orders."RigaOrdine"`). **Eseguito con `--solo` sulle due vendite di #12901**: vendite portate a 260 € × 2 e 325 € × 5, nata la consegna **#101130** (Rizzi, 09/09, fascia 14-15, DDT 12901, due righe: Macarons ×2 a 130, Praline ×5 a 65).
> - **Le altre 13, NON toccate (decide l'utente)**: #12895 La Dolceroma ha già la #101104 (DDT 12895) legata a un'altra vendita dello stesso ordine → niente da fare; #12879 Stefanelli è la riga «Selections» da 5 € (generico Torte e dolci, data 06/09 passata) → da non creare; #2729 Lami Fiori (Flowers, consegna 17/08, passata); #12804 e #12818 Stefanelli senza data; 8 accettate il 03/09 alle 14:08 SENZA partner (Flowers/cakedesign, date di agosto: 2650, 1758, 2684, 1780, 2717, 1785, 2732, 2789). Per crearle: `npx ts-node scripts/crea-consegne-vendite-ferme.ts --applica --solo <id,…>` dopo aver messo partner/data.
> - ⚠️ **Aperto**: oggi ogni vendita accettata crea la SUA consegna; con due righe dello stesso ordine per lo stesso partner (come #12901) nascerebbero DUE consegne allo stesso indirizzo. Lo script le unisce; il codice di `accetta` no. Proposta: in `accetta`, se esiste già la consegna di una vendita sorella (stesso ordine, stesso partner), aggiungere la riga lì. `Sale.deliveryId` è `@unique`: la seconda vendita resterebbe senza `deliveryId`, va deciso come marcarla.
> - Manuale (64), guida visiva (regola del posto), MANUALE-DELUXY (riga), Word rigenerato.
>
> 📬 **07/09/2026 (13) — RECAP GIORNALIERO AL PARTNER (7:00 e 18:30) + mail «Nuovo servizio» formale** (regole utente: «manda ogni mattina alle 7:00; se ci sono consegne per il giorno dopo anche un reminder alle 18:30»; «riformatta questa mail: indica data, orario di ritiro e usa un testo più formale»). IN LOCALE, committato, NON pubblicato.
> - **Modulo nuovo** `api/src/recap-partner/recap-partner.module.ts` (`RecapPartnerModule` in `app.module`): `GET /cron/recap-partner-mattina` e `GET /cron/recap-partner-sera` (Bearer `CRON_SECRET`, `?forza=1` salta il controllo dell'ora), `GET /recap-partner/anteprima?partnerId=&giorno=&tipo=` e `POST /recap-partner/prova {a?, partnerId?, giorno?, tipo?}` (ADMIN/OPERATION). Consegne VIVE del giorno (`deletedAt null`, stato non in `DELIVERY_CLOSED_STATUSES`), raggruppate per partner; destinatario = `Partner.email` (attivo, non cancellato, non in `recapPartnerEsclusi`); NON dipende da `mailNotifications`. **Finestra**: la mattina oggi + 6 giorni (`GIORNI_RECAP_MATTINA = 7`, regola utente «non solo di oggi ma dei prossimi giorni»), un blocco per giorno con oggi in testa; la sera solo domani. Idempotenza: marcatore `[recap-partner:<tipo>:<giorno dell'INVIO>]` in UNA riga `DeliveryLog` (`type note`) per partner e invio, sulla prima consegna dell'elenco, cercato con `deliveryId in (…)` (indice) — un invio solo per partner e giorno.
> - **Ora di Roma**: `vercel.json` crons `0 5,6 * * *` (mattina) e `30 16,17 * * *` (sera); la rotta parte solo se `oraRoma()` è 7 (o 18). Helper nuovi in `api/src/common/giorno-roma.ts` (`giornoRoma`, `oraRoma`, `dataLungaRoma`, `dataBreveRoma`, `fasciaOraria`): le date delle consegne sono mezzanotte UTC del giorno civile, si formattano in UTC.
> - **Mail**: HTML (una scheda per consegna: numero, fascia, ritiro con valet/telefono o «a cura tua», destinatario+indirizzo+citofono+telefono, prodotti con nota di specifica in oro, note, link) via `SettingsService.inviaHtmlViaAiMail` (nuovo, corpo HTML grezzo, timeout 45 s). Mai `internalNotes` né prezzi. Impostazioni: chiave `recapPartnerEsclusi` (SETTING_KEYS), sezione «Recap giornaliero ai partner» in `settings.component` con «Manda una prova a me» (salva prima solo quel campo, poi `POST /recap-partner/prova`); i18n `settings.recap.*`.
> - **Mail «Nuovo servizio» riscritta** (`notificaInserimentoAlPartner`): rilegge la consegna e scrive numero+servizio, data lunga, ritiro (fascia + valet e telefono, o «a vostra cura»), fascia di consegna, destinatario con indirizzo/citofono/telefono, prodotti con nota di specifica, note; oggetto «Nuova consegna Deluxy n. … · gg/mm/aaaa · ritiro dalle … alle …»; «Gentile …, vi informiamo …, Cordiali saluti». Resta condizionata a `mailNotifications`.
> - ⚠️ **Difetto trovato e corretto insieme**: `settings.component` non ricaricava `homePartnerEmails` → ogni «Salva» in Impostazioni lo AZZERAVA (stesso difetto già visto su Hub/Finance). Da riscrivere il valore dopo il deploy se risultasse vuoto.
> - **Verificato**: `tsc` API verde, `ng build` verde. NON provato dal vivo: l'API locale non parte (vedi voce 12) — la prova vera è il bottone «Manda una prova a me» dopo il deploy. Docs: manuale (62)(63) + sezione «Recap giornaliero al partner», Word rigenerato, guida visiva aggiornata e **ripubblicata su https://claude.ai/code/artifact/17c9fcad-6a0e-4da7-982f-fb546431d1d1** (⚠️ il vecchio indirizzo cd3c9488… non esiste più: era un'altra copia; da qui la guida sta lì), MANUALE-DELUXY (scoutwt) con la riga nel registro, ripubblicato allo stesso indirizzo.
> - 🔖 Al deploy: le env Vercel non cambiano (`CRON_SECRET`, `MAIL_*` già ci sono). Poi: Impostazioni → «Manda una prova a me», e mettere in `recapPartnerEsclusi` le email dei marchi interni (Deluxy Flowers, Cakedesignme, Artista Locale) se non devono riceverlo.
>
> 🔀 **07/09/2026 (12) — Riallineamento della cartella principale: `platform-0409` fuso in `piattaforma-ricerca-insensitive`** (comando utente «lavora su questo pc… leggi handoff e aggiorna memoria… lavora su locale prima del push»).
> - **Stato trovato**: la cartella principale `C:\Users\nicol\app` era su `piattaforma-ricerca-insensitive` a `a1a91301` con 15 commit locali (segnalazioni UX, Personale, design system, sito cakedesign, fix canale app del 06/09) e modifiche NON committate (regola «Deploy e costi di build» nel `CLAUDE.md`, `ignoreCommand` in ogni `vercel.json`, segnalazioni performance/UX del 07/09). La linea viva della piattaforma era invece `platform-0409` (worktree `.claude/worktrees/deploy-delivery`, pushato e in produzione: `delivery-iug01f74i`), 134 commit più avanti della base comune `ee0124b5` (04/09).
> - **Fatto**: commit `81e4b729` (docs + vercel.json), poi merge `319e8862` di `platform-0409`: per `app-api.module.ts`, `deliveries.service.ts` e questo handoff vale la versione di `platform-0409` (conteneva già, identico, il fix del 06/09 fatto di qua); in `SEGNALAZIONI-UX.md` tenute entrambe le righe del registro e lo stato più recente. Da qui `platform-0409` è ANTENATO di `piattaforma-ricerca-insensitive`: i due rami vanno tenuti allo stesso commit (push di entrambi; nel worktree `git pull --ff-only`).
> - **Verificato in locale**: `prisma generate` + `tsc --noEmit` API verde; `ng build --configuration development` verde (solo il warning NG8107 pre-esistente in `receipts-list`); web locale su **4230** in configurazione di produzione con proxy verso l’API pubblicata (`web/proxy.prod.json`, ignorato da git; 4200/4210 erano occupate da server di altre sessioni). ⚠️ L’avvio dell’API locale sul database vero (lanciatore che legge `DATABASE_URL` da `deluxy-tasks/.env`, come `avvia-api-reale.mjs` delle sessioni precedenti) è stato BLOCCATO dal classificatore di permessi: `api/.env` ha ancora `DATABASE_URL="file:./dev.db"`, quindi `npm run dev:api` da solo cade al `$connect`. Per provare l’API in locale serve che l’utente lo consenta o avvii lui il lanciatore.
> - ⚠️ **Non toccato, da decidere**: `api/scripts/rimedia-recap-inviati.ts` risulta CANCELLATO nel working tree (non committato; non so da chi né perché: era il rimedio una tantum del 03/09); `api/zz-ddt.mjs` e `promossi-tutti-anni.json` alla radice sono file di lavoro non tracciati. Restano come erano.
> - **Nessuna funzionalità nuova** in questo giro: manuale, guida visiva e Registro delle novità invariati. Memoria aggiornata (regole di lavoro + dove/come della piattaforma).
>
> 🪗 **07/09/2026 (11) — tendina prodotti: altezza dallo spazio vero, e apertura verso l'alto** (in locale, solo web; segnalazione utente «la lista a un certo punto si interrompe e non si può scorrere di più»). `delivery-form.component.ts`: signal `altezzaTendina()`/`tendinaSopra()`, `misuraTendina(campo)` chiamata dal focus (che ora passa `$event.target`) e da `@HostListener('window:scroll'|'window:resize')` tramite `campoTendina`; regola `.prod-risultati.su { top: auto; bottom: calc(100% + 4px) }`, tolta la `max-height` fissa dal CSS (la mette il template con `[style.max-height.px]`), aggiunto `overscroll-behavior: contain`. ⚠️ **Ci sono ricascato**: un backtick dentro un commento nel blocco `styles: […]` (che è un template literal) rompe la compilazione con «Failed to resolve styles at position 1 to a string» — è la seconda volta oggi. Nei commenti CSS niente backtick. `ng build` verde dopo la correzione. ✅ **PUBBLICATO**: `delivery-iug01f74i`, bundle **main-E4PQDHKG**; verificato nel bundle servito da app.deluxy.it che ci siano `overscroll-behavior:contain` e `.prod-risultati.su{top:auto;bottom:calc(100% + 4px)}`, e che la vecchia `max-height:280px` fissa sia sparita.
> ✅ **07/09/2026 sera — LIVE `delivery-a9alt7s48`** (prebuilt dalla RADICE del worktree `.claude/worktrees/deploy-delivery/`). Verificato dal bundle, non dal messaggio di Vercel: app.deluxy.it risponde 200 col bundle **main-HCBS2JIZ**, il chunk `chunk-QJIYYY6L.js` servito dalla produzione contiene `suoDelPartner` (la tendina prodotti), `/api/v1/deliveries` risponde 401 senza token. builds.json con **entrambi** i builder (@vercel/node + @vercel/static-build), 85 file statici, funzione `api` presente.
>
> ⚠️ **Ripresa la trappola del link annidato**: `vercel pull` lanciato dentro `deluxy-platform-next/` aveva creato lì un `.vercel` col progetto **deluxy-platform-next** (quello sbagliato, senza alias) — è lo stesso errore che il 06/09 lasciò app.deluxy.it in NOT_FOUND per 8 minuti. Il link giusto (progetto **delivery**) sta SOLO alla radice del worktree. Il `.vercel` annidato è stato spostato in `scratchpad/vercel-annidato-tolto-0709` (non cancellato). **Pull, build e deploy si lanciano da `.claude/worktrees/deploy-delivery/`, mai da dentro.**
>
> ⚠️ **Prima del build: merge di `origin/platform-0409`** — 3 commit del canale app (`GET /app/consegne?ddt=`, `POST /app/consegne/:id/consegnata`, `giaConsegnata`) erano già IN PRODUZIONE ma non nel ramo locale: deployare senza li avrebbe tolti, rompendo il Customer Service collaudato ieri. Merge pulito, `tsc --noEmit` verde.
>
> ✅ **PUSHATO** il 07/09: `origin/platform-0409` allineato a `9572e2ab` (0 commit locali, 0 divergenza). ⚠️ Il ramo contiene anche il merge di `origin/platform-0409` coi 3 commit del canale app dell'altra sessione: chi lavora di là faccia `git pull` prima di ripartire.
> 🔖 **07/09/2026 (10) — TRE PUNTI APERTI, lasciati aperti su decisione dell'utente** («per ora lascia il punto aperto»). Nessuno è stato implementato.
>
> **(a) Le righe «- Selections» degli ordini Shopify.** Non sono prodotti: sono la PERSONALIZZAZIONE della torta, che l'app Shopify fattura come riga a sé, senza SKU. In Orders sono **317** (`RigaOrdine`, campo `proprieta`), formato costante: titolo = «<nome del prodotto padre> - Selections», e nelle proprietà la scritta sulla torta, la candelina, la foto, col prezzo («| €6»). Oggi ognuna genera una vendita «da gestire» senza prodotto (ordine 12895: Sacher 35 € + «Sacher - Selections» 17 €; anche 12879 Tiramisù + 5 €). Le tre strade proposte: **1)** fondere la riga nel prodotto padre — importo sommato alla vendita del padre e proprietà come NOTA DI SPECIFICA per chi prepara (era la mia raccomandazione: il padre si riconosce dal titolo prima del « - », nello stesso ordine); **2)** scartarla (si perdono i soldi e la scritta); **3)** lasciarla com'è. ⚠️ Orders espone alle altre app solo `sku/titolo/quantita/prezzo` (`orders-sync.module.ts` riga 42): per la strada 1 servirebbe esporre anche `proprieta`.
>
> **(b) 12 vendite ACCETTATE rimaste senza consegna** (su 460), tutte per lo stesso motivo: `creaConsegna` esce se manca `serviceTypeId`, e la vendita non ce l'ha. Il registro scrive «Accettata dal partner — consegna NON creata (dati mancanti)» e nessuno se ne accorge. Le ultime: #12895 La Dolceroma (07/09), #12804 Stefanelli (05/09), otto del 03/09. Proposta non attuata: la vendita accettata prende il servizio predefinito «Vendita Deluxy» (che è quello che l'ufficio mette poi a mano) invece di fermarsi in silenzio.
>
> **(c) Recap giornaliero al partner: NON esiste.** Verificato: i cron della piattaforma sono tre (margini, smistamento, non-consegnate) e nessuno manda riepiloghi. L'unico recap è **mensile e a mano** (`POST /invoices/recap/:partnerId/invia`, HTML nel corpo + allegato, via AI Mail). L'unica mail automatica al partner è quella dell'inserimento di una consegna (`notificaInserimentoAlPartner`), tre righe e un link. Proposta non attuata: un cron serale che manda a ogni partner le consegne dell'indomani (numero, ora, destinatario, indirizzo, prodotti, note di specifica) — da concordare l'orario.
>
> Riscontro dato all'utente sul caso La Dolceroma: la mail è partita davvero, il 07/09 alle 11:20 («Nuovo servizio Deluxy · consegna #101104» → dolceste@yahoo.it), ma per l'inserimento a mano della consegna, non per l'accettazione delle 11:01 — sono i 19 minuti del punto (b). Domani (08/09) il partner non ha consegne: la Sacher è il 09/09.
> ✅ **07/09/2026 (9) — PRODUZIONE FERMA e RIMESSA IN PIEDI: mancavano gli indici sulle chiavi esterne** (segnalazione utente: «live non carica più nulla»). Nello schema `platform` le tabelle figlie della consegna avevano SOLO la primary key: `DeliveryLog` (36.548 righe), `DeliveryProduct` (59.727), `Activity` (58.429), e `Delivery` non aveva niente su `parentDeliveryId` (63.137 righe, 89 MB). Ogni lettura delle righe di UNA consegna scorreva tutta la tabella. Misura: log di una consegna = Seq Scan, 36.547 righe scartate per restituirne 2, **4.541 ms**. Con l'elenco che si riaggiorna ogni 30″ le richieste si accumulavano: transazioni ferme da 94 e 117 s, e con loro rallentava AI Mail (stesso Postgres). **Creati 7 indici** (lanciati dall'utente, `scratchpad/indici-emergenza.mjs`; ⚠️ il pooler NON accetta `CONCURRENTLY` — lo script ripiega sull'indice normale, che blocca le sole scritture: 66 s il più lento, su Delivery): `DeliveryLog(deliveryId)`, `DeliveryProduct(deliveryId)`, `Activity(deliveryId)`, `Activity(valetId)`, `Delivery(parentDeliveryId)`, `Sale(partnerId)`, `Sale(productId)`. **DOPO: 4.541 ms → 8 ms** (piano da Seq Scan a Index Scan, 862 buffer → 4); connessioni attive 10-14 → 2. In locale: `@@index` aggiunti a `schema.prisma` con dentro il perché (`prisma validate` verde) e registro in SEGNALAZIONI-PERFORMANCE. ⚠️ Due lezioni: **nessun deploy risolveva** (il codice non c'entrava), e la prima diagnosi — il solo `parentDeliveryId` — era sbagliata perché misurata a database già congestionato: sotto contesa OGNI query sembra lenta, quindi si misura la singola query con EXPLAIN, non l'orologio.
> 🪗 **07/09/2026 (8) — tendina prodotti: prima quelli del partner, davvero** (in locale, solo web): in `web/src/app/pages/delivery-form.component.ts` — `suoDelPartner(p)` (unico + partner della consegna, letto da `partnerId` piatto o da `partner.id`), `ordinaConSuoiPrima(items)`, `caricaProdottiIniziali()` con DUE letture (`?partnerId=&unique=true&active=true&pageSize=100` + `?active=true&pageSize=20`, unite con `unisciProdotti`), `active: true` e riordino anche nella ricerca, `[class.suo]` in tendina e CSS `.prod-risultati .ris.suo` (spostato DOPO `.ris`, che azzera il peso con font: inherit). ⚠️ Il pezzo che mancava davvero: **`onPartnerChange()` ora richiama `caricaProdottiIniziali()`** — la tendina si riempiva prima che il partner fosse scelto e non si rileggeva più. ⚠️ La modifica del 06/09 sera era andata PERSA: il commit d6bc5ad0 conteneva di quel file la sola riga CSS. `ng build` verde; verificato che `suoDelPartner` sia nel bundle (`dist/web/browser/chunk-QJIYYY6L.js`). 🔴 Da guardare a schermo e da pubblicare.
> 🪗 **07/09/2026 (7) — ordini generici ricostruiti coi generici** (in locale; colonna `Sale.quantity` creata in prod): `OrdersSyncService.genericoPerTitolo()` (tabella di parole in ordine, pezzi letti dal titolo), indice `generici` sugli sku `GEN-*`, `ingest`/`create` accettano `quantity` e `productName`, `creaConsegna` scrive la riga con quella quantità e col prezzo unitario ricavato (`alPartner / pezzi`) quando il listino è 0. Provato sui titoli veri: bouquet→Bouquet, 50 rose rosse→Rose ×50, cappelliera di rose→Cappelliera, cesto di frutta→Gastronomia, biglietto personalizzato→nessun generico.
> 🪗 **07/09/2026 (6) — riga senza SKU, ricerca, bottoni del preventivo** (in locale): `orders-sync` riconosce la riga senza SKU dal NOME (indice `perNome`, nomi ambigui esclusi) e, in mancanza, crea una vendita da gestire col titolo se la riga ha un prezzo; `ingest` deduplica sul titolo quando non c'è prodotto; `textSearch` accetta i campi «solo dall'inizio» e `realOrderNumber` ci finisce (2804 pescava 22 righe, ora 12); `findAll` calcola `preventivoMancante` con due letture e la lista nasconde accetta/rifiuta/inserisci finché il preventivo non c'è; cron smistamento con `?da=` (max 90 giorni). Verificato: ordine 12893 → 2 vendite (Spring Cake 90 € Cakedesignme, Van Gogh 85 € Negozio Fiori Cannavo); nessun altro ordine composto degli ultimi 30 giorni ha vendite mancanti (542 ordini controllati uno per uno contro le righe di Orders).
> ✅ **07/09/2026 sera — LIVE**: piattaforma `delivery-g31h9bpta` (bundle main-CRM73WY5 su app.deluxy.it) e Merchandising `deluxy-merchandising-l7f5uavam` (⚠️ il deploy di Merchandising vuole `--scope deluxy`, altrimenti «Not authorized»). Dopo il deploy di Merchandising le note sono state importate nella piattaforma: `542 prodotti e 522 varianti` (`node api/scripts/importa-note-da-merchandising.mjs --scrivi`), da rifare a ogni giro di note nuove.
> 🪗 **07/09/2026 (5) — note di specifica, vendita per riga, preventivo→riconciliazione** (in locale; colonne e note già in prod). Merchandising: `Prodotto.note` e `Variante.note` (`scripts/applica-note-prodotto.mjs`), `scripts/note-da-descrizione.mjs` (681+795 note ricostruite), campo nel modulo (prodotto e riga variante), `/api/v1/prodotti` espone note e varianti. Piattaforma: colonne `Product.note`/`ProductVariant.note`, `merchandising-sync.allineaNote()` + rotte, nota mostrata nella riga della consegna e nelle righe dell'ordine (con foto, produttore, prezzo, dove è finita), `ingest` deduplica per riga, `orders-sync` crea UNA VENDITA PER RIGA, `dettaglioOrdine` torna `vendite` e `incompleto` (alert ordine composto), `salvaPreventivo()` + `POST /sales/:id/preventivo` (riconciliazione accettata) e cancello in `prendiInMano`, bottone Archivia nella scheda prodotto, generico «Cappelliera fiori» (29 archiviate). 🔴 Le note si importano DOPO il deploy di Merchandising (`node api/scripts/importa-note-da-merchandising.mjs --scrivi`).
> 🪗 **07/09/2026 (4) — Chanel intoccabile + secondo giro dei generici** (dati già in prod): `scripts/chanel-fuori-dalle-pulizie.mjs` (12 prodotti Chanel rimessi a catalogo, Sushi Floreale in Fiori e in archivio), guardia `partnerIntoccabili()` nelle tre pulizie, `scripts/prodotti-generici-adhoc.mjs` (86 righe archiviate, generici Rose/Sushi/Varie creati). Otto generici di Deluxy: Bouquet, Fiori, Rose, Torte e dolci, Palloncini, Vino e bollicine, Sushi e gastronomia, Varie. Catalogo a 2.033.
> 🪗 **07/09/2026 (3) — nascondi da consegne + pulizia catalogo** (in locale; colonne e archivi già in prod): `Delivery.nonConsegnataChiusaIl/Da` (`scripts/applica-nascondi-non-consegnata.mjs`), viste attive/storico e cron aggiornati, rotte `POST/DELETE /deliveries/:id/nascondi`, terza scelta nella finestra Riconsegna. Script `archivia-consegna-ed-extra.mjs` (109 archiviati, generico del canale app salvo) e `prodotti-generici-deluxy.mjs` (4 generici di Deluxy al posto di 93 doppioni col prezzo nel nome; somma consegne invariata 66.611,53 €). Restano fuori famiglia: SLG ×2 e «Sushi Floreale - Medio €70».
> 🪗 **07/09/2026 (2) — riconciliazioni ripulite** (in locale; riclassificazione già in prod): regola palloncini ristretta alla categoria in `classifica-tipologia-vendita.mjs` (552 prodotti riscritti con `--forza --scrivi`); `genera` e `lista` saltano i partner non attivi, cancellati o esclusi dalle proposte. Proposte in pagina: 99 → 6 (Summer Star/Tortatelier, Wildflower e Torta Crema e Fragole/Dante, Tiramisù/Pappagallo, Millefoglie/Cakelab Firenze, Red Velvet Rose Cake/Lijoi).
> ✅ **07/09/2026 — LIVE**: piattaforma `delivery-7rdhyca7k` (prebuilt, bundle main-HJ7X4FA3 su app.deluxy.it, pushato 05ae398e) e Customer Service `deluxy-messaging-kfgtrq86b` (build remota, scout-ui f00633fc). Dentro: preventivo che si propone da solo, riconciliazioni filtrate, non consegnata NERA e con la sua etichetta, riconsegna agganciabile col legame in elenco, listino dentro Prodotti, rifiuto al secondo della lista.
> 🪗 **07/09/2026 — riconsegna agganciabile, legame in elenco, «Non consegnata» in etichetta e in nero** (in locale): API `agganciaRiconsegna`/`sciogliRiconsegna` + rotte `POST/DELETE /deliveries/:id/riconsegna/:childId`; `DELIVERY_LIST_SELECT` porta `parentDelivery` e il `code` delle figlie; nuovo `shared/riconsegna-dialog.component` (due scelte, ricerca su `GET /deliveries?q=&view=tutte`) usato da elenco e dettaglio; frecce ↩/↪ nella cella del numero; i18n `riconsegna.*` e `deliveries.legame.*`. Customer Service: la riapertura «non consegnata» guarda la CONSEGNA e non il cambio di stato (colonna `appNonConsegnataId`, creata in prod) — 5 ordini erano rimasti chiusi con la consegna fallita e rientrano al prossimo giro del cron (ogni 15 minuti).
> 🪗 **07/09/2026 — preventivo automatico, riconciliazioni filtrate, non consegnata NERA** (in locale; dati già scritti in prod): `preventiviDelProdotto()` + filtro in `scegliPartner` (solo chi ha un prezzo, e a quel prezzo; base = SKU della VARIANTE, mai quello del prodotto quando la variante c'è); `create` non blocca più a priori i preventivi, il motivo «nessun partner ha dato un prezzo» lo scrive `bloccoPreventivo`; riconciliazioni `genera` e `lista` solo NON_UNICO+preventivo (100 → 21 proposte); colore `not_delivered` → nero in `stati-consegna.ts` + pastiglie/pallini/legenda in elenco, dettaglio e pagina di conferma. Dati in prod: Lijoi senza Cake designer e fuori dalla lista RM (ora 1° Il Pappagallo); Cake designer ha lo smistamento automatico (acceso dall'utente). 🔴 Da guardare a schermo e pubblicare.
> 🪗 **07/09/2026 — rifiuto al secondo della lista, fiori a stelo «a numero», listino dentro Prodotti** (in locale; 29 prodotti riclassificati in PROD): `sales.rifiuta` cerca il prossimo con `scegliPartner(..., rifiutati)` e avvisa il nuovo partner; `app/prodotti` cerca anche per SKU di VARIANTE e restituisce `tipologia` e `varianti` (prima `forma()` non le esponeva: il CS non poteva fare il controllo a monte); `scripts/fiori-a-stelo-sono-a-quantita.mjs` (29 scritti) e precedenza corretta in `classifica-tipologia-vendita.mjs`; linguetta «Il mio listino» in Prodotti (partner) e linguette anche nella pagina Listino. CS: `propostaVendita` riconosce a-numero e a-preventivo dalle righe dell'ordine (`righe2`), `liste-prodotto` importa anche i prezzi da DDT (sku PP-*) riportandoli al codice di catalogo. 🔴 Da guardare a schermo e da pubblicare.
> ✅ **06/09/2026 notte — LIVE tutte e tre**: piattaforma `delivery-9zk1xd5s7` (prebuilt dalla radice del worktree, bundle main-JOU56BM4 su app.deluxy.it, pushato fino a 1dc43e21), Merchandising `deluxy-merchandising-6yncyp8cz` e Customer Service `deluxy-messaging-4crxbij9y` (build remota; scoutwt pushato sul ramo `liste-prodotto-0609` perché scout-ui locale è divergente — il diff verso origin/scout-ui è di sole aggiunte). Verificato: app.deluxy.it 200 col bundle nuovo, /api/v1/activities 401 senza token, i due progetti Vercel «Ready · Production».
> 🪗 **06/09/2026 sera — prodotti A PREVENTIVO e liste di prodotto** (in locale; tabella del CS creata in prod): piattaforma — `sales.create` blocca sempre i prodotti `tipologiaVendita=preventivo` (nemmeno l'eccezione «un solo partner» vale), `/app/prezzi-partner` e `/app/prodotti` espongono `tipologia`. Customer Service (scoutwt/deluxy-messaging, commit 01d9bc3f): `PrezzoProdottoPartner`, `lib/liste-prodotto.ts`, rotta `/api/vendite/prodotti`, linguetta «Liste di prodotto», preventivi nella proposta di vendita. 🔖 Da fare: importare i prezzi (bottone in pagina) e guardarlo a schermo.
> 🪗 **06/09/2026 sera — TIPOLOGIA DI VENDITA: casa in Merchandising** (in locale; COLONNA CREATA e 2.878 righe scritte in prod). Merchandising: `Prodotto.tipologiaVendita`, legenda in `src/lib/dominio.ts` (TIPOLOGIE_VENDITA / ETICHETTA / SPIEGAZIONE), tendina obbligatoria in `FormProdottoNuovo` con legenda + CSS `.legenda-tipologia`, validazione in `azioni-prodotto-nuovo` (create e update), campo esposto in `/api/v1/prodotti`, script `scripts/applica-tipologia-vendita.mjs` (colonna + travaso dalla piattaforma per codice). Piattaforma: `merchandising-sync.allineaTipologie()` + rotte `GET /merchandising-sync/tipologie/prova` e `POST /merchandising-sync/tipologie`, tipologia sui prodotti creati dal tiraggio, DTO `tipologiaVendita` con `@IsIn`, form prodotti con tendina obbligatoria + legenda + i18n `productForm.tipologia.*`. 🔖 Aperto: liste di prodotto nel CS e preventivo obbligatorio.
> 🪗 **06/09/2026 sera — vendita non unica con UN solo partner in provincia → PROPOSTA** (in locale, API compilata): `sales.create` separa `bloccoGrezzo` da `bloccoNonUnico` e toglie il blocco quando `candidati(..., soloLettura=true)` ne trova esattamente uno; `candidati` ha il quarto parametro `soloLettura` che NON crea la lista di priorità automatica. Misure: province con un solo partner — Cake designer 5 (LU PI TO VT NA), Gastronomia 93, Regali 4, Feste 2, Pasticceria 2, Originali 1; le 9 vendite ferme oggi non si muovono (7 con zero partner, 2 con più d'uno). 🔴 Da guardare a schermo dall'utente.
> 🪗 **06/09/2026 sera — Attività: niente consegne da partner, filtri città/valet, «solo io»** (in locale, API e web compilati): `activities.service.findAll(user, date, limite, stato, soloIo, conPartner)` con due condizioni in `where.AND` (`delivery.deliveredByPartner=false` salvo `?conPartner=1`, `valetId` se `?mie=1` e ruolo VALET), controller con le due query; `activities-list.component` con la riga filtri (tendine città e valet ricavate dalle righe caricate, segmento Tutti/Solo io per il valet, spunta consegne da partner) e i18n `activities.filtri.*`. Misurato sul database di oggi: 46 attività, 8 da partner escluse, 38 restano; tendine Milano 26 / Roma 6 / Firenze 2…, valet Cassoli 16 / Bergamasco 10 / Hmamly 6. 🔴 Non guardato a schermo: il tab 4210 è sul login e in questa sessione non si fanno accessi.
> ✅ **06/09/2026 sera — 179 prodotti UNICI caricati alle pasticcerie dai DDT** (`carica-prezzi-partner-da-ddt.mjs --scrivi`, sku `PP-*`, soglia 3 consegne, niente interni, prezzo ≠ pubblico): Clivati 56, Martesana 29, Stefanelli 25, Pappagallo 16… Vintage Ribbon Cake 6 di Pappagallo a 47 € (pubblico 90). Analisi in `scratchpad/ddt-prezzi-padre.{json,md,html}` + artifact 018251b3. 🔖 Restano: liste di prodotto nel CS, preventivi obbligatori sui prodotti a preventivo, tipologia prodotto (proposta: casa in Merchandising).
> ✅ **06/09/2026 sera — LIVE delivery-pbmohor11 (main-XS4HKISD, pushato f88df67f)**: Listino del fioraio, fiori sbloccati, pop-up vendite corretto.
> 🪗 **06/09/2026 sera — Listino del fioraio + pop-up vendite**: modulo `listino` (`GET`/`POST /listino-fiori`, 16 fiori principali, vuoto = non lo faccio), pagina `/listino` + voce di menu PARTNER + avviso sulla home, `Partner.listinoFioriCompilatoIl` (migrazione fatta in prod, 32 fioristi); fiori sbloccati (`notEditable` false su 29 prodotti e nello script); pop-up vendita con ✕ nell'angolo, righe dell'ordine e fascia oraria (`GET /sales/:id/ordine`); nuova `GET /app/prezzi-partner` (riconciliazioni accettate + prodotti unici) per le liste di prodotto del CS. Verificato via API: listino Maryflor 5 prezzi da confermare e 11 vuoti, come partner 16 righe.
> ✅ **06/09/2026 sera — 29 prodotti UNICI «Fiori a stelo» caricati in prod** (`carica-fiori-a-stelo.mjs --scrivi`, categoria «Fiori a stelo» creata, `notEditable`); LIVE piattaforma `delivery-hyh7rl5ez` (flag escluso dalle proposte) e CS `deluxy-messaging-emgyvd20s`. In corso: tabella prezzi torte per variante a catalogo (agente).
...
> 🪗 **06/09/2026 sera — NUOVA ARCHITETTURA VENDITE (piattaforma)**: `PartnerConsegnaProvincia` (area di consegna: provincia, minimo, raggio; semina = province già abilitate dei 12 col flag), scheda partner «Dove consegna da solo», `scegliPartner` con minimo/raggio per provincia, consegna «da partner» solo dentro l'area di consegna, app-api `vendita/provincia/:sigla`, `aree-commerciali`, `liste-priorita`; `quotaDaOrders` chiede prima al Customer Service (AppSetting `customerServiceUrl`/`customerServiceApiKey`). Il CS (scoutwt/deluxy-messaging, branch `cs-vendite-custode`): lib/vendite.ts, pagina /vendite (sconti, partner per provincia, liste per area), `GET /api/v1/quota-fornitore` (casa della quota), blocco «Proposta di vendita» sulla scheda ordine. Orders: `/api/v1/quota-fornitore` delega al CS (env CUSTOMER_SERVICE_URL/API_KEY). ⚠️ Nel CS l'«extra fuori provincia» si legge come totale − righe prodotto (Orders non espone le shipping lines).
> 🔧 **06/09/2026 sera — FIX profilo partner — LIVE `delivery-drk3zs16a`, pushato aa2c6797**: la riassegnazione del DTO per il PARTNER in `partners.update` scartava i tre campi di vendita (200 «aggiornato» ma niente scritto). Ora passano (gated dal servizio VENDITA). Verificato con chanel_consegne@deluxy.it via API locale.
...
> 🪗 **06/09/2026 sera — sconto dalla REGOLA DEL TERRITORIO di Orders**: `quotaDaOrders` manda `conPartner=1|0` (partner attivo con la provincia fra le effettive) e cache per (provincia, categoria, p/np); Orders risponde `regola=territorio` (40 senza partner; 20 MI / 30 altrove con partner; prezzo a 5). Orders: `deluxy-orders` in scoutwt (controllo.ts, quota-fornitore/route.ts, piattaforma.ts `provinciaHaPartner`). 🔴 Orders in prod ha bisogno di `PLATFORM_API_KEY` solo per chi NON passa conPartner (CS, Budgets).
> ✅ **06/09/2026 ore 16 — LIVE `delivery-uby7myxyo` (bundle main-AETIQEQK) su app.deluxy.it**, pushato su origin (17e03614+): aree (partner e valet, «Tutto il mondo», «Milano e hinterland»), province a mano, mestieri, minimo/raggio, link di conferma, riconciliazione per variante, regole di smistamento. ⚠️ Incidente durante il deploy: (1) il primo deploy è finito sul progetto «deluxy-platform-next» (innocuo, senza alias); (2) avevo poi fatto `vercel link` DENTRO `deluxy-platform-next/` — con quel `.vercel` annidato il build dalla radice ha ignorato il vercel.json (builds.json = solo `@vercel/static`, 0 file statici) e il deploy `delivery-banuqhekz` ha lasciato app.deluxy.it in NOT_FOUND per ~8 minuti; rollback e alias bloccati dal classificatore, risolto togliendo il `.vercel` annidato, ricostruendo e rideployando. Regola: il link Vercel sta SOLO alla radice del worktree (progetto «delivery», root directory `deluxy-platform-next`); prima di `vercel deploy` controllare `.vercel/output/builds.json` (deve avere @vercel/node E @vercel/static-build) e che `.vercel/output/static` non sia vuoto.
> ✅ **06/09/2026 sera — commit ffb15d69 ripreso col cherry-pick** (canale app: 500 dal CS, /app/partner coi servizi, GET /app/prodotti): il worktree contiene ora tutto quello che è in produzione; via libera dell'utente a push e deploy.
> 🔴 **06/09/2026 sera — DEPLOY FERMO, da finire a mano**: il build precompilato è passato ma il deploy è finito sul progetto Vercel SBAGLIATO («deluxy-platform-next», nato il 05/09, senza alias: innocuo). app.deluxy.it sta sul progetto **«delivery»** (la cartella è ora ricollegata lì con `vercel link --project delivery`). PRIMA di deployare su «delivery» va ripreso nel worktree il commit `ffb15d69` (canale app: 500 dal Customer Service, /app/partner con i servizi, GET /app/prodotti), che sta nella copia `app/` (branch locale `piattaforma-ricerca-insensitive`, 14 commit non su origin) ed è già IN PRODUZIONE (bundle main-2MBI6FNY, deploy di 2 ore prima): senza, il deploy lo toglierebbe. Sequenza per l'utente, dal worktree: `git cherry-pick ffb15d69` → `git push origin HEAD:piattaforma-ricerca-insensitive` → `npx vercel pull --yes --environment=production && npx vercel build --prod && npx vercel deploy --prebuilt --prod` con i due server locali SPENTI (npm install del build rigenera Prisma e vite: EPERM se girano). Il classificatore di questa sessione blocca push, merge e cherry-pick.
> 🪗 **06/09/2026 notte — Smistamento: provincia anche per gli UNICI, riconciliazione per VARIANTE** (in locale; colonna e backfill FATTI in prod): `FinestraConsegna.variantId` (create e riproponi), `candidati(product, provinceId, variantId)` → regola accettata solo con `productVariantId` uguale, `esisteCandidato` con provincia anche per l'UNICO (+ variante, passata da orders-sync), riconciliazioni `genera`/`daVendita`/`lista` per terna (prodotto, variante, provincia), pagina Riconciliazioni con la variante. Verificato: GET /riconciliazioni mostra 79 proposte con variante. 🔖 Aperto: 1 proposta con più varianti da sdoppiare al prossimo giro notturno.
> 🪗 **06/09/2026 notte — Province a mano oltre alle aree + «Milano e hinterland» ai valet di Milano** (in locale; colonna e dati FATTI in prod): `manuale` su PartnerProvince/ValetProvince (`applica-migrazione-province-manuali.mjs`), `provinceIds` = province a mano (`manuale: true`, dedup), ricalcolo dalle aree solo sulle righe non manuali e sempre dopo un update con soli `provinceIds`; form partner/valet: sezione province sempre visibile («Province in più» con le aree), chip = solo le manuali, «province coperte» = unione, payload manda aree E province. `applica-valet-milano-hinterland.mjs`: 61 valet → area «Milano e hinterland». Verificato via API sul valet «Prova Verifica» (4 casi, ripristinato a vuoto). 🔖 Aperto: 6 aree senza nessuno (Lecce, Lecco, Lodi, Piacenza, Teramo, Tutta Italia) e 40+ aree di una provincia: da accorpare dalla pagina Aree quando si vuole.
> 🪗 **06/09/2026 notte — AREE anche per i VALET + «Tutto il mondo»** (in locale; tabella e semina FATTE in prod): schema `ValetArea`, `AreeService.assegnaAlValet/ricalcolaProvinceValet` (+ conteggio valet e ricalcolo dei valet quando un'area cambia), valets DTO `areaIds` + service (create/update → aree → `findOne` fresco; `valets.module` importa `AreeModule`), `VALET_INCLUDE.aree`, scheda valet «Aree servite» (province nascoste se ci sono aree), models `Valet.aree`, i18n `valetForm.areeHint`. `applica-migrazione-aree-valet.mjs`: area «Tutto il mondo» (108 province, EE compreso) a 6 partner + 3 valet (province ricalcolate a 108), copertura esatta per gli altri 94 valet (49 aree di una provincia create → 83 aree in tutto). Verificato via API locale (GET /aree, /valets/:id). 🔖 Aperto: 49 aree di una sola provincia nate dai valet — l'ufficio può accorparle dalla pagina Aree; «Blulogistica» resta un partner cancellato con utente e valet vivi.
> 🪗 **06/09/2026 notte — AREE + profilo partner + Consegna da Partner nei mestieri** (in locale; tabelle e semina FATTE in prod: 33 aree, 126 partner): `aree/aree.module.ts` (lista/crea/aggiorna/elimina, `assegnaAlPartner`, `ricalcolaProvincePartner`), schema (Area, AreaProvincia, PartnerArea, Partner.aree, Province.aree), partners DTO `areaIds` + service (create/update → aree → ricalcolo province), `applica-migrazione-aree.mjs`; web `aree.component` + rotta/nav, scheda partner (Aree servite, province nascoste se ci sono aree, «Consegna da Partner» nei mestieri con raggio condizionato, «Categorie vendute» nascosta e non più inviata), profilo partner «Le tue vendite» (`haVendita` da `auth.profilo`, `aggiornaProfilo` gated su servizio VENDITA, whitelist partners.service), i18n `aree.*`, `partnerForm.aree.*`, `profilo.vendite.*`. 🔖 Aperto: 4 partner senza province né aree (Basara Trasferimenti, Olfattorio Rinascente, Partner Firenze, Pasquale Bruni Milano).
> 🪗 **06/09/2026 notte — Mestieri: CSS e «Nuovo mestiere»** (segnalazioni utente «sistema css» ×2): in /mestieri le categorie erano testo nudo che sfondava le colonne → chip che vanno a capo (`.chip-cat`, cella al 55%), numeri e interruttore `nowrap`; nella scheda partner i campi minimo/raggio avevano la nota in linea → `.mestieri-campi` (griglia, etichetta sopra, nota sotto). Nuovo: `POST /mestieri` (`CreaMestiereDto`, chiave dal nome, ordine ultimo prima di «interni») e pannello «+ Nuovo mestiere» nella pagina.
> 🪗 **06/09/2026 notte — LINK DI CONFERMA per le consegne da fornitore** (in locale, API e web compilati): `DeliveriesService.linkConsegnata()`, token generato in `create()` se `deliveredByPartner`, `linkConsegnata` in findAll/findOne, `notDeliveredByToken()` + `POST /deliveries/not-delivered/:token`, `deliveredAt` nel confirm; app-api `CONSEGNA_SELECT` + `consegnaPartner/linkConferma` nella serializzazione; web `confirm-delivery` con due bottoni e pop-up (`scelta`, `nonConsegnata()`), detail: bottone link anche al partner; i18n `confirmDelivery.*`. Da avvisare il Customer Service: leggere `linkConferma` nella risposta di creazione consegna.
> 🪗 **06/09/2026 notte — MESTIERI + minimo/raggio partner** (in locale, API e web compilati; tabelle e semina FATTE in prod): `mestieri/mestieri.module.ts`, schema (Mestiere, PartnerMestiere, Category.mestiereId, PriorityList.mestiereId, Partner.minimoOrdineVendita/raggioMaxConsegnaKm), `applica-migrazione-mestieri.mjs` (9 mestieri, 65 categorie, 149 partner-mestiere, 45 partner senza), partners DTO/service (`mestiereIds`), sales `candidati()` per mestiere + interruttore `smistamentoAutomatico` + minimo sul prezzo partner stimato + raggio geocodificato (`SettingsModule` importato in SalesModule), web `mestieri.component` + rotta/nav + scheda partner (chip, minimo, raggio) + models + i18n. 🔖 Aperti: Originali Deluxy dai componenti (step 4), quota per mestiere in Orders (step 5), pagina di revisione dei 45 partner senza mestiere, deprecare le liste per categoria quando quelle per mestiere coprono tutto.
> 🪗 **06/09/2026 sera — CONSEGNA PARTNER AUTOMATICO** (in locale; colonna `Partner.autoDeliveredByPartner` CREATA in prod, 0 accesi): DTO partner, `create()` consegne (flag del partner → `deliveredByPartner`; da fornitore senza valet → `valetPartnerConsegna()`), `update()` idem; partner-form toggle + hint, models, i18n `partnerForm.setup.autoDeliveredByPartner*`. Verificato: GET /partners/:id → campo presente.
> 🪗 **06/09/2026 sera — VENDITE: Cerca fornitore, fuori catalogo, foto, ultima volta** (in locale, API e web compilati): `linkRicercaFornitore()` + `RICERCA_FORNITORI` in sales-list (riga + pan-azioni), i18n `sales.cercaFornitore/fuoriCatalogo`, `sales.history.consegnata`; cella Prodotto con `productName` di ripiego; `.foto-velo/.foto-box` z 95/96; API `storicoFornitori` select + `ultimoProdotto/ultimaVariante/ultimaConsegna`. #12892: riga Shopify senza SKU (Monet Giverny Medio) → da gestire; si assegna a mano (lista MI × Fiori d'Arte: Maryflor > Angolo Fiorito > Lami Fiori > Oasi dei Fiori) o con «Cerca fornitore».
> 🪗 **06/09/2026 sera — REGISTRO REGOLE + NUMERO + FIORI** (in locale, API verificata su 3010: `GET /delivery-rules/<37>/registro` → 200 []). Schema: `DeliveryRuleLog` (tabella CREATA in prod, 0 righe); `DeliveryRulesService.registra/foto/registro`, ETICHETTE dei campi, `fmt()`; controller passa `@CurrentUser`; create numera «Regola N · nome»; web: sezione Registro nel modale (`delivery-rule-form`), i18n `deliveryRules.registro.*`. Vendite: `categoriaFiori()` + `bloccoNonUnico` in `create()` (non unici fuori dai fiori → DA_GESTIRE con motivo). Utente: Brioni ha cancellato «consegna carnet» (resta la 37). Da pubblicare insieme a galleria DDT e chip «da gestire».
> 🪗 **06/09/2026 sera — «Da gestire» senza colore** (segnalazione utente: in tabella Consegne l'evidenza arancione si confondeva col rosso di «Non consegnata»): riga su fondo neutro (`--surface-sunken`) con barra nera e chip nero «DA GESTIRE» accanto allo stato (`.chip-gestire`, i18n `deliveries.nonConsegnate.chip`); avviso in elenco e dettaglio su fondo neutro con bordo nero. Niente tinte semantiche nuove: il rosso resta dello stato.
> 🪗 **06/09/2026 sera — GIURIA SUL DUMP LEGACY (localhost.sql, 62.592 consegne): Regola 27 importata FEDELE (28/28 regole, 34/34 partner); `payable=0` della regola = valet NON pagato quando il legacy la applicava (flag withDaily/withTotal=1: 2.612/2.612 con payable=0 e expertSalary=0, zero controesempi; inverso 0/175). MA il legacy la applicava in modo incoerente: 723 consegne con deliveryRuleId senza flag (legacy, non nostre), pagate a record nel 53%; Bellavia (r25) mai applicata (15/16 pagate); Vranjes/Brioni/Taveggia applicata solo dal 03/08/2026. Le 4 consegne Postgres con regola 27 le ha agganciate il NOSTRO script del 04/09 (nel dump: 0). ⇒ La scelta 02/09 (toPay=false → paga nulla) e il codice di oggi sono GIUSTI; la mia proposta di togliere il flag era sbagliata (avevo scambiato consegne non agganciate per «regola ignorata»). Correzione delle 62 consegne di settembre: coerente con la regola, decide l'utente. Aperti: Brioni ha due regole (37 toPay=false e «consegna carnet» 03/09 toPay=true); regole 12 e 34 incoerenti già nel legacy. Script della giuria nello scratchpad (a1–a5, analisi-regole-1…5, confronta.mjs).**
> 🪗 **06/09/2026 sera — DDT dalla galleria** (in locale): tolto `capture="environment"` dai 3 input immagine (delivery-detail ×2, delivery-form AI). 🔍 In corso: giuria di 3 agenti sul dump del legacy (`C:\Users\nicol\Downloads\localhost.sql`, estratto in NDJSON nello scratchpad con `legacy-estrai.mjs`) per la semantica di `partner_delivery_rules.payable` e la fedeltà dell'import (Regola 27). Primi fatti: la Regola 27 è importata campo per campo (payable=0→toPay=false, −15, km 4, 07–22, 50 totali, partner 262 DR Vranjes Corso Vercelli); nel legacy NESSUNA consegna era agganciata alla 27 (le 4 in Postgres sono del nostro script del 04/09); dove il legacy applicava una regola payable=0 (Regola 28, agosto: 62474, 63134, withTotalDeliveryRule=1) scriveva payable=0 e expertSalary=0. Il server legacy è VIVO: 74.208.248.54 (Ubuntu/Apache, SSH aperto) e manda «Nuova Vendita – NON-UNIQUE PRODUCT» da support@deluxy.it (ordine #12889 a Fabbrica delle Feste): istruzioni di spegnimento date all'utente.
> 🪗 **06/09/2026 15:xx — LIVE delivery-djxnlb1xo (prebuilt dal worktree ROOT, bundle main-QDKDZIXX.js su deluxy-delivery.vercel.app e app.deluxy.it)**: dentro tutto il lavoro di oggi (statistiche, puntualità, prova al partner, DDT a consegna chiusa, tipo vendita dall'ordine, cron non-consegnate, regola non-paga) + merge canale-app-0609. Verificato: /api/v1/cron/non-consegnate → 401 senza segreto (la rotta c'è), /statistiche → 401, woo-order → 401. 🔴 NON PUSHATO: il push da sessione autonoma è bloccato dal classificatore — fare `git push origin HEAD:piattaforma-ricerca-insensitive` dal worktree deploy-delivery (commit daa0b5e2).
> 🪗 **06/09/2026-duotricesimo — NON CONSEGNATE A OGGI OGNI NOTTE + AVVISO; REGOLA «DA PAGARE = NO» → VALET 0 (101061); merge canale-app-0609** (in locale, API e web compilati). API: `riportaNonConsegnateAOggi()` (30 gg, idempotente, log), `NonConsegnateCronController` (`/cron/non-consegnate`, CRON_SECRET), cron in `vercel.json` «5 23 * * *»; lista con `childDeliveries`; aggancio regola scrive `payable/billable` dalla regola; findOne `valetSalaryDalListino=0` con toPay=false; Finance passa `deliveryRule` a `pagaConsegna` e azzera con toPay=false (include in entrambe le query). Web: banner `.warn-card.gestire` + righe `.da-gestire` in Consegne (`eDaGestire/daGestire`), avviso nel dettaglio con «Crea la riconsegna», dd paga «0 € (regola …)»; i18n `deliveries.nonConsegnate.*`, `deliveryDetail.nonConsegnata.*`, `deliveryDetail.valetPayRule`. Misure: 1.694 non consegnate senza riconsegna (684 del 2021) → finestra 30 gg; 2.205 consegne con regola non-paga ma payable=true (legacy le pagava: paga scritta su 88/94 a luglio) → NON corrette, decide l'utente. Merge di `canale-app-0609` (00b693bf, 18eeba17, a3ee7c62) fatto PRIMA del deploy come chiesto dal Customer Service. ⚠️ node_modules del worktree web erano svuotati (build-angular vuoto): rifatto `npm ci` alla radice. Mail a Fabbrica delle Feste: nei registri di tutte le app (mail.Messaggio, hub.InvioPosta, mail.InvioApp, crm.MailInviata, messaging.Messaggio, orders.MessaggioAutomazione, transactions.NotificaInvio) in 60 giorni risultano SOLO 3 «Nuovo servizio Deluxy» (01/09, 05/09 ×2): la valanga non parte dai nostri sistemi — serve l'oggetto/mittente di una di quelle mail.
> 🪗 **06/09/2026-untricesimo — DDT ANCHE A CONSEGNA CHIUSA** (regola utente, link consegna 101059 `cmtpha5gv0001i904schmgzbe`). API: `AllegaDdtDto`, `POST /deliveries/:id/ddt`, `DeliveriesService.allegaDdt` (valet = quello della consegna; Drive; log). Web: `puoAllegareDdt()`, `allegaDdt()`, `ridimensionaFoto()` (estratta da `onDdt`), blocco `.ddt-extra` in Allegati, i18n `deliveryDetail.prova.ddt*`. Verificato su API reale: 400 su corpo non immagine, 404 per un valet estraneo; il caso buono (valet della consegna) lo prova l'utente sul telefono. Poi: commit + push + deploy prebuilt (ordine dell'utente «fai commit e deploy quando hai finito tutto»).
> 🪗 **06/09/2026-tricesimo — STATISTICHE, classifiche tagliate dalla tessera (screenshot utente «sistema css»)**: griglia `.griglia-3` a `minmax(min(100%, 480px), 1fr)` (2×2 su desktop), tabelle `.classifica` con numeri e Δ `nowrap`, colonne numeriche a contenuto, nome min 120px, tabella dentro `.scorri` (overflow-x auto). Registrato in SEGNALAZIONI-UX (9). Non ho potuto misurarlo a schermo: il tab 4210 è sul login (l'API reale è stata riavviata) — da guardare a localhost:4210/statistiche, in fondo.
> 🪗 **06/09/2026-undetricesimo — PUNTUALITÀ su ogni consegna, PROVA DI CONSEGNA al partner, filtro VALET, TIPO DI VENDITA dall'ordine** (in locale, verificato su API reale 3010 + web 4210). API: `common/puntualita.ts` (`puntualitaConsegna`, tolleranze 10′/30′, minuti dalla fine/inizio fascia — allineato all'SQL di Statistiche), `puntualita` in findAll/findOne consegne; Statistiche `valetIds`; `dettaglioOrdine` → `tipoVendita/pezzi/totale`, gateway letto da `shopify.gateway`. Web: badge `.punt` in elenco e dettaglio; dettaglio: `<dl class="prova">` (consegnata alle, ritirata da, valet, documenti via `documentiProva()`) dentro Allegati; Statistiche filtro valet; form consegna: `tipoVenditaOrdine` signal + `servizioToccato`, `venditaPreferita()` per tipo (regex corretta), contrassegno con importo = totale ordine. Verifica: `GET /sales/cmtnsorwm0001k4048crpybun/ordine` (12879) → `{contrassegno:false, tipoVendita:'singola', pezzi:1, totale:50}`. ⚠️ Il web build era rotto per `DeliveryDetail` senza i campi nuovi: aggiunti. Scratchpad: `avvia-api-reale.mjs` ora conserva il segreto JWT in `jwt-locale.txt` (un riavvio non butta fuori chi è dentro). 🔖 Aperto: la regola del tipo vendita vale nel FORM; la via automatica `creaConsegna` resta senza servizio (le vendite da Orders nascono con `serviceTypeId` null) — se si vuole la consegna automatica, la stessa regola va lì. Utente 06/09: «la sku rimarrà sempre quella legata a prodotto e variante, non ci saranno più prodotti con provincia associata, lo sconto per la provincia sarà calcolato automaticamente grazie a orders» — misurato: 16.289 prodotti, 2.910 con `legacyProvince`, 489 con SKU «MI» e 233 «RM» (lo SKU È la provincia), 234 SKU ripetuti, 66 copie di «Monet - Giardino a Giverny - Medio»; 6.955 CategoryDiscount locali, 28 liste; Orders `QuotaRegola` vuota → oggi comanda ancora la regola locale.
> 🪗 **06/09/2026-duodetricesimo — STATISTICHE: macrotipologia, tempi nel confronto, tendine; form: Vendita Deluxy prima** (in locale). API: `Filtri.pricingModel` (sottoquery su ServiceType), GROUP BY `st."pricingModel"` (il campo `serviceTypeId` di perTipologia ora porta il modello), ritardi con `startedAt/indirizzo/ritiro/km/ddt/partnerId/valetId/modello`. Web: select macrotipologia (`MODELLI`), `espansa` signal + righe `.tendina` (tipologia → `vociTipologia()` corrente/confronto/Δ; ritardo → dl + bottoni; classifiche/stati → numeri + bottoni), barra sottile del confronto, `prima:` nei tempi, Δ nelle celle e nel Totale; `vaiConsegne({pricingModel})`. Form consegna: `venditaPreferita()` = «Vendita Deluxy» fra le VENDITA (default da vendita e ordine dell'elenco).
>
> 🔎 **06/09/2026-septvicesimo — STATISTICHE: filtri, ritardi apribili, tabelle ordinabili e cliccabili** (in locale). API: `Filtri {serviceTypeId, provinceId, partnerIds[]}` in un solo frammento SQL (`filtroSql`) usato da tutte le query; economia filtrata via id consegna; `GET /statistiche/ritardi` (stessa regola della puntualità, LIMIT 500 + totale). Web: select servizio/provincia, elenco partner con ricerca e checkbox (max 200 mostrati) + chip; `ordinaLista/righeOrdinate` generico per ritardi, classifiche e stati; righe → `vaiConsegne` (`/deliveries?view=tutte&date&dateTo&serviceTypeId|status|partnerId|q`), `/partners/:id`, `/valets/:id`. Misura prod: mese con filtri MI+Cannavò 1,9 s; ritardi mese 137 ms (94 righe). ⚠️ Provincia → elenco consegne via `q=<nome>`: l'elenco non filtra per provincia (da aggiungere `provinceId` a findAll + lista). ⚠️ Trappola scoperta: `String.replace(a, b)` con `$'` nel testo inserito duplica il seguito — negli script di patch usare `replace(a, () => b)`. Mock: `/auth/login` finto (ruolo dall'email). API vera locale: `avvia-api-reale.mjs` (3010, DB prod).
>
> 🔗 **06/09/2026-sexvicesimo — QUOTA DA ORDERS nello smistamento** (in locale; regola utente). `SalesService.quotaDaOrders(provinceId, categoryId)`: chiede `GET {ordersUrl}/api/v1/quota-fornitore?provincia=CODE&categoria=<nome categoria minuscolo>` con `ordersApiKey`; usa la risposta solo se `regola ≠ default` e 0<quota<100; sconto = 100 − quota (2 decimali); cache 5′ per (prov, cat). Chiamata SOLO quando non c'è prezzo specifico (riconciliazione/UNICO) né % sull'ordine. `assignmentReason` aggiunge «sconto da Orders (regola: fornitore N%)». MISURA: Orders `QuotaRegola` in prod = 0 righe, `controllo.quotaFornitore` non impostata → oggi Orders risponde sempre «default» → resta CategoryDiscount (6.955 righe). Per far comandare Orders davvero: migrare le regole in Orders (script nella cartella di Orders, categoria = nome minuscolo — Orders confronta in minuscolo) e/o impostare la quota globale. ⚠️ Lista di priorità: 28 liste/48 voci dal legacy, NESSUNA pagina/rotta per gestirle nella nuova app; le auto-create finiscono nella stessa tabella. API vera avviata in locale su 3010 (`scratchpad/avvia-api-reale.mjs`: DB di prod, JWT locale casuale, CORS 4210) su richiesta utente «fallo parlare col database reale».
>
> 📊 **06/09/2026-quinvicesimo — STATISTICHE + codice valet + smistamento automatico + sconto da Orders** (in locale, NON deployato). (1) `api/src/statistiche/statistiche.module.ts`: `intervalli()` (Roma, pari giorni, settimana −364 per anno prima), 5 query SQL con bucket corrente/confronto (per tipologia, per stato, top partner/valet/province), puntualità in SQL (fascia `date+deliveryTimeTo AT TIME ZONE 'Europe/Rome'`, deliveredAt UTC, +10′/−30′), economia via `FinanceService.corrispettivi` sotto 2.000 righe (`FinanceModule` ora esporta il service); `web/src/app/pages/statistiche.component.ts` (segmented periodo/confronto nell'URL, tessere con Δ e base, barra puntualità, tabella tipologie ordinabile, classifiche, stati); rotta `/statistiche` ADMIN/OPERATION, voce menu prima di Finanza, i18n `statistiche.*`. Misura prod: mese 4,4 s (106 righe Finanza), anno 2,0 s; payload 23–40 KB. Verificata a schermo sul web 4210 con i payload VERI salvati su file (mock legge `stat-<periodo>-<confronto>.json`). ⚠️ Ritardo medio agosto 1.145 min: orari legacy scritti a posteriori — la giuria chiede mediana/p90 (da fare). Verdetti della giuria: KPI (controller), UX (§6 «tessera KPI con confronto», soglia copertura 50 %), performance (tabella persistita `DeliveryEconomia` proposta per togliere il tetto 2.000; nessun indice nuovo; niente cache in v1). (2) Codice valet: `auth.profilo` manda `legacyId`, profilo lo mostra con avviso; elenco Valet colonna Codice (`Valet.legacyId` nel modello web). (3) `SalesService.candidati`: abilitati = attivi con categoria E provincia; 1 → proposta; >1 → `priorityList.create` ordinata per `sale.groupBy(partnerId, status accettata, provinceId)`; log. (4) `ingest/create`: `discountPercent` da Orders vince (0–100, 2 decimali); orders-sync passa `o.scontoPartnerPercent ?? o.smistamento?.scontoPartnerPercent` — 🔴 Orders NON lo manda ancora: va aggiunto lì (`scoutwt/deluxy-orders`). Partner: apertura su «In lavorazione». Ricorrenti: Elimina annulla le future.
>
> 🏁 **06/09/2026-quattuorvicesimo — PARTNER: apertura su «In lavorazione» senza data** (in locale; regola utente). `deliveries-list` init: ruolo PARTNER → `vista='attive'`, `dateFilter=''` (già vuoto per il partner dal 02/09); arrivando dalla scheda partner (`qPartner`, ufficio) resta «tutte».
>
> ⏱️ **06/09/2026-tervicesimo — ORARI REALI: `startedAt`/`deliveredAt` scritti al cambio di stato** (in locale, NON deployato). Censimento per le Statistiche: concluse 2026 con deliveredAt — gen 978/1289 … ago 678/826, **set 0/230**: la nuova app non li scriveva mai (li scriveva il legacy fino al 31/08), la pagina tracking li legge. `updateStatus`: in_delivery → startedAt; delivered/delivered_time_to_approve → deliveredAt (+ startedAt se manca), solo se vuoti. Backfill ESEGUITO in prod dai log «Stato: … -> in_delivery/delivered» (`scripts/completa-orari-reali-dai-log.mjs`): 286 deliveredAt e 257 startedAt ricostruiti; 2078 esaminate senza log utile (import legacy: hanno già i loro orari o non li hanno mai avuti). Giuria Statistiche: verdetti KPI e UX ricevuti, performance in arrivo — implementazione a seguire.
>
> 🔁 **06/09/2026-duovicesimo — RICORRENTI: Elimina annulla le FUTURE** (in locale, NON deployato; regola utente). `RecurringService.remove()`: prima del delete, le consegne del ricorrente con `date >= domani (Roma)`, non cancellate e in created/assigned/in_preparation/accepted → `deletedAt` + `cancelled` + log `cancelled` con userId; risposta `{ ok, annullateFuture }`. Le altre restano (FK SetNull). ⚠️ Non passa da `stock.rientra` (le ricorrenti non portano prodotti a stock; se servisse, iniettare StockService).
>
> 📍 **06/09/2026-unvicesimo — INDIRIZZI: Google solo se si tocca il campo** (in locale, NON deployato; regola utente). `delivery-form`: `indirizzoToccato/indirizzoDaGoogle` per consegna e ritiro; `(input)` sugli input segna «digitato», `place_changed`/`onPlaceSelected` segnano «da Google»; `normalizzaIndirizzo(campo)` esce subito se non toccato o scelto da Google. Prima ogni blur (anche col tab su una consegna in modifica) mandava l'indirizzo salvato al Geocoder e lo riscriveva. Stessa regola in `indirizzo-google.directive.ts` (`toccato`/`sceltoDaGoogle`). Il server già geocodifica in `update()` solo se `recipientAddress` cambia. Compila; non provato a schermo (pannello nascosto).
>
> 🚀 **06/09/2026-vicesimo — DEPLOY `delivery-7u7h5y2w1` (comando utente «Fai push & deploy di tutto e crea anche questa tabella»)**: bundle `main-2MBI6FNY.js` sul dominio; `POST /api/deliveries/sync/woo-order` risponde 401 (era 405) sia su deluxy-delivery.vercel.app sia su app.deluxy.it. Tabella `platform."StockMovement"` CREATA in prod (script `applica-migrazione-stock.mjs`, 0 righe). In produzione da questo deploy: Finanza con la formula dell'utente + cache Orders agganciata, notifica al partner sulla proposta, numero d'ordine sulle consegne di vendita (108 completate), rotta WooCommerce legacy, fee visibile nel form, stock in piattaforma, assegnazioni idempotenti. Push su `piattaforma-ricerca-insensitive` subito dopo.
>
> 🔁 **06/09/2026-undevicesimo — ASSEGNAZIONI DOPPIE (#101058: 19 «Assegnata al valet» in 5″, a raffiche di 5 in 50 ms; anche #101050, #101024, #100196)**. Il server scriveva un log per OGNI chiamata; il bottone «Scegli» della lista aveva `[disabled]=salvandoAssegna()` ma nessuno accendeva il segnale → ogni tocco/Invio ripetuto = una chiamata. Ora: `assignValet` è idempotente (stesso valet + stato assigned → risponde senza scrivere), lista e dettaglio chiudono la porta alla prima chiamata. Le 18 righe doppie restano nello storico (non cancello dati senza ordine). Notifiche `delivery_assigned` per quella consegna: 0 trovate.
>
> 📦 **06/09/2026-duodevicesimo — STOCK IN PIATTAFORMA (regola utente «per i prodotti con flag magazzino lo stock deve essere gestito su app delivery»)** — in locale, NON deployato, **NON deployabile finché non esiste la tabella**. `api/src/stock/stock.module.ts`: `StockService.verifica/scala/rientra/rettifica/movimenti` + `GET /stock/movimenti`; modello `StockMovement` in schema.prisma; hook in `DeliveriesService`: `create()` verifica prima e scala dopo (`dto.ignoraStock`, solo ufficio), `update()` con righe → rientra + verifica + scala, `updateStatus()` su cancelled/invalidated/not_accepted/not_delivered → rientra, `annulla()` e `remove()` → rientra; `ProductsService.update` scrive «rettifica» se la giacenza cambia; `DeliveriesModule`/`ProductsModule`/`AppModule` importano `StockModule`. Web: colonna Giacenza in Prodotti (rossa a ≤0), «Giacenza: N / insufficiente» sulla riga prodotto del form, `ProductRef.controlStock/stock/variants`. 🔴 **MIGRAZIONE DA FARE A MANO PRIMA DEL DEPLOY**: il classificatore ha bloccato la scrittura dello script (schema condiviso, regola del Libro Performance): la tabella va creata con `CREATE TABLE IF NOT EXISTS platform."StockMovement" (id TEXT PRIMARY KEY, "productId" TEXT NOT NULL, "productVariantId" TEXT, "deliveryId" TEXT, quantity INTEGER NOT NULL, reason TEXT NOT NULL, note TEXT, "userId" TEXT, "createdAt" TIMESTAMP(3) NOT NULL DEFAULT CURRENT_TIMESTAMP)` + indici su ("productId","createdAt") e ("deliveryId"). Giacenza NULL = controllo non attivo (i 99 prodotti col flag e senza numero NON si bloccano e NON toccano la tabella): senza tabella si rompe solo la prima consegna di un prodotto con giacenza SCRITTA — quindi prima la tabella, poi si caricano le giacenze. Misura: 99 prodotti col flag, 0 con giacenza, 0 varianti. Compila (tsc + ng), non provato a schermo (pannello nascosto). Aperto: Merchandising/Shopify non legge il saldo; smistamento vendite non guarda ancora lo stock.
>
> 🛒 **06/09/2026-septendecies — ROTTA COMPATIBILE COL PLUGIN WOOCOMMERCE (regola utente «Procedi»)** (in locale, NON deployato). `WooLegacyService/WooLegacyController` in `woocommerce.module.ts`: `POST /api/deliveries/sync/woo-order` (fuori dal prefisso: `exclude` in main.ts e vercel.ts + rotta in vercel.json), header `x-deluxy-partner-key` (accetta anche `x-api-key`) → `Partner.woocommerceApiKey`; idempotente su (partner, ddtNumber, nota «[WooCommerce #»); servizio = listino partner PREZZO_FISSO («consegn…» se c'è) o primo a catalogo; prodotti per SKU (varianti poi prodotti, del partner o senza partner), i non trovati in nota; `DeliveriesService.create(dto, utente PARTNER sintetico)` così valgono le stesse regole del form; risposta `{status:true, deliveryId: code, statusCode}` con HTTP 200 come si aspetta il plugin. Provato in locale (API su 3020 contro il DB di prod, SOLO letture): rotta mappata, 401 senza chiave e con chiave sbagliata, 404 sotto /api/v1. ⚠️ Non provato un ordine vero: servirebbe la chiave di un partner e creerebbe una consegna in prod. Chiavi in prod: Clivati-Consegne, Martesana Ecommerce, Fioraio Milano Centro («demo…»). Da fare dall'utente dopo il deploy: nel WordPress di Clivati controllare che il plugin abbia Debug mode = No (di fabbrica manda a dev.deluxy.it) e la chiave della scheda «Clivati-Consegne». Finanza: prezzo pubblico 0 = non indicato (`prezzoRiga`). Notifiche: NIENTE mail — campanella (tabella Notification) + web push; una mail passerebbe da AI Mail (`MAIL_API_KEY`), non c'è. **Form consegna**: `feeVendita()` (fee% del listino partner × base fee delle righe senza «Senza fee») mostrato come placeholder e `slot-hint` sotto Prezzo sulla VENDITA (regola utente «i numeri sempre visibili»; il campo resta vuoto per la regola 01/09). Compila; non provato a schermo (pannello nascosto).
>
> 🔔 **06/09/2026-sedecies — PROPOSTA CON NOTIFICA, NUMERO D'ORDINE SULLE CONSEGNE DI VENDITA, cache di Orders agganciata** (in locale, NON deployato). (1) `SalesService.avvisaProposta` → `notifications.notifyUsers` (tipo nuovo `NotificationType.SALE_PROPOSED`) agli utenti attivi del partner, chiamata in `create()` (quando nasce PROPOSTA) e in `proponiAPartner`; `SalesModule` importa `NotificationsModule`; un avviso fallito non tocca la vendita. Prima NESSUNA notifica: il partner scopriva la proposta solo entrando in Vendite. (2) `creaConsegna` scrive `realOrderNumber = numeroShopify(ordine.orderId)`. (3) Finanza `clientePagato`: per le righe senza numero risale Sale.deliveryId → externalOrderId → OrdineCliente.ordersId e scrive la chiave sulla riga. (4) Backfill ESEGUITO in prod: 108 consegne completate, 1 senza cache (ordine non ancora nella corsa notturna). Misura 20/08–06/09 per ordine DOPO: venduto 48.216 € (prima 34.892 solo righe), pagato 29.118, netto 15.480, fee 3.089, valet 2.127, commissione 1.294 (da Orders), margine 15.149 €. ⚠️ Correzione a quanto detto all'utente: la cache è per id Shopify (`OrdineCliente.orderId`), NON per numero: la «collisione #2874» che avevo segnalato era un artefatto della MIA query per numero; il vero difetto era il numero mancante sulle consegne. ⚠️ #100099 nel recap vale 90 € (ordine 12835) e non 145: la seconda vendita 12860 non può collegarsi (Sale.deliveryId unique) — punto aperto già noto. Smistamento: #12890 → Maryflor (1ª della lista priorità MI, 5 partner), #2874 → Montenero In Fiore (nessuna lista per FI: scelto fra chi tratta «Fiori d'Arte»; Arte E Fiori Firenze ha Fiori Classici/Rose/Cappelliere/Fiori (2) ma NON «Fiori d'Arte» → esclusa: decisione dati dell'utente).
>
> 💶 **06/09/2026-quindecies — FINANZA: margine per riga con la formula dell'utente** (in locale, NON deployato). `computeRow(d, ctx)` con `contestoMargine()` (listini valet, fee % partner-servizio per le vendite senza quota, tariffe): venduto = prezzo pubblico di listino (variante > prodotto) o riga a prezzo partner segnata `vendutoStimato`; pagato = `productValue` o somma righe a prezzo partner (`prezzoPartnerRiga`); netto = differenza ÷ 1,22; fee = `price+additional+regola`, o fee % × `baseFee` sulle VENDITE senza prezzo; valet = scritto o `scegliListinoValet`+`pagaConsegna` (import da salaries); commissione di riga dalla tariffa del gateway, nel recap quella di Orders. Query: `productVariant.price`, `valet.minimumKmIncluded/extraOutOfCityPrice`. Provato su prod 20/08–06/09 (script `q-finance.mjs`): #100099 venduto 145 / pagato 132 / netto 10,66 / fee 22,44 / valet 26,60 → margine 6,50; Tiramisù #101007 30/28 → −0,20. ⚠️ 189 righe su 283 hanno venduto = prezzo partner perché il prodotto non ha `publicPrice` a catalogo: lì il netto è 0 e il margine è solo fee − valet (per ordine il recap usa la cache di Orders dove c'è). **Backfill di `productValue` NON fatto**: con la somma delle righe come ripiego in Finanza e Fatturazione non serve scrivere 131 righe in prod. Correzioni dette all'utente: `INCASSI` vale 0 (non 3 %); productValue mancante dava incasso 0 + anomalia sulla riga, ma nel recap per ordine pagato 0 = tutto margine — ora il ripiego sulle righe chiude anche quello.
>
> 🚀 **06/09/2026-quattuordecies — DEPLOY `dpl_6DyTxQrKbpkSUnevyLpDSEUQdcR9` (comando utente «una volta finito fai push & deploy»)**: build precompilata dalla radice del worktree (`vercel pull` + `vercel build --prod` + `vercel deploy --prebuilt --prod`), bundle `main-AOPPZCHE.js` servito da deluxy-delivery.vercel.app. In produzione da oggi: AI solo ufficio (dcd19640), ore del valet in colonna + «Inserisci codice valet» in riga (2dff7a0c), auto-aggiornamento del dettaglio (d8c06566). Push su `piattaforma-ricerca-insensitive` subito dopo (vedi log git).
>
> 📋 **06/09/2026-tredecies — LISTA CONSEGNE: ore del valet in colonna e codice dal partner in riga** (in locale, non deployato; regola utente «senza messaggi: mostra solo i nuovi orari in tabella, coi bottoni per accettare o rifiutare»). `DELIVERY_LIST_SELECT` porta `hoursFrom/hoursTo/hoursOriginal`; in riga, con `status=delivered_time_to_approve`, la colonna «Consegna» mostra `hoursFrom–hoursTo` in `<strong class="ore-valet">` al posto della fascia prevista; Approva/Rifiuta erano già in riga. Il PARTNER (e l'ufficio) ha in riga «Inserisci codice valet» (`codiceDaInserire(d)`: verifica richiesta, non fatta, valet assegnato, stato assigned/accepted/in_preparation) → pop-up `codiceFor` con lo stesso endpoint del dettaglio; `deliveries.codice.inserisci` in it/en. Provato sul web locale 4210 + API finta: pop-up, codice 4711 accettato, bottone sparito al ricarico. **Auto-aggiornamento anche nel DETTAGLIO** (`avviaAutoAggiornamento` nel costruttore, `load(true)` silenzioso che non riapre il pop-up da `?chiudi`, sospeso con pop-up/azioni/codice digitato); in lista il pop-up del codice sospende il giro. Provato: stato cambiato lato server, dopo 33″ il dettaglio del valet mostrava da solo il bottone spento e l'avviso. ⚠️ Il giro salta con la scheda NASCOSTA (`visibilityState`): nel pannello browser nascosto sembra non funzionare, e invece è la regola «educata». ⚠️ Le schermate dei flussi sono ferme perché il pannello del browser è nascosto: mancano «esito approvazione» (visivo), il servizio a ora standard (#100203) e il form dell'ufficio con la casella «Verifica identità del valet».
>
> 🤫 **05/09/2026-duodecies — «Compila con l'AI» per ora SOLO per l'ufficio** (in locale, `dcd19640`). Regola utente: ai PARTNER la sezione «✨ Compila con l'AI» del form nuova consegna non si mostra (`aiPossibile() && !isPartner()`) e `POST /ai/consegna-da-testo` accetta solo ADMIN/OPERATION. Per riaccenderla: togliere `!isPartner()` nel form e rimettere `Role.PARTNER` sul controller in `ai.module.ts`. Le schermate dei tre flussi chieste dall'utente (codice valet al ritiro, ore con approvazione, ore standard) si fanno SENZA toccare la produzione: web del worktree su 4210 (`ng serve --configuration development`) + API finta su 3010 (`scratchpad/mock-api.mjs`: tre consegne in memoria, ruolo letto dal token `mock.<RUOLO>` messo in localStorage) — nessuna password.
>
> 🪪 **05/09/2026-undecies — CODICE DEL VALET AL RITIRO, regola definitiva** (in locale). Decide il flag DELLA CONSEGNA `Delivery.valetIdentityCheck` (nel DTO, casella in alto a destra del form: era `deliveryCodeRequired`, il codice del CLIENTE, che resta fuori da questa logica). Il flag del PARTNER (`Partner.valetIdentityCheck`, oggi solo Zegna) è la politica di default: in creazione il form accende la casella scegliendo un partner che lo ha (modificabile, finché l'utente non la tocca); se inserisce il PARTNER stesso, il server la forza accesa (la sua politica non si aggira). Verifica: `POST /deliveries/:id/ritiro/verifica {codice}` — il codice è `Valet.legacyId` (tutti e 52 gli attivi lo hanno) — scrive `pickupVerifiedAt/By`; senza verifica il VALET non può mettere «in consegna» (403 + bottone spento con motivo in dettaglio e lista); l'ufficio può forzare, registro «ritiro NON verificato». Migrazione additiva eseguita. 142 Restaurant: i 6 servizi (Stock Pallet compreso, 1 €/pezzo) sono identici al legacy campo per campo — nati tutti il 15/07/2026 09:53:43 nello stesso secondo (listino applicato in blocco nella vecchia app).
>
> ⏱️ **05/09/2026-decies — ORE CON APPROVAZIONE: solo «Servizio Ora con Approvazione»** (regola utente). Colonna `ServiceType.hoursApproval` (migrazione additiva ESEGUITA in prod, flag true solo su `SERVIZIO_ORA_CON_APPROVAZIONE`, 2.715 consegne, 7 partner). `updateStatus`, dettaglio e lista chiedono/esigono le ore SOLO su quel servizio; gli altri 9 A_ORA (Chanel Milano/Firenze/Roma a ora, Servizio a Ora, Ritiro Catering…) si chiudono come le altre consegne. **Le 6 ferme in «ore da approvare» erano tutte su servizi senza approvazione: chiuse → `delivered`** con le ore dichiarate e riga di registro (`scripts/ripristina-ore-senza-approvazione.mjs`). 🔴 DA FARE: «Codice richiesto valet» — regola utente 05/09: se abilitato, al RITIRO il PARTNER vede un pop-up e inserisce il codice del valet; se combacia con l'id del valet che ritira, il valet può mettere «in consegna» e poi consegnare. Oggi i flag esistono senza logica.
>
> ⏱️ **05/09/2026-nonies — SERVIZI A ORE «non si chiudono»: diagnosi e correzione (in locale, non deployato).** Le chiusure dei VALET funzionano (5 oggi, stato `delivered_time_to_approve`, 110 notifiche `delivery_hours_to_approve` mandate a partner + ufficio). Il blocco era doppio: (1) l'UFFICIO dal pop-up di stato in lista mandava solo `{status}` e il server — giustamente, regola del 04/09 — rifiutava con «servono l'ora di inizio e di fine»; ora il pop-up mostra i due orari (precompilati coi previsti) sui servizi A_ORA e li manda; (2) **6 consegne ferme in «ore da approvare» con ZERO decisioni in tutto il giorno**: Approva/Rifiuta esisteva solo nel dettaglio; ora anche sulla riga di lista (ufficio e partner della consegna). ⚠️ «Codice richiesto valet» (`deliveryCodeRequired`, `deliveryCodeCheckType`, `valetIdentityCheck`): SOLO campi salvati e toggle nel form/dettaglio — nessuna logica li legge (nessun codice generato, nessun controllo alla chiusura): il flag oggi non fa niente.
>
> 🚀 **05/09/2026-octies — PUSH FATTO (`757e4f80..30df633f` su `piattaforma-ricerca-insensitive`, stavolta il classificatore ha lasciato passare) e DEPLOY `delivery-apx6xbdag`, bundle `main-3HLFCZHG.js` verificato sul dominio.** In produzione: partner che si salva in modifica (+ riga di registro), paga da listino visibile nel form, tendina partner SOLO attivi (eccezione: modifica, etichetta «non attivo»), commento Finanza corretto. Dati: 24 vendite UNICO allineate al listino (23 + 12829 estendendo a tutto il 2026), 14 «da decidere» con un prezzo solo a catalogo. Un solo commit locale non pushato: questo dell'handoff.
>
> 🧾 **05/09/2026-septies — PARTNER CHE NON CAMBIAVA in modifica (corretto), paga da listino nel form, 23 vendite UNICO allineate al listino, punti aperti aggiornati** (in locale, non deployato; push FATTO dall'utente via PowerShell fino a `757e4f80`).
> - **Partner in modifica**: `update()` toglieva `partnerId` dal dto (serve al listino) e NON lo scriveva mai in `data` → si sceglieva un altro partner, il ritiro si spostava al suo indirizzo, la consegna restava del vecchio (#101015/#101016). Ora si scrive (solo ufficio, mai il PARTNER) con riga di registro «Partner cambiato: A → B». ⚠️ «Si vedono i partner non attivi» NON confermato: 130 partner vivi, **1 solo inattivo**, escluso da `filteredPartners`; i 165 cancellati non escono dalla lista. Probabile: provincia dell'indirizzo non ancora dedotta → tendina con TUTTI i 129 attivi. Da verificare col nome del partner visto.
> - **Paga valet nel form**: il campo mostra solo la paga SCRITTA; se vuota gli stipendi la ricavano dal listino (`valetSalaryDalListino`, finora solo nel dettaglio). Ora sotto il campo: «Vuoto: gli stipendi useranno la paga da listino, X €». Cambiando servizio/ore l'update AZZERA `valetSalary` apposta (regola 31/08).
> - **23 vendite UNICO allineate** (`scripts/allinea-prezzo-partner-unici.mjs --applica`, eseguito): cliente = riga Orders (×qta), partner = listino variante, quota derivata; SOLO dove il catalogo ha `price` ≠ `publicPrice`. **9 «da decidere»** (catalogo con un prezzo solo: 12716, 12719, 12745, 12751, 12761, 12765 Charlotte ×6, 12818, 2757, 2807): non toccate. 2 già giuste.
> - **12736/7/8**: import senza «creata», Rose Rosse NON_UNICO 180 € l'una, quota 0% → al partner 180; regola Rose×MI = 20% → 144 al partner, 36 a Deluxy ciascuna. NON corrette (l'utente ha chiesto il dettaglio).
> - **productValue**: 131 consegne di vendita da agosto con righe e campo NULL, 12.441,39 € letti come guadagno lordo. ⚠️ Sulle consegne «fornitore diretto» (Artista Locale) la RIGA è il costo del fornitore e la vendita l'importo cliente: il «vero» per riga non si ricava dalla vendita in automatico — serve la decisione del punto 1 dell'architettura.
> - **69 consegne di VENDITA senza vendita collegata** dal 25/08 su 224 (35 legacy 63xxx create a mano con DDT = numero ordine; fornitore diretto/Papera Flowers assegnate; 5 annullate/non accettate; righe «€0 consegna»). Elenco stampato dallo script (sezione 4 della versione precedente, ora nel report all'utente).
> - ✅ CHIUSO: le 9 vendite cakedesign.me aperte senza consegna si gestiscono a mano (decisione utente 05/09).
>
> 🚀 **05/09/2026-sexies — DEPLOY `delivery-r7lpwqgrq`, bundle `main-NB4YWCNS.js` verificato sul dominio.** In produzione da ora: «Senza fee» per riga, avviso contanti al valet, provincia dedotta dall'indirizzo nella sincronizzazione. 🔴 **Push su GitHub ANCORA BLOCCATO** dal classificatore dei permessi (quarto tentativo): **10 commit locali** `92a667cb`…HEAD sul worktree `platform-0409`, da pushare a mano con `git push origin platform-0409:piattaforma-ricerca-insensitive`. Finché non è pushato, il lavoro di oggi vive solo in questo worktree.
>
> 📍 **05/09/2026-quinquies — ORDINI SENZA PROVINCIA: la vendita entra lo stesso.** Caso #12881 («four season milano», città vuota, CAP e provincia nulli, conforme, consegna oggi 08–09): `orders-sync` lo scartava come `senza-provincia` in silenzio (~25% degli ordini, dice il commento). **Fatto entrare a mano** in prod (`ingest` via dist, provincia MI, da gestire senza prodotto a catalogo, riga nel registro). **Correzione**: `deduciProvincia()` in `orders-sync.module.ts` — se la sigla manca o è sconosciuta si deduce (1) dai nomi di città (`City`, 46) e provincia in banca dati cercati come parola intera nel testo indirizzo+città+CAP, dal nome più lungo, nomi ≥ 4 lettere (provato: «via romagnosi» NON è Roma); (2) altrimenti da `settings.geocode()` (Google). L'esito dice «provincia dedotta dall'indirizzo: MI («milano»)» o «da Google». Il residuo resta `senza-provincia` ma VISIBILE: warn nel log con l'indirizzo e lista `senzaProvincia` nel rendiconto. ⚠️ La riga `sicuro()` (escape della regex) era stata storpiata dalla shell e ricommittata: riscritta e provata su 5 casi (`6b95215d`). ⚠️ Caso da aggiungere al diagramma dell'architettura vendite: «indirizzo che non dice dove va».
>
> 🏷️ **05/09/2026-quater — «SENZA FEE» sulle righe di vendita e AVVISO CONTANTI al valet** (in locale, non deployato).
> - **Senza fee** (regola utente): flag per riga prodotto nel form consegna, SOLO sui servizi di VENDITA (`isVendita()`), sul campo `DeliveryProduct.withoutCommission` che esisteva nello schema e nessuno usava. Fotografato in `fotografaProdotti`. `baseFee()` in `common/valore-prodotti.ts` = venduto MENO le righe senza fee (se tutte senza fee → 0, NON il ripiego su `productValue`); la usano `invoices.module.ts` (fee% × base, quindi in fattura quota 0 su quelle righe) e il conto della vendita nel dettaglio (`quota`). Il VENDUTO resta intero (`valoreProdotti`): al partner è dovuto tutto. ⚠️ Con `Delivery.price` congelato > 0 vince il congelato, come prima. Il dettaglio marca la riga con il badge «senza fee».
> - **Contrassegno** (regola utente): flag+importo ESISTEVANO già (`paymentOnDelivery`/`paymentAmount`, form e dettaglio), gli Stipendi già scalano i contanti (`contanti`, `cashDeductions`), la Finanza usa `valetSalary` LORDO per il costo consegna → i contanti NON abbassano il margine: era già così, verificato (`finance.module.ts:1028`). Mancava l'avviso: ora «Metti in consegna» (dettaglio `avviaConsegna()` e riga di lista `valetInConsegna()`) apre PRIMA un `app-conferma` con l'importo da ritirare in contanti e la nota che sarà scalato dal bonifico; senza contrassegno si parte subito.
>
> 🧭 **05/09/2026-ter — ARCHITETTURA VENDITE: studio della giuria (3 agenti) CONSEGNATO.** Artifact «Architettura Vendite Deluxy» https://claude.ai/code/artifact/2740f619-61e2-4b8a-8f47-dee975e12254 (copia nel repo: `docs/architettura-vendite-2026-09-05.html`). Contiene: diagramma di flusso completo (chi decide: sistema/ufficio/partner), le 5 DECISIONI da prendere prima del codice (unità = ordine o riga; patto unitario o totale, per prodotto o variante; quando si fotografano importo/prezzo partner/quota; zero valore o buco; cosa succede quando l'unico candidato non c'è), la regola di precedenza patto > listino proprietario (UNICO) > regola categoria > null, il modello dati (Sale con quantity/unitPrice/partnerPrice/source, DeliveryProduct.saleId, productValue = Σ, ProductReconciliation per variante e unitario, deliveryId da UNIQUE a indice), dove vivono % e liste (raccomandazione: piattaforma, con API di lettura per Orders/CS e correzione di §7.4), migrazione in 7 passi reversibili. Fatti nuovi trovati: quantità assente in tutta la catena (24 rose × 5 € = «1 rosa a 5 €»); prezzo partner ricostruito dalla % in 3 posti con 3 arrotondamenti (Tiramisù: patto 28 → Orders legge 30, margine 0); cascata di `creaConsegna` salta `product.price`; `escludi` sempre vuoto → il rismista ripropone a chi ha rifiutato; `esisteCandidato` e `scegliPartner` si contraddicono sulla provincia dell'UNICO. NESSUNA riga di codice scritta: si parte dopo le 5 decisioni dell'utente.
>
> 💸 **05/09/2026-bis — VENDITE: DDT come legame, orari sulla fascia di consegna, prezzo partner dal listino della variante; colonne congelate; il DDT in tabella** (LIVE, deploy `delivery-ae3gggl65`, bundle `main-2RF44XJG.js` verificato sul dominio; commit `92a667cb`…`fab5f987` NON pushati: il push è bloccato dal classificatore, comando in coda all'utente).
> - **Riconcilia parte dal DDT** (numero d'ordine, e per sicurezza anche l'id Orders), poi indirizzo con il biglietto tagliato via; ogni riga dice da cosa è stata trovata. `creaConsegna` scriveva l'id Orders (cuid) nel DDT: ora il numero d'ordine.
> - **Orari**: `aperto()` confronta la FASCIA di consegna (da Orders) con l'apertura; senza fascia «è aperto quel giorno». Prima confrontava l'ora dentro la data (mezzanotte UTC = 02:00): 46% delle vendite a mezzanotte senza partner contro 7%. `rismistaAperte()` + `scripts/rismista-vendite-senza-partner.mjs` (usa il codice vero via `dist/`): 6 rismistate (12879→Clivati, 12736/7/8→Angolo Fiorito, 12874→Fao Schwarz, 1830→Cakedesignme), 28 ferme con motivo.
> - **DDT uguale = storico**: `scripts/collega-vendite-per-ddt.mjs`, 9 vendite chiuse (12832, 12840, 12841, 12842, 12847→#100788, 12850, 12857→#100826, 2833, 2839); 9 cakedesign.me NON toccate (collisione di numero con consegne Flowers nov–dic 2025: destinatari diversi). Utente: «hai fatto bene a non toccarle».
> - **Prezzo partner UNICO = listino della variante** (`ProductVariant.price`; `publicPrice` = cliente; verificato: Tiramisù «4» 28/30, Orders 30, 15 consegne a 28). `create` ora usa `body.amount` (prezzo di riga da Orders) e `quotaPerDare`. 12879 corretto in prod: 30/28/6,67%. ⚠️ 23 vendite UNICO divergono dal listino ma dove il catalogo ha un prezzo solo NON si può dire quale sia il partner: da separare prima di correggere.
> - **Consegne**: ordinamento data DESC anche in ricerca (la data è chiave primaria, l'orario ordina dentro il giorno); id vendita → ora **DDT + brand** sotto il numero, link a Vendite se collegata. **Vendite**: colonne congelate (`.col-fisse` in `styles.css`, Libro UX §8 v2.0, decisione in SEGNALAZIONI-UX.md), Modifica nel pop-up, produttore + foto del prodotto, indirizzo con Google Maps, inserimento che aspetta la chiusura della vendita (corsa di 3,1 s). Riconciliazioni ordinabile.
> - 🔴 **APERTI (in ordine di denaro)**: (1) `Delivery.productValue` letto dalla Finanza come «dato al partner» ma nullo su 116 consegne di vendita da agosto → 11.165 € letti come guadagno (#100099: 104,24 € contro 28,50 €); (2) `Sale.deliveryId` UNIQUE impedisce due vendite su una consegna (#100099 = 12835 + 12860): serve `DROP INDEX` concordato + `DeliveryDdt` + form a righe + riconciliazione su tutti i DDT + margini sommati; (3) 308 vendite importate con sconto 0% (215 con regola 20–40%, 186 già accettate); (4) 12736/7/8 a 180 € con quota 0%; (5) 9 vendite cakedesign.me aperte senza consegna; (6) architettura vendite nuova: giuria di 3 agenti in corso (flusso, dove vivono % e liste, casistica ostile) — l'esito va nel prossimo handoff.
>
> 🔎 **05/09/2026 — FILTRI PER TIPOLOGIA DI SERVIZIO in Consegne, e la RICONSEGNA finita davvero.**
> - **Filtri per tipologia** (regola utente: «in consegne, anche storico, consenti filtri veloci su tipologie di servizi in alto»). Due livelli: linguette per **famiglia** (`pricingModel`: PREZZO_FISSO 39.402 · VENDITA 13.735 · A_ORA 9.165 · CORPORATE 200 · MAGAZZINO 104 consegne — nessuna consegna senza famiglia) e menu per **servizio preciso** (48), che si restringe alla famiglia scelta. API: `GET /deliveries?pricingModel=&serviceTypeId=` (più id separati da virgola), applicato **sia in `findAll` sia in `mapPoints`**. Entra in «Filtri (N)», si azzera con «Azzera», torna col tasto indietro (sta nell'indirizzo). Verificato sul database di produzione: le cinque famiglie sommano esattamente lo storico (57.244).
> - ⚠️ **La riconsegna era rimasta a metà**: il commit `7f46bca1` conteneva API, DTO, chiavi i18n e il prefill del modulo, ma **NON** il bottone «Riconsegna» in lista né i collegamenti nel dettaglio (le chiavi `deliveries.redeliver` e `deliveryDetail.redelivery.*` erano orfane). Aggiunti ora: bottone sulle non consegnate (solo ufficio) e riquadro «Riconsegna della #N / Riconsegnata con #N» nel dettaglio, col motivo della mancata consegna.
> - **Documentazione allineata** (era indietro di tre funzionalità): `docs/COME-FUNZIONA-APP-DELUXY.md` (changelog 05/09 + §3.1 filtri e riconsegne + §3.8 Costi rimborsati e Conto Economico), `docs/guida-visiva.html` (tre avvisi nuovi), e **5 righe** nel registro di `scoutwt/MANUALE-DELUXY.html` (filtri, riconsegne, costi rimborsati che **sostituisce** la riga 04/09 sulla paga del valet, ore da approvare, verdetto delle segnalazioni).
> - Verifiche fatte: `npx tsc --noEmit` (api) pulito, `ng build` (web) pulito, `controlla-traduzioni.mjs` 1.655 chiavi · 0 mancanti.
...
> 🚚 **06/09/2026 (2) — Canale app: i DEFAULT delle consegne create dal Customer Service (regola utente, consegna #101065).** In `AppApiService.creaConsegna`, solo sui campi vuoti: **ritiro un'ora prima della consegna** (`pickupTimeFrom/To` = `deliveryTimeFrom/To` − 1h); **indirizzo di ritiro = sede del partner** (com'è scritta sul partner, con la provincia) e per **«Artista Locale» = l'indirizzo di consegna per intero**; per «Artista Locale» **`deliveredByPartner = true`** salvo ordine di **deluxy.it** (`ddtBrand`) o **valet assegnato**. E in `ritiroInCittaDiConsegna` (25/08) il ritiro di Artista Locale diventa l'INDIRIZZO di consegna, non la sola città («Modena» lasciava il valet senza un posto). ⚠️ Il brand del DDT lo passa il CS: prima mandava «FLowers» (nome del negozio), che non è fra i quattro della tendina (`deluxy.it`, `Flowers`, `cakedesign.me`, `Business`) — corretto di là. #101065 sistemata a mano nel DB (brand, ritiro 11-15, indirizzo, da fornitore, assegnata). Ramo `canale-app-0609`, in locale, NON pubblicato.
>
...
> - **Province in italiano** (regola utente): 11 nomi erano inglesi dall'import legacy (Florence, Naples, Milan, Venice, Padua, Mantua, Genoa, Syracuse, South Tyrol, South Sardinia, Monza and Brianza). Script `api/scripts/traduci-province-in-italiano.mjs` (prova senza argomenti, `--applica` per scrivere): **eseguito in produzione il 04/09**, 11 aggiornate, codici intatti. La geocodifica chiede già `language=it`, quindi le nuove nascono in italiano.
>
