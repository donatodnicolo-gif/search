...
> 🪗 **06/09/2026 sera — Attività: «Da fare» / «Storico»** (in locale, commit da pubblicare): `activities.service.findAll(user, date, limite, stato)` con `conteggi`, controller `?stato=`, `activities-list.component` con le due linguette e i numeri, i18n `activities.sezione.*`. Verificato via API locale: oggi 10 da fare / 28 concluse.
