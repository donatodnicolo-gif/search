...
- **Finanza** — 20/07, branch `worktree-vercel-deploy`. Sezione `/finance` (era stub), solo ADMIN. Due tab come §3.8 dell'app reale: **Corrispettivi** (una riga per consegna a buon fine: stato, id consegna, data, prodotto, categoria, partner, valore vendite, prezzo pubblico, prezzo partner, fee %/value/+IVA, costo consegna, primo margine e %) con **export CSV** e filtri data; **Margini** (totali del periodo). Backend `finance.module.ts`: `GET /finance/corrispettivi` e `GET /finance/summary` (from/to). ⚠️ **Formule DERIVATE, da confermare sullo schermo reale**: lo schema non ha una "fee/commissione" esplicita né lega Vendite↔Consegne, quindi `fee = valoreVendite − prezzoPartner`, `fee+IVA ×1.22`, `costo = valetSalary`, `primoMargine = fee − costo`. Se sull'app reale la fee ha un'altra fonte, cambiare `computeRow()`. Testato a runtime (SQLite usa-e-getta): consegna 95€ vendite − 29€ partner = fee 66€, −12€ costo → margine 54€; non-admin 403; filtro data ok.
