...
> - ✅ **LO SCALO AUTOMATICO È SCRITTO** (assenso dell'utente, 09/09: «fai anche quello»). ⚠️ **La migrazione la lancia l'utente PRIMA del deploy** — `api/prisma/migrations-manuali/20260909_recupero_anticipi_valet.sql`, due colonne su `Payment`: `recuperatoImporto` (Float, default 0) e `recuperatoSuSalaryId` (Text). Aggiungere colonne è retrocompatibile, quindi l'ordine è **migrazione → deploy**; al contrario ogni query su `Payment` darebbe P2022 e la pagina Pagamenti resterebbe vuota.
