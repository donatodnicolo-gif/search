...
>
> 💶 **11/09/2026 (mattina, 28) — ANTICIPO/EXTRA DA STIPENDI** (manuale 130). Web `salaries-list`: bottone «Richiesta pagamento» nella riga di «Da pagare» + finestra in fondo al template (`richiestaPer`, `ibanValet`, `apriRichiesta(valetId, nome)`, `richiestaValida`, `inviaRichiesta`), stili della modale copiati dalla scheda valet; l'IBAN si legge con `GET /valets/:id` all'apertura. Nessuna rotta nuova: `POST /valets/:id/richiesta-pagamento` è quella del 09/09. i18n riusate (`valetDetail.pagamento.*`). `ng build` ok.
