...
> ✅ **07/09/2026 (17) — il 12902 È ENTRATO** (LIVE `delivery-cncq8e3af`, pushato `086565b5`): alle 19:15:03 il cron ha creato la vendita «Bouquet Portofino, 135 €, da gestire» su **PV/Pavia** (provincia giusta), e alle 19:15:05 #2880 su **CA/Cagliari**. La correzione funziona sul vivo. ⚠️ **Ma il motivo mentiva**: `ingest` col flag `senzaProposta` scriveva un `assignmentReason` CABLATO — «Ordine estero: nessuno smistamento automatico» — nato per l'estero del 03/09 e finito tale e quale su due vendite italiane. Corretto: `ingest` accetta ora un `motivo` da chi la chiama (l'estero resta il ripiego) e lo smistamento passa «Nessun partner a cui proporla in questa provincia (prodotto unico di un partner escluso dalle proposte, o provincia scoperta): si gestisce a mano». 🔴 **Le DUE vendite già create portano ancora il motivo sbagliato nel database** (solo il testo: prodotto, importo e provincia sono corretti). Script pronto per riscriverle: `scratchpad/correggi-motivo.mjs` — da lanciare a mano, non fatto di iniziativa.
