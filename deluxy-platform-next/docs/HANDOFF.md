...
> - 🔖 **RESTA DA FARE, e serve il tuo assenso: LO SCALO AUTOMATICO.** Recuperare un anticipo dallo stipendio richiede di sapere **su quale stipendio è già stato recuperato**, cioè una **colonna nuova** su `Payment` (es. `recuperatoSuSalaryId`). Lo schema del Postgres condiviso non si tocca in autonomia (regola): la proposta è pronta, la migrazione no. Riusare `Payment.salaryId` sarebbe la scorciatoia sbagliata — quel campo risponde già a un'altra domanda (su quale stipendio è stato aperto il reclamo), vedi [[trappola-campo-di-stato-che-risponde-a-due-domande]]. Finché non c'è, l'anticipo si scala **a mano** e il riquadro del valet lo dice.
