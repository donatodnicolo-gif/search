    ...
    <div class="form-head">
      <div>
        ...
        <p class="page-caption">
          Anagrafica, province servite, servizi abilitati e fatturazione.
        </p>
      </div>
    </div>
    ...
    <form (ngSubmit)="submit()" class="form-grid">
      ...
      <section class="card block">
        ...
        <div class="grid-2">
          <label class="fld">
            <span>Insegna *</span>
            <input class="field" name="insegna" [(ngModel)]="model.insegna" required placeholder="Es. Fioraio Milano Centro" />
          </label>
          <label class="fld">
            <span>Email *</span>
            <input class="field" type="email" name="email" [(ngModel)]="model.email" required placeholder="ordini@partner.it" />
          </label>
          <label class="fld">
            <span>Ragione sociale</span>
            <input class="field" name="businessName" [(ngModel)]="model.businessName" placeholder="Partner S.r.l." />
          </label>
          <label class="fld">
            <span>Telefono</span>
            <input class="field" name="phone" [(ngModel)]="model.phone" placeholder="+39 …" />
          </label>
          <label class="fld">
            <span>Partita IVA</span>
            <input class="field" name="vatNumber" [(ngModel)]="model.vatNumber" placeholder="IT01234567890" />
          </label>
          <label class="fld">
            <span>Codice fiscale</span>
            <input class="field" name="fiscalCode" [(ngModel)]="model.fiscalCode" />
          </label>
          <label class="fld span-2">
            <span>Indirizzo</span>
            <input class="field" name="address" [(ngModel)]="model.address" placeholder="Via …, CAP Città (PR)" />
          </label>
          <label class="fld">
            <span>Referente</span>
            <input class="field" name="contactName" [(ngModel)]="model.contactName" placeholder="Nome e cognome" />
          </label>
        </div>
      </section>
      ...
      <section class="card block">
        ...
        @if (provinces().length === 0) {
          <p class="muted">Nessuna provincia configurata.</p>
        } @else {
          <div class="chips">
      ...
      <section class="card block">
        ...
        @if (categories().length === 0) {
          <p class="muted">Nessuna categoria configurata.</p>
        } @else {
          <div class="chips">
      ...
      <section class="card block">
        <header class="block-head">
          <h2>Fatturazione e notifiche</h2>
        </header>
        <div class="grid-2">
          <label class="fld">
            ...
            <select class="field" name="paymentMethod" [(ngModel)]="model.paymentMethod">
              <option value="">—</option>
              @for (m of paymentMethods; track m[0]) {
                <option [value]="m[0]">{{ m[1] }}</option>
              }
            </select>
          </label>
          <label class="fld">
            <span>Email per le fatture</span>
            <input class="field" type="email" name="invoiceEmail" [(ngModel)]="model.invoiceEmail" placeholder="fatture@partner.it" />
          </label>
        </div>
        <div class="toggles">
          ...
          <label class="toggle"><input type="checkbox" name="smsTemplatesEnabled" [(ngModel)]="model.smsTemplatesEnabled" /><span>Modelli SMS abilitati</span></label>
          <label class="toggle"><input type="checkbox" name="notificationsEnabled" [(ngModel)]="model.notificationsEnabled" /><span>Notifiche attive</span></label>
        </div>
      </section>
  ...
  styles: [
    `
      ...
      .chip {
        ...
        border-radius: 980px; padding: 6px 14px; font-size: 13px; font-family: inherit; color: var(--text);
      ...
      .icon-btn {
        width: 34px; height: 34px; border: none; border-radius: 8px; background: var(--fill);
        color: var(--text-secondary); cursor: pointer; font-size: 13px; transition: all 0.15s var(--ease);
      }
      .icon-btn:hover { background: rgba(215,0,21,0.09); color: var(--red); }
      .add { margin-top: 4px; align-self: flex-start; }
      .toggles { display: flex; flex-wrap: wrap; gap: 18px; margin-top: 16px; }
...
export class PartnerFormComponent {
  ...
  submit(): void {
    ...
    const payload: Record<string, unknown> = {
      insegna: this.model.insegna.trim(),
      email: this.model.email.trim(),
      notificationsEnabled: this.model.notificationsEnabled,
      invoicingEnabled: this.model.invoicingEnabled,
      smsTemplatesEnabled: this.model.smsTemplatesEnabled,
    };
    // Campi opzionali: inclusi solo se valorizzati
    for (const key of [
      'businessName', 'phone', 'vatNumber', 'fiscalCode', 'address',
      'contactName', 'paymentMethod', 'invoiceEmail', 'woocommerceApiKey', 'notes',
    ] as const) {
      const v = (this.model as Record<string, unknown>)[key];
    ...
    const services = this.serviceRows
      ...
      .map((r) => ({
        ...
        extraOutOfCityPrice:
          r.extraOutOfCityPrice != null ? Number(r.extraOutOfCityPrice) : undefined,
      }));
    ...
    this.http.post(`${environment.apiUrl}/partners`, payload).subscribe({
      ...
      error: (err) => {
        ...
        this.error.set(
          Array.isArray(msg) ? msg.join(' · ') : msg ?? 'Errore nella creazione del partner',
        );
      },
    });
  }
