...
@Component({
  ...
  template: `
    <div class="form-head">
      <button type="button" class="back" (click)="indietro()">← {{ 'products.title' | translate }}</button>
      @if (product(); as p) {
        <div class="title-row">
          <h1>{{ p.name }}</h1>
          @if (p.active === false) {
  ...
  styles: [
    `
      ...
      .title-row { display: flex; align-items: center; gap: 14px; margin-top: 6px; flex-wrap: wrap; padding-right: 96px; /* spazio per le bandierine fisse della lingua (28/08) */ }
