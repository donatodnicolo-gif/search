...
- URL di default: **`https://deluxy-anagrafiche.vercel.app`** (registro online → funziona anche da mobile). In ⚙️ Admin si impostano `anagKey` (chiave di **sola lettura** `dlxk_…`, obbligatoria) e opzionalmente `anagUrl` per puntare altrove (es. `http://localhost:3060` in sviluppo); vivono nella config su KV (`api/config.js`). La chiamata parte **dal browser**.
...
- **📣 Segnala al commerciale**: `POST /api/segnala` (pass code) → la funzione server legge dalla config KV `anagWriteKey` (chiave di **scrittura** del registro, impostata in Admin, **mai mandata al browser** — in `sanitize` solo `hasAnagWriteKey`) e crea il prospect: `stato:'prospect'`, `fonte:'manuale'`, `ultimaVisita` = ora, nota con origine, sito e **ordine per cui è stato contattato** (`ordine:{numero,valore,brand}` dal browser, se un ordine è caricato → «Contattato per l'ordine #2403 deluxyflowers.com — € 85.»). Il timestamp arriva dal browser (`quando` ISO: niente `new Date()` nella funzione, regola §12.5). Prima controlla i duplicati (`q=` + confronto nome/città normalizzati): se esiste fa **PATCH** aggiornando `ultimaVisita` («ultimo contatto») e accodando la riga `[data] Contattato per l'ordine …` alle note → risponde `{esistente:true, aggiornato:true}`. Le schede Google già matchate col registro hanno il pulsante disabilitato («Già nel registro»).
