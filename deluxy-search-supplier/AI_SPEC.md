...
- Dopo ogni ricerca, l'app interroga **deluxy-anagrafiche** (`GET {anagUrl}/api/v1/partners?categoria=FIORISTA|PASTICCERIA&citta=<CITTÀ>` con header `x-api-key`, timeout 5 s, `cache:no-store`); se in città non c'è nulla riprova con `provincia=`. **Tutti i valori dei filtri vanno in MAIUSCOLO** (il registro salva tutto così; ci pensa `anagQuery`). Risposta `{ totale, dati:[...] }`; stato `attivo` (confronto case-insensitive) = partner, gli altri stati = prospect.
