...
- **Anteprima locale**: server statico **PowerShell** (`C:/Users/nicol/scoutwt/.claude/serve.ps1`, config `search-main` in `C:\Users\nicol\app\.claude\launch.json`, porta **5511**). Node 24 c'è (per `node --check`), Python no.
...
```
- I **token Shopify NON escono mai dal server**: `GET /api/config` restituisce `hasToken:true/false`, mai il token. Stessa regola per `anagKey`/`anagWriteKey` (→ `hasAnagKey`/`hasAnagWriteKey`) e per **`mailApiKey`** (→ `hasMailKey`): la chiave di AI Mail resta sul server, usata solo da `/api/mail`.
- `mailUtente`/`mailUrl` (27/08/2026): casella mittente e (opzionale) URL di **AI Mail** per «Invia richiesta via email». La casella e l'URL tornano al browser, la chiave no. URL vuoto = `https://deluxy-mail.vercel.app`. Vedi `/api/mail` in §5.
- `ordersUrl`/`ordersKey` (27/08/2026): URL (vuoto = `https://deluxy-orders.vercel.app`) e **chiave di sola lettura** di **Orders** (`→ hasOrdersKey`, la chiave resta sul server). Con la chiave impostata, `/api/order` legge gli ordini da Orders invece che da Shopify (Standard §7: Orders è l'unica app collegata a Shopify). I brand di Orders (`deluxy.it`/`deluxyflowers.com`/`cakedesign.me`) coincidono con quelli di quest'app. La chiave si genera in Orders con `npm run chiave -- deluxy-search`.
- `googleKey`/`googleOauthClientId` sono restituite al browser (servono a mappa/rubrica; proteggi la chiave con restrizione referrer su Google Cloud) **ma NON alle chiavi API `dlxs_`** (27/08): `sanitize(c, soloBrowser)` le omette quando il chiamante è una chiave API (`auth.viaChiave`), che non ne ha bisogno.
...
- `cacheRicerca` (27/08/2026): `'1'` = **cache condivisa delle ricerche (48h)** accesa, `'0'`/mai salvata = spenta (predefinito). Con la stessa ricerca (indirizzo+categoria) già fatta da chiunque <48h, `run()` ripropone i risultati salvati (`/api/ricerca`) senza chiamare Google. Interruttore in ⚙️ Impostazioni. Vedi §5.
...
| GET | `/api/order?brand=&number=&ts=&debug=` | ordine per numero. **Ordine di ricerca (27/08)**: (0) **Orders** `GET <ordersUrl>/api/v1/ordini?brand=&q=<num>` se `ordersKey` è configurata — è la fonte di verità (Standard §7), foto e bigliettino inclusi; (1) KV webhook; (2) Shopify Admin col token. `mapOrdersOrder` converte la forma di Orders in quella del front-end. Se Orders non ha l'ordine (o non è configurata) si ripiega su (1)/(2). `debug=1` elenca i nomi ordini recenti. **Ogni check viene registrato nello Storico** (tipo `check`; esito «ordine trovato (Orders)» / «(magazzino webhook)» / «(Shopify)») |
...
| GET | `/api/contatti?url=<sito>` | **email + Instagram dal sito del negozio**, lato server (niente CORS, nessun proxy da configurare): legge homepage + pagine contatti tipiche → `{emails:[…max 3], instagram}`. Google Places non dà l'email: la ricava da qui. Best effort: errori di rete → liste vuote. Il front-end lo chiama per ogni scheda con sito dopo la ricerca; se in config c'è un `proxy` CORS usa quello (legacy, scraping dal browser). **Anti-SSRF (27/08)**: l'host dell'URL viene risolto e rifiutato se punta a IP interni (loopback/rete privata/link-local/metadata), ricontrollato a ogni redirect |
| POST | `/api/mail` | **invia la richiesta ordine via EMAIL passando da AI Mail** (deluxy-mail): body `{a, cc?, oggetto, corpo, corpoHtml?, allegati?:[{nome, contenuto(base64), tipo}]}`. Inoltra a `POST <mailUrl>/api/v1/invia` con `x-api-key: mailApiKey` + `x-utente: mailUtente` (dalla cassaforte, mai nel browser). La foto del bouquet viaggia come **vero allegato** (base64 dal blob già scaricato per gli appunti WhatsApp), tetto 8 MB. La mail parte dalla casella vera dell'operatore, copia negli «Inviati». UI: pulsante «📧 Invia richiesta via email» → **pop-up** `#mailModal` (destinatario/oggetto/corpo modificabili + anteprima foto) → invio, stella + stato «in corso» + log `canale:email` |
| GET/POST | `/api/ricerca?key=` | **cache condivisa delle ricerche in zona (48h)** — evita di ripetere le chiamate Google a pagamento. GET `?key=<hash>` → `{hit, salvatoIl, payload}` (o `{hit:false}`); POST `{key, salvatoIl, payload}` salva in KV `ricerca:v1:<key>` con TTL 48h (tetto ~900 KB, oltre non salva). `payload` è opaco lato server: lo scrive/rilegge il front-end (HTML dei risultati già renderizzati + `CARD_DATA` + `PLACE_FOTOS` dei pid mostrati). Chiave client = `normName(indirizzo) + '|' + categorie ordinate`. Attiva solo con l'interruttore `cacheRicerca` (Impostazioni); su cache hit `renderDaCache` ripristina le schede **senza chiamare Google** e mostra «💾 Risultati salvati il …» + «🔄 Forza su Google» (`run(true)`); **mappa e «aperto ora» sono congelati** al salvataggio (per averli aggiornati → Forza) |
| POST | `/api/webhook?brand=` | riceve ordine da Shopify (HTTPS diretto **o** envelope Google Pub/Sub) e lo salva in KV `order:{brand}:{num}` (TTL 60gg). **Sicurezza (27/08)**: fail-closed via `verificaOrigine` — se è impostato `SHOPIFY_WEBHOOK_SECRET` verifica l'HMAC `X-Shopify-Hmac-Sha256` sui byte grezzi, se è impostato `WEBHOOK_SECRET` accetta `?key=`/`x-webhook-key`; con almeno un segreto impostato rifiuta 401 tutto il resto. **Finché nessun segreto è impostato accetta senza verifica, come prima** (da chiudere impostando il segreto in Vercel) |
...
`APP_PASSWORD`, `KV_REST_API_URL`, `KV_REST_API_TOKEN` (+ `KV_URL`, `REDIS_URL` iniettati da Upstash), `SHOPIFY_CLIENT_ID`, `SHOPIFY_CLIENT_SECRET`.
**Sicurezza webhook (27/08)**: impostane ALMENO uno per chiudere `/api/webhook` agli estranei —
`SHOPIFY_WEBHOOK_SECRET` (il «secret di firma» in Shopify → Impostazioni → Notifiche, per i webhook HTTPS diretti; verifica l'HMAC) **oppure** `WEBHOOK_SECRET` (chiave condivisa da passare come `?key=…`/`x-webhook-key`, utile per Pub/Sub o test). Finché nessuno dei due è impostato l'endpoint accetta senza verifica (comportamento storico). La chiave di AI Mail NON è una env: sta in `config:v1.mailApiKey`.
...
- Dopo ogni ricerca, l'app interroga **deluxy-anagrafiche** (`GET {anagUrl}/api/v1/partners?…` con header `x-api-key`, timeout 5 s, `cache:no-store`) **sempre per provincia**: `provincia=` con sigla (FI), nome (FIRENZE) e nome completo (CITTÀ METROPOLITANA DI FIRENZE), ricavati dal geocoding della zona di consegna. Ripiego per CITTÀ (27/08): se la provincia non trova nulla si riprova con `citta=` usando la **città di consegna vera** (`geo.city`) e poi il capoluogo. Serve alle consegne **all'estero** (es. deluxyflowers a Ludwigsburg): lì nessuna provincia italiana combacia, ma un fornitore che abbiamo in quella città sì — così sale in cima come i prospect invece di restare invisibile. **Tutti i valori dei filtri vanno in MAIUSCOLO** (il registro salva tutto così; ci pensa `anagQuery`). Risposta `{ totale, dati:[...] }`; stato `attivo` (confronto case-insensitive) = partner, gli altri stati = prospect.
...
- **💾 Salva in rubrica**: con `googleOauthClientId` configurato in Admin usa Google Identity Services (`accounts.google.com/gsi/client`, scope `auth/contacts`) e la **People API**; il Client ID (tipo Web) deve avere la People API attiva e l'origine `https://search-deluxy.vercel.app`. Prima di creare, **cerca il numero** con `people:searchContacts` (con richiesta di warm-up, confronto sulle ultime 9 cifre): se il contatto esiste mostra «Già in rubrica» e non duplica. Il contatto viene salvato col nome **`FORNITORE [NOME] [TIPO] PROV. [PROVINCIA]`** (`contactName()`: tipo FIORAIO/PASTICCERE dalla categoria, provincia dal negozio o dall'ultima ricerca `lastGeo`). Senza Client ID (o se l'OAuth fallisce) ripiega su un **file .vcf** con lo stesso nome, che si apre con la rubrica di telefono/PC (lì niente controllo duplicati). **Dal 27/08 «Salva in rubrica» crea ANCHE il fornitore in Anagrafiche**: al clic manuale parte, oltre al salvataggio contatti, `creaFornitoreRegistro()` che fa lo stesso `POST /api/segnala` di «Segnala al commerciale» (idempotente, `segnalaPayload()` condiviso) — così un fornitore salvato è anche nel registro e la volta dopo compare in cima come prospect. Best effort: se manca la chiave di scrittura, il contatto si salva lo stesso e la riga di stato avvisa. NON parte sul percorso automatico della riconciliazione (`saveContact(...,{auto:true})`), che il registro lo gestisce già. Il pulsante ora si chiama «💾 Salva in rubrica + registro».
- **📣 Segnala al commerciale**: `POST /api/segnala` (utenze) → la funzione server legge dalla config KV `anagWriteKey` (chiave di **scrittura**, mai al browser — `hasAnagWriteKey`) e fa **un solo POST upsert-merge** al registro seguendo le sue regole d'ingaggio: `sistema:'deluxy-suppliers'` + `idEsterno` (= place_id Google: il registro ci riconosce alla prossima segnalazione), `asOf` e `ultimaVisita` = `quando` (ISO dal browser, regola §12.5), nota `[data] Segnalato dall'app search/supplier (utente). Ordine #2403 valore € 85. Sito: …`. **Niente `stato`** commerciale (le nuove nascono `prospect`; stato/interessi/account li cura il team del registro), **ma sì `statoFornitore:'da_provare'`** (27/08): il rapporto di FORNITURA è la dimensione che il registro si aspetta dalla ricerca fornitori (`segnalato → da_provare → abituale`; il CS promuove ad abituale dopo un pagamento; `da_evitare` è una bocciatura che il merge del registro non lascia ribaltare da qui). ✅ Nota merge (fatto lato registro, commit anagrafiche `42dcfb1d`): il merge di Anagrafiche garantisce che `statoFornitore` **può solo avanzare, mai retrocedere** dalle app (gradini `segnalato < da_provare < abituale`; `da_evitare` fuori scala e protetto). Quindi un `da_provare` in arrivo su un record già `abituale` viene ignorato: nessuna retrocessione. Solo la UI del registro può retrocedere a mano. Anti-doppioni, append delle note e merge per campo li fa il registro: risposta `{esito:'creato'|'merged', applicati, in_revisione}` → tradotta in `{creato:true}` o `{esistente:true, aggiornato:true}`. La `fonte` del record risulta `deluxy-suppliers` (dedotta da `sistema`). Le schede Google già matchate col registro hanno il pulsante disabilitato («Già nel registro»).
...
6. **Anteprima locale**: server statico PowerShell (`serve.ps1`, porta **5511**) — serve `deluxy-search-supplier/`. In locale le `/api/*` **puntano alla produzione** (`API_BASE` in index.html, riga ~1088): rispondono 401 senza credenziali; per collaudare l'interfaccia senza login si nasconde `#lockScreen` e si chiama `populateOrder({...})` da console. **Node 24 e npm ci sono** — `node --check api/*.js` per il lint di sintassi. Python no.
