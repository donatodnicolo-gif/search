...
- Dopo ogni ricerca, l'app interroga **deluxy-anagrafiche** (`GET {anagUrl}/api/v1/partners?categoria=FIORISTA|PASTICCERIA&citta=<CITTÀ>` con header `x-api-key`, timeout 5 s, `cache:no-store`) allargando via via: **città esatta → `provincia=` → città capoluogo** ricavata dal nome della provincia («Città Metropolitana di Firenze» → `citta=FIRENZE`): così i partner registrati sul capoluogo escono anche per le consegne nei comuni intorno (es. Greve in Chianti). **Tutti i valori dei filtri vanno in MAIUSCOLO** (il registro salva tutto così; ci pensa `anagQuery`). Risposta `{ totale, dati:[...] }`; stato `attivo` (confronto case-insensitive) = partner, gli altri stati = prospect.
