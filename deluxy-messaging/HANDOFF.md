...
> 📨 **12/09/2026 — CHIESTO ALLA PIATTAFORMA: il valore dell'ordine sulla consegna.** In attesa di risposta, niente da fare da questa parte finché non arriva.
>
> Segnalazione dell'utente sulla consegna **101402** (ordine **#2923**, Flowers, Giarre): «il valore passato è 90 € mentre il cliente ha pagato 150 €».
> **Verificato, e 90 non è un errore**: è il **costo del fornitore** («Vecchio Maurizio»), ed è esattamente la quota — `leggiQuotaFornitore(150,'CT')` risponde *quota 60%, atteso 90*. «Manda in app» mette in `products[0].price` quello che prende chi prepara, per disegno (07/09).
> **Quello che manca è il 150.** Riletto dalla loro API su quella consegna: `valoreProdotti 90`, `prezzoConsegnaCliente null`, `numeroShopify null`, e dell'ordine arriva solo `ddt "2923"` + `ddtBrand "Flowers"`. Nessuna Vendita (da noi `appVenditaId` vuoto): ordine lavorato a mano e mandato in app come «Vendita Deluxy» con partner «Artista Locale». Quindi 90 è l'unico numero sulla pagina, e il loro riquadro dei margini non ha da dove leggere il totale del cliente.
> ⚠️ **Non è un caso isolato**: 49 ordini mandati in app, **44 con un costo fornitore** — tutti così. Oggi sei fra le 15:29 e le 15:34 (#2923 150→90, #2922 87,65→60, #2921 135→80, #2919 85→60, #2906 85→50, #2908 185→120).
> **Chiesto** (sessione «Deluxy Delivery handoff e punti aperti», messaggio consegnato il 12/09): che siano **loro** a valorizzare `numeroShopify` e il prezzo cliente leggendoli da Orders con `ddt` + `ddtBrand`, che già ricevono. Il perché sta nello Standard §7: il prezzo del cliente vive in Orders e il proprietario si interroga, non si ricopia — e loro Orders lo leggono già per i margini. ⚠️ Segnalato anche che l'abbinamento va fatto su **ddt + marchio insieme**, o si ripete il caso #1834 di Cake che mostrava la consegna del #1834 di Flowers.
> **Se rispondono che preferiscono riceverlo da noi**: aggiungono un campo al `CreateDeliveryDto` (es. `orderNumber` / totale cliente) e da questa parte è **una riga** in `NuovaConsegna` (`src/lib/piattaforma.ts`) + il valore in `manda-in-app.ts`. Oggi nel DTO che usiamo quel campo **non esiste**.
...
>
> ⭐ **Poi riempiti COL VERO** (utente: «riempi le impostazioni di pagamento cercando già ora cosa dice deluxy.it in caso di selezione di questi altri metodi di pagamento»), con `scripts/metodi-dai-negozi.mts --esegui`: **10 righe**, nomi e testi **letti dal checkout vero** dei tre negozi (una bozza di prova per negozio, poi cancellata — #D5717, #D277, #D515). L'Admin API non serve a niente qui: sull'API 2025-01 **non esiste nessun tipo per i gateway manuali** (introspezione fatta: solo `PaymentSettings`, coi portafogli digitali). Misurato:
> · **deluxy.it**: Contrassegno «Paga comodamente in contanti alla consegna» · Deposito bancario «Paga tramite bonifico bancario» · PostePay «Effettuare il pagamento e inviare la ricevuta a +39 02 8295 2899» · Binance / Crypto «Account deluxy_white_gloves» (+ Satispay, Klarna, PayPal, Carta, Crypto USDC: gateway veri, non manuali)
> · **cakedesign.me**: Deposito bancario «Pagamento tramite bonifico» · Postepay (Italia) · Binance / Crypto — **niente contrassegno**
> · **deluxyflowers.com**: come Cake, ma **PostePay (Italia) non dice niente** al cliente
> ⚠️ Ogni riga è legata al **suo** negozio perché nome e testo cambiano («PostePay» / «Postepay (Italia)» / «PostePay (Italia)»; «Paga tramite bonifico bancario» / «Pagamento tramite bonifico»). Il **Contrassegno** invece vale per tutti: qui l'ordine nasce da una telefonata, non dal checkout, e il contrassegno è un accordo col cliente — il testo però è quello di deluxy.it, parola per parola.
> ⚠️⚠️ **DUE BUCHI DEI SITI, da sistemare di là**: (1) **nessuno dei tre dà le coordinate del bonifico** al checkout — il cliente legge «paga con un bonifico» e non sa dove; (2) su **Flowers PostePay non ha nessun testo**. Nelle impostazioni ho scritto per Flowers la stessa frase degli altri due (è lo stesso numero) e l'IBAN **non l'ho inventato**.
>
...
> · **Se il negozio non ha i termini si RIFIUTA**: senza, l'ordine nascerebbe pagato, cioè si dichiarerebbero incassati dei soldi che nessuno ha preso.
> · Il contrassegno viaggia in **due** modi: nella **nota** («DA INCASSARE ALLA CONSEGNA — …») per chi consegna, e nell'**attributo `Pagamento_Alla_Consegna`** per le macchine — è quello che in piattaforma fa nascere «Vendita con Pagamento alla Consegna».
> · Nella tendina dei mezzi, oltre ai metodi veri del negozio, entrano **«Contanti alla consegna»** e **«POS alla consegna»**: su Shopify non esistono come gateway perché quei soldi non ci passano.
> · ⚠️ `aggiornaBozza` rifiuta `alla-consegna`: non è uno stato che una bozza possa prendere.
> · **Provato dal vivo** con `scripts/prova-pagamento-alla-consegna.mts`: 8/8 su un ordine vero (#12938, poi annullato; prima la prova grezza #12937, anch'essa annullata). Entrambi col cliente «Test», che per la regola di casa li rende ordini di prova.

> ✅ **11/09/2026 sera (2) — IN PRODUZIONE con `ddazwl4uu`.** Tre segnalazioni dell'utente:
> 1. **I rimborsi già resi su Shopify si chiudono da soli** («#12868 risulta già rimborsato su shopify, in questi casi chiudi in automatico»). `src/lib/rimborsi-gia-resi.ts` + cron `/api/cron/rimborsi` ai minuti 18 e 48. Chiude solo se Shopify dice che è già stato reso **almeno** quanto si chiede, **scalando le altre richieste dello stesso ordine già eseguite**; una coperta in parte NON si tocca. La riga si prende con `updateMany` sullo stato, come il rimborso vero. ⚠️⚠️ **IL CONTO SI FA NELLA VALUTA DEL CLIENTE**, non in euro: corretto subito dopo la segnalazione dell'utente «perché il 2789 è ancora visibile?». Su #2789 il cliente aveva pagato **82,00 CHF** e Shopify gliene ha resi **82,00 CHF** (in franchi `netPayment` è zero), ma in euro il reso vale 87,55 € contro 87,65 € incassati — **dieci centesimi di differenza di cambio**, non un rimborso a metà — e il confronto in euro l'avrebbe lasciata aperta per sempre. Si converte col cambio DI QUELL'ORDINE (rapporto fra i due `totalPriceSet`), con **un solo centesimo** di tolleranza e solo quando le valute differiscono; sugli ordini in euro il rapporto è 1 e il confronto resta esatto. ⚠️ **Arretrato GIÀ pulito in produzione** con `scripts/chiudi-rimborsi-gia-resi.mts --esegui` (prova a secco di suo): chiuse **#2585, #1735, #1758, #12868** e poi **#2789**. Oggi la coda rimborsi è vuota (8 righe, tutte eseguite). Il cron però parte solo col deploy.
> 2. **Reclami: la riga si apre col click** («al click su elenco non apre dettagli»). ⚠️ Il difetto vero non era il click: il modulo sta SOPRA la tabella e l'apertura avveniva fuori schermo — adesso `modifica()` fa anche `scrollIntoView`, quindi anche il vecchio bottone «Apri» finalmente si vede lavorare.
> 3. **La ✕ del pop-up ordine è nell'angolo** («manca la "x" per chiudere pop-up»). C'era, ma era l'ultimo elemento della fila delle azioni, che va a capo: finiva sotto il titolo a sinistra, vestita da pillola. Ora `.pannello-chiudi-angolo` la ancora alla testata (sticky).
>
> ✅ **Confermato in produzione che il rimborso in valuta straniera funziona**: #2846 è stato reso dall'utente alle 16:29 e l'esito porta «il cliente riceve 160.00 USD».
> ⚠️ Non viste nel browser (login sul dev locale): la ✕ nell'angolo e il click sulla riga dei reclami.

> ✅ **11/09/2026 16:21 — IN PRODUZIONE `deluxy-messaging-gfzd90dnk`** (col «fai push & deploy» dell'utente; commit `eb533912`, già su `origin/scout-ui`). Verificato dopo il deploy: build Ready, alias `deluxy-messaging.vercel.app` sul deploy nuovo, `/rimborsi` e `/reclami` rispondono 307 al login (come da middleware), `/api/pubblico/consegna` risponde `configurato: true`. Tre cose:
>
> 1. **RIMBORSI IN VALUTA STRANIERA: il blocco è superato.** Nato dall'ordine **#2846** (utente: «non si può superare questo problema?»). Prima `preparaRimborso` si fermava con `stato: 'valuta'` su ogni ordine con `presentmentCurrencyCode ≠ currencyCode`. Il motivo era giusto ma la conclusione no: **Shopify vuole il rimborso nella valuta del CLIENTE**. Ora `src/lib/rimborso-shopify.ts` legge `shopMoney` **e** `presentmentMoney` di `netPaymentSet` e di ogni transazione, manda `RefundInput.currency = presentmentCurrencyCode` e gli importi delle transazioni **nella valuta del cliente**. Tutto il residuo → si prende la cifra di Shopify così com'è (zero arrotondamenti); una parte → in proporzione **col cambio di quell'ordine**, mai un cambio di mercato. Il tetto resta nei nostri euro. Nuovo: `GET /api/rimborsi/[id]/shopify` è l'**anteprima** (admin, non muove un euro) e la riga di conferma dice «Ha pagato in USD: riceve 160,00 $». Misurato sul vivo: #2846 = 160,00 USD / 138,20 EUR, 9/9 controlli in `scripts/prova-rimborso-valuta.mts`, e gli ordini in euro invariati.
> 2. **RIMBORSI: sezione STORICO** (utente: «crea una sezione storico dove far andare i rimborsi approvati o meno»). Chip «Da lavorare · Storico · Tutti» col conteggio, sul sistema di pillole dell'app (`stato-pill`+`attuale`, Libro UX v1.9 §8-bis); `stato=storico` nell'API; `rimborsoStorico()`, `STATI_DA_LAVORARE`, `STATI_STORICO` in `src/lib/rimborsi.ts`. ⚠️ **«Approvato» NON è storico**: decisione presa, soldi non usciti — resta nella coda finché non è «Rimborsato».
> 3. **SEGNALAZIONE AL DELIVERY, metà fatta** (utente: «se all'ordine è associato una consegna su app delivery invia una segnalazione all'app delivery di vedere il reclamo»). Fatto qui: colonne `Reclamo.segnalatoIl` + `segnalazioneEsito` (migrazione additiva **già applicata in produzione** con `scripts/applica-reclamo-segnalato.mjs`), `src/lib/segnala-reclamo.ts` (trova la consegna col numero+marchio, scarta le annullate, una volta sola, best-effort, non manda MAI la colpa), invio automatico alla nascita del reclamo in `POST /api/reclami`, riprova a mano con `POST /api/reclami/[id]/segnala`, e l'esito scritto nel messaggio di conferma.
>    ⚠️⚠️ **MANCA LA METÀ DI LÀ**: la piattaforma consegne non ha `POST /api/v1/app/segnalazioni` (misurato: risponde 404, vedi `scripts/prova-segnala-reclamo.mts`). Il modello `Segnalazione` di là **esiste già** (`tipo`, `deliveryId`, `oggetto`, `testo`, `apertaDaNome`, `stato`): serve solo la rotta app-to-app con `@UseGuards(ScritturaRichiestaGuard)` in `api/src/app-api/app-api.module.ts`, che accetti `{ tipo, deliveryId, oggetto, testo, riferimento, apertaDaNome }` e sia idempotente su `riferimento` (`reclamo:<id>`). **Non l'ho scritta io**: la cartella `C:/Users/nicol/app` è di un'altra sessione (ramo `piattaforma-ricerca-insensitive`, modifiche non committate) e la regola del repo è una sola sessione per cartella. Fino ad allora il CS dice «avvisa a voce» invece di far finta.
>
> ⚠️ **Da guardare al primo uso** (non viste nel browser, il dev locale chiede il login): la riga di conferma di un rimborso estero con la nota oro, le tre chip dei rimborsi, e il messaggio di conferma di un reclamo nuovo.
>
> ⚠️⚠️ **TRAPPOLA PAGATA, da non ripetere**: negli script di patch usare `split/join` e **mai `String.replace`** quando il testo NUOVO contiene un dollaro. In una stringa di sostituzione `$&` significa «il testo appena trovato»: scrivendo la riga «riceve 160,00 $&raquo;» nel Manuale Deluxy mi ha infilato dentro mezza riga della tabella, ed è finita così anche nella versione pubblicata. Riparata e ripubblicata (`8ea42502`); nello stesso giro ho rimesso nel registro una riga che un'altra sessione aveva lasciato dentro la tabella delle app del capitolo 2.

> ✅ **11/09/2026 15:02 — IN PRODUZIONE `deluxy-messaging-qxzs30frd`** (col «fai push & deploy» dell'utente; ramo `scout-ui` allineato, ultimo commit `4b28a650`). Va live tutto quello che era in locale dal 10/09 notte: **modifica di una bozza non ancora pagata**, **Orari negozi** col layout dell'architetto UX&UI e i **giorni con fasce speciali**, **Rimborsi** con la riga che si apre col click. Nessuna migrazione: i giorni speciali riusano la colonna `fasce` di `OrarioNegozio`. Verificato dopo il deploy: build Ready e alias attivo, `/orari-negozi` e `/rimborsi` rispondono (307 al login, come da middleware), API pubblica `/api/pubblico/consegna` risponde `configurato: true`.
> ⚠️ **Da guardare al primo uso** (le tre pagine non sono state aperte nel browser: il dev server locale chiede il login): scheda «Orari negozi» a 1440 e a 1000 px — l'anteprima a destra sticky, le etichette dei campi su una riga —, il pannello di un rimborso aperto col click, e la modifica di una bozza vera dal bottone «Modifica».

> ✅ **10/09/2026 ~17:50 — IN PRODUZIONE `deluxy-messaging-nkljka0iy`** (col «fai push & deploy» dell'utente; pushati `4378bb31`, `fd220d7d`, `cf93355a` su origin/scout-ui): motore col calendario del partner (`codici=`), drop-off per brand, «Chiudi il negozio oggi». Verificato: la rotta pubblica risponde 60 giorni, con `codici=` esce `partner.errore: piattaforma: non-trovato` finché la piattaforma non pubblica la sua rotta (`dc7acdc7` sul ramo `platform-0409`, da deployare dalla sessione della piattaforma).
> ✅ **11/09/2026 — IN PRODUZIONE `deluxy-messaging-3mu9ky7u7`** (deploy autorizzato dall'utente, chiesto dalla sessione del CRM). Ci vanno **quattro** cose: le due rotte v1 nuove di questa sessione (reclami, opzioni+tariffe del nuovo ordine) e **due funzioni di questa app che erano committate e non pubblicate** — il layout Orari negozi con i giorni a fasce speciali e la modifica di una bozza non pagata: ⚠️ **nessuna delle due è stata vista a schermo da chi le ha scritte** (login), quindi vanno guardate ora che sono live. Verifica fatta sulle API: `/api/v1/reclami` 12 reclami in 0,37 s, `/api/v1/nuovo-ordine/opzioni` (Deluxy) 1,66 s con le fasce di oggi, 8 voci di spedizione e 3 metodi.
> ⭐ **11/09/2026 — I RECLAMI DETTI ALLE ALTRE APP** (utente dal CRM: «aggiungi sezioni reclami all'app e fatti passare i reclami dall'app customer service»). Due rotte v1 in sola lettura (chiave API): **`GET /api/v1/reclami`** — filtri `cliente` (email uguale, o telefono confrontato sulle **ultime 9 cifre**: +39 347… e 0347… sono la stessa persona), `ordine`, `stato` (con la vista `aperti` = aperto + in lavorazione), `colpa`, `gravita`, `q`, `periodo`, `page`/`limit`; risponde con le righe, `totale`/`pagine`, `perStato` **sullo stesso filtro dell'elenco**, `domandeAperte` per riga, il `link` alla scheda (`/reclami?apri=<id>`) e le **etichette** (stati, colpe, gravità) così chi legge non si riscrive i vocabolari — e **`GET /api/v1/reclami/<id>`** col filo dei messaggi e `senzaRisposta` per ogni domanda. ⚠️ `cliente=` che non assomiglia né a un'email né a un telefono torna **zero**, non tutto: dentro la scheda di una persona i reclami di altri sarebbero peggio del vuoto. `prodotti` e `azioni` viaggiano come elenchi (a database sono righe separate da `\n`). Le due funzioni condivise stanno in **`src/lib/reclami-api.ts`**: un `route.ts` di Next può esportare solo i verbi HTTP. **Niente scrittura**: il reclamo si apre e si chiude qui. Provato in locale con la chiave del CRM: elenco 0,79 s (11 reclami, `perStato` giusto), filtro per email 1 riga, per telefono 1 riga, `cliente=xyz` 0 righe, dettaglio con azioni e descrizione, senza chiave 401. `tsc` 0. **Pubblicato** (deploy `deluxy-messaging-3mu9ky7u7`).
> ⭐ **11/09/2026 — LE OPZIONI DI NUOVO ORDINE DETTE ALLE ALTRE APP** (utente dal CRM: «le fasce orarie devono essere dei negozi o a inserimento manuale; copia tutte le opzioni da app customer service, chiedi espressamente all'app quali ha»). Due rotte v1 nuove (chiave API): **`GET /api/v1/nuovo-ordine/opzioni?negozio=<id>[&giorni=45]`** — fasce dagli orari del negozio (`configurato`, `primoGiornoAperto`, griglia `oltre`, `calendario` con ok/motivo/fasce per giorno; `storiche` se il negozio non ha orari scritti), voci di spedizione usate, metodi di pagamento visti, `iva` (aggiungibile, predefinito falso, spiegazione), `campi` = l'elenco a parole dei facoltativi del POST — e **`POST /api/v1/nuovo-ordine/tariffe`** (indirizzo + righe → tariffe del sito + stima fuori zona). Il conto delle tariffe sta ora in **`src/lib/tariffe-con-stima.ts`**, usato sia dalla rotta interna `/api/nuovo-ordine/tariffe` sia dalla v1 (un conto solo, due porte). **`POST /api/v1/nuovo-ordine`** accetta anche `destinatario`, `anonima`, `consensoMarketing`, `aggiungiIva`, `eccezioneOrari` (prima li ignorava: una data chiusa restava rifiutata anche se concordata). Provato in locale con la chiave del CRM: opzioni 0,6–1,1 s per negozio (Deluxy oggi 12-14…20-22, Cake 08-12/12-16/16-20; spedizioni e metodi veri), tariffe Milano 2,1 s → 15 €, Bergamo 3,4 s → 80 € del sito + stima 75 €; chiave sbagliata → 401. `tsc` 0. **Pubblicato l'11/09** (`deluxy-messaging-3mu9ky7u7`): il CRM live le usa. Registrato in README (Per le altre app), docs/MANUALE.html, MANUALE-DELUXY.html.
> ⭐ **11/09/2026 — RIMBORSI: LA TABELLA DICE COSA SI CONCRETIZZA** (utente: «mostra in tabella l'azione che si concretizza», dopo aver chiesto «cosa succede se faccio approva?»). `azioneRimborso(r, ruolo)` in `src/lib/rimborsi.ts` torna `{ cosa, ora }`: l'azione chiesta (totale/parziale con gli importi) e che cosa fa il bottone visibile adesso — `richiesto` → «Approva registra la decisione: nessun soldo si muove»; `approvato` → «“Rimborsa su Shopify” rende X sul metodo con cui ha pagato» (admin) o «Il rimborso vero lo fa un amministratore»; `eseguito` → «Niente da fare: i soldi sono usciti»; rifiutato/annullato → «Nessun rimborso». Nuova colonna **«Cosa si fa»** fra Stato e Chiesto in `RimborsiLista`, e lo stesso blocco nel pannello sopra Approva/Rifiuta. Il motivo: approvare e rimborsare si somigliano e fanno cose opposte, e da fuori non si distinguevano. `tsc` verde; ⚠️ non visto nel browser (login). In LOCALE, non pushato.
> ⭐ **11/09/2026 — RIMBORSI: LA RIGA SI APRE COL CLICK** (utente: «al click non apre i dettagli», su /rimborsi). `RimborsiLista.tsx`: `<tr className="riga-apribile" tabIndex={0}>` con click che ignora `a,button,input,select,label,textarea` (Libro v1.6, stesso pattern di `ChiamateLista`) e Invio da tastiera; si apre un pannello (`.velo`/`.pannello`, Esc e click fuori) con **motivo per intero** (in tabella è tagliato a 110 caratteri), importo/tipo, stato, chiesto da e quando, deciso da, esito, Approva/Rifiuta se `stato === 'richiesto'`, e «Apri l'ordine» che monta `DettaglioOrdine`. Un effetto risincronizza la riga aperta quando l'elenco si ricarica. CSS `tr.riga-apribile` (cursore, hover, focus oro) in fondo a `globals.css`. `tsc` verde; ⚠️ non provato nel browser (login). ✅ **LIVE `qxzs30frd`** (11/09 15:02).
> ⭐ **11/09/2026 mattina — ORARI NEGOZI: LAYOUT DELL'ARCHITETTO UX&UI + GIORNI CON FASCE SPECIALI** (utente: «sistema css e layout su base architetto ux&ui» e «consenti di specificare per specifici giorni fasce orarie specifiche»). **Layout** (`OrariNegozi.tsx` riscritto nel render, CSS in fondo a `globals.css`: `.orari-*`, `.blocco*`, `.riga-campi`, `.segmenti`, `.tabella-anteprima`, `.pillola-fascia`, `.bottone.pericoloso`): una colonna con sezioni, anteprima a tabella in `aside` sticky 360 px sopra i 1100 px, campi 112/88 px con etichette ≤ 22 caratteri e `align-items: end`, un `testo-guida` per blocco, preset a segmenti con «Personalizzato» (attivo da solo quando nessun preset coincide), «Chiudi il negozio oggi» `.bottone.secondario`, barra azioni `[Cancella (pericoloso)] … [Annulla] [Salva]`. Regola nuova nel **Libro UX&UI v1.11 §4** (campi a valore corto) come proposto dall'architetto. **Giorni speciali**: `GiornoSpeciale {data, ogniAnno, fasce, testo?}` in `orari-regole.ts`; `OrarioNegozioDati.giorniSpeciali?`; salvati nella colonna `fasce` (libera dal 10/09) da `salvaOrario`, letti da `leggiOrario`; `leggiFasceTesto('08-12, 14-18')`/`scriviFasceTesto`; `giornoSpeciale(dati, iso)`; `giornoSelezionabile` dice aperto se speciale (anche in un giorno della settimana chiuso; una chiusura sulla stessa data è rifiutata da `validaOrario`); `fasceDelGiorno` usa le fasce del giorno speciale al posto delle regole (oggi: drop-off e via le fasce già cominciate; oraMinima e partner come sempre) e mette `speciale: true` nell'esito. Test motore +16 (tutti passati), `tsc` verde. ⚠️ **La pagina non è stata guardata nel browser** (il dev server chiede il login): controllare a 1440 e a 1000 px che l'aside stia a destra e che le etichette non vadano a capo; se qualcosa stona, il verdetto dell'architetto è nel transcript e le regole nel Libro §4. ✅ **LIVE `qxzs30frd`** (11/09 15:02).
> ⭐ **10/09/2026 notte (3) — MODIFICA DI UNA BOZZA NON ANCORA PAGATA** (utente: «consenti di modificare una bozza non ancora pagata», da /nuovo-ordine). Nelle «Bozze mandate» ogni bozza aperta ha **Modifica** → `/nuovo-ordine?bozza=<id OrdineCreato>`: `NuovoOrdine` (prop `bozzaId`) legge `GET /api/bozze/[id]` (`leggiBozzaPerModifica` in `src/lib/bozze.ts`: la bozza **riletta da Shopify** — `draftOrder` con email/phone/note2/taxExempt/customAttributes/customer/shippingAddress/shippingLine/lineItems — tradotta nei campi del modulo; la nota si riparte in biglietto e note di consegna, la riga «Mittente (chi ordina)» VINCE sul cliente Shopify perché senza email Shopify inventa un cliente col nome del destinatario) e al salvataggio fa `PUT /api/bozze/[id]` → `aggiornaBozza` in `src/lib/nuovo-ordine.ts` (rifiuta: già pagata/annullata, negozio diverso, `pagamento: 'pagato'`; richiede lo stato a Shopify PRIMA di scrivere; `draftOrderUpdate` con lo stesso input della creazione — estratto in `preparaBozza`, condiviso; `reinviaLink` opzionale; aggiorna importo/cliente sulla riga OrdineCreato). Il corpo della richiesta si traduce in `src/lib/nuovo-ordine-corpo.ts` (`datiDaCorpo`), usato anche da POST /api/nuovo-ordine. In modifica: negozio bloccato, niente «Ha già pagato» (c'è la spunta «Rimanda il link per email»), niente bozza locale (né proposta né salvataggio), bottone «Salva le modifiche», esito «Bozza #D… aggiornata» con lo stesso link. **Provato dal vivo** con `scripts/prova-modifica-bozza.mts` su deluxy.it (bozza di prova creata → riletta → modificata → riletta → rifiuti → cancellata): 22/22. `tsc` verde. ⚠️ La UI in modifica NON è stata guardata nel browser (il dev server locale chiede il login): al primo uso controllare che il modulo si riempia e che il salvataggio mostri «Bozza #D… aggiornata». Commit `b5fbe10a`. ✅ **LIVE `qxzs30frd`** (11/09 15:02).
> ⭐ **10/09/2026 sera — IL CALENDARIO DEL PARTNER ENTRA NEL MOTORE, E I TEST DI ACQUISTO** (utente: «abbiamo aggiornato anche l'app merchandising con ora l'orario di chiusura di un partner e inoltre abbiamo previsto i giorni di chiusura dello stesso con l'app delivery — STUDIA TUTTO e unisci anche questi sviluppi alle modifiche del codice su shopify, implementa e testa»). **Studio** (due agenti + lettura diretta): l'orario del partner vive nella PIATTAFORMA (`OpeningHour` settimanale, `PartnerDayException` per data, `PartnerDaySlot`; cascata in `common/disponibilita-partner.ts`); il worktree `app/.claude/worktrees/deploy-delivery` (ramo `platform-0409`) ha `merchandising-sync/calendario-unici.module.ts`: `CalendarioUniciService.calendario()` costruisce per ogni prodotto UNICO (per SKU) 14 giorni `{data, aperto, dalle, alle, origine}` con `chiuso-per-oggi` passata l'ora di chiusura, e lo manda ogni mezz'ora a Merchandising (`POST /api/v1/prodotti/disponibilita`), che ricalcola `prodotto.consegna`/`custom.minimo_orario` e li scrive sui negozi (`disponibilita-unici.ts`). Quindi preavviso e orario minimo arrivavano già al tema; mancavano le CHIUSURE nei giorni intermedi e l'ora di chiusura di oggi come regola esplicita. **Fatto**: (1) nella piattaforma, nel suo worktree, **`GET /api/v1/app/prodotti-unici/calendario?codici=&giorni=`** (additiva, dietro il guard `x-api-key`; riusa `calendario()` filtrato per SKU; `tsc` API 0; commit sul ramo `platform-0409` con riga di handoff — ⚠️ da pubblicare col loro deploy: finché non è live il CS risponde `partner.errore: piattaforma: non-trovato`); (2) nel CS: `piattaforma.ts: calendarioUniciPiattaforma(codici, giorni)` con cache 5 min per insieme di SKU; `orari-regole.ts`: tipi `GiornoPartner`/`CalendarioProdotto`, `VincoliCarrello.prodotti`, e nel motore per ogni giorno: partner chiuso (o `chiuso-per-oggi`, o `adesso ≥ alle` oggi) → giorno spento col nome del partner e del prodotto; aperto → `dalle` arrotondata all'ora piena alza l'orario minimo di quel giorno; giorni oltre il calendario → nessun vincolo; la rotta pubblica accetta `codici=` (max 30) ed esce con `partner.prodotti` e `partner.errore`; (3) nel tema: il modulo `DeluxyConsegna` raccoglie gli SKU (`item.sku` del carrello + variante della scheda), li passa in `codici=` e li mette nella chiave della cache — `all_tags_and_script.liquid` ricaricato sul tema di lavoro (md5 verificato). Prove: 8 casi nuovi nel motore, sandbox del carrello 19/19. **Test di acquisto con agenti** (desktop col browser + HTTP): il tema di lavoro rende bene, Liquid pulito, i vincoli nel modulo coincidono coi metafield, fasce = API; **trovato e corretto un difetto GRAVE della rotta pubblica**: `Number(null)` valeva 0 e senza `giorni=` uscivano 1 solo giorno e `primoGiorno` null → il tema conosceva solo «oggi» (commit `4378bb31`, **LIVE `ar09ix206`**, verificato: 60 giorni, primo giorno 12/09 con preavviso 2). ⚠️ Riserva: `s-maxage` non è visibile dall'esterno (Vercel lo toglie), ma l'edge fa HIT/STALE come atteso.⚠️ Riserva: `s-maxage` non è visibile dall'esterno (Vercel lo toglie), ma l'edge fa HIT/STALE come atteso. **Test desktop (agente col browser, 17:04–17:20)**: flusso completo home → scheda → modale indirizzo (Google) → carrello → pagina di checkout OK; data = primoGiorno, tendina = API, attributi `Data_Consegna`/`Fascia_Oraria_Consegna`/`Fasce_Fonte` salvati, data passata bloccata; **due difetti medi corretti subito e ricaricati sul tema** (md5 verificati: all_tags `668a2b60…`, delivery `a107cd32…`): `Fasce_Fonte` restava «customer-service» anche sul ripiego → ora `segnaFonte('tema')` scrive «tema (regole cablate)» e si riscrive a ogni cambio; `#date_disp2` chiamava «prima data disponibile» la data ricordata → ora mostra la prima disponibile e «data scelta: …». Non nostri, già noti: `fnCheckDelivery` legge `#DeliveryDate` inesistente (main-cart, 99 KB), errori console `OK is not defined` da script esterni, calendario della modale prodotto non patchato (file da incollare). **Test mobile (375 px, ~18:00)**: flusso OK, `Fasce_Fonte` api/tema e «prima data · data scelta» verificati; tre ritocchi al carrello ricaricati sul tema (md5 `158b7812…`): datepicker in italiano dal lunedì, bersagli 44 px sul touch, trattino «–» anche nel ripiego. I difetti UX preesistenti del tema (calendario home 43×22 px, CONFERMA muto senza data, × e celle modale 31 px…) sono registrati in STATO-DELUXY-IT.md §6, non toccati.
> ✅ **10/09/2026 17:00 — IN PRODUZIONE `deluxy-messaging-jp74xzuzc`** (col «fai push & deploy» dell'utente; il commit `e2b0f7e8` era già su origin, pushato da un'altra sessione): `/api/pubblico/consegna` risponde in prod per i tre domini (deluxy configurato, alle 16:56 propone 20-22). Da ora il tema di lavoro di deluxy.it riceve le date dal Customer Service.
> ⭐ **10/09/2026 notte (2) — DOMANI A FASCE ORARIE, E DOPO LE 20 LA PRIMA DI DUE ORE** (utente: «domani deve essere fascia un'ora a parte se io acquisto dopo le 20 e in quel caso la fascia oraria di consegna diventa minimo orario di consegna del carrello con fascia di due ore, per le restanti ore la fascia diventa di un'ora»). Nuovo campo `domani.primaFasciaOre` (0 = come le altre): dopo `saltaDopoOra` la prima fascia di domani dura tanto e parte dall'orario minimo del carrello, le restanti `fasceIntere(…, da + prima)`. `REGOLE_DELUXY.domani` = { durataOre 1, salta 0, soglia 20:00, primaFasciaOre 2 } → alle 15: 08-09 … 21-22; alle 21: 08-10, 10-11 … 21-22; con orario minimo 9 alle 21: 09-11, 11-12 …. Flowers/Cake `primaFasciaOre: 0` (restano «dalle 12»). Lettura/validazione/UI (campo «Dopo quell'ora la prima fascia dura (ore)») aggiornate; righe deluxygifts e 90bfeb-f5 **riscritte nel DB**; test motore riscritti (6 casi su domani), `tsc` verde. Tema: ripiego del carrello per domani dopo le 20 allineato (prima fascia 2 ore da `max(ora_inizio, ora_min_def)`, poi orarie). ✅ **18:22 LIVE `deluxy-messaging-fgaz1mcmz`** (col «fai push & deploy» dell'utente; pushato `0bbbb449`): verificato in produzione alle 18:19 (prima delle 20) domani orario da 08-09 (14 fasce) e con `oraMinima=9` da 09-10 (13); il caso «dopo le 20» è coperto dal test del motore (alle 21: 08-10, 10-11 … 21-22) e dall'anteprima «prova come se fossero le 21:00» in Orari negozi. Tema: `delivery_date_hour_c` ricaricato col ripiego allineato, md5 `0033e4d7bd047252368aa507ef8ac864` verificato.
> ⭐ **10/09/2026 notte — «DOMANI DOPO LE 20»: DECISO DALL'UTENTE** («Prime due ore da quando l'orario minimo del carrello è attivo»): scartate la 10-12 fissa (architetto UX) e la 08-10 fissa (vecchio tema). Nel motore `fasceIntere(regole, durata, daMinuti)` aggancia la griglia all'orario minimo del carrello per **domani e oltre** (dalle 9 → 09-11, 11-13…, non 10-12 da griglia fissa); `REGOLE_DELUXY.domani.dopoLimiteSaltaFasce` = 0 (Flowers e Cake restano a 1). Righe di deluxygifts e 90bfeb-f5 **riscritte nel DB** con `imposta-orari-deluxy.mts --anche-se-scritta --solo`. Test motore aggiornati (+4 casi), `tsc` verde. Il ripiego del tema (08-10 dopo le 20) coincide col caso senza vincoli. Difetti UX preesistenti di home e modale: **non decisi dall'utente, non toccati**. ✅ **18:05 LIVE `deluxy-messaging-5ixmkf4ue`** (col «fai push & deploy di tutto»; pushati `2ba58f48`, `ad6428c9`, `e1bfd5e2`): verificato in produzione con `oraMinima=9` → domani 09:00–11:00, 11:00–13:00…, dopodomani 09-10, 10-11….
> ⭐ **10/09/2026 sera — ORARIO DI DROP-OFF PER BRAND e «CHIUDI IL NEGOZIO OGGI»** (utente: «aggiungi su app anche l'orario di dropoff di default quello previsto ora per ogni brand che è l'ora a partire dalla quale gli ordini potranno arrivare solo dal giorno dopo» e «consenti sempre in app di chiudere il negozio oggi e così dire ai clienti che possono ordinare da giorno dopo»). **In locale, NON pushato né pubblicato.** `RegoleConsegna` ha due campi nuovi: `oggi.notteDalle` (di notte la prima fascia parte da qui: il motore ora lo usa ESPLICITAMENTE al posto del conteggio «dalla fascia prima dell'apertura») e `domani.saltaDopoOra` (l'ora da cui domani perde le prime N fasce — Flowers 22:00, diverso dal drop-off 16:00); `leggiRegole` li completa col preset se mancano nelle righe vecchie. Preset per brand **`REGOLE_PER_DOMINIO`**: deluxy.it e business = REGOLE_DELUXY (drop-off 20:00, notte 10:00, domani salta 1 dopo le 20), Flowers = REGOLE_FLOWERS (= FASCE_AMPIE: drop-off 16:00, notte 08:00, domani salta 1 dopo le 22), Cake = REGOLE_CAKE (drop-off 14:00, notte 12:00, domani salta 1 dopo le 20) — presi da REGOLE_BRAND.md. **Righe scritte per tutti e quattro i negozi** con `imposta-orari-deluxy.mts --anche-se-scritta` (tiene giorni e chiusure esistenti, riscrive le regole): da ora Flowers e Cake sono «impostati» (i loro temi non leggono ancora l'API: nessun effetto sui siti; in Nuovo ordine le fasce calcolate coincidono con le tre storiche). Pagina: etichetta «Orario di drop-off (da quest'ora si ordina solo per il giorno dopo)», campi notte/soglia, tre bottoni preset (deluxy.it/business · Flowers · Cake). **«Chiudi il negozio oggi»** in testata della scheda (admin, sempre visibile): `Conferma` → oggi entra in `giorniChiusura` con motivo «chiusura di oggi: si ordina per domani» e si salva SUBITO (anche le modifiche non salvate della scheda, e se il negozio era «senza orari» salva anche le regole mostrate — lo dice nella conferma); badge rosso «oggi chiuso», «Riapri oggi» toglie la chiusura di oggi (solo quella non «ogni anno» con quel motivo). Sul sito: oggi spento col motivo, `primoGiorno` = domani. Prove aggiunte (Flowers notte/soglia, Cake, chiusura di oggi): tutte passate; `tsc` 0. REGOLE_BRAND.md: sezioni Flowers e Cake annotate con la fonte CS e il drop-off.
> ⭐⭐ **10/09/2026 sera — IL MOTORE DELLE FASCE E I SITI CHE LEGGONO DA QUI** (utente: «prepara le version to work di shopify in modo tale che tutte le tabelle recepiscano le date da te … deluxy.it consegna nello stesso giorno in fasce di 2 ore a partire dal secondo paio d'ore successivo a quello in corso, dalle 20 (chiedi conferma ad architetto) l'ordine è possibile per il giorno dopo con fascia di 2 ore. Per i giorni successivi fascia di un'ora a partire dall'orario di disponibilità minima di tutti i prodotti nel carrello. Fai stessa cosa per sito business.deluxy.it — iniziamo con deluxy.it»). **In locale, NON pushato né pubblicato** — ⚠️ e finché `/api/pubblico/consegna` non è in produzione il tema di lavoro NON riceve le date (il modulo resta inattivo e il tema usa le regole cablate). **Architetto UX consultato** (punti A–F): conferma 20:00 con l'eccezione esplicita «18:00–19:59 → solo 20-22»; di notte si conta dall'apertura (prima 10-12); domani dopo le 20 dalle 10-12 (coerenza: stesso stato operativo) — il tema oggi dava 08-10, **da confermare l'utente**; granularità per distanza del giorno, mai per ora; giorno senza fasce = data spenta col motivo; etichette «14:00–16:00» + «Oggi/Domani» sul sito, value «HH-HH». **Nel CS**: `orari-regole.ts` riscritto — tipo `RegoleConsegna` (finestraDa/A, oggi{attivo,durataOre,saltaFasce,limiteOra,ultimaFasciaFinoAlLimite}, domani{durataOre,dopoLimiteSaltaFasce}, oltre{durataOre}, giorniMostrati), preset `REGOLE_DELUXY` e `REGOLE_FASCE_AMPIE`, motore `fasceDelGiorno(dati, iso, adesso, {oraMinima, leadGiorni})` → {ok, motivo, fasce, etichette, quando}, `calendarioConsegna`, `adessoRoma()` (Intl Europe/Rome), `fasceIntere`; la colonna `fasce` a mano è ABBANDONATA (resta vuota per compatibilità), nuova colonna **`OrarioNegozio.regole`** (JSON, aggiunta da `applica-migrazione-orari-negozi.mjs` con ADD COLUMN IF NOT EXISTS). Pagina Orari negozi: blocco «Regole delle fasce orarie» con i due preset e l'anteprima 14 giorni con fasce calcolate ADESSO + «prova come se fossero le HH:MM». Nuovo ordine: fasce dal motore per la data scelta (`fasceDelGiorno`), avviso oro se per quell'ora non resta nessuna fascia (l'operatore usa «Flessibile»). **Rotta PUBBLICA `GET /api/pubblico/consegna?dominio=&oraMinima=&leadGiorni=&giorni=`** (senza chiave, fuori dal middleware — `api/pubblico` nel matcher —, CORS *, `Cache-Control public, max-age=30, s-maxage=60, stale-while-revalidate=60`; una query sola; `configurato:false` = il sito tiene le sue regole). v1 `/api/v1/orari-negozi` ora esce con `regole` e `calendario`. **Righe scritte in tabella** con `npx tsx scripts/imposta-orari-deluxy.mts`: Deluxy (deluxygifts) e Business (90bfeb-f5) con REGOLE_DELUXY, aperti 7 giorni, nessuna chiusura (`--anche-se-scritta` per riscrivere). Prove: `prova-orari-negozi.mts` riscritta (validazione, giorno, **motore**: 10:30→14-16…, 10:00→14-16, 16:00→20-22, 19:59→20-22, 20:00→niente, 03:00/07:00→10-12, domani 7×2h, domani dopo le 20 da 10-12, oltre 14×1h, oraMinima, leadGiorni, fasce ampie), tutte passate; `tsc` 0; API provata in locale per i tre domini. **Nel TEMA deluxy.it** (Version to work on `207333458250`): 5 file caricati con md5 verificato (all_tags con il modulo `DeluxyConsegna`, delivery_date_hour_c, header, home-delivery, home-delivery-section-new); `product-delivery-date.liquid` (59 KB) **bloccato dal classificatore**: incollato a mano dall'utente la notte del 10/09, md5 verificato `6f405ef2…` → **tutti e sei i file sono sul tema di lavoro**; resta da provare l'anteprima e pubblicare il tema dall'admin. Dettagli: REGOLE_BRAND.md §deluxy.it (riscritta: la fonte è il CS), TEMA_DELUXY_IT.md §10/09, STATO-DELUXY-IT.md. Sandbox del carrello: 19 prove passate. 🔴 **Prossimi passi**: (1) push & deploy del CS (senza, il tema non riceve niente); (2) incollare il file prodotto nel tema; (3) provare l'anteprima del tema con un articolo in carrello (`?preview_theme_id=207333458250`) e poi pubblicare dall'admin; (4) Flowers e Cake: scrivere le loro regole (limiti 16:00/14:00, tre fasce ampie: preset pronto) e patchare i loro temi con lo stesso modulo; (5) decidere «domani dopo le 20: da 10-12 (architetto) o da 08-10 (tema di prima)».
> ⭐ **10/09/2026 — ECCEZIONE CONCORDATA in Nuovo ordine** (utente: «aggiungi anche l'eccezione per data chiusa in nuovo ordine»). **In locale, non pushato né pubblicato.** Quando la data è un giorno chiuso (orari salvati) e NON è passata, sotto il campo Giorno compare la spunta **«Eccezione concordata»** con un **motivo obbligatorio** (max 200 caratteri); si azzera da sola se la data torna buona. Il client manda `eccezioneOrari` (stringa = motivo; vuota = nessuna eccezione) e `crea()` si ferma senza spunta o senza motivo. **Il server decide** (`creaOrdine`, nuovo campo `DatiNuovoOrdine.eccezioneOrari`): data passata → rifiuto sempre («Una data passata non si può concordare»); data chiusa senza motivo → rifiuto con l'invito a spuntare; con motivo → passa e scrive nella NOTA dell'ordine la riga `ECCEZIONE ORARI: consegna dom 13/09/2026 — Il negozio è chiuso di domenica. Concordato: …` (subito dopo CONSEGNA ANONIMA, in maiuscolo) e l'attributo **`Eccezione_Orari`** = motivo (la nota per le persone, l'attributo per Orders/piattaforma). La rotta interna `/api/nuovo-ordine` passa il campo; `/api/v1/nuovo-ordine` NO (le altre app non hanno l'operatore davanti: una data chiusa resta rifiutata col motivo). Il blocco della spunta sta FUORI dalla label del Giorno (label dentro label non è HTML valido) e prende tutta la riga della griglia. `tsc` 0. Documentato in README, docs/MANUALE.html, MANUALE-DELUXY.html. ⚠️ Non visto a schermo (login): provare su /nuovo-ordine con un negozio che ha orari salvati e una domenica.
> ✅ **10/09/2026 15:56 — ORARI NEGOZI IN PRODUZIONE** (col «fai push & deploy» dell'utente): commit `693b46f4` su `origin/scout-ui` (il ramo era già stato pushato da un'altra sessione insieme ai suoi commit Finance; `origin/scout-ui..HEAD` vuoto), deploy **`deluxy-messaging-7y8kukmeg`** (`vercel deploy --prod --yes` dalla cartella; il primo tentativo ha stampato «fetch failed» DOPO il build, ma il deploy era stato creato ed è Ready — controllare `vercel ls` prima di rilanciare). Alias verificato con `vercel inspect` (→ 7y8kukmeg) e dal vivo: `/api/v1/orari-negozi` senza chiave risponde 401 col nostro messaggio, `/orari-negozi` 307 al login, `/api/health` ok. ⚠️ La tabella `OrarioNegozio` è vuota: finché un amministratore non salva un negozio, in produzione non cambia niente per nessuno.
> ⭐ **10/09/2026 pomeriggio — ORARI NEGOZI** (richiesta dell'utente: «crea una sezione ORARI NEGOZI dove impostiamo per i negozi shopify apertura del negozio (quindi se la data è selezionabile), fasce orarie di consegna con orario minimo e massimo, giorni di chiusura»). Nuova tabella `OrarioNegozio` (una riga per negozio, FK su `NegozioShopify` con cascade; `giorniApertura` "1,2,3,4,5,6" con 0 = domenica, `fasce` e `giorniChiusura` JSON in colonne di testo) — creata in produzione con `scripts/applica-migrazione-orari-negozi.mjs` (solo additiva; ⚠️ mai `prisma db push` qui). Le REGOLE stanno in **`src/lib/orari-regole.ts`**, senza import dal server (le usa anche il client): `validaOrario` (zero giorni aperti si RIFIUTA, zero fasce si accetta, fascia col massimo prima del minimo no, doppioni no), `giornoSelezionabile` (tre motivi a parole: passato · chiusura con motivo, «ogni anno» confronta MM-GG · giorno della settimana chiuso; le date si confrontano come stringhe AAAA-MM-GG, mai come Date, per il fuso UTC del server), `etichettaFascia` («08:00»→«08-12», come gli attributi `Fascia_Oraria_Consegna`), `prossimiGiorni`, `primoGiornoAperto`. `src/lib/orari-negozi.ts` legge/scrive; **`orarioConfigurato()` torna null senza riga, e senza riga NON c'è regola** (né in Nuovo ordine né in `creaOrdine`): `ORARIO_PREDEFINITO` (7 giorni, 08-12·12-16·16-20) è solo il punto di partenza della scheda — ⚠️ la prima versione aveva «lun–sab» come default APPLICATO, e avrebbe chiuso la domenica a tutti i negozi senza che nessuno lo decidesse (trappola del default che si spaccia per decisione); corretto prima del commit, la prova sul DB l'ha mostrato. Rotte: `GET/PUT/DELETE /api/orari-negozi` (lettura per tutti, scrittura admin — il cancello nella rotta), **`GET /api/v1/orari-negozi?dominio=&data=`** con chiave ApiKey per le altre app (esce anche `configurato` e `primoGiornoAperto`). Pagina **`/orari-negozi`** (`OrariNegozi.tsx`: una scheda per negozio, giorni a pillole lun→dom, fasce con due `type=time` ed etichetta, chiusure con data/motivo/«ogni anno», anteprima 14 giorni a badge verde/rosso col motivo nel title, Salva/Annulla/«Cancella gli orari» con `Conferma`); voce «Orari negozi» nel menu Configurazione sotto Negozi (admin). **Aggancio a Nuovo ordine**: la tendina delle fasce prende quelle impostate — SOLO per i negozi con `configurato = true` (ripiego: `fascePerNegozio` storico), sotto il campo Giorno si legge il motivo se la data non va, `crea()` si ferma, e **il divieto sta nel server**: `creaOrdine` in `nuovo-ordine.ts` rifiuta la data chiusa — quindi vale anche per riconsegne, preventivi e `/api/v1/nuovo-ordine`; senza data non si controlla (caso ammesso). Prove: **`npx tsx scripts/prova-orari-negozi.mts`, 27 casi, tutti passati**; `tsc` 0; dal dev server `/orari-negozi` e `/api/orari-negozi` senza sessione → 307 al login, `/api/v1/orari-negozi` senza chiave → 401. Documentato in README (sezione in cima), `docs/MANUALE.html` (riga menu + novità), `MANUALE-DELUXY.html` (registro), `scripts/README.md` (catalogo). ⚠️ **Non verificato a schermo da me**: la pagina sta dietro il login e non ho credenziali — da guardare su http://localhost:3140/orari-negozi (admin) e in /nuovo-ordine scegliendo una domenica. 🔴 **Decisioni aperte per l'utente**: (1) i **siti Shopify non leggono ancora questi orari** — il selettore data/fascia dei temi va agganciato all'API, e servirebbe una lettura SENZA chiave per il browser del cliente (oggi la v1 vuole la chiave); (2) il blocco in Nuovo ordine è **secco** (nessuna «eccezione concordata» per una data chiusa): se serve, si aggiunge una spunta esplicita, non si toglie il controllo.

> 📌 **10/09/2026 pomeriggio — RIPRESA (sessione nuova): solo lettura, riconteggio e memoria; nessun codice toccato.** Locale `71bae381` = `origin/scout-ui`, `npx tsc --noEmit` 0, dev server :3140 ok. Prod **`cp0fd8whd`** (= API fornitori qui sopra; senza chiave risponde 401). ✅ **Correzione a una riga di sotto**: il widget dell'08/09 (`deluxy-chat-pannello`) **è in produzione** dal deploy `31eepkqu3` del 09/09 — verificato leggendo `widget.js` in prod — quindi la nota «NON ancora pubblicato» del 08/09 è storia; resta da fare la misura su un sito vero (contatore su `fetch` a pannello chiuso). **Ricontato sul DB** (solo pieno/vuoto): ordini 1.623 → gestito 1.589 · da gestire 19 · in_app 6 · attesa consegna 4 · comunicazione 3 · in pagamento 1 · ricerca fornitore 1; **reclami 11 aperti**; utenti 3; ApiKey 5. `piattaformaApiKey` e `merchandisingApiKey` sono **PIENE** (i due «assenti» della memoria del 06/09 sono chiusi). 🔴 **CHARGEBACK: `#2714` (84,54 €) needs_response SCADE OGGI 10/09, prove mai inviate, bozza 0 caratteri; `#12829` (115,77 €) needs_response scade il 17/09, bozza 0; `#1741` under_review (prove inviate il 05/09).** ⚠️ **Da guardare**: `#2843` è `in_app` con `appConsegnaStato = cancelled` e consegna del 04/09 — la regola del 06/09 (43) dice che «cancelled» torna a noi, ma è fermo lì; gli altri 5 in_app hanno `appConsegnaStato` vuoto (#1835, #1827, #1834, #2905, #2904). 🔴 Business ancora senza credenziali Shopify (vedi MANCA). Memoria canonica aggiornata: `projects/C--Users-nicol-app/memory/progetto-deluxy-messaging.md`.

> ✅ **10/09/2026 — NUOVA API `GET /api/v1/fornitori?giorni=180` (sola lettura, chiave `ApiKey`) · IN PRODUZIONE** (commit `9b81aa1c`, `vercel deploy --prod`; senza chiave risponde «Chiave API mancante» dal codice; con la chiave `deluxy-scout` — creata oggi, sola lettura — 200 con 118 ordini su 98 fornitori). Per Scout (schermate Fornitori e Segnalazioni CS, richiesta dell'utente): per ogni fornitore a cui abbiamo affidato ordini, `ordini30 / ordiniLunga / venduto30 / vendutoLunga / ultimoIl / ultimoNumero`, con `id` (registro Anagrafiche, quando c'è) e `chiave` (`chiaveNome`). Conta l'ORDINE DATO (`Ordine.fornitoreNome`, data `fornitoreIl` con ripiego `data`), non il pagamento (quello è `fornitori-usati.ts`); fuori annullati e uniti (`unitoA`). **Non esce `fornitoreCosto`.** ⚠️ Misurato il 10/09 sul DB: 118 ordini in 180 giorni, 98 fornitori, **solo 6 con `fornitoreId`**: chi legge aggancia per nome. `tsc` 0. Per Scout serve la chiave: `npm run chiave -- deluxy-scout` (sola lettura). File: `src/app/api/v1/fornitori/route.ts`.

> 📍 **09/09/2026 — DOVE STA LA CHAT SUI TRE SITI (verificato dal vivo, e l'utente ha detto «come ora va bene»: NON si cambia).** Cercando `widget.js` avevo concluso che «su due siti la chat non c'è»: **era sbagliato**. La chat c'è su tutti e tre, in due forme diverse.
> · **cakedesign.me** — `widget.js` incorporato con `data-sito=cake`, `data-tema=minimale`, `data-accento=#9c5b3f`, `data-etichetta='Scrivici 🎂'` e **`data-bottone="no"`**: la bolla flottante è SPENTA e la chat si apre dal link già in pagina, agganciato con `data-apri-da="a[href*='chatting.page']"`. ⚠️ Quel link punta al **vecchio fornitore** (`chatting.page/zbcza0…`, testo «Live Chat») e `widget.js` lo dirotta al volo. Verificato oggi: l'elemento c'è (1 corrispondenza) e l'ospite del widget viene montato. **Fragilità da sapere**: il giorno che qualcuno toglie quel link dal menu, su cake la chat diventa irraggiungibile e **niente dà errore** — non c'è bolla di ripiego.
> · **deluxy.it** e **deluxyflowers.com** — nessuno `widget.js`. Nel menu contatti c'è un link (`<a class="… dialogify">`) alla pagina `/chat/<codice>` del CS: `uwwar1vl…` per deluxy.it, `3sp1k5o0…` per flowers. Provato in produzione: `/chat/3sp1k5o0…` risponde «Deluxy Flowers — chat» col saluto dell'atelier e il campo per scrivere, e sotto è `/widget?sito=flowers&tema=minimale&pagina=link&dominio=deluxyflowers.com`. **Differenza per il cliente**: su cake la chat si apre SOPRA il negozio, sugli altri due lo porta via su un altro dominio.
> · Tutti e tre hanno **`apreSulSito = false`** in `WidgetSito`, quindi `/chat/<codice>` non rimanda mai al negozio: nessun vicolo cieco. ✅ **RISOLTO IL DUBBIO (agente, 09/09)**: la riga «17/08/2026: deluxyflowers.com sì, cakedesign.me sì, deluxy.it no» — che stava sia in `README.md:2847` sia nel commento di `src/app/chat/[codice]/page.tsx` — **era sbagliata su Flowers, e non è una regressione**: dal connettore Shopify, il `layout/theme.liquid` del tema pubblicato di Flowers ha lo **stesso checksum** di «Version to work on 1.2» di giugno, quindi quello snippet non c'è **mai** stato. Probabilmente fu scritta guardando le schede in «Widget dei siti» invece dei siti veri. **Le due righe sono state corrette** con la misura e con l'avvertimento: ⚠️ **non accendere `apreSulSito`** per deluxy.it e flowers finché lo snippet non è davvero nel tema, o il link manderebbe i clienti su una vetrina senza chat. ⚠️ Punto cieco dichiarato dall'agente: su Flowers è attiva l'app embed `cmp-insert-code` (iniezione di codice arbitrario) il cui contenuto non è leggibile dai file del tema — ma l'HTML servito oggi non contiene `widget.js`, quindi comunque oggi non c'è.
> ✅ **Regressione controllata sulla modifica di stamattina**: la pagina `/chat/<codice>` ospita il widget SENZA `widget.js` attorno, quindi nessuno le manda `deluxy-chat-pannello`. Il valore di partenza `pannelloAperto = true` — messo per i siti con lo snippet vecchio in cache — copre anche questo caso: provata in produzione, la chat risponde. Con il valore opposto il link che mandiamo ai clienti sarebbe rimasto muto.
> ⭐ **09/09/2026 — SINCRONIZZAZIONE COL DELIVERY: consegnata di là = «Gestito» qui, davvero** (utente: «quando un ordine è in app accettato da un partner e poi viene consegnato dal valet o dal partner in automatico deve andare in gestito»). La regola c'era dal 06/09 e **non bastava**, per due motivi indipendenti, tutti e due misurati.
> 1️⃣ **Gli stati «consegnata» sono TRE, ne guardavamo uno.** La piattaforma usa `delivered`, `approved` e **`delivered_time_to_approve`** («consegnata, ore del valet ancora DA approvare»), e le tratta insieme lei stessa (`deliveries.service.ts:2362` chiude tutte le attività su quei tre; `puntualita.ts` li conta fra le CONCLUSE). Contati il 09/09: **3** consegne in `delivered_time_to_approve` e **1.257** in `approved`. Un ordine consegnato da un valet che dichiara le ore restava «In App» finché non gliele approvavano. Ora c'è `STATI_CONSEGNATA` + `eConsegnata()` in `piattaforma.ts`, usati sia da `sync-piattaforma.ts` sia da `consegne-piattaforma.ts` (`STATI_CHE_CHIUDONO` = i tre + `in_delivery`). ⚠️ NON si usa la lista «chiusa» della piattaforma: comprende anche `not_delivered`, `cancelled` e `archived`, e due di quelle devono TORNARE a noi.
> 2️⃣ **I due giri di cron guardano l'orologio SBAGLIATO, e nessuno dei due può rimediare.** La sync delle vendite è incrementale su `Sale.updatedAt`; ma quando il valet chiude, di là si muove la **consegna**, non la vendita. Caso vero **#12905**: vendita ferma su «accettata» dalle **07:54** dell'08/09, consegna passata a `delivered` alle **10:04** — due ore dopo, quindi già dietro al segnaposto. Quella vendita non è più stata letta e l'ordine è rimasto «In App» un giorno intero. Rimedio: **giro di RECUPERO** in fondo a `sincronizzaConPiattaforma`, che parte da casa nostra — prende i nostri ordini fermi in `in_app` (oggi **5**), i più vecchi per primi, e per ognuno chiede alla piattaforma `GET /api/v1/app/vendite/by-ref/deluxy-orders/<id>` passando l'esito allo stesso `allineaUno`. Tetto **`TETTO_RECUPERO = 40`** chiamate a giro, e se ce ne sono di più **lo dice** nell'esito. In un `try`: un guasto qui non fa perdere il segnaposto del giro principale.
> ✅ Verificato in locale con un giro in **prova** (non scrive niente): «lette 6 · gestite 1 · RECUPERATE 1 · recupero · #12905: consegnata di là (Martesana Milano) → Gestito». Prove nuove: **`npx tsx scripts/prova-stati-consegnata.mts`** (9 stati + coerenza fra le due sync, tutte passate). ✅ **LIVE `deluxy-messaging-31eepkqu3`** (09/09 11:20, alias verificato). ✅ **E HA FUNZIONATO DA SOLO**: il cron delle **09:27:42 UTC** (il primo col codice nuovo) ha chiuso #12905 — `esito: lette 6 · gestite 1 · **recuperate 1**`, ordine `gestito`, `gestioneDaNome: 'Piattaforma consegne'`. Il conteggio «recuperate» dice che è stato il giro NUOVO e non il cursore: senza, quell'ordine sarebbe ancora fermo.
> 🪤 **Trappola presa in faccia**: `sync-piattaforma.ts` ha i fine riga **misti** (4 CRLF su 419 righe). Un patcher che deduce il fine riga da `t.includes('
')` sceglie CRLF e **non trova più niente** in un file quasi tutto LF. Su questo repo si applica con testo esatto, senza dedurre.
> ✅ **09/09/2026 — SISTEMATO: il modulo pagamenti legge gli IBAN anche dal registro** (utente: «sistema pure»). `partnerAttivi` ha l'opzione **`conDatiFinanziari`, predefinita a FALSO**, e `normalizza` butta `iban`/`intestatarioConto` se non sono stati chiesti — seconda serratura dopo quella del registro. La chiede **una sola rotta**, `/api/fornitori/cerca`, che gira sul server; ⚠️ `/api/partner` (tendina reclami, risposta che va nel browser) NON la chiede: verificato sull'app, **96 partner e 0 IBAN**. Nuovo campo `FornitoreTrovato.ibanDa` (`pagamento` | `registro`): il modulo scrive a schermo da dove viene l'IBAN, con un avviso oro sotto il campo quando è del registro («da qui non gli abbiamo mai fatto un bonifico»), perché un IBAN provato da un bonifico e uno dichiarato in anagrafica sono due gradi di fiducia diversi davanti alla stessa casella piena. Regole tenute ferme in `unisci`/`scegliIban`: il pagamento batte sempre il registro (anche se il registro arriva prima), e con **IBAN discordi** non si propone nessuno — il registro non fa da arbitro. Prova: **`npx tsx scripts/prova-iban-da-registro.mts`, 6 casi, tutti passati**. 🔧 Corretto per strada un difetto che c'era già e riguarda i soldi: scegliendo il fornitore A (IBAN compilato) e poi il fornitore B senza IBAN, **il campo restava pieno con le coordinate di A** mentre l'avviso diceva «va scritto a mano» — ora si azzera.
> ✅ **AMBITO CONCESSO il 09/09 su ok dell'utente** («si fai pure»): `leggeDatiFinanziari = true` sulla chiave `deluxy-messaging` (UPDATE diretto su `ApiKey` del registro — non c'è una rotta per farlo; annotato in SEGNALAZIONI-SICUREZZA.md). Ora ce l'hanno tre chiavi su undici: delivery, messaging, partner. **Verificato in produzione**: «FAG Torino Fiori» → `IT25B0…4971` conto «EMMEGIFLOWERS SRL», «Il Fiore Di Fabula» → conto «Martina Cozzella» (due nomi sul conto DIVERSI dall'insegna), `/api/partner` resta a 96 partner e **0 IBAN**. ✅ **LIVE `deluxy-messaging-31eepkqu3`**. ~~la chiave aveva `leggeDatiFinanziari = false`~~, quindi il blocco `datiFinanziari` torna `null` e il codice è **inerte** — verificato oggi: «FAG Torino Fiori» torna ancora con IBAN vuoto. L'ambito fu ristretto il 27/08 con una motivazione seria («col registro usato anche per i FORNITORI è una superficie da frode»), quindi la richiesta è registrata in `deluxy-design-system/SEGNALAZIONI-SICUREZZA.md` («AMBITO DA CONCEDERE») e **decide l'utente**. Si concede in Anagrafiche sulla chiave `deluxy-messaging`. ⚠️ **In locale, non pubblicato.**
> 🔎 **09/09/2026 — «per Petit Jardin doveva già risultare l'IBAN salvato?» (domanda dell'utente): NO, e la verifica lo dimostra — ma sotto c'è un buco vero.** Il modulo pagamenti prende l'IBAN **solo** dalle nostre `RichiestaPagamento` passate (`/api/fornitori/cerca`, fonte 1; il commento in testa alla rotta dice «Il registro NON ha gli IBAN»). Per Petit Jardin - Fioreria non c'era nessun pagamento precedente: la scheda nel registro esiste dal **31/08** come `prospect` (CUNEO) ma **senza IBAN**, e l'IBAN `IT56U08397…1930` è comparso **oggi alle 09:23** scritto dalla richiesta stessa (`RichiestaPagamento` creata 09:16, pagata 09:23, `registroIl` 09:23:23.713 ≡ `Partner.aggiornatoIl` 09:23:23.503). Quindi al momento dello screenshot non l'avevamo da nessuna parte e l'avviso era corretto. Controprova: **adesso** `GET /api/fornitori/cerca?q=Petit Jardin` in produzione risponde con `iban: IT56U0…1930, ibanDiversi: 1`.
> ⚠️⚠️ **IL BUCO: il CS SCRIVE gli IBAN nel registro e non li RILEGGE mai.** `registro-fornitori.ts:371` manda `iban` + `intestatarioConto` ad Anagrafiche a ogni pagamento verificato, ma `src/lib/anagrafiche.ts` **non mappa nessuno dei due** e `/api/fornitori/cerca` non li guarda. Misurato il 09/09 (raw SQL qualificato, `current_schema()` era `mail`): il registro ha **80 IBAN distinti** su 1.179 partner, i pagamenti del CS ne conoscono **70**, e **24 IBAN distinti stanno SOLO nel registro** — fra questi partner che usiamo di continuo: Enrico Rizzi Milano, Il Fiore Di Fabula, FAG Torino Fiori, MARYFLOR di GERARDI VINCENZO, Angolo Fiorito, Mastrofioraio, Cakelab Firenze, Lijoi Roma. Per tutti e 24 il modulo dice «l'IBAN no — va scritto a mano» pur avendolo il registro. **Proposta, NON applicata** (decide l'utente): in `partnerAttivi` mappare `iban` e `intestatarioConto`, e in `/api/fornitori/cerca` usarli come **secondo** ripiego dopo i nostri pagamenti — mai al posto loro, perché l'IBAN che abbiamo davvero usato vale più di quello scritto in anagrafica, e con più IBAN diversi si continua a non proporne nessuno.
> ⚡ **08/09/2026 sera — IL WIDGET SUI SITI DEI CLIENTI NON SONDA PIÙ A VUOTO** (chiesto dal custode delle prestazioni: è l'unico punto dell'ecosistema che cresce coi CLIENTI). L'iframe `/widget` è appeso al DOM appena la pagina del cliente si carica, e sondava `/api/widget/messaggi` ogni 3,5 s senza nessuna guardia: un visitatore che ha scritto una volta (token in `localStorage`) continuava a chiamarci **a pannello chiuso, su ogni pagina, anche in secondo piano** — e a pannello chiuso non c'è niente da mostrare, sul bottone NON esiste nessun bollino di messaggio nuovo. ⚠️ `document.hidden` da solo non basta: dice se la SCHEDA è in secondo piano, non se il pannello è chiuso. Rimedio in due pezzi: `public/widget.js` manda all'iframe `{tipo:'deluxy-chat-pannello', aperto}` a ogni apertura/chiusura e al `load`; `src/app/widget/page.tsx` sonda solo con **token + scheda in primo piano + pannello aperto**, e aggiorna SUBITO al ritorno sulla scheda o alla riapertura. `pannelloAperto` parte da **`true`** apposta: chi ha lo snippet vecchio in cache non manda il messaggio, e per quei siti tutto resta come prima. Misurato sul dev server con un contatore su `fetch`, 11 s per stato: scheda nascosta **3 → 0**, pannello chiuso **3 → 0**, pannello aperto e scheda davanti **3 → 4** (ritmo intatto), riapertura **1 chiamata entro 500 ms**. Scala: 56 conversazioni widget (45 attive in 30 giorni) su 3 siti. ⚠️ Difetto mio, trovato provando: la prima versione saltava anche la PRIMA lettura a scheda nascosta e il widget restava su «sto caricando» — ora la prima si fa sempre.
> ⚡ **08/09/2026 sera — la rubrica clienti leggeva 78 colonne per usarne 9.** `src/app/api/clienti/route.ts`: `db.ordine.findMany` senza `select`. Misurato a database tranquillo su 1.587 ordini: **0,88 MB e 266/166/161 ms** contro **0,13 MB e 88/85/90 ms**. ⚠️ Il `take` NON si può mettere: qui si RAGGRUPPA per persona e si sommano spesa e conteggi — tagliare darebbe totali sbagliati. Controllo di non-regressione: 1.374 gruppi contro 1.374, zero diversi, 295.836,36 € contro 295.836,36 €. 📦 **Su `origin/scout-ui` come `7f9f33af`** (08/09 sera, cherry-pick di un'altra sessione col metodo del worktree staccato, autore e data originali; verificato da qui: `git diff origin/scout-ui` sui quattro file è **vuoto**). ~~⚠️ **NON ancora pubblicato su Vercel**: il deploy lo decide l'utente, e il widget gira sui siti dei CLIENTI.~~ ✅ **10/09: è in produzione** dal deploy `31eepkqu3` del 09/09 (verificato su `widget.js` in prod). ⚠️ Quando si pubblica, la misura che chiude il cerchio è il contatore su `fetch` **su un sito vero**: a pannello chiuso deve restare a zero, non solo sul dev server. ⚠️ Trappola vista da chi ha pushato: su questo repo un `diff -q` fra checkout fresco e albero di lavoro **mente** per la conversione CRLF di Windows — `git diff` no. Le risposte complete alle sei domande del custode stanno in `deluxy-design-system/SEGNALAZIONI-PERFORMANCE.md`, sezione del 08/09; lì c'è anche la cosa che NON ho applicato (le due chiamate a Orders di `dettaglio-ordine.ts` non sono la stessa richiesta: una porta `annullati=inclusi`, unirle cambia il comportamento).
> ⭐ **08/09/2026 — RECLAMO: SU QUALI PRODOTTI, E IL PARTNER DELLA PIATTAFORMA** (due richieste dell'utente: «in caso di reclamo di una vendita con più prodotti fai scegliere uno o più prodotti su cui è aperto il reclamo» e «in scelta partner segnala tutti i partner attivi se l'ordine è stato gestito da app associa già il reclamo al partner che l'ha fatto»). Nuova colonna `Reclamo.prodotti` (`String @default("") @db.Text`, creata in prod con `ALTER TABLE … ADD COLUMN IF NOT EXISTS` — ⚠️ mai `prisma db push` qui: propone di togliere tutte le foreign key). Nuova rotta **`GET /api/reclami/contesto?ordine=&ordineId=`**: legge la nostra copia dell'ordine (`numero`, `shopifyId`, `appPartner`), chiede le righe a Orders con `righeOrdineDaOrders` e cerca il partner in Anagrafiche. Non fallisce mai in blocco: se Orders tace tornano campi vuoti e una nota. `ReclamiLista.tsx`: un `useEffect` con **500 ms di attesa** chiede il contesto ogni volta che il form è aperto e c'è un numero d'ordine (prefill da chat, apertura in modifica, o digitato a mano); il blocco «Su quali prodotti» compare **solo con ≥2 righe** (etichetta `N × Titolo · Variante`, SKU sotto), le spunte si salvano una per riga in `bozza.prodotti`, nessuna spunta = tutto l'ordine (ed è scritto); i prodotti scelti si rileggono in elenco sotto la casistica. Il partner si propone **solo su un reclamo nuovo** (`proponiPartner = !bozza.id`) e **solo se la colpa è ancora `nessuno`**, con l'avviso «Dalla piattaforma consegne: l'ordine è stato preparato da ‹nome›».
> ⚠️⚠️ **Trappola trovata e chiusa nello stesso giro (misurata sull'ordine #12901)**: la ricerca del partner era su `stato: 'tutti'` e il registro Anagrafiche ha **schede doppie** per la stessa azienda — «Enrico Rizzi» (attiva, `cmrovrthj…`) e «Enrico Rizzi Milano» (`cmruwipns…`, non attiva). Il nome della piattaforma combaciava alla lettera con la seconda, il reclamo si prendeva quell'id, e nella tendina — che mostra i soli **88 attivi** — il campo restava **vuoto**: colpa scritta addosso a una scheda morta. Ora si cerca **solo fra gli attivi**, cioè la stessa popolazione della tendina, con `paroleTrovate` di `cerca-fornitore` e la soglia `0,7 × parole`, accettando solo un vincitore unico. Verificato su 8 ordini: 6 su 6 dei partner presenti fra gli attivi ora combaciano con un'opzione della tendina (#12905 «MARTESANA», #12904 «MARYFLOR di GERARDI VINCENZO», #12901 «Enrico Rizzi»); «Artista Locale» e «Angolo Fiorito» **non sono fra gli 88 attivi** e giustamente non vengono proposti — il nome si legge lo stesso e il modulo chiede di scegliere. Aggiunto anche l'avviso se il registro dichiarasse più partner di quanti ne arrivano (la pagina è da 200, oggi sono 88: mai un troncamento muto).
> Verificato in locale su :3140 con l'ordine **#12901** (2 righe, partner Enrico Rizzi): il blocco compare, la spunta scrive «Il reclamo è su 1 prodotto di 2», la tendina partner mostra «Enrico Rizzi · MILANO», `npx tsc --noEmit` pulito, e giro completo scrittura/rilettura/cancellazione sulla colonna `prodotti`. ✅ **LIVE `deluxy-messaging-gdn0x4gb6`** (08/09 18:11, alias `deluxy-messaging.vercel.app` verificato: `GET /api/reclami/contesto?ordine=%2312901` in produzione risponde 200 con i due prodotti e `partner.id = cmrovrthj00udi6pcsygleazy` «Enrico Rizzi», `nelRegistro: true`). Pushato su `origin/scout-ui` come commit **13434670** (worktree temporaneo su origin, refspec `cs-reclami-0809:scout-ui`, avanti di 1 e indietro di 0 — nessun merge, nessun doppione degli altri 71 commit locali). Nello stesso push è andato in produzione anche il tetto di connessioni del CS (`urlPooler()` in `db.ts`, cron contatti dal minuto 7 al 52). ⚠️ Il MANUALE-DELUXY.html di origin aveva due righe (Transactions, AI Mail) che il ramo locale non ha: la riga nuova è stata **innestata** nella copia di origin, non è stato preso il file intero.
> ⭐ **07/09/2026 sera — L'INTESTATARIO DEL CONTO PUÒ ESSERE DIVERSO DAL FORNITORE** (regola dell'utente: «l'intestatario conto di un fornitore può essere diverso da ragione sociale quindi deve essere consentito»). Su `RichiestaPagamento` c'era UN nome (`intestatario`) che faceva due mestieri: beneficiario del bonifico e fornitore che prepara l'ordine. Riscrivere sul conto il nome giusto per la banca («Mario Rossi» per «C&G Sweet Bakery») azzerava il fornitore scelto nel modulo e il POST rispondeva 409. Ora sono due campi: nuova colonna `RichiestaPagamento.fornitore` (creata in prod con `scripts/applica-fornitore-pagamento.mjs`; le 92 righe vecchie restano vuote = vale l'intestatario, ripiego unico in `chiPrepara()` di `src/lib/chi-prepara.ts`). Modulo: lo stato `intestatarioScelto` si chiama `fornitoreScelto`, riscrivere l'intestatario NON lo cancella più, sotto il campo si legge «diverso dal fornitore scelto: va bene…» con «Rimetti X»; `usaFornitore` compila l'intestatario col nome sul conto dell'ultimo pagamento (`FornitoreTrovato.intestatarioConto`, nuovo, dai pagamenti insieme all'IBAN). Server: 409, `riconcilia.ts`, `decidi()`, riconciliazione a mano, `registro-fornitori` (nome = fornitore, `intestatarioConto` = intestatario, campo che Anagrafiche aveva già), `fornitori-usati`, `fornitori-da-collegare`, `consegne-da-pagamenti`, ricerca `/api/fornitori/cerca` (cerca su tutti e due i nomi, raggruppa per chi prepara) usano `chiPrepara`. A Transactions va sempre l'intestatario come `beneficiario`. Tabella pagamenti: sotto l'intestatario «per ‹fornitore›» quando differiscono; il rosso sul fornitore dell'ordine confronta il fornitore, non il nome della banca. Prova: `npx tsx scripts/prova-ordine-obbligatorio.mts` (caso nuovo «intestatario diverso dal fornitore scelto passa»). ⚠️ Il codice sta nel commit **820c8eb0** («Manuale: vendita accettata → consegna sempre…»): il messaggio è di un'altra sessione che ha committato mentre i miei file erano nell'indice — in `scoutwt` l'indice git è UNO per tutte le cartelle, quindi `git add` e `git commit` vanno nello stesso comando. ✅ **LIVE `deluxy-messaging-h97iezp6x`** (07/09 16:36, col «lancia i due comandi» dell'utente; push già fatto dall'altra sessione, origin/scout-ui = 6b23cfe5).
> 🔧 **07/09/2026 sera — dopo «No, è un fornitore nuovo» si cerca su Google Maps** (utente: «al click di no è un fornitore nuovo metti possibilità di ricercarlo tramite maps come prima»): con i tre passi chi diceva «nuovo» passava alle coordinate e Maps spariva. `CercaFornitore` ha la prop `soloMaps` (niente ricerca in casa, barra Maps sempre a vista, Invio = cerca, bottone «Cerca su Google Maps»); al passo 2, se `dichiaratoNuovo`, in cima c'è «Lo troviamo su Google Maps?» con quella casella: scegliendo un risultato `usaFornitore` compila nome/città/telefono, tiene la scheda per Anagrafiche e passa al modulo. ✅ LIVE `deluxy-messaging-3hkbjs9bw` (commit 14a626de, pushato).
> 🔧 **07/09/2026 sera — «è un fornitore nuovo l'ho già scritto prima»** (utente, con la schermata di #1833 / Monopoli Angelo): al passo 1 «No, è un fornitore nuovo →» cambiava solo pagina, e al passo 3 ricompariva «Da dove viene…?» col secondo bottone. Ora `dichiaratoNuovo` (stato in `RichiediPagamento.tsx`) tiene la risposta: `fornitoreEffettivo = fornitoreScelto || (dichiaratoNuovo ? intestatario : '')` è quello che passa a `cosaManca`, al POST/PATCH (`fornitore`) e alla domanda, che con la dichiarazione non compare; la testata del modulo dice «Fornitore nuovo (detto al passo 1)». Decade scegliendo uno dei nostri, correggendo una riga o svuotando il modulo. ✅ LIVE `deluxy-messaging-ipwuznc7q` (commit 550de03b, pushato).
...
a posto col caso nuovo; `npm run build` ok; `scripts/prova-intestatario-diverso.mts`
(nuovo) tutto a posto. A schermo NON provato: il locale chiede il login e le
password non le inserisce una sessione. **Stato**: pushato e **LIVE**
`deluxy-messaging-h97iezp6x` (16:36). Da provare dall'utente: Pagamenti → Paga
fornitore, riscrivere l'intestatario e salvare.
