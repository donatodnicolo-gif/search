...
> · Tutti e tre hanno **`apreSulSito = false`** in `WidgetSito`, quindi `/chat/<codice>` non rimanda mai al negozio: nessun vicolo cieco. ⚠️ Il commento in `src/app/chat/[codice]/page.tsx` dice «verificato il 17/08/2026: deluxyflowers.com sì, cakedesign.me sì, deluxy.it no» — **oggi flowers non ha più lo snippet**: o è una regressione del tema, o quella verifica riguardava altro. Non è un problema finché `apreSulSito` resta falso, ma se un giorno lo si accende per flowers **il link manderebbe i clienti su una vetrina senza chat**.
