...
{
  const r = decidiPallini({ '/inbox': { ultimo: A }, '/ordini': { ultimo: A } }, {}, '/', true)
...
console.log('\n=== POI ===')
{
  const r = decidiPallini({ '/inbox': { ultimo: B }, '/ordini': { ultimo: A } }, { '/inbox': A, '/ordini': A }, '/', false)
...
console.log('\n=== STANDO SULLA PAGINA ===')
{
  const r = decidiPallini({ '/inbox': { ultimo: B } }, { '/inbox': A }, '/inbox', false)
...
{
  const r = decidiPallini({ '/inbox': { ultimo: B } }, { '/inbox': A }, '/inbox?c=abc', false)
...
{
  const r = decidiPallini({ '/reclami': { ultimo: B } }, { '/reclami': A }, '/reclami/casistiche', false)
  prova('una sotto-pagina conta come «ci sono sopra»', r.accesi.length === 0)
}

console.log('\n=== SEZIONI VUOTE ===')
{
  const r = decidiPallini({ '/chiamate': { ultimo: '' }, '/inbox': { ultimo: B } }, { '/inbox': A }, '/', false)
...
{
  const visto = { '/inbox': A }
  const r = decidiPallini({ '/inbox': { ultimo: B } }, visto, '/', false)
