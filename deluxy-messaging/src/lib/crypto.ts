...
// I token Meta servono in chiaro al momento della chiamata alle Graph API,
// quindi non basta un hash: vanno cifrati e poi decifrati. AES-256-GCM con una
// chiave derivata da APP_SECRET, che sta solo nell'ambiente del server.

function chiave(): Buffer {
  const segreto = process.env.APP_SECRET
  if (!segreto) {
    throw new Error(
      'APP_SECRET mancante: impossibile cifrare i token dei canali. Vedi .env.example.'
    )
  }
  // scrypt normalizza qualsiasi lunghezza di segreto a 32 byte.
  return crypto.scryptSync(segreto, 'deluxy-messaging', 32)
}
