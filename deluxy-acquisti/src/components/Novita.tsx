...
import { leggiIo } from "./Identita";
import { avvisaSessioneScaduta } from "./PalliniNav";

// ── I RIQUADRI IN BASSO A DESTRA: «è appena arrivata una richiesta» ──
//
// Versione snella dei riquadri del Customer Service (deluxy-messaging,
// Novita.tsx). Qui hanno senso perché una richiesta di acquisto la scrive
// quasi sempre qualcun altro — un collega o un'altra app via API — e chi deve
// approvarla non lo sa finché non ricarica la pagina. Il pallino sulla voce di
// menu resta (segnalibro), il riquadro dice cosa è appena successo e sparisce.
//
...
export function Novita() {
  ...
  const chiedi = useCallback(async () => {
    ...
    try {
      ...
      // ⚠️ Le richieste fatte da me non mi si annunciano: chi ha appena premuto
      // «Invia richiesta» sa benissimo di averla inviata, e un avviso per una
      // cosa fatta da sé fa sembrare gli avvisi rumore. «Io» qui è l'identità
      // leggera della topbar (localStorage), la stessa che firma le richieste.
      const io = leggiIo().email.trim().toLowerCase();
      let nuove = (d.novita ?? []).filter((a) => !viste.current.has(a.id));
      if (io) nuove = nuove.filter((a) => (a.autore ?? "").trim().toLowerCase() !== io);
      ...
      setCoda((c) => {
        ...
        if (nuove.length > TETTO) {
          return [
            {
              id: `riassunto:${adesso}`,
              titolo: `${nuove.length}${d.troncato ? "+" : ""} richieste nuove`,
              dettaglio: "Arrivate mentre non guardavi",
