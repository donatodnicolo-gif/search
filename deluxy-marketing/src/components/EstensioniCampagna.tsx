      ...
      {righe.length === 0 ? (
        ...
        <>
          <div className="kpi-riga" style={{ marginBottom: 14 }}>
            <div className="kpi">
              <div className="kpi-valore">{titoli.length}<span style={{ fontSize: 15, color: "var(--text-tertiary)" }}>/15</span></div>
              <div className="kpi-etichetta">Titoli · max 30 caratteri</div>
            </div>
            <div className="kpi">
              <div className="kpi-valore">{descrizioni.length}<span style={{ fontSize: 15, color: "var(--text-tertiary)" }}>/4</span></div>
              <div className="kpi-etichetta">Descrizioni · max 90 caratteri</div>
            </div>
            {conteggi.map((c) => (
          ...
          {(titoli.length > 0 || descrizioni.length > 0) && (
            <div style={{ marginBottom: 14 }}>
              ...
              <ul className="storia">
                {[...titoli].sort(ordina).concat([...descrizioni].sort(ordina)).map((t) => (
                  <li key={t.id}>
                    ...
                    <span className="storia-autore" style={{ display: "flex", gap: 10, alignItems: "center" }}>
                      ...
                      <span style={(t.caratteri ?? 0) > (t.tipo === "titolo" ? 30 : 90) ? { color: "var(--red)", fontWeight: 600 } : undefined}>
                        {t.caratteri} car.
                      </span>
                    </span>
                  </li>
