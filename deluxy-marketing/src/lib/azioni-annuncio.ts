...
export async function creaAnnuncioConAi(input: {
  ...
  const esito = await chiediAllAi({
    istruzioni: [
      ...
      "- I titoli devono coprire angoli DIVERSI (prodotto, consegna, occasione, prova sociale, chiamata all'azione): quindici modi di dire la stessa cosa non servono a Google, che li ruota — e i quasi-doppioni ABBASSANO l'Ad Strength.",
      "- Per il giudizio «Eccellente» di Google: le keyword principali del gruppo compaiono TESTUALI in almeno 5 titoli, in modo naturale; la maggior parte dei titoli sfrutta i 20-30 caratteri, 2-3 restano corti; le 4 descrizioni sono PIENE (70-90 caratteri).",
