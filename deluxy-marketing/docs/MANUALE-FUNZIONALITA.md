...
|---|---|---|
| 2026-09-11 | **«Confronta con» su tutte le pagine che hanno il periodo**: home, elenco campagne, scheda campagna e scheda gruppo — prima c'era solo sulla dashboard di brand. La scelta è una sola per tutta l'app, come il periodo. Nell'**elenco campagne** ogni card mostra la variazione di **spesa** e **ricavi** rispetto al confronto (sulla spesa il verde vuol dire *meno* speso): si legge in una query sola per tutte le campagne, non una per card, e con «nessuno» non si fa affatto | `/`, `/campagne`, `campagne/[id]`, `gruppi/[id]` |
| 2026-09-11 | **Il confronto si sceglie nei filtri, e arriva dentro le tabelle**: accanto al periodo c'è «Confronta con» — **periodo precedente · anno precedente · personalizzato · nessuno** — e la scelta si ricorda in tutta l'app come il periodo, e viaggia nel link. Le tabelle per categoria e per area mostrano la **variazione** sotto incasso e spesa (sulla spesa il verde vuol dire *meno* speso: lì scendere è un miglioramento), e le tessere mostrano **un** delta — quello scelto — invece dei due fissi che c'erano sempre. Con «Nessuno» la seconda lettura **non si fa**: chi cerca una riga non paga il conto di un confronto che non guarda | `lib/periodo.ts`, `lib/periodo-condiviso.ts`, `components/SceltaPeriodo`, `components/Delta`, `BrandPerCategoria`, `BrandPerArea` |
| 2026-09-11 | **Il pallino colorato non sballa più i titoli**: `.sb-dot` è la classe della sidebar e porta `margin: 0 auto`; usata come pallino generico dentro un contenitore flex si mangiava tutto lo spazio libero — **600,4 px per lato**, misurati — e sulla dashboard di un brand il titolo «Gifts» finiva all'estrema destra col pallino in mezzo alla riga. Ora il centraggio vale solo dentro la sidebar e c'è `.pallino` per tutti gli altri usi (erano **nove** pagine) | `globals.css`, `brand/[brand]` |
| 2026-09-11 | **Su una campagna Meta non compaiono più consigli e testi di Google**: «Prossime azioni» proponeva «Aggiungere snippet» (un'estensione che su Meta non esiste) perché il conteggio era zero — e su Meta è zero per costruzione; il budget era etichettato «al giorno su Google»; e il check VALORE vs NUMERO diceva «lancia lo script di Google Ads», che su Meta non c'è (là la strategia la legge l'app dalla Graph API). Trovati verificando in produzione dopo il deploy | `lib/opportunita.ts`, `campagne/[id]`, `lib/guardrail.ts` |
| 2026-09-11 | **Diagnostica delle creatività su ogni inserzione Meta**: i tre voti di pertinenza (qualità, coinvolgimento, conversione), il CTR sul link, e per i video la **curva di ritenzione** (quanti arrivano al 25 / 50 / 75 / 100% e quanto se ne perde). È l'unico segnale nativo di «creatività stanca»: senza, per capire se un annuncio ha smesso di funzionare si guardava la spesa e si indovinava. ⚠️ «Non giudicabile» (poche impression) **non** si colora di rosso: bocciare una creatività che nessuno ha visto farebbe spegnere quella sbagliata. Letta viva e messa in cache mezz'ora — **non si salva**, perché servirebbero sei colonne nuove per numeri che Meta ricalcola ogni giorno | `lib/meta.ts` (`leggiDiagnosticaAnnunciMeta`), `components/AnnunciMeta` |
| 2026-09-11 | **Il modulo di lancio Meta dice quali scelte sono irreversibili**: dopo la pubblicazione l'**obiettivo** non si cambia più (su Meta diventa sola lettura: per cambiarlo si spegne la campagna e se ne fa un'altra, perdendo l'apprendimento) e il **tipo** di budget (giornaliero o totale) nemmeno — si cambia solo l'importo. Verificato aprendo i pannelli di modifica in Ads Manager | `/campagne/lancia` (`modulo-meta`) |
| 2026-09-11 | **Il pubblico di un ad set è diviso in «vincoli» e «suggeriti»**, come fa Business Meta: con **Advantage+ acceso** luoghi, età **minima**, lingue ed esclusioni sono limiti che Meta non scavalca; età massima, genere, interessi e pubblici inclusi sono **suggerimenti** che scavalca quando le conviene. Mostrarli in un elenco unico — come faceva questo riquadro fino a stamattina — dice una cosa falsa sulla consegna: «età 27-60» sembra un filtro e non lo è. Anche l'operazione di cambio pubblico lo scrive fra gli avvisi | `lib/meta.ts` (`leggiSpecTargeting`), `AdSetMeta`, `lib/azioni.ts` |
| 2026-09-11 | **Ogni inserzione Meta dice se è tracciata**: il riquadro degli annunci mostra i **parametri utm** di ciascuna, e conta quante accese non ne hanno. Senza utm il traffico arriva a Shopify come «diretto» o «social»: la spesa si vede, il ricavo non si attribuisce, e il **ritorno Meta** delle tabelle per categoria e per area risulta più basso del vero (in tutto il 2026 gli ordini attribuiti a Meta sono 16 su Gifts, 19 su Flowers, 8 su Cake) | `lib/meta-annunci.ts`, `components/AnnunciMeta` |
| 2026-09-11 | **La finestra di attribuzione dell'ad set è scritta dove si guardano i numeri** («conta: 7 giorni dal clic · 1 giorno dalla visualizzazione», più l'evento ottimizzato): Meta si attribuisce un ordine arrivato giorni dopo il clic, e perfino di chi ha solo **visto** l'inserzione, mentre Orders conta l'ordine il giorno del pagamento. È la ragione per cui ROAS di piattaforma e MER non coincidono, e adesso si vede. Nello stesso giro: un ad set a **budget totale** non viene più chiamato «CBO» (aveva un budget suo), e il tipo di budget non si cambia dopo la pubblicazione | `lib/meta.ts`, `components/AdSetMeta` |
| 2026-09-11 | **Impostazioni dice se un conto Meta è FUORI dall'app**: chiede a Meta quali conti pubblicitari vede il nostro token e li confronta con quelli censiti. Un conto fuori non dà errore da nessuna parte — le sue campagne semplicemente non esistono per l'app, e la loro spesa non entra nel MER né esce da `/api/v1/spesa`, che le altre app leggono per i budget. L'app **non** lo aggiunge da sola: a quale brand appartiene un conto è una decisione, e indovinarla attribuirebbe spesa al marchio sbagliato. Se la lettura non arriva lo dice: «non è un no, è un non lo so» | `components/ContiMetaNonCensiti`, `lib/meta.ts` (`leggiContiMeta`), `/impostazioni` |
| 2026-09-11 | **Allarme «accesa e vuota» (A6)**: una campagna Search che su Google è ENABLED ma non ha **nessuna keyword e nessun annuncio** non eroga — quindi non brucia budget — ma resta «accesa» in ogni elenco e nessuno se ne accorge. Ora lo dice, col budget dichiarato, e l'allarme compare dove compaiono gli altri (scheda campagna, alert aperti del brand). Servono **due zeri**: una Performance Max senza keyword è normale, segnalarla sarebbe un allarme che grida sempre; e non scatta nelle prime 24 ore, quando lo script può non essere ancora passato. Misurato: su 23 campagne Google accese ne scatta **una**, quella che il problema ce l'ha davvero. Nello stesso giro, il `completa_campagna` che non porta né keyword né annunci lo **dichiara a chi approva** invece di riuscire in silenzio | `lib/guardrail.ts` (`alertAccesaEVuota`), `GuardrailCampagna`, `lib/dopo-il-lancio.ts` |
| 2026-09-11 | **La stessa analisi non entra più due volte**: nella cartella ADV ogni analisi arriva come `.md` (il testo) e come `.xlsx` (le tabelle), e l'import ne faceva **due** — quella sul foglio di calcolo non si può elaborare, quindi restava «da elaborare» per sempre. Ora il gemello non crea una seconda riga, e se il foglio è arrivato **prima** del testo la riga esistente si **sposta** sul testo invece di sdoppiarsi. Sulle coppie già nate, la scheda del foglio di calcolo **porta il link** all'analisi in testo. Misurato: 132 analisi, 94 senza scheda di cui **46 su file illeggibili**, e 23 titoli doppi | `lib/analisi-drive.ts`, `/analisi/[id]` |
| 2026-09-11 | **Su Meta si chiamano «ad set», e hanno la loro voce di menu**: la voce era stata tolta il 09/08 perché portava a una pagina vuota (gli ad set non erano censiti); ora che la sync li porta (171) torna, l'elenco prende il nome del filtro («Ad set» su Meta, «Gruppi di annunci» su Google) e le due voci del menu si accendono ciascuna per il suo canale. Sulla scheda di una campagna Meta il riquadro generico dei gruppi **non compare più**: mostrava gli stessi ad set del riquadro nuovo, col nome sbagliato e senza budget né comandi | `components/Sidebar`, `/gruppi`, `campagne/[id]`, `components/AdSetMeta` |
| 2026-09-11 | **Il pubblico di un ad set Meta si legge e si cambia dall'app**: sotto il riquadro degli ad set, «A chi parla ogni ad set» mostra **età, genere, luoghi e pubblici letti vivi da Meta**, e un modulo li cambia — età e genere sono un **L2**, luoghi e pubblici un **L3** (cambiano il mercato, non la domanda dentro lo stesso mercato). Quello che si lascia vuoto non si tocca; i pubblici si modificano solo spuntando «cambia anche i pubblici», e togliere tutti i pubblici è scritto fra gli avvisi. Il pubblico di **prima** resta scritto nell'operazione, così tornare indietro non è a memoria. Una sola modifica in volo per ad set | `components/AdSetMeta`, `lib/azioni.ts` (`creaOperazioneTargeting`), `lib/meta.ts`, `lib/meta-scrittura.ts` |
| 2026-09-11 | **Un annuncio Meta si mette in pausa (e si riattiva) dalla scheda della campagna**: nel riquadro «Annunci su Meta (dal vivo)» ogni annuncio ha il suo bottone, così si spegne una creatività che non rende **senza fermare l'ad set**. Passa dalla coda con approvazione (L1) e la esegue l'app; finché non è approvata lo stato mostrato resta quello vero di Meta, con «⏳ pausa chiesta» accanto. Se è l'ultimo annuncio acceso del suo ad set, chi approva lo legge nell'avviso. Un annuncio acceso che Meta dà come fermo (ad set o campagna in pausa) lo dice, e uno archiviato non offre bottoni invece di offrirne uno che fallisce | `components/AnnunciMeta`, `lib/azioni.ts` (`creaOperazioneAnnuncioMeta`), `lib/meta-scrittura.ts` |
