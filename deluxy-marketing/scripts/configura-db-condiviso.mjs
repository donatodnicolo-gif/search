...
// dedicato "marketing". Non stampa mai le stringhe di connessione.
//
...
function conSchemaAnagrafiche(url) {
  if (!url) return null;
  if (/[?&]schema=/.test(url)) return url.replace(/([?&]schema=)[^&]*/, "$1marketing");
  return url + (url.includes("?") ? "&" : "?") + "schema=marketing";
}
...
const contenuto = [
  '# Postgres condiviso delle app Deluxy (stesso cluster di hub/partner), schema "marketing"',
  `DATABASE_URL="${db}"`,
  `DIRECT_URL="${direct}"`,
  "",
].join("\n");
...
console.log("Scritto .env con schema=marketing (stringhe di connessione non mostrate).");
