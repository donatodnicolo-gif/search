...
> 6. **Migrazioni** applicate fino a **0012**; **⚠️ la 0013 (`visits.concorrenti`) è scritta ma NON ancora applicata** — va eseguita prima di usare il campo Concorrenti, altrimenti il salvataggio visita erra sulla colonna mancante: `node scripts/mgmt-query.mjs supabase/migrations/0013_concorrenti.sql` con `SUPABASE_PAT`. Anche la **Edge Function `hubspot-sync` va rideployata** (append concorrenti alle note del deal). Se aggiungi altre migrazioni, stesso comando.
