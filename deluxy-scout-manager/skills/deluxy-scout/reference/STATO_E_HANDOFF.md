...
> 6. **Migrazioni** applicate fino a **0012**; **⚠️ la 0013 (`visits.concorrenti`) è scritta ma NON ancora applicata** — da eseguire per **persistere** il campo Concorrenti: `node scripts/mgmt-query.mjs supabase/migrations/0013_concorrenti.sql` con `SUPABASE_PAT`. (Il salvataggio NON si rompe se manca: `inserisciVisita` degrada e salva senza concorrenti.) Poi **rideploya `hubspot-sync`** (append concorrenti alle note del deal). Se aggiungi altre migrazioni, stesso comando.
> 6b. **Web deployato** con: campo Concorrenti + **fix leggibilità nomi su mobile** (card a colonna sotto 560px, nome per esteso, icone a capo). URL live aggiornato.
