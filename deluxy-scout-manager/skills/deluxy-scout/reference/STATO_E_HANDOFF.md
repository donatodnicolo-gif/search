...
- **🎨 Adottato il Deluxy Design System v1.0 (14 lug 2026)**: il DS ufficiale vive in `deluxy-design-system/` (root del repo, committato sul branch `deluxy-scout` come `93f0429`). Scout lo adotta: `lib/theme.ts` **rimappato sui token DS** (sfondo `#F5F5F7`, testo `#1D1D1F`, azioni primarie **ink `#111318`**, accento **oro `#B8963E`** usato poco, hairline/fill neutri, ombre `shadow.card/float`). I nomi storici restano (`navy`→ink, `oro`→gold) per non rompere le schermate. **Login** ridisegnato stile DS (card chiara, logo D oro su quadrato ink, campi raggruppati, CTA pillola nera, footnote brand) — verificato live. Bottoni primari → ink in VisitaModal/nuovo-target/modifica. Commit `6c13b2f` + `79b26bc`. ⏳ Da rifinire (opzionale): header/tab bar ancora scuri, FAB e dock "Naviga" ancora oro (accenti tollerati), tabelle/badge di stato secondo §Componenti del DS.
