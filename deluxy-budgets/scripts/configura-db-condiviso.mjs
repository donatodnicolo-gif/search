...
// dedicato "budgets". Non stampa mai le stringhe di connessione.
//
...
function conSchemaApp(url) {
  if (!url) return null;
  if (/[?&]schema=/.test(url)) return url.replace(/([?&]schema=)[^&]*/, "$1budgets");
  return url + (url.includes("?") ? "&" : "?") + "schema=budgets";
}
...
const contenuto = [
  '# Postgres condiviso delle app Deluxy (stesso cluster di hub/partner), schema "budgets"',
  `DATABASE_URL="${db}"`,
  `DIRECT_URL="${direct}"`,
  ...esistenti,
  "",
...
console.log('Scritto .env con schema=budgets (stringhe di connessione non mostrate).');
