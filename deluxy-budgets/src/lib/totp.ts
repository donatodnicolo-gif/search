...
// TOTP (RFC 6238) scritto a mano: sono trenta righe e evitano una dipendenza in
// più. È lo stesso identico file di deluxy-transactions — un solo modo di fare
// il secondo fattore in tutto l'ecosistema, invece di due implementazioni che
// col tempo divergono. Compatibile con Google Authenticator,
...
export function uriTotp(segreto: string, conto: string): string {
  const etichetta = encodeURIComponent(`Deluxy Budgets:${conto}`);
  return `otpauth://totp/${etichetta}?secret=${segreto}&issuer=Deluxy%20Budgets&algorithm=SHA1&digits=6&period=30`;
}
