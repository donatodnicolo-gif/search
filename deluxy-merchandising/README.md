...
| **Per fornitore** | Chi ci fornisce i prodotti (il «Venditore» letto da Shopify) con quanto pesa ciascuno: prodotti, senza costo, esclusi, pezzi e venduto 90gg. Vista per fornitore o fornitore × categoria; da ogni riga si entra nei suoi prodotti. | `/fornitori` |
| **Per categoria** | Il catalogo per categoria, in due viste tenute separate: dal negozio (il «Tipo» di Shopify) e interna. | `/categorie` |
| **Griglie** | Due lenti incrociate (categoria × fascia, fornitore × fascia…): ogni casella dice quanti SKU ci stanno e quanto vendono, con le quote e lo scarto in punti. Verde = rende più di quanto occupa, ambra = il contrario. Righe e colonne si ordinano come si vuole. | `/griglie` |
| **Multi prodotto** | Più prodotti agganciati insieme diventano un prodotto nuovo (cesto, kit): si vedono costo e listino di ogni componente e i totali. Il costo del composto non si scrive — è la somma dei componenti, e si aggiorna con loro. | `/multi-prodotto` |
| **Per linea** | Le famiglie commerciali decise da noi, con quanto pesa ciascuna e quanti prodotti non ne hanno ancora una. | `/linee` |
| **Fasce di prezzo** | Sei scalini ricavati dai prezzi veri del catalogo. La fascia di un prodotto è quella in cui **cade il suo prezzo**: non si assegna a mano. Qui si spostano i confini, e l'app segnala buchi e sovrapposizioni. | `/fasce` |
| **Riconciliazione** | Schede diverse che sono lo stesso prodotto: l'app le propone dicendo perché (stesso SKU, stessa pagina Shopify, stesso nome), tu scegli. Unendo, il venduto passa sulla scheda buona; l'altra resta con scritto a chi è unita, e si può separare. | `/prodotti/riconcilia` |
