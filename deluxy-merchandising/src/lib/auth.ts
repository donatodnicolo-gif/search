...
// deluxy-partner e deluxy-anagrafiche. Se MERCHANDISING_APP_PASSWORD non è
// impostata (sviluppo locale) la UI è aperta.
...
export async function sessionToken(password: string): Promise<string> {
  const data = new TextEncoder().encode(`deluxy-merchandising::${password}`);
